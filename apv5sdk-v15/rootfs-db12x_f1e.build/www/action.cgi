...
"Open")
    if [ -f "/tmp/udisk/www/video/$FORM_filename" ]; then
		cgi_open "/tmp/udisk/www/video/$FORM_filename" "$FORM_filename"
    fi    
    cgi_exec index.htm
    ;;
esac
...
"Open&Download")
    if [ -f "/tmp/udisk/$FORM_filename" ]; then
		cgi_oem_download "/tmp/udisk/$FORM_filename" "$FORM_filename"
    fi    
    cgi_exec ltesetup.htm
    ;;  
   
 "Apply_Setup")
	lte_setup "$FORM_apn" "$FORM_acc" "$FORM_wifidog"
    cgi_exec ltesetup.htm
    /sbin/sysreboot &
	;;    
