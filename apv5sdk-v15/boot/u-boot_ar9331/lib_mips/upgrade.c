...
#define BIN_SIZE 64*1024

/*kernel1 start*/
unsigned char *kernel_crc_addr = PERFORM_BASE_ADDR + KERNEL_CRC_OFFSET;
unsigned char *rootfs_crc_addr = PERFORM_BASE_ADDR + ROOTFS_CRC_OFFSET;
unsigned char *kernel_lenth_addr = PERFORM_BASE_ADDR + KERNEL_LENGTH_OFFSET;
unsigned char *rootfs_lenth_addr = PERFORM_BASE_ADDR + ROOTFS_LENGTH_OFFSET;
/*kernel1 end*/

/*kernel2 start*/
unsigned char *kernel2_crc_addr = PERFORM_BASE_ADDR + KERNEL2_CRC_OFFSET;
unsigned char *rootfs2_crc_addr = PERFORM_BASE_ADDR + ROOTFS2_CRC_OFFSET;
unsigned char *kernel2_lenth_addr = PERFORM_BASE_ADDR + KERNEL2_LENGTH_OFFSET;
unsigned char *rootfs2_lenth_addr = PERFORM_BASE_ADDR + ROOTFS2_LENGTH_OFFSET;
/*kernel2 end*/

/*crc start address*/
unsigned char *kernel_crc_start = FLASH_BASE_ADDR + 0x050000;
unsigned char *rootfs_crc_start = FLASH_BASE_ADDR + 0x130000;
unsigned char *boot_flag_addr = PERFORM_BASE_ADDR + FLAG_OFFSET;
unsigned char *boot_mark_addr = PERFORM_BASE_ADDR + MARK_OFFSET;

unsigned char *kernel2_crc_start = FLASH_BASE_ADDR + 0x7e0000;
unsigned char *rootfs2_crc_start = FLASH_BASE_ADDR + 0x8c0000;

unsigned char *magic_begin = PERFORM_BASE_ADDR;
unsigned char *magic_end = PERFORM_BASE_ADDR + 0xffff;

char mark2flag1[] = "console=ttyS0,115200 root=31:03 rootfstype=jffs2 init=/sbin/init mtdparts=ar7240-nor0:256k(u-boot),64k(u-boot-env),896k(kernel1),6848k(rootfs1),896k(kernel2),6848k(rootfs2),448k(config),64k(productinfo),64k(ART)";
char mark2flag2[] = "console=ttyS0,115200 root=31:05 rootfstype=jffs2 init=/sbin/init mtdparts=ar7240-nor0:256k(u-boot),64k(u-boot-env),896k(kernel1),6848k(rootfs1),896k(kernel2),6848k(rootfs2),448k(config),64k(productinfo),64k(ART)";
char mark1[] = "console=ttyS0,115200 root=31:10 rootfstype=jffs2 init=/sbin/init mtdparts=ar7240-nor0:256k(u-boot),64k(u-boot-env),896k(kernel1),6848k(rootfs1),896k(kernel2),6848k(rootfs2),448k(config),64k(productinfo),64k(ART)";
...
#if 1
//	if(len_k > 1114112 || len_r > 6815744){
	if(len_k > (rootfs_crc_start - kernel_crc_start) || len_r > (kernel2_crc_start - rootfs_crc_start)){//wangyu change the len_r for apv5
		printf("len is too long , now jump the crc,default bootm kernel2\n");
		setenv ("bootcmd", "bootm 0x9f7e0000");
		setenv ("bootargs", mark2flag2);
		return 1;
	}else{
	...
	if(crc_kernel == crc1 && crc_rootfs == crc2){
		...
		*(ulong *)(config_magic_data + FLAG_OFFSET) = 1;
		int rc = flash_write((char *)config_magic_data, magic_begin, BIN_SIZE);
		if (rc != 0) {
			flash_perror (rc);
			return 1;
		} else {
...
void crc_flash(void)
{
	...
	*(ulong *)(config_magic_data + FLAG_OFFSET) = 2;
	...
	if (mark == 2){
		...
		if (flash_sect_erase (magic_begin, magic_end))
			return 1;
		*(ulong *)(config_magic_data + MARK_OFFSET) = 1;
		int rc = flash_write((char *)config_magic_data, magic_begin, BIN_SIZE);
		if (rc != 0) {
			flash_perror (rc);
			return 1;
