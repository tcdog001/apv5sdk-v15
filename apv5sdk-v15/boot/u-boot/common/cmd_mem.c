...
int do_nfcprd (int rd, cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	switch (rd) {
		case 0: return write_buff(NULL, addr, dest, count);
		case 1: return read_buff(NULL, addr, dest, count);
		case 2: return write_buff((void *)1, addr, dest, count);
		case 3: return read_buff((void *)1, addr, dest, count);
	}
}
