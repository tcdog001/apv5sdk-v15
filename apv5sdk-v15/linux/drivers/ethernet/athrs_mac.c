...
/*Begin: Added by pengruofeng record last time for 2011-6-3*/
extern capwap_record txcapwap;
extern capwap_record rxcapwap;
//extern capwap_record capwap;
#define RX_PACKET 0
#define TX_PACKET 1
...
static int athr_gmac_is_capwap(struct sk_buff *skb,int flag) 
{
	struct iphdr *ip;
	/*check the net work protocal*/
    capwap_record *pcapwap = NULL;
	unsigned char capwap_type = 0;
	if ((skb == NULL) || (skb->protocol != htons(ETH_P_IP))) {
       		return 0;
	}
	...
	if ((flag == TX_PACKET && udp->dest != AC_CONTROL_PORT) || (flag == RX_PACKET && udp->source!= AC_CONTROL_PORT)) 
    {

		//printk("--4 : udp->source %#X :: %#X\n", udp->source, ntohs(udp->source));	
		return 0;
		
	}
	struct timespec now;
	now = current_kernel_time();
	pcapwap->third_time = pcapwap->second_time;
	pcapwap->third_msgtype = pcapwap->second_msgtype;
	pcapwap->second_time = pcapwap->first_time;
	pcapwap->second_msgtype = pcapwap->first_msgtype;
	pcapwap->first_time = now.tv_sec;
	if(flag == TX_PACKET){
		capwap_type = *(skb->data + 28+14);
	}
	else{
		capwap_type = *(skb->data + 28);
	} 
	capwap_type = capwap_type >> 4;
	if(!capwap_type)
	{
		if(flag == TX_PACKET){
			memcpy(&(pcapwap->first_msgtype),(skb->data + 28 + 14 + 8),4);
		}
		else{
			memcpy(&(pcapwap->first_msgtype),(skb->data + 28 + 8),4);
		} 
	}
	else
	{
		pcapwap->first_msgtype = 0;
	}
	return 1;
}
/*End: Added by pengruofeng record last time for capwap 2011-6-3*/
...
athr_gmac_mii_setup(athr_gmac_t *mac)
{
    ...
    if (is_ar934x() && (is_f1e() || is_vir_phy())) {
        ...
        mgmt_cfg_val = 0xa;//wangyu change from 0x6  to 0xa 	
        if(mac->mac_unit == 0)
            athr_reg_wr(ATHR_GMAC_ETH_CFG, ATHR_GMAC_ETH_CFG_RGMII_GE0);  // External RGMII Mode on GE0

		athr_reg_rmw_clear(ATHR_SWITCH_CLK_SPARE, (1 << 6));	//wangyu add for clear switch_clk		   //MII clock is 40Mhz.
        athr_gmac_reg_wr(mac, ATHR_GMAC_MII_MGMT_CFG, mgmt_cfg_val | (1 << 31));
        athr_gmac_reg_wr(mac, ATHR_GMAC_MII_MGMT_CFG, mgmt_cfg_val);
...
athr_gmac_hw_setup(athr_gmac_t *mac)
{
    ...
    athr_gmac_reg_wr(mac, ATHR_GMAC_CFG1, (ATHR_GMAC_CFG1_RX_EN | ATHR_GMAC_CFG1_TX_EN));//wangyu add as it in 616
...
athr_gmac_set_mac_from_link(athr_gmac_t *mac, athr_phy_speed_t speed, int fdx)
{
    ...
    case ATHR_PHY_SPEED_10T:
	   ...
	   ops->set_pll(mac,mac->mac_speed);
		...
		/*chenxf added for 10Mbps problem 2011-05-21*/
        if (is_ar934x() && mac->mac_unit == 0 && is_f1e()) {
...
#ifdef AE5000ENT
int net_packets_num = 0;
int lights_flag;/*Add bu zhangsiyu for AE5000ENT turn off all lights 2012-12-12*/
#endif
...
athr_gmac_check_link(void *arg,int phyUnit)
{
	...
	lights_flag = (athr_reg_rd(ATHR_GPIO_OUT) & (1 << 15));
	athr_reg_wr(ATHR_GPIO_OE, (athr_reg_rd(ATHR_GPIO_OE) | (1 << 16)));
	athr_reg_wr(ATHR_GPIO_OUT, (athr_reg_rd(ATHR_GPIO_OUT) & (~(1 << 16))));
	#endif
    ...
    if (!phy_up)
    {
        if (carrier)
        {   
            /*Begin:Add by zhangisyu for AE5000ENT turn off all lights 2012-12-12*/
            #if defined(AE5000ENT)
			if(lights_flag)
            {
			    /*Begin:Add by zhangisyu for AE5000 transplantation 2012-11-22*/    
                if(mac->mac_unit == 0) 
				    athr_reg_wr(ATHR_GPIO_OUT, (athr_reg_rd(ATHR_GPIO_OUT) & (~(1 << 1))));
			    if(mac->mac_unit == 1) 
				    athr_reg_wr(ATHR_GPIO_OUT, (athr_reg_rd(ATHR_GPIO_OUT) & (~(1 << 13))));
            }
			    /*End:Add by zhangsiyu for AE5000 transplantation 2012-11-22*/
			#endif
			/*End:Add by zhangisyu for AE5000ENT turn off all lights 2012-12-12*/
        ...
        if (mac_has_flag(mac,ETH_SOFT_LED)) {
            ...
            */
            if (is_ar933x()) {
                ...
                #endif
			        /*End:Add by zhangsiyu for AE5000ENT turn off all lights 2012-12-12*/
				    //printk("ATHR_GPIO_FUNCTIONS=%x\n",ATHR_GPIO_FUNCTIONS);
                    athr_reg_wr(ATHR_GPIO_FUNCTIONS, 
                        (athr_reg_rd(ATHR_GPIO_FUNCTIONS)& (~(1 << (phyUnit + 3)))));
				    //printk("athr_reg_rd(ATHR_GPIO_FUNCTIONS)=%x\n",athr_reg_rd(ATHR_GPIO_FUNCTIONS));
                    /*Begin:Add by zhangsiyu for AE5000ENT turn off all lights 2012-12-12*/
				#if defined(AE5000ENT)
                    athr_reg_wr(ATHR_GPIO_OUT, (athr_reg_rd(ATHR_GPIO_OUT) & (~(1 << 16))));
				}
...
/*chenxf added for ethernet gmac tx timeout 2011-7-27*/
       /* 
...
/*BEGIN: enable active led blink [liuqiang] 2012-11-12 */
#ifdef AQ2000_X
unsigned int packet_count = 0;
#endif
/*END: enable active led blink [liuqiang] 2012-11-12 */
...
athr_gmac_hard_start(struct sk_buff *skb, struct net_device *dev)
{
...
#if 0
	#ifdef AQ2000_X
	#if defined (LTEFI)||defined (AK2000_EN2T)
	packet_count++;
	athr_reg_wr(ATHR_GPIO_OUT, athr_reg_rd(ATHR_GPIO_OUT) & (0xfff7ffff));
	if(packet_count == 10)
	{
		athr_reg_wr(ATHR_GPIO_OUT, athr_reg_rd(ATHR_GPIO_OUT) | (0x1 << 19));
		packet_count = 0;
	}	
	#else
	packet_count++;
	(*(volatile int *)KSEG1ADDR(0x18040034U)) = 0x1180000U;
	if(packet_count == 10)
	{
		(*(volatile int *)KSEG1ADDR(0x18040034U)) = 0x2d180000U;
		packet_count = 0;
	}
	#endif
	#endif
	...
	if(capwap_get_uplink_mac_hook)
	{
		capwap_get_uplink_mac_hook(skb);// liuqiang add find AC or Router MAC in uplink.
	}
    /*Begin: Added by pengruofeng record last time for capwap 2011-6-3*/
    if (strncmp(skb->dev->name, "eth", 3) == 0)
    {
        athr_gmac_is_capwap(skb,TX_PACKET);
    }
    /*End: Added by pengruofeng record last time for capwap 2011-6-3*/
	...
	/*Begin:Add by zhangisyu for AE5000 transplantation 2012-11-22*/
          ...
          /*End:Add by zhangsiyu for AE5000ENT turn off all lights 2012-12-12*/
			  ...
			  else 
			  {
					net_packets_num ++;
					athr_reg_wr(ATHR_GPIO_OUT, (athr_reg_rd(ATHR_GPIO_OUT) & (~(1 << 1))));
					if(net_packets_num >= 20)
						net_packets_num = 0;
			  }

		      }
          }/*Add by zhangsiyu for AE5000ENT turn off all lights 2012-12-12*/
		  
#endif
	/*End:Add by zhangsiyu for AE5000 transplantation 2012-11-22*/

    athr_gmac_trc(athr_gmac_reg_rd(mac, ATHR_GMAC_DMA_TX_CTRL),"dma idle");
/*added by chenming for ethernet statics--PCAPVXN-124*/
    if((PACKET_BROADCAST == skb->pkt_type) ||(is_broadcast_ether_addr(skb->data)))    
    {	
        #ifdef CONFIG_64BIT		
            atomic64_add(1,&mac->mac_net_stats.tx_broadcast_packets);	
        #else		
            atomic_add(1,(atomic_t *)&mac->mac_net_stats.tx_broadcast_packets);	
        #endif	  
            mac->mac_net_stats.tx_broadcast_bytes += skb->len;	    
    } 
    if((PACKET_MULTICAST == skb->pkt_type) ||is_multicast_ether_addr(skb->data))    
    {	
        #ifdef CONFIG_64BIT		
            atomic64_add(1,&mac->mac_net_stats.tx_multicast_packets);	
        #else		
            atomic_add(1,(atomic_t *)&mac->mac_net_stats.tx_multicast_packets);	
        #endif	 
            mac->mac_net_stats.tx_multicast_bytes += skb->len;	   
    }   
/*ended by chenming*/
    if (mac_has_flag(mac,WAN_QOS_SOFT_CLASS)) {
...
athr_gmac_intr(ATHR_MAC_ISR_ARGS)
{
	 ...
	 * ignore it.
 	 */
/*begin:wangyu add for 9344+8035 ethernet interrupt problem*/
#if 0
        if (mac->dma_check)
	{
            mac->dma_check = 0;
...
#endif
/*end:wangyu add for 9344+8035 ethernet interrupt problem*/
...
//chenxf changed it for  cache alignment 2011-01-12
	athr_mac_cache_inv((unsigned long)(skb->data - 2), skb->len);
//chenxf changed end
...
/*End:Add by zhangsiyu for AE5000 transplantation 2012-11-22*/
      ...
      {
          #ifdef CONFIG_64BIT		
              atomic64_add(1,&mac->mac_net_stats.rx_broadcast_packets);
          #else			
              atomic_add(1,(atomic_t *)&mac->mac_net_stats.rx_broadcast_packets);
          #endif            
              mac->mac_net_stats.rx_broadcast_bytes += skb->len; 
      }    
      if(( PACKET_MULTICAST == skb->pkt_type) ||is_multicast_ether_addr(skb->data))   
      {
          #ifdef CONFIG_64BIT		
              atomic64_add(1,&mac->mac_net_stats.rx_multicast_packets);
          #else			
              atomic_add(1,(atomic_t *)&mac->mac_net_stats.rx_multicast_packets);
          #endif            
              mac->mac_net_stats.rx_multicast_bytes += skb->len;
      }
/*ended by chenming*/
        /*
	...
	if(packet_count == 10)
	{
		...
		if (strncmp(skb->dev->name, "eth", 3) == 0)
		{
			athr_gmac_is_capwap(skb,RX_PACKET);
		}
		/*End: Added by pengruofeng record last time for capwap 2011-6-3*/
...
//chenxf added for buffer replenish method 2011-01-12

/*
*here is the detail for this method:
*we always reamain 3 skb buffers disreplenished
*/
	head = (head >= 3)?(head - 3):(r->ring_nelem - 3 + head);
//chenxf added end
...
//chenxf added for flushing descriptors 2011-01-12
		wmb();
//chenxf added end
