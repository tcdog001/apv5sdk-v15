...
//chenxf added for ethreg tool 2011-05-21
...
static athrPhyInfo_t athrPhyInfo[] = {
    ...
    {TRUE,  /* port 5 -- WAN Port 5 */
     FALSE,
//     ENET_UNIT_WAN,
	ENET_UNIT_LAN, //wangyu change this for xj9344
     0,
//     ATHR_PHY0_ADDR,
	ATHR_PHY4_ADDR,//wangyu change this for xj9344
     ATHR_LAN_PORT_VLAN    /* Send to all ports */
    },
...
//        int phyUnit = 0;
	#if defined ( XJ9344 )
	int phyUnit = 4; //wangyu change the phyUnit for xj9344
	#else
	int phyUnit = 0;
	#endif
...
//	if (athr_phy_is_link_alive(phyUnit)) {
	if (athr_phy_is_link_alive(ethUnit)) {//wangyu change the arg of ....alive
		liveLinks++;
...
athr_phy_is_fdx(int ethUnit,int phyUnit)
{
	...
	#if defined ( XJ9344 )
	phyUnit = 4; //wangyu change the phyUnit for xj9344
	#else
	phyUnit = 0;
	#endif
...
//    if (athr_phy_is_link_alive(phyUnit)) {
        ...
        do {
            phyHwStatus = phy_reg_read(ethUnit, ATHR_PHYADDR(phyUnit),ATHR_PHY_SPEC_STATUS);
/*begin:wangyu add for 8035 enet drop packet problem*/
#if 0
            mdelay(10);
       } while((!(phyHwStatus & ATHR_STATUS_RESOVLED)) && --ii);
#else
...
#endif
/*end:wangyu add for 8035 enet drop packet problem*/
        phyHwStatus = ((phyHwStatus & ATHER_STATUS_LINK_MASK) >>
                       ATHER_STATUS_LINK_SHIFT);

        switch(phyHwStatus) {
        case 0:
    	    /*chenxf added for 10Mpbs problem 2011-05-21*/
...
athr_phy_is_up(int ethUnit)
{
	...
	#if defined ( XJ9344 )
	for (phyUnit=4; phyUnit < 5; phyUnit++) {
	#else
...
//chenxf added for ethreg tool 2011-05-21
int athrf1e_ioctl(struct net_device *dev, void *args, int cmd)
{
	...
	if(cmd  == ATHR_PHY_RD) {
	if(ethcfg->portnum != 0xcc) {
	    printk("---mii read---\n");
	    ethcfg->val = phy_reg_read(mac->mac_unit, ethcfg->portnum,ethcfg->phy_reg);
	}
	else {
	    printk("---cpu read---\n");
	    ethcfg->val = athr_reg_rd(ethcfg->phy_reg);
	}
    }
    else if(cmd  == ATHR_PHY_WR) {
	if(ethcfg->portnum != 0xcc) {
	    printk("---mii write---\n");
            phy_reg_write(mac->mac_unit, ethcfg->portnum ,ethcfg->phy_reg,ethcfg->val);
	}
	else {
	    printk("---cpu write---\n");
	    athr_reg_wr(ethcfg->phy_reg,ethcfg->val);
	}
    }
    else
        return -EINVAL;
...
//ops->ioctl          =  NULL;
  ops->ioctl          =  athrf1e_ioctl; //add chenxf
