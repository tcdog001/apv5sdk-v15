...
#if defined (LTEFI) //add for lte-fi by liuhj//20130730
	for (phyUnit = 0; phyUnit < S27_PHY_MAX ; phyUnit++) {
		phyAddr = S27_PHYADDR(phyUnit);
		s27_wr_phy(0,phyAddr,S27_PHY_INTR_ENABLE,S27_LINK_INTRS);
	}
#else
...
void athrs27_disable_link_intrs(void *arg) 
{
	...
	for (phyUnit = 0; phyUnit < S27_PHY_MAX ; phyUnit++) {
		phyAddr = S27_PHYADDR(phyUnit);
		s27_wr_phy(0,phyAddr,S27_PHY_INTR_ENABLE,0x0);
	}
#else
     if (mac->mac_unit == ENET_UNIT_WAN) {
...
#ifdef AQ2000_X
     ops->port_map    = 0x73; /*set GE0 and port - 0,1, 4,5,6 - eth0*/
#else
