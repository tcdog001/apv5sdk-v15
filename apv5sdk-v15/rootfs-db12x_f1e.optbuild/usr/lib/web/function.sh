...
{
    echo "Content-Type: application/octet-stream;"
    echo "Content-disposition: inline; filename=$2"
    echo
    cat "$1"
}
...
{   
    ...
    local xml_wifi=`config_read /config/network/$xml_wlan_wifi`
      ...
      xml_config=`config_setoption "$xml_config" puren "$puren"`
	    config_write /config/network/$vapname "$xml_config"
...
lte_setup()
{
	local apn_name="$FORM_apn"
	local acc_time="$FORM_acc"
	local wifidog_name="$FORM_wifidog"
#	local commoand_print

	local xml_lte=`config_read /config/lte`
	
	if [ -n "$apn_name" ];then
#		local cur_apn=`/bin/cat /etc/config/profile.ini | /bin/grep 'apn=' | /usr/bin/awk -F'=' '{print $2}'`
#		commoand_print=`/bin/sed -i "s/apn=$cur_apn/apn=$apn_name/g" /etc/config/profile.ini`
		xml_lte=`config_setoption "$xml_lte" apn "$apn_name"`
	fi
	if [ -n "$acc_time" ];then
		xml_lte=`config_setoption "$xml_lte" acc "$acc_time"`
	fi
	config_write  /config/lte "$xml_lte" > /dev/null 2>&1


	if [ -n "$wifidog_name" ];then
		local xml_lte=`config_read /config/wifidog`
		xml_lte=`config_setoption "$xml_lte" enable "$wifidog_name"`
		config_write  /config/wifidog "$xml_lte" > /dev/null 2>&1
	fi

	if [ $? != 0 ]; then
		false
	else		
#		commoand_print=`/usr/bin/killall check_lte`
#		commoand_print=`/usr/sbin/check_lte > /dev/null 2>&1 &`
		true
	fi
}
