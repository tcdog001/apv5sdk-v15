...
#ifdef __cplusplus
SOAP_SOURCE_STAMP("@(#) stdsoap2.cpp ver 2.7.8 2006-06-03 12:00:00 GMT")
extern "C" {
#else
SOAP_SOURCE_STAMP("@(#) stdsoap2.c ver 2.7.8 2006-06-03 12:00:00 GMT")
#endif
...
{ return soap_copy_context((struct soap*)malloc(sizeof(struct soap)), soap);
  //printf("soap copy\n");
  //return (struct soap*)malloc(sizeof(struct soap));
}
#endif
...
{ int i;
  ...
  { if (soap->local_namespaces && (t = strchr(tag, ':')))
    ...
    { t = tag;
      if (soap_element_begin_out(soap, t, 0, NULL))
        return soap->error;
    }
  }
  if (p && *p)
...
{ 
  printf("soap_connect_command: soap: %x\n", soap);
  printf("soap_connect_command: endpoint: %s\n", endpoint);
  ...
  printf("soap_connect_command: host: %s\n", soap->host);
#ifndef WITH_LEANER
  if (soap->fconnect)
