  ...
  { for (p = t->list; p; p = p->next)
	...
	q = (Entry*)p->info.typ->ref;
        for (r = t->list; r; r = r->next)
          if (r->info.typ->type == Tfun && !(r->info.sto & Sextern) && q == r->info.typ->ref)
	    q = NULL;
	  ...
	  { if (is_document(method_style))
	    ...
	    { for (q = ((Table*)q->info.typ->ref)->list; q; q = q->next)
            ...
            for (e = t->list; e; e = e->next)
              if (e->info.typ->type == Tfun && !(e->info.sto & Sextern) && q == e->info.typ->ref)
	        q = NULL;
            if (q && !is_transient(q->info.typ))
  ...
  { case Tstruct:
	...
	{ p = table->list;
	  ...
	  if(p->info.typ->type==Tarray)
	    ...
	    fprintf(fout,"\n\treturn soap_outliteral(soap, tag, &a->%s);", p->sym->name);
	  else if (is_XML(p->info.typ) && is_wstring(p->info.typ))
	    fprintf(fout,"\n\treturn soap_outwliteral(soap, tag, &a->%s);", p->sym->name);
	  else if (p->info.typ->type != Tfun && !is_void(p->info.typ))
	    fprintf(fout,"\n\treturn soap_out_%s(soap, tag, id, &a->%s, \"%s\");", c_ident(p->info.typ), p->sym->name, xsi_type_u(typ));
	  else
            fprintf(fout,"\n\treturn SOAP_OK;");	 
   ...
   {  if (!is_invisible(typ->id->name))
	...
	{ if (p->info.typ->type == Tpointer)
	  ...
	  if (is_XML(p->info.typ) && is_string(p->info.typ))
	    fprintf(fout,"\n\treturn soap_outliteral(soap, tag, &(((%s*)a)->%s));", t->sym->name,p->sym->name);
	  else if (is_XML(p->info.typ) && is_wstring(p->info.typ))
	    fprintf(fout,"\n\treturn soap_outwliteral(soap, tag, &(((%s*)a)->%s));", t->sym->name,p->sym->name);
	  else if (table->prev)
	  {
	    if(p->info.typ->type==Tarray)
	...
	fprintf(fout,"\n\t\tif ((*i).soap_out(soap, tag, id, \"%s\"))", xsi_type_u(typ));
        ...
        fprintf(fout,"\n\t\tif (soap_outliteral(soap, tag, &(*i)))");
      else if (is_XML(n) && is_wstring(n))
        fprintf(fout,"\n\t\tif (soap_outwliteral(soap, tag, &(*i)))");
