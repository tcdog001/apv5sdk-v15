...
* All rights reserved.
*
* filename: dhcp_snooping_options.c
* description:  implementation for DHCP server option packet tools 
* 
*
* 
************************************************************************************/
...
void* get_option(dhcpMessage_t *packet, int code)
{
	int i, length;
...
int add_option_string(unsigned char *optionptr, unsigned char *string)
{
	...
	{
		printf("Option 0x%02x did not fit into the packet!", string[OPT_CODE]);
		return DHCP_FAIL;
	}
	printf("adding option 0x%02x\n", string[OPT_CODE]);
...
int add_simple_option(unsigned char *optionptr, unsigned char code, u_int32_t data)
{
	...
	if (!length) 
	{
		printf("Could not add option 0x%02x", code);
		return DHCP_FAIL;
	}
...
void attach_option(struct option_set **opt_list, struct dhcp_option *option, char *buffer, int length)
{
	...
	{
		printf("Attaching option %s to existing member of list", option->name);
	...
	{
		printf("Attaching option %s to list", option->name);
