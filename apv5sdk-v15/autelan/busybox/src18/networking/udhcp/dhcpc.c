	...
	*curr = xmalloc(sizeof("ip=255.255.255.255"));
	...
	while (*opt_name) {
		...
		if (!temp)
			goto next;
	/*
	 * Add by wangxl for option43
	   original code is :
	    *curr = xmalloc_optname_optval(temp, &dhcp_optflags[i], opt_name); 
	   option43 
	   The first Byte is type,second Byte is length of ip address in option43, Ignore them
	*/
    // original:  *curr = xmalloc_optname_optval(temp, &dhcp_optflags[i], opt_name); 
	if ( dhcp_optflags[i].code == 0x2b )
	{
	  *curr = xmalloc_optname_optval(temp+2, &dhcp_optflags[i], opt_name); 
	}
	else
	{
	  *curr = xmalloc_optname_optval(temp, &dhcp_optflags[i], opt_name);
