...
static len_and_sockaddr* remoteAddr[5];
static unsigned int remoteAddrCount = 0;
#endif
...
static void do_syslogd(void)
{
	...
	#ifdef APV5 
        	dev_log_name="/tmp/log";         // wangjianru  add for APV5  syslogd
	#endif
	...
	for (;;) {
		...
		if (FD_ISSET(sock_fd, &fds)) {
			...
			if (remoteAddr[0]) {
				if (-1 == remoteFD) {
					remoteFD = socket(remoteAddr[0]->sa.sa_family, SOCK_DGRAM, 0);
				}
				if (-1 != remoteFD) {
					/* send message to remote logger, ignore possible error */
                    int iter = 0;
                    for(; iter<remoteAddrCount; iter++)
					sendto(remoteFD, RECVBUF, i, MSG_DONTWAIT,
						&remoteAddr[iter]->sa, remoteAddr[iter]->len);
				}
			}
#endif
			RECVBUF[i] = '\0';
...
static int set_server_addr(char *host)
{
    char *pStr = NULL;
    ...
    while( pStr != NULL )
    {
        remoteAddr[remoteAddrCount++] = host2sockaddr(pStr, 514);
        pStr = strtok(NULL, ";");
    }
    return 0;
}
