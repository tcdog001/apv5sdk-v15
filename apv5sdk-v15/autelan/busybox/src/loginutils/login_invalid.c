...
prompt:
	...
	*buf++ = c;
	...
	if (!strchr(buf, '\n'))
		exit(1);
	/*begin:zengmin modify 2013-05-15*/
   //while (isgraph(*buf)) buf++;
   while (*buf != '\n') buf++;
   /*end:zengmin modify 2013-05-15*/
	*buf = '\0';
}
...
int logininvalid_main(int argc, char **argv)
{
	enum {
		LOGIN_OPT_f = (1<<0),
		LOGIN_OPT_h = (1<<1),
	...
	while (1) {
		if (!username[0])
		{
			...
			if(strncmp(username, "adminconsole", strlen(username))==0)
			{
