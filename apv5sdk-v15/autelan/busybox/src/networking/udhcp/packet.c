...
{	
	int min_len=300 ;
	int len ;
	len = sizeof(struct dhcpMessage) - MAX_OPTIONS_LEN + end_option(payload->options) + sizeof(payload->options[0]);
	return len< min_len ? min_len:len ;
	/* udhcpc package can not be relayed by some vendor.
	return sizeof(struct dhcpMessage) - MAX_OPTIONS_LEN + end_option(payload->options) + sizeof(payload->options[0]);*/ /*modified by czm */
}
