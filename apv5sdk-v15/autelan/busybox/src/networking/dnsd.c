...
static const char *anwser_interface = "8.8.8.8";
...
static int process_packet(uint8_t * buf)
{
	...
	lookup_result = 0;
	strcpy((char *)answstr, anwser_interface);
	if (lookup_result != 0) {
...
get_iface_ip(const char *ifname)
{
	...
	if ((sockd = socket (AF_INET, SOCK_PACKET, htons(0x8086))) < 0) {
		bb_info_msg("socket(): %s", strerror(errno));
		return NULL;
	}
	...
	if (ioctl (sockd, SIOCGIFADDR, &if_data) < 0) {
		bb_info_msg("ioctl(): SIOCGIFADDR %s", strerror(errno));
		return NULL;
	}
...
//	dnsentryinit();
	...
	if ((gw_address = get_iface_ip(listen_interface)) == NULL) {
		bb_info_msg("Could not get IP address information of %s, exiting...", listen_interface);
		exit(1);
	}
	bb_info_msg("%s = %s", listen_interface, gw_address);

	lsa = host2sockaddr(gw_address, port);
