		...
		const char *server_path, char *local_path)
{
    ...
    {
        servp++;
        strp = strchr(servp, '/');    // 查找到路径的位置；
        strncpy(df, servp, strp - servp);   //把路径名提取出来；            
            
    	memset(buf, 0 , sizeof(buf));
    	sprintf(buf, "MKD %"OFF_FMT"s", dbuf);
