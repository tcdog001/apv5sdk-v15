...
#define IPV6_ADDR_STRING_LEN 47
//#define IPV6_SAVE_ADDR_PATH "/tmp/ac-addr2"
#define IPV6_DNS_SERVER_PATH  _PATH_SAVE_DNS_SERVER_ADDRESS

#define OPTION_52_TYPE_ADDR	0x11
#define OPTION_52_TYPE_LEN	2
#define OPTION_52_DATA_LEN	2




struct option *capwap_option = NULL;
extern const char *path_save_ipv6_addr;
...
dhcpv6_client_assignments(void)
{
	...
	/***************************add by zhl*************************************/
	#if 0
	code = D6O_CAPWAP_AC_V6;
	if (!option_code_hash_lookup(&capwap_option, dhcpv6_universe.code_hash,
				     &code, 0, MDL))
		log_fatal("Unable to find the CAPWAP option definition.");
	#endif
	/*************************************end*********************************/
...
/*****************************************add by zhl***********************************************/
/*
 * add parse function for option52
 */
#if 0
void print_IPv6_addr(char IPv6_addr[][IPV6_ADDR_LEN], int len)
{
	int i = 0;
	for(i = 0; i < len; i++)
	{
...
#endif
int write_conf_file(char IPv6_addr[][IPV6_ADDR_STRING_LEN], int len)
{
	int i = 0;
	FILE *fp = NULL;
	//
	fp = fopen(path_save_ipv6_addr, "w+");
	if(fp == NULL)
	{
	...
	for(i = 0; i < len; i++)
	{
		...
		{
			fclose(fp);
			fprintf(stderr, "fprintf:%s", strerror(errno));
			return -1;
		}
	}
	//	
...
//
int IPv6_get_optiondata_length(const unsigned char *option_data)
{
	return option_data[0]*256+option_data[1];
}
#if 0
static isc_result_t dhc6_parse_capwap_ac_addr(const unsigned char *option_data, u_int16_t num)
{
	char IPv6_addr[num][IPV6_ADDR_STRING_LEN];
	
	int i = 0;
	char *p = NULL;
		//starting from the third byte of data, 
		//because this is a convention of option52.
		//for more information see option52 document please.
		option_data = option_data + OPTION_52_DATA_LEN + OPTION_52_TYPE_LEN;
		log_debug("RCV:  | X-- IPv6 address number: %u.", num);
		for(i = 0; i < num ; i++)
		{
#if 1
				sprintf(IPv6_addr[i], "%.2x%.2x:%.2x%.2x:%.2x%.2x:%.2x%.2x:%.2x%.2x:%.2x%.2x:%d.%d.%d.%d",
						*option_data, 
						*(option_data+1),
						*(option_data+2),
						*(option_data+3),
				...
				p = inet_ntop(AF_INET6, addr, addr1, sizeof(addr1));
				log_debug("RCV:  | | X-- IPv6 address: %s", IPv6_addr[i]);

				option_data += IPV6_ADDR_LEN;
#endif
		}
		//
		//print_IPv6_addr(IPv6_addr, num);
		//write the IPv6 address to the configure file
		write_conf_file(IPv6_addr, num);
		system("touch /tmp/dhcpflag");			
...
void test_option52()
{
	unsigned char p[66] = {0,};
		...
		p[64] = 61;
		p[65] = 4;
		printf("option52_length = %d\n", IPv6_get_optiondata_length(p));
		dhc6_parse_capwap_ac_addr(p, 4);
}
#endif
#if 0
static isc_result_t
dhc6_parse_capwap_ac_v6(struct dhc6_ia **pia, struct packet *packet,
		 struct option_state *options)
{
	//test_option52();
	...
	struct dhc6_capwap_ac *capwap;
	struct option_cache *oc;
	isc_result_t result;

	memset(&ds, 0, sizeof(ds));

	oc = lookup_option(&dhcpv6_universe, options, D6O_CAPWAP_AC_V6);
	for ( ; oc != NULL ; oc = oc->next) {
		if(evaluate_option_cache(&ds, packet, NULL, NULL,
			 ...
			 if((ds.len - OPTION_52_TYPE_LEN - OPTION_52_DATA_LEN) % IPV6_ADDR_LEN != 0)
			 {
			 	log_error("Invalid OPTION52 option cache.");
			 	return ISC_R_UNEXPECTED;
			 }
			 unsigned char buf[ds.len+1];
			 memcpy(buf, ds.data, ds.len*sizeof(char));
			 buf[ds.len] = '\0';
			 if(buf[1] == OPTION_52_TYPE_ADDR)
			 	dhc6_parse_capwap_ac_addr(ds.data, (unsigned) (buf[2]*256 + buf[3]));
...
#endif
//modify by zhl 2010-09-01
static isc_result_t dhc6_parse_capwap_ac_addr(const unsigned char *option_data, u_int8_t num)
{
	unsigned char IPv6_addr[num][IPV6_ADDR_STRING_LEN];
	unsigned char addr1[sizeof("ffff:ffff:ffff:ffff:ffff:ffff:255.255.255.255")] = {0,};
	unsigned char addr[sizeof("ffff:ffff:ffff:ffff:ffff:ffff:255.255.255.255")] = {0,};
	int i = 0;
	char *p = NULL;
		//starting from the third byte of data, 
		//because this is a convention of option52.
		//for more information see option52 document please.
		log_debug("RCV:  | X-- [Options52]IPv6 address number: %u.", num);
		for(i = 0; i < num ; i++)
		{	
#if 1
				memcpy(addr1, option_data +(i * IPV6_ADDR_LEN), IPV6_ADDR_LEN * sizeof(char));
				p = inet_ntop(AF_INET6, addr1, addr, sizeof(addr));
				strcpy(IPv6_addr[i], p);
				log_debug("RCV:  | | X-- IPv6 address: %s", addr);
#endif
		}
		//
		//print_IPv6_addr(IPv6_addr, num);
		//write the IPv6 address to the configure file
		write_conf_file(IPv6_addr, num);
		//system("touch /tmp/dhcpflag");//cerate dhcpflag in dhclient-script
		 ...
		 struct option_state *options)
{
	...
	for ( ; oc != NULL ; oc = oc->next) {
		if(evaluate_option_cache(&ds, packet, NULL, NULL,
						 ...
						 &global_scope, oc, MDL) &&
			   ds.len >= 16) {
			 if((ds.len < 1) || (ds.len % IPV6_ADDR_LEN) != 0)
			 {
			 	log_error("Invalid OPTION52 option cache.");
				if(ds.len != 0)
					data_string_forget(&ds, MDL);
			 ...
			 memcpy(buf, ds.data, ds.len*sizeof(char));
			 buf[ds.len] = '\0';
			 dhc6_parse_capwap_ac_addr(ds.data, ds.len/IPV6_ADDR_LEN);
			data_string_forget(&ds, MDL);
			continue;
		}
		}
...
/********************************parse dns**************************************************/
static isc_result_t
dhc6_parse_dns(struct packet *packet, struct option_state *options)
{
struct data_string ds;
	struct dhc6_capwap_ac *capwap;
	struct option_cache *oc;
	...
	memset(&ds, 0, sizeof(ds));
	oc = lookup_option(&dhcpv6_universe, options, D6O_NAME_SERVERS);
	for ( ; oc != NULL ; oc = oc->next) {
		if(evaluate_option_cache(&ds, packet, NULL, NULL,
						 options, NULL,
						 &global_scope, oc, MDL) &&
			   ds.len >= IPV6_ADDR_LEN) {
			   int i = 0;
			   int n = 0;
			   n = ds.len / IPV6_ADDR_LEN;
			   unsigned char buf[ds.len+1];
			   unsigned char addr[IPV6_ADDR_LEN] = {0,};
			   unsigned char addr1[sizeof("ffff:ffff:ffff:ffff:ffff:ffff:255.255.255.255")] = {0,};
			   memcpy(buf, ds.data, ds.len*sizeof(char));
			   char *p = NULL;
			   FILE *fp = NULL;
			   //
			   fp = fopen(IPV6_DNS_SERVER_PATH, "w");
			   if(fp == NULL)
			   {
				   fprintf(stderr, "fopen:%s", strerror(errno));
				   return ISC_R_UNEXPECTED;
			   ...
			   //
			   log_debug("RCV:  | X-- IPv6 DNS server address number: %u.", n);
			   for(i = 0; i < n; i++)
			   {
			   		memcpy(addr, buf +(i * IPV6_ADDR_LEN), IPV6_ADDR_LEN * sizeof(char));
					p = inet_ntop(AF_INET6, addr, addr1, sizeof(addr1));
					if(fprintf(fp, "nameserver %s\n", addr1) == -1)
				    {
					   fclose(fp);
					   fprintf(stderr, "fprintf:%s", strerror(errno));
					   data_string_forget(&ds, MDL);
					...
					log_debug("RCV:  | | X-- IPv6 DNS server address: %s.", p);
			   }
			   fclose(fp);
			data_string_forget(&ds, MDL);
			continue;
...
dhc6_leaseify(struct packet *packet)
{
	...
	/************add by zhl*************************/
	/*
	 * Dig into recursive DHCPv6 pockets for CAPWAP_AC 
	 */
	if(dhc6_parse_capwap_ac_v6(&lease->bindings, packet, lease->options) != ISC_R_SUCCESS)
	{
		dhc6_lease_destroy(&lease,MDL);
		return NULL;
	}
	/************add by zhl*************************/
	/*
	 * Dig into recursive DHCPv6 pockets for DNS SERVERS 
	 */
	if(dhc6_parse_dns(packet, lease->options) != ISC_R_SUCCESS)
	{
		dhc6_lease_destroy(&lease,MDL);
		return NULL;
	}
		     ...
		     struct dhc6_lease *lease, u_int8_t message)
{
	...
	#if 0
	if ((oc = lookup_option(&dhcpv6_universe, *op,
				D6O_CAPWAP_AC_V6)) == NULL) {
		if (!option_cache(&oc, &default_duid, NULL, capwap_option,
				  MDL))
			log_fatal("Failure assembling a capwap_ac_v6.");
	 ...
	 * 建立和保存一个ORO(选项请求选项),这是一个致命的错误无法发送一个0长度的ORO
	 ...
	 * 讨论: RFC3315 显示出ORO的声明是否采用强制性，有不一样的看法。
	 *
