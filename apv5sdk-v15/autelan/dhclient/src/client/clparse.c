...
#define NUM_DEFAULT_REQUESTED_OPTS	10				// default 9  modify by zhl
...
isc_result_t read_client_conf ()
{
	...
	/* 10 */
	/*
	 * add option52 option for default
	 */
	#if 1
	...
	int jj = 0;
	for(jj = 0; jj < 10; jj++)
		printf("default_reqested_options[%d] = %d\n", jj,top_level_config.requested_options[jj]->code);
	#endif
	/*********************test****************************/
			   ...
			   struct client_config *client)
{
	...
	cfile = NULL;
	status = new_parse(&cfile, file, NULL, 0, path_dhclient_conf, 0);	//把文件内容映射到cfile中，填写cfile结构体
