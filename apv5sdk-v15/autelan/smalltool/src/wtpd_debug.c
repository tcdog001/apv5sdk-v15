...
int main(int argc,char **argv)
{
	...
	mysigval.sival_int = stdout_dev;
    	...
    	autelan_printf("Sender gets wtpd process id:%d\n",proc_id);
	 //main,contrl,rev_d,send_d,rrm_r,sta_r,if_r,ext_r,stat_t
	 /*   int opt = 0 ;
...
int PopenFile(char *cmd_str,char *str,int len)
{
 ...
 int GetProcID(int *proc_id)
 {
	...
	autelan_sprintf(str_cmd,"ps |awk '/wtpd/{if($5==\"wtpd\"){print $1}}' | wc -l");
	PopenFile(str_cmd,str,sizeof(str));
	autelan_printf("wtpd count:%d\n",atoi(str));
	if(atoi(str) == 0){
		autelan_printf("get wtpd process id failed!\nplease make sure wtpd has been started.\n");
		return 0;
	}

	if(PopenFile("ps | awk '/wtpd/{if($5==\"wtpd\"){print $1}}' | awk '{if(NR==1){print $1}}'",str,sizeof(str))){
		*proc_id = atoi(str);
