...
static void *cronometer(void *arg) 
{
	...
	while(1) 
	{
		...
		{ /* peiwh modified at 111220, for fixing timer bug */
			pthread_mutex_unlock(&timerq.mutex);
			continue;
		}
...
void timer_rem(int id, int isFree) {
	...
	if (t->in_use == 1) 
	{
		...
		t->cancelled = 1;
		pthread_kill(timerq.ticker, SIGALRM); 
		pthread_mutex_unlock(&timerq.mutex);
		return;
	}
...
unsigned long info_report_get_current_time()
{
...
unsigned long get_ap_start_time()
{
	char cmd[128] = {0};
	char str[64] = {0};
	unsigned long current_time = 0;
	
	if(access("/tmp/ap_start_time",0) == 0){
		sprintf(cmd, "cat /tmp/ap_start_time");
		if(popen_fgets(cmd, str, sizeof(str)) == NULL)
			return 0;
		current_time = atol(trim(str));
		return current_time;
	}else{
		struct timeval	tv;
		gettimeofday(&tv, NULL);
		...
		current_time += tv.tv_sec*1000;
		current_time = current_time/10;
		sprintf(cmd, "echo %lu  >  /tmp/ap_start_time",current_time);
		system(cmd);
		return current_time;
	}
}
