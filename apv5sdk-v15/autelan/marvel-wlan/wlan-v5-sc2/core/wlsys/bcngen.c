...
static void InitHT_IE(vmacApInfo_t *vmacSta_p,IEEEtypes_HT_Element_t *elment)
{
	...
	//modify by guijinlin for low speed begin
	elment->SupportedMCSset[0] =0xff&vmacSta_p->mcs_mask;
	//modify by guijinlin for low speed end
	...
	{
		//modify by guijinlin for low speed begin
		elment->SupportedMCSset[1] =0xff&(vmacSta_p->mcs_mask >> 8);
		//modify by guijinlin for low speed end
	}
	...
	{
		//modify by guijinlin for low speed begin
		elment->SupportedMCSset[2] =0xff&(vmacSta_p->mcs_mask >> 16);
		//modify by guijinlin for low speed end
	}
	else
	{
...
* @return Number of bytes in the probe response
*/
static void InitHT_IE_Ext(vmacApInfo_t *vmacSta_p,IEEEtypes_HT_Element_t *elment,UINT32 SupportedMCSMask)
{
#if 1 //def COMMON_PHYDSSS
	MIB_PHY_DSSS_TABLE *PhyDSSSTable=vmacSta_p->Mib802dot11->PhyDSSSTable;
	UINT8 *mib_guardInterval_p = vmacSta_p->Mib802dot11->mib_guardInterval;
...
UINT16 AddHT_IE_Ext(vmacApInfo_t *vmacSta_p,IEEEtypes_HT_Element_t* pNextElement,UINT32 SupportedMCSMask)
{
	...
	{
		InitHT_IE_Ext(vmacSta_p,pNextElement,SupportedMCSMask);
