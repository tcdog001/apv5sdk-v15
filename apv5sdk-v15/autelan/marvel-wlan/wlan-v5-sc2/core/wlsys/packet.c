...
#endif
#include "ap8xLnxWmmTraffic.h"      //lhj//2012.06
...
extern u_int32_t tunnel_fast_forward;
    ...
    while ((skb = skb_dequeue(&pStaInfo->aggr11n.txQ)) != NULL)
    {
        if(wlxmit(netdev, skb, IEEE_TYPE_DATA, pStaInfo, 0, FALSE))
        {
            ...
            if(skb->truesize > MAX_AGGR_SIZE)
            {
                ...
                Statistics_TX_Error_Packet(netdev,skb->cb[0]);
                /********************* end **************************/
        ...
        if(cap == 2)
        {
            if(CurrentFrag(q).skb->len > WL_MAX_AMSDU_SIZE_8K-netdev->mtu||CurrentFrag(q).status>*(mib->mib_amsdu_pktcnt))
            {
                ...
                if(wlxmitmfs(netdev, CurrentFrag(q).skb, pStaInfo, CurrentFrag(q).status))
                {
                    ...
                    /************* zhouke add ,Interface Statistics *************/
                    Statistics_TX_Error_Packet(netdev,CurrentFrag(q).status);
                    /********************* end **************************/
...
void user_stations_info(struct net_device *netdev, struct sk_buff *skb)
{
    struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
    vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
    struct ieee80211_frame *wh = (struct ieee80211_frame *)(skb->data + 2);
    
    extStaDb_StaInfo_t *pStaInfo = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *)(wh->addr1), 0);
    if(NULL == pStaInfo)
    {
        return;
    }
    /*
    pStaInfo->sta_stats.ns_rx_data++;
    pStaInfo->sta_stats.ns_rx_bytes += skb->len;
    */
    ...
    /*
    if(0 == wh->FrmCtl.MoreFrag && 0 == (wh->seq >> 12))
    {
        return;
    }
    
    pStaInfo->sta_stats.rx_frag_packets++;  
    */
    return;
}
...
struct sk_buff *ieee80211_encap(struct sk_buff *skb, struct net_device *netdev, BOOLEAN eap)
{
    ...
    vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
    MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
    struct ieee80211vap *vap = &(wlpptr->vap); //athx-x//lhj
    struct ether_header eh;
    struct llc *llc;
    ...
    if(!IS_GROUP((UINT8 *) &(eh.ether_dhost)))
    {
        ...
        if(*(mib->QoSOptImpl))
        {
            ...
            if(pStaInfo->IsStaQSTA)
            {
                ...
                //struct ieee80211vap *vap = &(wlpptr->vap); //athx-x
                if ((vap->priv_wmm).vlan_flag) {
                    Priority = vlan_to_wmm_map(tid, vap);
                }else if ((vap->priv_wmm).tos_flag) {
                    Priority = tos_to_wmm_map(tid, vap);
...
#endif
/*add by zhanxuechao for llc analysis*/
UINT16 ieee80211_autelan_get_4_address_packet_llc_analysis(struct sk_buff *skb)
{
    UINT16 ether_type = 0;
    struct llc *llc;
    struct llc_rptr *llc_rptr = NULL;
    //unsigned char *key_8021x = NULL;
    //UINT32 replay_cnt[2];
...
UINT16 ieee80211_autelan_get_3_address_packet_llc_analysis(struct sk_buff *skb)
{
    ...
    hdrspace = ieee80211_hdrspace(skb->data);
    
    //llc = (struct llc *) skb_pull(skb, sizeof(struct ieee80211_frame) - 6);
    llc = (struct llc *) skb_pull(skb, hdrspace);
    ...
    //skb_push(skb, sizeof(struct ieee80211_frame) - 6);
    skb_push(skb, hdrspace);

    return ether_type;
}

struct sk_buff *ieee80211_decap(struct net_device *dev, struct sk_buff *skb, extStaDb_StaInfo_t *pStaInfo)
{
...
#endif
    {
        ...
        {
            WlLogPrint(MARVEL_DEBUG_WARNING, __func__, "class3 frame from %x %d\n", pStaInfo, pStaInfo?pStaInfo->State:0);
...
#ifdef SOC_W8764
int ieee80211_input(struct net_device *dev, struct sk_buff *skb, u_int32_t rssi, u_int32_t rssiPaths, u_int8_t ampdu_qos, u_int32_t status, u_int8_t rxRate)
#else
...
#endif
{
    ...
    switch (wh->FrmCtl.Type)
    {
        case IEEE_TYPE_DATA:
            ...
            //wlpptr->netDevStats.rx_packets++;                 //zhouke add,for athors drivers rx_packet is data
            //wlpptr->netDevStats.rx_bytes += skb->len;         //zhouke add,for athors drivers rx_packet is data
            ...
            WlLogPrint(MARVEL_DEBUG_ALL, __func__,"Receive Data Packet!\n");

            /*added by chenming for rssithre*/
            if (rssi < wlpptr->vap.iv_rssi_threshold)
            {
                WlLogPrint(MARVEL_DEBUG_DEBUG, __func__,"rssi=%d < %d (vap_rssi_threshold),Send DeauthenticateMsg To Sta\n",\
                    rssi,wlpptr->vap.iv_rssi_threshold);

                macMgmtMlme_SendDeauthenticateMsg(vmacSta_p,&(wh->addr2),  0, IEEEtypes_REASON_UNSPEC);
            ...
            {
                WlLogPrint(MARVEL_DEBUG_ALL, __func__,"Data Packet is WDS!\n");
            ...
            /******* zhouke add ,Interface Statistics ***********/
            if(stationPacket == FALSE){              //add3
                Statistics_Recv_Data_Packet(dev,(UINT8 *)&(wh->addr3),skb->len);
            }
#if 0                   //AP to STA packets not statistics
            else if(stationPacket == TRUE){     //addr1
                Statistics_Recv_Data_Packet(dev,(UINT8 *)&(wh->addr1),skb->len);
            }
#endif
            /********************* end **************************/
            ...
            if ((status == (u_int32_t)GENERAL_DECRYPT_ERR) ||(status & (u_int32_t)DECRYPT_ERR_MASK)) 
            {
                ...
                Statistics_Recv_Error_packet(dev);
                /********************* end **************************/
                ...
                WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"status has a specific error bit\n");
                goto err;
            }
#ifdef AMPDU_SUPPORT
            ...
            if (!stationPacket)
            {
                ...
                if(pStaInfo)
                {
                    WlLogPrint(MARVEL_DEBUG_ALL, __func__,"Packet from %02X:%02X:%02X:%02X:%02X:%02X:\n",\
                        wh->addr2[0],wh->addr2[1],wh->addr2[2],wh->addr2[3],wh->addr2[4],wh->addr2[5]);
                    ...
                    //add by yuxiaona for stastats begin
                    pStaInfo->sta_stats.ns_tx_data++;
                    pStaInfo->sta_stats.ns_tx_bytes += skb->len;
                    //add by yuxiaona for stastats end
    
                    /************* zhouke add ,Interface Statistics *************/
                    if((ieee80211_node_is_authorize(pStaInfo))&&(wh->FrmCtl.Retry == 1))
                    {
                        Statistics_Recv_Packet_Retry(vmacSta_p,skb->len,IEEE_TYPE_DATA);
                    }
                    /********************* end **************************/
                }
                else
            ...
            if(ieee80211_autelan_traffic_limit_rx(dev, skb, pStaInfo))
            {
                WlLogPrint(MARVEL_DEBUG_DEBUG, __func__,"traffic limit rx,packet is drop\n");
                goto out;
            }
            //modify by lirui for traffic limit begin
            ...
            //add by yuxiaona for station info begin
            pStaInfo->Rate = rxRate;
            //add by yuxiaona for station info end
#endif
...
blkackoutcontinue:
#endif 
                {
                    ...
                    if(thinap)
                    {
                        ...
                        //NULL Data filter
                        //add by zhanxuechao
                        if ((wh->FrmCtl.Subtype == QoS_NULL_DATA) ||(wh->FrmCtl.Subtype == NULL_DATA))
                        {
                            WlLogPrint(MARVEL_DEBUG_ALL, __func__,"Packet is null data or qos null data\n");
                            dev_kfree_skb_any(skb);
                            skb = NULL;
                            goto out;
                        }
                        ...
                        //add by guijinlin for attack detection begin
                        attack_detection(vmacSta_p, (IEEEtypes_Frame_t *)(skb->data), (void *)rssi);
                        //add by guijinlin for attack detection end
                    
                        /* add by zhanxuechao for thinap */
                        ret = ieee80211_autelan_recv_data_frame_filter_process(skb, dev, pStaInfo);
                        if (ret == AP8X_80211_DATA_DROP)
                        {
                            goto err;
                        }
                        else if (ret == AP8X_80211_DATA_DEAUTH)
                        {
                            goto deauth;
                        }

                        frametype = ieee80211_autelan_get_4_address_packet_llc_analysis(skb);

#ifdef AE5200_RT_SUPPORT
                        ret = ieee_autelan_qos_decap(&skb,dev,frametype);
#endif

                        /*Begin:Added by pengruofeng for develop the policy of mac binding 2011-5-24*/
                        ret = ieee80211_autelan_ip_mac_binding_process(skb, pStaInfo, dev, frametype);
                        if (ret != 0)
                        {
                            goto err;
                        }
                        ...
                        /*add by zhanxuechao for thinap support*/
                        ret = ieee80211_autelan_recv_data_frame_thinap_process(&skb, dev, pStaInfo, frametype);                
                        if (ret == AP8X_THINAP_MODE_TUNNEL_SUCC)
                        {
                            goto out;
                        }
                        else if (ret == AP8X_THINAP_MODE_TUNNEL_DROP 
            ...
            if (skb == NULL)
            {
                WlLogPrint(MARVEL_DEBUG_DEBUG, __func__,"Packet decap Error!\n");
                goto err;
            }
...
//thinapout:        
        ...
        case IEEE_TYPE_MANAGEMENT:
            ...
            subtype = wh->FrmCtl.Subtype;
            //WlLogPrint(MARVEL_DEBUG_ALL, __func__, "rssi: %d\n", rssi);
            ...
            if(!stationPacket)
            {
                ...
                if(rssi < wlpptr->vap.iv_rssi_threshold)
                {
                    if (subtype == IEEE_MSG_ASSOCIATE_RQST ||
                        subtype == IEEE_MSG_REASSOCIATE_RQST ||
                        subtype == IEEE_MSG_AUTHENTICATE ||
                        subtype == IEEE_MSG_PROBE_RQST)
                    goto out;
                }
            }
            
#ifdef AE5200_RT_SUPPORT
            ret = ieee80211_autelan_recv_managent_frame_thinap_process(skb, dev, subtype,vmacSta_p,rssi);
#else
            ret = ieee80211_autelan_recv_managent_frame_thinap_process(skb, dev, subtype);
#endif
            if (ret == AP8X_THINAP_MODE_SKB_RELOCATE_FAIL && ret == AP8X_THINAP_MODE_NETIF_RX_DROP)
            {
                goto out;
            }
        ...
        case IEEE_TYPE_CONTROL:
            {
                ...
                /************* zhouke add ,Ctl Statistics *************/
                Statistics_Recv_Ctl_Packet(dev,skb->len);
                /********************* end **************************/
...
struct sk_buff *ieee80211_getmgtframe(UINT8 **frm, unsigned int pktlen)
{
...
struct sk_buff *ieee80211_getDataframe(UINT8 **frm, unsigned int pktlen)
{
    const unsigned int align = sizeof(u_int32_t);
    struct sk_buff *skb;
    unsigned int len;
...
void sendLlcExchangeID(struct net_device *dev, IEEEtypes_MacAddr_t *src)
{
    ...
    if(skb)
    {
        ...
        *(bufptr++) = 0x00;  /* LLC data */
        ...
        //((NETDEV_PRIV_P(struct wlprivate, skb->dev)))->netDevStats.rx_packets++;      //zhouke add,for athors drivers rx_packet is data
#ifdef NAPI
...
/********** zhouke add,l2 isolation is disable **********/
int Vap_L2_Isolation_Disable(struct net_device *dev,struct sk_buff *skb)
{
    struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
    vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
    MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
    ...
    struct ether_header *eh;
    extStaDb_StaInfo_t *pStaInfo = NULL;
    //extStaDb_StaInfo_t *pStaInfo1 = NULL;
    eth_StaInfo_t *ethStaInfo_p = NULL;
    ...
    if (!(skb->protocol & WL_WLAN_TYPE_STA))
    {
        ...
        ret = ieee80211_autelan_vlan_recv_mac_binding_process(skb, pStaInfo);
        if (ret != 0)
        {
            return -1;
        }
        ...
        if(IS_GROUP((UINT8 *) &(eh->ether_dhost)))
        {
            ...
            //add by zhanxuechao for multicast isolation
            if ((wlpptr->vap.iv_switch & VAP_MULTICAST_ISOLATION) == 0)
            {
            ...
            {
                WlLogPrint(MARVEL_DEBUG_DEBUG, __func__, "MULTICAST ISOLATION is enable\n");
            }
...
#endif /*CLIENT_SUPPORT*/
    {
        if(pStaInfo->State == ASSOCIATED)
        {
            //add by zhanxuechao for unist isolation
            if ((wlpptr->vap.iv_switch & VAP_UNICAST_ISOLATION) == 0) 
            {
                /* Intrabss packets get txed AP mode only*/
                if (*(mib->mib_intraBSS) && (vmacSta_p->VMacEntry.modeOfService != VMAC_MODE_CLNT_INFRA))
                {
            ...
            {
                WlLogPrint(MARVEL_DEBUG_DEBUG, __func__, "UNICAST ISOLATION is enable\n");
                return -1;
            }
        } 
        else
...
int ForwardFrame(struct net_device *dev,struct sk_buff *skb)
{
    ...
    if (!(IEEE80211_VAP_IS_NOBRIDGE_ENABLED(wlpptr->vap))) 
    {
        int retu = Vap_L2_Isolation_Disable(dev,skb);
        if(retu < 0)
        {
            //WlLogPrint(MARVEL_DEBUG_WARNING, __func__, "received err and drop!\n");
            goto err;
        }
        else if(retu == 1)
        {
    ...
    {
        WlLogPrint(MARVEL_DEBUG_DEBUG, __func__, "Vap L2 Isolation is enable\n");
    }
    cnt++;
#ifdef CLIENT_SUPPORT
    if (!(skb->protocol & WL_WLAN_TYPE_STA))
...
mprxycontinue:
#endif
    WlLogPrint(MARVEL_DEBUG_ALL, __func__, "Packet go to Linux kernel.\n");
    /*increment Rx packet counter per interface */
    //((NETDEV_PRIV_P(struct wlprivate, skb->dev)))->netDevStats.rx_packets++;          //zhouke add,for athors drivers rx_packet is data
#ifdef NAPI
    netif_receive_skb(skb);
