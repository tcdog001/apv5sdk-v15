...
struct MIB_s
{
    ...
    UINT8 mib_rxchainmask[MAX_CARDS_SUPPORT];//added by pengdecai
    UINT8 mib_txchainmask[MAX_CARDS_SUPPORT];//added by pengdecai
...
#define MAX_WDS_PORT 6 // Also in wds.c 
	...
	UINT8 mib_MaxAmpduLen[MAX_CARDS_SUPPORT][MAX_VMAC_MIB_INSTANCE]; 
	UINT8 mib_MaxAmpduNum[MAX_CARDS_SUPPORT][MAX_VMAC_MIB_INSTANCE]; 
	UINT16 mib_MaxPacketLifetime[MAX_CARDS_SUPPORT][MAX_VMAC_MIB_INSTANCE];
#endif
#ifdef MPRXY
...
static UINT8 wlanmib_initflag[2]={0};  //added by pengdecai for wlanmib initialize one time
static UINT8 global_initflag = 0;      //added by pengdecai for global variables initialize one time
static UINT8 wifi_initflag[MAX_CARDS_SUPPORT]={0}; // added by pengdecai for every wifi card initialize one time

#if 1

/*
*madified by pengdecai for  some parameters of wlanmib, wifi card and global variables only be initialized one time
*/
extern BOOLEAN mib_InitAp(MIB_802DOT11 * mib, char *addr, int phyMacId, int vMacId, int shadow)
{
    ...
    mib->mib_rxchainmask =  &wlanmib[shadow].mib_rxchainmask[phyMacId];//added by pengdecai
    mib->mib_txchainmask =  &wlanmib[shadow].mib_txchainmask[phyMacId];//added by pengdecai
	...
	mib->mib_MaxAmpduLen = &wlanmib[shadow].mib_MaxAmpduLen[phyMacId][vMacId];		
	mib->mib_MaxAmpduNum = &wlanmib[shadow].mib_MaxAmpduNum[phyMacId][vMacId];	
	mib->mib_MaxPacketLifetime = &wlanmib[shadow].mib_MaxPacketLifetime[phyMacId][vMacId];	
#endif
#ifdef MPRXY
    ...
    if(!wifi_initflag[phyMacId]) //madified by pengdecai 
    {
    ...
    if(!wifi_initflag[phyMacId])//madified by pengdecai 
    {
#ifdef IEEE80211H
#ifdef IEEE80211_DH
        mib_SpectrumMagament_p->spectrumManagement = 1;   //enable. pengdecai modify it to 1 from 0
        ...
        mib_SpectrumMagament_p->multiDomainCapability = 1;  //enable. pengdecai modify it to 1 from 0
#else
    ...
    *(mib->mib_wmmAckPolicy) = 0; // Auto
    ...
    if(!wifi_initflag[phyMacId]) //madified by pengdecai 
    {
        ...
        *(mib->mib_agingtime) = 40*10; //3 min       //zhouke add,3*60 change 400 s;
        *(mib->mib_PadAndSnap) = TRUE; 
        *(mib->mib_autochannel) = FALSE;
        *(mib->mib_MaxTxPwr) = 0x18; //madifiy 0xff to 0x18 by pengdecai

        *(mib->mib_rxAntenna) = 0; /* 0:ABC(Auto), 1:A, 5:B, 4:C 2:AB, 3:ABC */ 
        *(mib->mib_txAntenna) = 0; /* this is bitmap: 0:AB(Auto), 1:A, 2:B */ 
        *(mib->mib_rxchainmask) = 0x7; //added by pengdecai
        *(mib->mib_txchainmask) = 0x7;//added by pengdecai
#ifdef SOC_W8366
    ...
    *(mib->mib_CSMode) = LINKADAPT_CS_ADAPT_STATE_CONSERV;  // 0:CONSERV, 1:AGGR, 2:AUTO_ENABLED,3:AUTO_DISABLED  
    ...
    {  
        ...
        *(mib->QoSOptImpl)= TRUE; //zhouke modify enable wmm
    }
    ...
    *(mib->mib_AmpduTx) = 1;    //zhouke modify enable ampdutx
	*(mib->mib_MaxAmpduLen) = 0;  //0:auto, val=overwrite default value
	*(mib->mib_MaxAmpduNum) = 0;  //0:auto, val=overwrite default value
	*(mib->mib_MaxPacketLifetime) = 512;
#endif
    ...
    *(mib->mib_ampdu_mintraffic[WME_AC_VI]) = 32;
    ...
    if(!wlanmib_initflag[shadow])  //madified by pengdecai 
    {   
        *(mib->mib_ampdu_bamgmt) = 1;
    }
    if(!wifi_initflag[phyMacId])
    {    
        //madified by pengdecai  
...
#endif
#ifdef QOS_FEATURE
    if (!global_initflag)  //madified by pengdecai 
    {
    ...
    *(mib->mib_agingtimeRptr) = 3*60; //3 min
    ...
    global_initflag = 1;    
    wlanmib_initflag[shadow] = 1;
    wifi_initflag[phyMacId] = 1;
    /*End:added by pengdecai */
...
extern BOOLEAN mib_InitAp(MIB_802DOT11 * mib, char *addr, int phyMacId, int vMacId, int shadow)
{
    ...
    MIB_PHY_DSSS_TABLE *PhyDSSSTable;
    UINT8 ssid[IEEEtypes_SSID_SIZE];
//    vmacApInfo_t *sSysCfg_p; //added by pengdecai
#ifdef MPRXY
    UINT32 index=0;
#endif
    sSysCfg_p = mib->mib_sSysCfg_p;
    mib->PhyDSSSTable = &wlanmib[shadow].Mib_PhyBsssTable[phyMacId];// 1
    ...
    mib->SpectrumMagament = &wlanmib[shadow].SpectrumMagament[phyMacId]; // 2
#ifdef BRS_SUPPORT
    mib->BssBasicRateMask = &wlanmib[shadow].BssBasicRateMask[phyMacId];  // 3
    mib->NotBssBasicRateMask = &wlanmib[shadow].NotBssBasicRateMask[phyMacId]; //4
#endif
    mib->mib_RtsThresh = &wlanmib[shadow].mib_RtsThresh[phyMacId];  // 4
    mib->mib_BcnPeriod = &wlanmib[shadow].mib_BcnPeriod[phyMacId]; // 5
    mib->mib_ApMode = &wlanmib[shadow].mib_ApMode[phyMacId];    // 6
    mib->mib_shortSlotTime = &wlanmib[shadow].mib_shortSlotTime[phyMacId]; // 7
    mib->QoSOptImpl = &wlanmib[shadow].QoSOptImpl[phyMacId];   // 8
    mib->mib_rxAntenna = &wlanmib[shadow].mib_rxAntenna[phyMacId]; //9
    mib->mib_txAntenna = &wlanmib[shadow].mib_txAntenna[phyMacId]; // 10
    mib->mib_txAntenna2 = &wlanmib[shadow].mib_txAntenna2[phyMacId]; // 11
    mib->mib_CDD = &wlanmib[shadow].mib_CDD[phyMacId]; // 12
    mib->mib_wlanfilterno = &wlanmib[shadow].mib_wlanfilterno[phyMacId][vMacId];
    mib->mib_wlanfiltermac = &wlanmib[shadow].mib_wlanfiltermac[phyMacId][vMacId][0];
    mib->mib_wlanfiltertype = &wlanmib[shadow].mib_wlanfiltertype[phyMacId][vMacId];
    mib->mib_guardInterval = &wlanmib[shadow].mib_guardInterval[phyMacId]; // 13
    mib->mib_extSubCh = &wlanmib[shadow].mib_extSubCh[phyMacId]; // 14
    mib->mib_agingtime = &wlanmib[shadow].mib_agingtime[phyMacId]; // 15
    mib->mib_PadAndSnap = &wlanmib[shadow].mib_PadAndSnap[phyMacId]; // 16
    mib->mib_autochannel = &wlanmib[shadow].mib_autochannel[phyMacId]; // 17
    mib->mib_MaxTxPwr = &wlanmib[shadow].mib_MaxTxPwr[phyMacId]; // 18
    mib->PowerTagetRateTable20M = &wlanmib[shadow].PowerTagetRateTable20M[phyMacId][0];
    mib->PowerTagetRateTable40M = &wlanmib[shadow].PowerTagetRateTable40M[phyMacId][0];
    mib->PowerTagetRateTable20M_5G = &wlanmib[shadow].PowerTagetRateTable20M_5G[phyMacId][0];
    mib->PowerTagetRateTable40M_5G = &wlanmib[shadow].PowerTagetRateTable40M_5G[phyMacId][0];
    mib->mib_rifsQNum = &wlanmib[shadow].mib_rifsQNum[phyMacId]; // 19 
    mib->mib_regionCode = &wlanmib[shadow].mib_regionCode[phyMacId]; // 20
    mib->mib_maxsta = &wlanmib[shadow].mib_maxsta[phyMacId][vMacId];    
#if defined ( INTOLERANT40)  || defined (COEXIST_20_40_SUPPORT)

    mib->mib_FortyMIntolerant = &wlanmib[shadow].mib_FortyMIntolerant[phyMacId]; // 21
    mib->USER_ChnlWidth = &wlanmib[shadow].USER_ChnlWidth[phyMacId]; // 22
    mib->mib_HT40MIntoler = &wlanmib[shadow].mib_HT40MIntoler[phyMacId]; // 23
#endif
    mib->mib_htProtect = &wlanmib[shadow].mib_htProtect[phyMacId]; // 25
    mib->mib_forceProtectiondisable = &wlanmib[shadow].mib_forceProtectiondisable[phyMacId]; // 26
    mib->mib_amsdu_flushtime = &wlanmib[shadow].mib_amsdu_flushtime[phyMacId]; // 27
#ifdef WDS_FEATURE
    mib->mib_wdsEnable = &wlanmib[shadow].mib_wdsEnable[phyMacId][vMacId];
    mib->mib_wdsno = &wlanmib[shadow].mib_wdsno[phyMacId][vMacId];
    mib->mib_wdsmac = &wlanmib[shadow].mib_wdsmac[phyMacId][vMacId][0];
    ...
    mib->mib_broadcastssid = &wlanmib[shadow].mib_broadcastssid[phyMacId][vMacId];
    mib->mib_defaultkeyindex = &wlanmib[shadow].mib_defaultkeyindex[phyMacId][vMacId];
    mib->mib_enableFixedRateTx = &wlanmib[shadow].mib_enableFixedRateTx[phyMacId]; // 28
    mib->mib_FixedRateTxType = &wlanmib[shadow].mib_FixedRateTxType[phyMacId]; // 29
    mib->mib_txDataRate = &wlanmib[shadow].mib_txDataRate[phyMacId]; // 30
    mib->mib_txDataRateG = &wlanmib[shadow].mib_txDataRateG[phyMacId]; // 31
    mib->mib_txDataRateA = &wlanmib[shadow].mib_txDataRateA[phyMacId]; // 32
    mib->mib_txDataRateN = &wlanmib[shadow].mib_txDataRateN[phyMacId]; // 33
    mib->mib_MulticastRate = &wlanmib[shadow].mib_MulticastRate[phyMacId]; // 34
    mib->mib_MultiRateTxType = &wlanmib[shadow].mib_MultiRateTxType[phyMacId]; // 35
    mib->mib_ManagementRate = &wlanmib[shadow].mib_ManagementRate[phyMacId]; // 36
    mib->mib_WPAPSKValueEnabled = &wlanmib[shadow].mib_WPAPSKValueEnabled[phyMacId][vMacId];
    mib->mib_WPA2PSKValueEnabled = &wlanmib[shadow].mib_WPA2PSKValueEnabled[phyMacId][vMacId];
    mib->mib_cipherSuite = &wlanmib[shadow].mib_cipherSuite[phyMacId][vMacId];
    mib->mib_wpaWpa2Mode = &wlanmib[shadow].mib_wpaWpa2Mode[phyMacId][vMacId];
...
#ifdef RXPATHOPT
    mib->mib_RxPathOpt= &wlanmib[shadow].mib_RxPathOpt[phyMacId]; // 37 
#endif
#ifdef DYNAMIC_BA_SUPPORT
    mib->mib_ampdu_bamgmt = &wlanmib[shadow].mib_ampdu_bamgmt; // 38 for erry shadow,so need  add flag in wlanmib
    ...
    mib->mib_ampdu_low_AC_thres[0] = &wlanmib[shadow].mib_ampdu_low_AC_thres[phyMacId][0]; // 39--42
    mib->mib_ampdu_low_AC_thres[1] = &wlanmib[shadow].mib_ampdu_low_AC_thres[phyMacId][1];
    mib->mib_ampdu_low_AC_thres[2] = &wlanmib[shadow].mib_ampdu_low_AC_thres[phyMacId][2];
    mib->mib_ampdu_low_AC_thres[3] = &wlanmib[shadow].mib_ampdu_low_AC_thres[phyMacId][3];
#endif    
#ifdef COEXIST_20_40_SUPPORT
    mib->mib_Channel_Width_Trigger_Scan_Interval =&wlanmib[shadow].mib_Channel_Width_Trigger_Scan_Interval[phyMacId];// 43
    mib->mib_Channel_Transition_Delay_Factor =&wlanmib[shadow].mib_Channel_Transition_Delay_Factor [phyMacId];//44
#endif
    mib->mib_HtGreenField= &wlanmib[shadow].mib_HtGreenField[phyMacId];// 45
    mib->mib_HtStbc= &wlanmib[shadow].mib_HtStbc[phyMacId]; // 46
    mib->mib_3x3Rate = &wlanmib[shadow].mib_3x3Rate[phyMacId]; // 47
    mib->mib_RptrMode = &wlanmib[shadow].mib_RptrMode[phyMacId][vMacId];
    mib->mib_RptrDeviceType = &wlanmib[shadow].mib_RptrDeviceType[phyMacId][vMacId][0];
    mib->mib_agingtimeRptr = &wlanmib[shadow].mib_agingtimeRptr[phyMacId][vMacId];
    mib->mib_bftype = &wlanmib[shadow].mib_bftype; // 48  for erry shadow,so need  add flag in wlanmib
...
//    *(mib->mib_rxAntenna) = 4; /* 0:ABC(Auto), 1:A, 5:B, 4:C 2:AB, 3:ABC */
//    *(mib->mib_txAntenna) = 0xf; /* this is bitmap: 0:AB(Auto), 1:A, 2:B */
    ...
    *(mib->mib_3x3Rate) = 0;           //0: Disable, 1:enable
    ...
    hal_InitApMIB(mib);
#ifdef MRVL_DFS
    mib->mib_CACTimeOut = &wlanmib[shadow].mib_CACTimeOut[phyMacId]; //49
    mib->mib_NOPTimeOut = &wlanmib[shadow].mib_NOPTimeOut[phyMacId]; // 50
    *(mib->mib_CACTimeOut) = 5 ; // 5 seconds
    *(mib->mib_NOPTimeOut) = 5 ; // 5 seconds
#endif
    *(mib->mib_RptrMode) = 0;   /* 0: Disable, 1:enable */
...
function : clean the mib status when destroying the VAP.
param    : vap->ShadowMib802dot11
*******************************************************/
extern BOOLEAN mib_CleanAp(MIB_802DOT11 *mib)
{
#ifdef MPRXY
    UINT32 index=0;
#endif
    memset(mib->mib_wlanfilterno,  0, sizeof(wlanmib[0].mib_wlanfilterno[0][0]));
    memset(mib->mib_wlanfiltermac, 0, sizeof(wlanmib[0].mib_wlanfiltermac[0][0][0]));
    memset(mib->mib_wlanfiltertype,0, sizeof(wlanmib[0].mib_wlanfiltertype[0][0]));
    memset(mib->mib_maxsta, 0, sizeof(wlanmib[0].mib_maxsta[0][0]));    
#ifdef WDS_FEATURE
    memset(mib->mib_wdsEnable, 0,  sizeof(wlanmib[0].mib_wdsEnable[0][0]));
    memset(mib->mib_wdsno,0, sizeof(wlanmib[0].mib_wdsno[0][0]));
    memset(mib->mib_wdsmac, 0, sizeof(wlanmib[0].mib_wdsmac[0][0][0]));
    memset(mib->mib_wdsBcastMode , 0, sizeof(wlanmib[0].mib_wdsBcastMode[0][0])); /* indicates how broadcast pks are sent over
                                                                                 * wds, ( broadcast or unicast to other APs */
#endif // WDS_FEATURE

    memset(mib->StationConfig , 0, sizeof(wlanmib[0].StationConfig[0][0]));   /* station configuration table */
    memset(mib->AuthAlg , 0, sizeof(wlanmib[0].AuthAlg[0][0]));            /* authentication algorithms table */
    memset(mib->WepDefaultKeys , 0, sizeof(wlanmib[0].WepDefaultKeys[0][0][0]));  /* wep default keys table */
   // memset(mib->Privacy , 0, sizeof(wlanmib[0].Privacy[0][0]));         /* privacy table */

    memset(mib->Privacy, 0, sizeof(wlanmib[0].Privacy[0][0]));         /* privacy table */

    memset(mib->OperationTable , 0, sizeof(wlanmib[0].OperationTable[0][0]));
    memset(mib->SuppDataRatesTx , 0, sizeof(wlanmib[0].SuppDataRatesTx[0][0][0]));
    memset(mib->RSNConfig , 0, sizeof(wlanmib[0].RSNConfig[0][0]));
    memset(mib->UnicastCiphers , 0, sizeof(wlanmib[0].UnicastCiphers[0][0]));
    memset(mib->RSNConfigAuthSuites , 0, sizeof(wlanmib[0].RSNConfigAuthSuites[0][0]));
    memset(mib->RSNStats , 0, sizeof(wlanmib[0].RSNStats[0][0]));
    memset(mib->thisStaRsnIE, 0, sizeof(wlanmib[0].thisStaRsnIE[0][0]));
#ifdef AP_WPA2
    memset(mib->RSNConfigWPA2 , 0, sizeof(wlanmib[0].RSNConfigWPA2[0][0]));
    memset(mib->WPA2UnicastCiphers , 0, sizeof(wlanmib[0].WPA2UnicastCiphers[0][0]));
    memset(mib->WPA2UnicastCiphers2 , 0, sizeof(wlanmib[0].WPA2UnicastCiphers2[0][0]));
    memset(mib->WPA2AuthSuites , 0, sizeof(wlanmib[0].WPA2AuthSuites[0][0]));
    memset(mib->thisStaRsnIEWPA2, 0, sizeof(wlanmib[0].thisStaRsnIEWPA2[0][0]));
    memset(mib->thisStaRsnIEWPA2MixedMode, 0, sizeof(wlanmib[0].thisStaRsnIEWPA2MixedMode[0][0]));
#endif
    memset(mib->mib_broadcastssid , 0, sizeof(wlanmib[0].mib_broadcastssid[0][0]));
    memset(mib->mib_defaultkeyindex , 0, sizeof(wlanmib[0].mib_defaultkeyindex[0][0]));

    memset(mib->mib_WPAPSKValueEnabled , 0, sizeof(wlanmib[0].mib_WPAPSKValueEnabled[0][0]));
    memset(mib->mib_WPA2PSKValueEnabled , 0, sizeof(wlanmib[0].mib_WPA2PSKValueEnabled[0][0]));
    memset(mib->mib_cipherSuite , 0, sizeof(wlanmib[0].mib_cipherSuite[0][0]));
    memset(mib->mib_wpaWpa2Mode , 0, sizeof(wlanmib[0].mib_wpaWpa2Mode[0][0]));
    memset(mib->mib_intraBSS , 0, sizeof(wlanmib[0].mib_intraBSS[0][0]));
    memset(mib->pMib_11nAggrMode , 0, sizeof(wlanmib[0].mib_11nAggrMode[0][0]));
    memset(mib->mib_wmmAckPolicy , 0, sizeof(wlanmib[0].mib_wmmAckPolicy[0][0]));
    memset(mib->mib_ampdu_factor , 0, sizeof(wlanmib[0].mib_ampdu_factor[0][0]));
    memset(mib->mib_ampdu_density , 0, sizeof(wlanmib[0].mib_ampdu_density[0][0]));
    memset(mib->mib_amsdu_maxsize , 0, sizeof(wlanmib[0].mib_amsdu_maxsize[0][0]));
    memset(mib->mib_amsdu_allowsize , 0, sizeof(wlanmib[0].mib_amsdu_allowsize[0][0]));
    memset(mib->mib_amsdu_pktcnt , 0, sizeof(wlanmib[0].mib_amsdu_pktcnt[0][0]));
#ifdef INTEROP
    memset(mib->mib_interop , 0, sizeof(wlanmib[0].mib_interop[0][0]));
#endif
    memset(mib->mib_optlevel , 0, sizeof(wlanmib[0].mib_optlevel[0][0]));
    memset(mib->mib_RateAdaptMode , 0, sizeof(wlanmib[0].mib_RateAdaptMode[0][0]));
    memset(mib->mib_CSMode , 0, sizeof(wlanmib[0].mib_CSMode[0][0]));
#ifdef WDS_FEATURE
    for (index=0; index<MAX_WDS_PORT; index ++)
        memset(mib->mib_WdsMacAddr[index] , 0, sizeof(wlanmib[0].mib_WdsMacAddr[0][0][index]));
#endif
    memset(mib->mib_strictWepShareKey , 0, sizeof(wlanmib[0].mib_strictWepShareKey[0][0]));
    memset(mib->mib_MrvlRSN_GrpKey , 0, sizeof(wlanmib[0].mib_MrvlRSN_GrpKey[0][0]));
    memset(mib->mib_ErpProtEnabled , 0, sizeof(wlanmib[0].mib_ErpProtEnabled[0][0]));
    memset(mib->mib_disableAssoc , 0, sizeof(wlanmib[0].mib_disableAssoc[0][0]));
#ifdef PWRFRAC
    memset(mib->mib_TxPwrFraction , 0, sizeof(wlanmib[0].mib_TxPwrFraction[0][0]));
#endif
    memset(mib->mib_psHtManagementAct , 0, sizeof(wlanmib[0].mib_psHtManagementAct[0][0]));
#ifdef CLIENT_SUPPORT
    memset(mib->mib_STAMacCloneEnable , 0, sizeof(wlanmib[0].mib_STAMacCloneEnable[0][0]));
#endif
#ifdef AMPDU_SUPPORT
    memset(mib->mib_AmpduTx , 0, sizeof(wlanmib[0].mib_AmpduTx[0][0]));
#endif
#ifdef MPRXY
    memset(mib->mib_MCastPrxy , 0, sizeof(wlanmib[0].mib_MCastPrxy[0][0]));
    memset(mib->mib_IPMcastGrpCount , 0, sizeof(wlanmib[0].mib_IPMcastGrpCount[0][0]));

    for (index=0; index<MAX_MULTICAST_ADDRESS; index ++)
    {
        memset(mib->mib_IPMcastGrpTbl[index] , 0, sizeof(wlanmib[0].mib_IPMcastGrpTbl[0][0][index]));
        memset(mib->mib_IPMFilteredAddress[index] , 0, sizeof(wlanmib[0].mib_IPMFilteredAddress[0][0][index]));
    }

    memset(mib->mib_IPMFilteredAddressIndex , 0, sizeof(wlanmib[0].mib_IPMFilteredAddressIndex[0][0]));
#endif

    memset(mib->mib_Rssi , 0, sizeof(wlanmib[0].mib_Rssi[0][0]));

#ifdef DYNAMIC_BA_SUPPORT
    memset(mib->mib_ampdu_mintraffic[0] , 0, sizeof(wlanmib[0].mib_ampdu_mintraffic[0][0][0]));
    memset(mib->mib_ampdu_mintraffic[1] , 0, sizeof(wlanmib[0].mib_ampdu_mintraffic[0][0][1]));
    memset(mib->mib_ampdu_mintraffic[2] , 0, sizeof(wlanmib[0].mib_ampdu_mintraffic[0][0][2]));
    memset(mib->mib_ampdu_mintraffic[3] , 0, sizeof(wlanmib[0].mib_ampdu_mintraffic[0][0][3]));
#endif    

    memset(mib->mib_RptrMode , 0, sizeof(wlanmib[0].mib_RptrMode[0][0]));
    memset(mib->mib_RptrDeviceType , 0, sizeof(wlanmib[0].mib_RptrDeviceType[0][0][0]));
    memset(mib->mib_agingtimeRptr , 0, sizeof(wlanmib[0].mib_agingtimeRptr[0][0]));
