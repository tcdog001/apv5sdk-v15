...
UINT8 isMacAccessList(vmacApInfo_t *vmacSta_p, IEEEtypes_MacAddr_t *destAddr_p)
{
	...
	for ( i = 0;i < *mib_wlanfilterno_p;i++ )
	{
		if ( memcmp(destAddr_p, mib->mib_wlanfiltermac + i*6, 6) == 0 )
		{
			...
			{
			    WlLogPrint(MARVEL_DEBUG_DEBUG, __func__,"FILTER_MODE find mac %02X:%02X:%02X:%02X:%02X:%02X\n",\
                   *(destAddr_p)[0],*(destAddr_p)[1],*(destAddr_p)[2],*(destAddr_p)[3],*(destAddr_p)[4],*(destAddr_p)[5]);
				return FAIL;
			}
			return SUCCESS;
		}
