...
void receiveWlanMsg(struct net_device *dev, struct sk_buff *skb, UINT32 rssi, BOOLEAN stationpacket)
{
	...
	case IEEE_TYPE_MANAGEMENT:
		{
			//add by guijinlin for attack detection begin
			attack_detection(vmacSta_p, wlanMsg_p, (void *)rssi);
			//add by guijinlin for attack detection end
...
/*zhaoyang add for priority join in 5G*/
            if(join5g_enable)
            {
                if((wlanMsg_p->Hdr.FrmCtl.Subtype == IEEE_MSG_PROBE_RQST) ||
                   ...
                   (wlanMsg_p->Hdr.FrmCtl.Subtype == IEEE_MSG_ASSOCIATE_RQST)
                   )
                {
                    int ret = ieee80211_autelan_5G_priority_enable(vmacSta_p,(UINT8 *)&wlanMsg_p->Hdr.Addr1,rssi,wlanMsg_p->Hdr.FrmCtl.Subtype);
                    if(ret == 0)
                    {
                    ...
                    {
                        WlLogPrint(MARVEL_DEBUG_WARNING, __func__, "5G priority failed\n");
                    }
                }
            }
