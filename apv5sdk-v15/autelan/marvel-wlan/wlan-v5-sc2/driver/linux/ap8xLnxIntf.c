...
#include "wllog.h"
#include "ap8xLnxWmmTraffic.h"  //add for autelan  wmm//lhj//2012.06
...
/** external functions **/
extern int ap8x_stat_proc_register(struct net_device *dev,int index);
extern int ap8x_dump_proc_register(struct net_device *dev);
extern int ap8x_stat_proc_unregister(struct net_device *dev,int index);
...
extern vmacApInfo_t * Mac_Init(struct wlprivate *wlp, struct net_device *dev,char *addr, UINT32 mode, int phyMacId,int index);
/************************* end ******************************/

/*Begin:added by pengdecai for txpower*/
extern int load_txpower_table(struct net_device *netdev,const char* pwrtable);
...
static int __devinit wlprobe(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    WlLogPrint(MARVEL_DEBUG_DEBUG, __func__,"%s: %s: mem=0x%lx, irq=%d, ioBase0=%x, ioBase1=%x\n",
...
{
	struct wlprivate_data *wlpd_p = (struct wlprivate_data *) data;
	...
	if (thinap)
	{
		if (thinap_state == 0)
		{
			wlpd_p->ic_wtpd_init_timer++;
			thinap_check_timer = 0;

			if (wlpd_p->ic_wtpd_init_timer >= 66)  //Modified by duanmingzhe change 60 to 66
			{
...
//#endif
	...
	mod_timer(&(wlpd_p->monitor_process_timer), jiffies + HZ * 10);
}
...
int wlInit(struct net_device *netdev, u_int16_t devid)
{
    ...
    printk("Mac_Init \n");
#endif
    wlpptr->vmacSta_p = Mac_Init(NULL, netdev, &wlpptr->hwData.macAddr[0], WL_OP_MODE_AP, wlinitcnt,0);
    ...
    ap8x_stat_proc_register(netdev,0);
#endif
#ifdef AP8X_DUMP
    ap8x_dump_proc_register(netdev);
#endif

//Add by zhanxuechao for sysctrl
/*Begin:Added by zhanxuechao for thinap proc*/
    ap8x_dynamic_sysctl_register_thinap(wlpptr);
/*End:Added by zhanxuechao for thinap proc*/

    if(wlinitcnt == 0)
    {
        init_timer(&wlpptr->wlpd_p->monitor_process_timer);
        wlpptr->wlpd_p->monitor_process_timer.function = monitor_process_timer_fn;
        wlpptr->wlpd_p->monitor_process_timer.data = (unsigned long)wlpptr->wlpd_p;
        wlpptr->wlpd_p->monitor_process_timer.expires=jiffies + HZ/10;
        add_timer(&wlpptr->wlpd_p->monitor_process_timer);
    }

    wlpptr->wlpd_p->ic_wtpd_init_timer = 0;
/*Begin:added by pengdecai for txpower*/
#define PWR_TABLE_PATH ("/jffs/txpwrtbl.ini")
    load_txpower_table(netdev,PWR_TABLE_PATH);
/*End:added by pengdecai for txpower*/
...
int wlDeinit(struct net_device *netdev)
{
...
#ifdef AP8X_STATISTICS
    ap8x_stat_proc_unregister(netdev,0);
#endif
#ifdef AP8X_DUMP
    ap8x_dump_proc_unregister(netdev);
#endif

    //add by zhanxuechao for sysctl
    ap8x_dynamic_sysctl_unregister_thinap(wlpptr);
    ...
    del_timer(&wlpptr->wlpd_p->monitor_process_timer);
...
static int wlopen(struct net_device *netdev)
{
    ...
    memset(&wlpptr->stats_info, 0x00, sizeof(wlan_stats_info));                 // zhouke add ,init stats_info
...
static int wlstop(struct net_device *netdev)
{
    ...
    if (netdev->flags & IFF_RUNNING)
    {
        ...
        {
            WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"disabling AP bss failed");
        }
        if (wlFwSetRadio(netdev, WL_DISABLE, WL_AUTO_PREAMBLE))
        {
            WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"disabling rf failed");
        }
        wlInterruptDisable(netdev);
    }
...
static int wlsetMacAddr(struct net_device *netdev, void *addr)
{
    ...
    if(NULL == wlpptr)
    {
        printk("%s: Cannot find wlprivate data\n\n",netdev->name);
        return 0;
    }
    ...
    if(NULL == vmacSta_p)
    {
        printk("%s: Cannot find vmacSta_p\n\n",netdev->name);
        return 0;
    }
    ...
    { 
        if (netdev->flags & IFF_RUNNING)
		{
		    printk("%s: Cannot set mac/laa address while interface is open\n\n",netdev->name);
		    return 0;
		}
        
        memcpy(netdev->dev_addr, &macAddr->sa_data[0], 6);
        memcpy(&wlpptr->hwData.macAddr[0], &macAddr->sa_data[0], 6);

        memset(vmacSta_p->VMacEntry.vmacAddr, 0, 6);
        memcpy(vmacSta_p->VMacEntry.vmacAddr, &macAddr->sa_data[0], 6);
        
        vmacSta_p->ShadowMib802dot11->OperationTable->StaMacAddr[0] = macAddr->sa_data[0];
        vmacSta_p->ShadowMib802dot11->OperationTable->StaMacAddr[1] = macAddr->sa_data[1];
        vmacSta_p->ShadowMib802dot11->OperationTable->StaMacAddr[2] = macAddr->sa_data[2];
        vmacSta_p->ShadowMib802dot11->OperationTable->StaMacAddr[3] = macAddr->sa_data[3];
        vmacSta_p->ShadowMib802dot11->OperationTable->StaMacAddr[4] = macAddr->sa_data[4];
        vmacSta_p->ShadowMib802dot11->OperationTable->StaMacAddr[5] = macAddr->sa_data[5]; 
        vmacSta_p->Mib802dot11->OperationTable->StaMacAddr[0] = macAddr->sa_data[0];
        vmacSta_p->Mib802dot11->OperationTable->StaMacAddr[1] = macAddr->sa_data[1];
        vmacSta_p->Mib802dot11->OperationTable->StaMacAddr[2] = macAddr->sa_data[2];
        vmacSta_p->Mib802dot11->OperationTable->StaMacAddr[3] = macAddr->sa_data[3];
        vmacSta_p->Mib802dot11->OperationTable->StaMacAddr[4] = macAddr->sa_data[4];
        vmacSta_p->Mib802dot11->OperationTable->StaMacAddr[5] = macAddr->sa_data[5];       

        memcpy(&vmacSta_p->macStaAddr, &macAddr->sa_data[0], 6);
        memcpy(&vmacSta_p->macBssId, &macAddr->sa_data[0], 6);
        memcpy(&vmacSta_p->macBssId2, &macAddr->sa_data[0], 6);
...
static int wlsetMacAddr(struct net_device *netdev, void *addr)
{
    ...
    if (NULL == netdev || NULL == macAddr)
        return -EADDRNOTAVAIL;
    wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	if (is_valid_ether_addr(macAddr->sa_data))
	{
        WlLogPrint(MARVEL_DEBUG_DEBUG, __func__,"%02x:%02x:%02x:%02x:%02x:%02x\n",\
        macAddr->sa_data[0],macAddr->sa_data[1],macAddr->sa_data[2],macAddr->sa_data[3],macAddr->sa_data[4],macAddr->sa_data[5]);
        ...
        if((memcmp(netdev->name,"wifi0",5) == 0))
        {
            wifi_flag = 0;
        }
        else
		...
		DisableMacMgmtTimers(wlpptr->vmacSta_p);
		MacMgmtMemCleanup(wlpptr->vmacSta_p);
		disable_wids_timer(wlpptr->vmacSta_p);
        wlDestroySysCfg(wlpptr->vmacSta_p);

        memcpy(netdev->dev_addr, &macAddr->sa_data[0], 6);
        memcpy(&wlpptr->hwData.macAddr[0], &macAddr->sa_data[0], 6);

        wlpptr->vmacSta_p = Mac_Init(NULL, netdev, &macAddr->sa_data[0], WL_OP_MODE_AP, wifi_flag);
...
int wlreset(struct net_device *netdev)
{
    ...
    {
        WlLogPrint(MARVEL_DEBUG_ALL, __func__,"Stop client netdev = %x \n", (int)wlpptr->txNetdev_p);
    ...
    if (wlFwSetAPBss(netdev, WL_DISABLE))
    {
        WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"disable AP bss failed");
        //if fw stop responding, do not block fw download
        //goto err_fw_cmd;
    }
    if (wlFwSetRadio(netdev, WL_DISABLE, WL_AUTO_PREAMBLE))
    {
        WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"disable rf failed");
        //if fw stop responding, do not block fw download
        //goto err_fw_cmd;
    }
    wlInterruptDisable(netdev);
    ...
    {
        WlLogPrint(MARVEL_DEBUG_PANIC, __func__,"loading firmware failed");
        goto err_fw_cmd;
    }
#endif
#endif
    ...
    if(wlFwApplySettings(netdev))
    {
        WlLogPrint(MARVEL_DEBUG_PANIC, __func__,"Settings Apply failed");
        return -EIO;
    }
...
static int wlreloadFirmwareAndInitDescriptors(struct net_device *netdev)
{
    ...
    if (wlFwDownload(netdev))
    {
        WlLogPrint(MARVEL_DEBUG_PANIC, __func__,"download firmware failed");
        return FAIL;
    }
#endif
    if (wlTxRingAlloc(netdev) == 0)
    {
        ...
        {
            wlTxRingFree(netdev);
            WlLogPrint(MARVEL_DEBUG_PANIC, __func__,"init TX failed");
            return FAIL;
        }
    }
    else
    {
        WlLogPrint(MARVEL_DEBUG_PANIC, __func__,"allocate TX failed");
        return FAIL;
    }
    ...
    if (wlRxRingAlloc(netdev) == 0)
    {
        if (wlRxRingInit(netdev) != 0)
        {
            wlRxRingFree(netdev);
            wlTxRingFree(netdev);
            WlLogPrint(MARVEL_DEBUG_PANIC, __func__,"init RX failed");
            return FAIL;
        }
    } 
    else
    {
        WlLogPrint(MARVEL_DEBUG_PANIC, __func__,"allocate RX failed");
        return FAIL;
    }
#ifdef MFG_SUPPORT
    if (!wlpptr->mfgEnable)
#endif
    {
        if (wlFwGetHwSpecs(netdev))
        {
            ...
            wlTxRingCleanup(netdev);
            wlTxRingFree(netdev);
            WlLogPrint(MARVEL_DEBUG_PANIC, __func__,"failed to get HW specs");
            return FAIL;
        }
    }
    ...
    if (wlFwSetHwSpecs(netdev))
    {
        WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"failed to set HW specs");
    }
...
static int wlstop_mbss(struct net_device *netdev)
{
    ...
    DisableMacMgmtTimers(wlpptr->vmacSta_p);
    //WlLogPrint(MARVEL_DEBUG_ALL, __func__,"Stop mbss name = %s \n", netdev->name);
...
int wlInit_mbss(struct wlprivate *wlp, unsigned char * macAddr)
{
    //  int retCode;
    struct wlprivate *wlpptr = NULL;
    struct ieee80211vap *vap;//lhj//2012.06
    /*guijinlin add for attack_detection 2012-07-03 start */
    extern int wids_init(vmacApInfo_t *vap);
    /*guijinlin add for attack_detection 2012-07-03 end */
#if LINUX_VERSION_CODE >=KERNEL_VERSION(2,6,25)
    struct net_device *dev;
    ...
    wlpptr->vmacSta_p = Mac_Init((void *)wlp, wlpptr->netDev, &macAddr[0], WL_OP_MODE_VAP, wlinitcnt,wlp->wlpd_p->vmacIndex);
    if(wlpptr->vmacSta_p == NULL)
    {
        printk(KERN_ERR  "%s: failed to init driver mac\n", wlpptr->netDev->name);
        goto err_out;
    ...
    //add by zhanxuechao for procsys
    ap8x_dynamic_vap_sysctl_register_thinap(wlpptr);
    ...
    /*Begin:Added by zhanxuechao for rssi*/
    vap->iv_rssi_threshold = 0;
    /*End:Added by zhanxuechao for rssi*/
    
    //add begin for autelan wmm,traffic_limit init //lhj//2012.06
    autelan_wmm_init(vap);  //add for autelan wmm init
    autelan_traffic_limit_init(&(vap->iv_tl));//add for autelan traffic_limit init
    autelan_traffic_limit_init(&(vap->iv_everynode_tl));//add for autelan traffic_limit init
    //add end for autelan wmm,traffic_limit init
...
#endif
        /* add by zhanxuechao for delete wiress proc */
        ap8x_dynamic_vap_sysctl_unregister_thinap(wlp);
...
int wlInit_client(struct wlprivate *wlp, unsigned char * macAddr_p, unsigned char *ApRootmacAddr_p)
{
    ...
    WLDBG_ENTER(DBG_LEVEL_2);
    {
        ...
        wlpptr->vmacSta_p = Mac_Init(wlp, wlpptr->netDev, macAddr_p, WL_OP_MODE_VSTA, wlinitcnt,wlp->wlpd_p->vmacIndex);
        if(wlpptr->vmacSta_p == NULL)
        {
            printk(KERN_ERR  "%s: failed to init driver mac\n", wlpptr->netDev->name);
            goto err_out;
        ...
        wlpptr->wlpd_p = wlp->wlpd_p;
        wlpptr->master= wlp->netDev;
        wlp->vdev[MAX_VMAC_INSTANCE_AP] = wlpptr->netDev;
        //wlp->vdev[wlp->wlpd_p->vmacIndex++] = wlpptr->netDev;
#if LINUX_VERSION_CODE >=KERNEL_VERSION(3,0,6)
#else
        atomic_set(&wlpptr->netDev->refcnt, 0);
...
#ifdef AP8X_STATISTICS
        ap8x_stat_proc_register(wlpptr->netDev,MAX_VMAC_INSTANCE_AP);
#endif
...
err_init:
#ifdef AP8X_STATISTICS
    ap8x_stat_proc_unregister(wlpptr->netDev,MAX_VMAC_INSTANCE_AP);
#endif
    //      wlRxRingCleanup(netdev);
err_out:
err_register_netdev:
...
#ifdef AP8X_STATISTICS
        ap8x_stat_proc_unregister(wlpptr->vdev[i],MAX_VMAC_INSTANCE_AP);
#endif
...
int wlInit_vap(struct wlprivate *wlp,unsigned char *macaddr,unsigned char *name,unsigned int index)
{
    ...
    if((wlp == NULL) || (macaddr == NULL) || (name == NULL))
    {
        WlLogPrint(MARVEL_DEBUG_PANIC, __func__,"netdev is NULL");
        return -1;
    }
    
    dev = alloc_netdev(sizeof(struct wlprivate), name, wlwlan_setup);
    if(dev) 
    {
        wlpptr = NETDEV_PRIV(struct wlprivate, dev);
        NETDEV_PRIV_S( dev) = wlpptr;
    ...
    //sprintf(wlpptr->netDev->name, "ap%1d", wlp->wlpd_p->vmacIndex);
    //sprintf(wlpptr->netDev->name, DRV_NAME_VMAC, wlinitcnt,  wlp->wlpd_p->vmacIndex);
    memcpy(wlpptr->netDev->name,name,strlen(name));
    wlpptr->netDev->irq    = wlp->netDev->irq;
    wlpptr->netDev->mem_start = wlp->netDev->mem_start;
    wlpptr->netDev->mem_end  = wlp->netDev->mem_end;
    NETDEV_PRIV_S( wlpptr->netDev)   = wlpptr;
    ...
    memcpy(wlpptr->netDev->dev_addr, &macaddr[0], 6);
    memcpy(&wlpptr->hwData.macAddr[0], &macaddr[0], 6);
    wlpptr->vmacSta_p = Mac_Init((void *)wlp, wlpptr->netDev, &macaddr[0], WL_OP_MODE_VAP,wlp->vmacSta_p->VMacEntry.phyHwMacIndx,index);
    if(wlpptr->vmacSta_p == NULL)
    {
        printk(KERN_ERR  "%s: failed to init driver mac\n", wlpptr->netDev->name);
        goto err_out;
    ...
    if(wlp->vdev[index] == NULL)
    {
        wlp->vdev[index] = wlpptr->netDev;
        wlp->wlpd_p->vmacIndex++;
    }
    else
    {
        WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"ath[%d] is exist!\n");
        return -1;
    }
    //wlp->vdev[wlp->wlpd_p->vmacIndex++] = wlpptr->netDev;
#if LINUX_VERSION_CODE >=KERNEL_VERSION(3,0,6)
    ...
    rtnl_lock();
#ifdef AP8X_STATISTICS
    ap8x_stat_proc_register(wlpptr->netDev,index);
#endif
    //add by zhanxuechao for procsys
    ap8x_dynamic_vap_sysctl_register_thinap(wlpptr);
    vap = &(wlpptr->vap);//ath.x-x
    ...
    autelan_traffic_limit_init(&(vap->iv_everynode_tl));//add for autelan traffic_limit init
    //add end for autelan wmm,traffic_limit init
    wlpptr->vmacSta_p->StopTraffic = FALSE;
    wlpptr->netDev->mtu = wlp->netDev->mtu;
    memcpy(wlpptr->netDev->dev_addr, macaddr, 6);
#ifdef WDS_FEATURE
    wlInit_wds(wlpptr);
...
int wlDeinit_vap(struct net_device *netdev,struct net_device *vapdev)
{
...
#endif
    {
        ...
        //if(memcmp(wlpptr->vdev[i]->name,devname,strlen(devname)) != 0)
        if(wlpptr->vdev[i] != vapdev)
        {
            continue;
        }
        
        WlLogPrint(MARVEL_DEBUG_DEBUG, __func__,"name %s,index %d,vmacIndex %d\n",wlpptr->vdev[i]->name,i,wlpptr->wlpd_p->vmacIndex);
        if (wlpptr->vdev[i]->flags & IFF_RUNNING)
        {
            printk("please ifconfig %s down first!\n",wlpptr->vdev[i]->name);
            return -1;
        }

        wlpptr->vmacSta_p->StopTraffic = TRUE;
        wlpptr->vmacSta_p->StaCtl->Initialized = FALSE;

        NETDEV_PRIV_P(struct wlprivate, vapdev)->vmacSta_p->StopTraffic = TRUE;
        ...
        wlp = NETDEV_PRIV_P(struct wlprivate, wlpptr->vdev[i]);
        DisableMacMgmtTimers(wlp->vmacSta_p);
        extStaDbMemCleanup(wlp->vmacSta_p);
        disable_wids_timer(wlp->vmacSta_p);
        wlDestroySysCfg(wlp->vmacSta_p);
#ifdef AP8X_STATISTICS
        ap8x_stat_proc_unregister(wlpptr->vdev[i],i);
#endif
        /* add by zhanxuechao for delete wiress proc */
        ap8x_dynamic_vap_sysctl_unregister_thinap(wlp);
        rtnl_unlock();
...
#endif
        wlpptr->vdev[i] = NULL;
        wlpptr->wlpd_p->vmacIndex--;
        wlpptr->vmacSta_p->StaCtl->Initialized = TRUE;
        wlpptr->vmacSta_p->StopTraffic = FALSE;
        break;
    }
