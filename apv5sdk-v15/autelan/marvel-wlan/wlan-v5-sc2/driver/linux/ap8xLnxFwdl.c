...
int wlFwDownload(struct net_device *netdev)
{
	...
	if (wlpptr->mfgEnable)	 
	{
        ...
        WlLogPrint(MARVEL_DEBUG_ALL, __func__,"client mode");
	}
	else
	{
		...
		sizeFwDownloaded = 0;
        WlLogPrint(MARVEL_DEBUG_ALL, __func__,"fw download start 88");
		...
		{			
			...
			if (currIteration == 0)
			{
                ...
                WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"Exhausted currIteration during fw download");
        ...
        WlLogPrint(MARVEL_DEBUG_DEBUG, __func__,"FwSize = %d downloaded Size = %d currIteration %d\n", 
		...
		if (downloadSuccessful) 
		{
			...
			if (currIteration == 0)
			{
                WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"Exhausted currIteration waiting for fw signature; firmware seems failed to operate\n");
    ...
    WlLogPrint(MARVEL_DEBUG_ALL, __func__,"wlFwDownload complete\n");
