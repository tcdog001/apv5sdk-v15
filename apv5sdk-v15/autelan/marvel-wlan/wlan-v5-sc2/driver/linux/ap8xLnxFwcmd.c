...
#endif
#include "ap8xLnxRate.h"//add by wangxl 20121227
...
static char *wlgetCmdString(u_int16_t cmd)
{
    ...
    static const struct
    {
        ...
        { HostCmd_CMD_HT_TX_STBC,            "SetHTStbc"              },
        { HostCmd_CMD_SET_BFTYPE,            "SetBFType"              },
        { HostCmd_CMD_SET_REGION_CODE,       "SetRegionCode"          },
        { HostCmd_CMD_GET_CALTABLE,          "GetCalTable"            },
        { HostCmd_CMD_802_11_TX_POWER,       "Settxpower"             },
        { HostCmd_CMD_SET_CDD,               "SetCDD"                 },
        { HostCmd_CMD_SET_IES,               "SetIEs"                 },
        { HostCmd_CMD_SET_WMM_MODE,          "SetWMM"                 },
        { HostCmd_CMD_SET_APMODE,            "SetApMode"              },
        { HostCmd_CMD_HT_GUARD_INTERVAL,     "SetHTGI"                },
        { HostCmd_CMD_SET_RATE_ADAPT_MODE,   "SetAdaptMode"           },
        { HostCmd_CMD_SET_LINKADAPT_CS_MODE, "SetCSAdaptMode"         },
#ifdef RXPATHOPT  /* RXPATHOPT is used only for V4-PIKA firmware.*/
        { HostCmd_CMD_SET_RXPATHOPT,         "SetRxPathOpt"           },
#endif
        { HostCmd_CMD_DWDS_ENABLE,           "SetDwdsStaMod"          },
        { HostCmd_CMD_FW_FLUSH_TIMER,        "SetFwFlushTimer"        },     
        { HostCmd_CMD_SET_OPTIMIZATION_LEVEL,"SetOptimizationLevel"   },
        { HostCmd_CMD_SET_FIXED_RATE,        "SetRate"                },
        ...
        { HostCmd_CMD_SET_N_PROTECT_FLAG,    "SetNProt"               },
        { HostCmd_CMD_SET_RIFS,              "SetRifs"                }, 
        { HostCmd_CMD_SET_NEW_STN,           "SetNewStn"              },
        { HostCmd_CMD_UPDATE_ENCRYPTION,     "SetSecurity"            },
    };
...
int wlFwGetHwSpecs(struct net_device *netdev)
{
    ...
    while (wlexecuteCommand(netdev, HostCmd_CMD_GET_HW_SPEC))
    {
        ...
        mdelay(1000);
        WlLogPrint(MARVEL_DEBUG_PANIC, __func__," Repeat wlFwGetHwSpecs = %x \n", (unsigned int) pCmd);
...
int wlFwSetHwSpecs(struct net_device *netdev)
{
    ...
    {
        WlLogPrint(MARVEL_DEBUG_PANIC, __func__,"failed execution");
        MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
        return FAIL;
    }
...
#endif
int wlFwGetHwStats(struct net_device *netdev, char *page,struct HWstats *total)
{
    struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
    char *p = page;
    int len =0;
    ...
    {
        WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"failed execution");
        MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
        return FAIL;
    }
    ...
    if(total)
    {
        total->RxWAPIMICErrors = ENDIAN_SWAP32((int)pCmd->RxWAPIMICErrors);
        total->RxWAPIPNErrors = ENDIAN_SWAP32((int)pCmd->RxWAPIPNErrors);
        total->WAPINoKeyErrors = ENDIAN_SWAP32((int)pCmd->RxWAPINoKeyErrors) + ENDIAN_SWAP32((int)pCmd->TxWAPINoKeyErrors);
        total->RxErrors = ENDIAN_SWAP32((int)pCmd->RxErrors);
        return SUCCESS;
    }
...
int wlFwHTGI(struct net_device *netdev, u_int32_t GIType)
{
    ...
    WlLogPrint(MARVEL_DEBUG_ALL, __func__,"SetFwHTGI type %d",GIType);
    MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);
...
int wlFwSetRadio(struct net_device *netdev, u_int16_t mode, wlpreamble_e preamble)
{
    ...
    WlLogPrint(MARVEL_DEBUG_ALL, __func__,"mode: %s,preamble: %i",
...
int wlFwSetAntenna(struct net_device *netdev, wlantennatype_e dirSet)
{
    ...
    WlLogPrint(MARVEL_DEBUG_ALL, __func__, 
        "set %s antenna Mode 0x%x", (dirSet == WL_ANTENNATYPE_RX)?"RX":"TX",pCmd->AntennaMode);
    WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *) pCmd, 
...
int wlFwSetRegionCode(struct net_device *netdev, UINT16 regionCode)
{
    ...
    */
    ...
    WlLogPrint(MARVEL_DEBUG_ALL, __func__,"SetRegionCode %d",regionCode);
#endif
...
/*************** zhouke add ,for set Management Rate **********************/
#if 0
    pCmd->ManagementRate = *(mib->mib_ManagementRate);
#else
    ratemask = ieee80211_get_rates_mask(vmacSta_p,1);

    while((mib->StationConfig->OpRateSet[i] != *(mib->mib_ManagementRate)) && i < IEEE80211_RATE_XRATE_SIZE)
    {
        i++;
    }   // get management rate mask;
    
    if(IEEE80211_RATE_XRATE_SIZE == i){
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"ManagementRate %d is error\n",*(mib->mib_ManagementRate));
        *(mib->mib_ManagementRate) = 2;
    }

    if(!((ratemask >> i) & 0x01))  //set min management rate 
    {
        i = 0;
        while( !((ratemask >> i) & 0x01) && i < IEEE80211_RATE_XRATE_SIZE){
            i++;
        }
        
        if(IEEE80211_RATE_XRATE_SIZE == i){
            WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"%d more than IEEE80211_RATE_XRATE_SIZE\n", i);
        }else{
            *(mib->mib_ManagementRate) = mib->StationConfig->OpRateSet[i];
            *(vmacSta_p->ShadowMib802dot11->mib_ManagementRate) = mib->StationConfig->OpRateSet[i];
        }
    }
...
/***************************** end ****************************************/
    ...
    if (*(mib->mib_enableFixedRateTx) == 2)
    {
        ...
        WlLogPrint(MARVEL_DEBUG_ALL, __func__,"%s rate at %i w/o auto drop",*(mib->mib_FixedRateTxType)?"HT":"legacy", *(mib->mib_FixedRateTxType)?*(mib->mib_txDataRateN):*(mib->mib_txDataRate));
    } else if(*(mib->mib_enableFixedRateTx) == 1)
    {
        ...
        WlLogPrint(MARVEL_DEBUG_ALL, __func__,"%s rate at %i w/ auto drop",*(mib->mib_FixedRateTxType)?"HT":"legacy", *(mib->mib_FixedRateTxType)?*(mib->mib_txDataRateN):*(mib->mib_txDataRate));
    } else
    {
        pCmd->AllowRateDrop = ENDIAN_SWAP32(FIXED_RATE_WITH_AUTO_RATE_DROP);
        WlLogPrint(MARVEL_DEBUG_ALL, __func__,"auto rate %i",*(mib->mib_enableFixedRateTx));
    }
...
int wlFwSetTxPower(struct net_device *netdev, UINT8 flag, UINT32 powerLevel)
{
    ...
    {
        if (found) {
             #if 0 /*deleted by pengdecai for txpower */
               if ((mibS->PhyTXPowerTable[index]->setcap)   
               && (mibS->PhyTXPowerTable[index]->TxPower[i] > mibS->PhyDSSSTable->maxTxPow[i])) /*deleted by pengdecai for txpower */
			   tmp = mibS->PhyDSSSTable->maxTxPow[i];
			 #endif
			 
			if ((*(mibS->mib_MaxTxPwr)!=0xFF) && (mibS->PhyTXPowerTable[index]->TxPower[i] > *(mibS->mib_MaxTxPwr))) /*added by pengdecai for txpower */
				 tmp =  *(mibS->mib_MaxTxPwr);	
            else
...
int wlFwSetWep(struct net_device *netdev, u_int8_t *staaddr)
{
    ...
    for (keyIndex = 0 ; keyIndex < 4; keyIndex++)
    {
        ...
        WlLogPrint(MARVEL_DEBUG_ALL, __func__,"HostCmd_FW_UPDATE_ENCRYPTION_KEY_SET Len = %d pCmd->KeyParam = %d\n", 
...
int wlFwSetMacAddr(struct net_device *netdev)
{
    ...
    WLDBG_ENTER(DBG_LEVEL_0);
    WlLogPrint(MARVEL_DEBUG_ALL, __func__,"%s Set MacAddr %02x:%02x:%02x:%02x:%02x:%02x",\
...
int wlFwSetChannel(struct net_device *netdev, u_int8_t channel, CHNL_FLAGS Chanflag, u_int8_t initRateTable)
{
    ...
    WlLogPrint(MARVEL_DEBUG_ALL, __func__,"set channel: %d", channel);
                       ...
                       u_int8_t *SrcMacaddr, UINT16 seqNo)
{
	...
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
	u_int8_t	bufsize_exp = (ParamInfo & 0x3);
	...
	if (*(mib->mib_MaxAmpduLen) && ( (*(mib->mib_MaxAmpduLen)-1) < bufsize_exp)) 
		ParamInfo = ((ParamInfo & 0xfc) | (*(mib->mib_MaxAmpduLen)-1));
			
	if (*(mib->mib_MaxAmpduNum) && *(mib->mib_MaxAmpduNum) < WindowSize)
		WindowSize = BarThrs = *(mib->mib_MaxAmpduNum);
...
int wlFwMaxPacketLifetime(struct net_device *netdev, UINT16 maxlifetime)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_FW_SET_PACKET_LIFETIME *pCmd = (HostCmd_FW_SET_PACKET_LIFETIME *) &wlpptr->pCmdBuf[0];
	int retval = FAIL;
	...
	//printk("setting MaxPacketLifetime = %d\n", maxlifetime);
	MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);
	memset(pCmd, 0x00, sizeof(HostCmd_FW_SET_PACKET_LIFETIME));
	pCmd->CmdHdr.Cmd    = ENDIAN_SWAP16(HostCmd_CMD_SET_MAX_PACKET_LIFETIME);
	pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_SET_PACKET_LIFETIME));
	pCmd->CmdHdr.macid = wlpptr->vmacSta_p->VMacEntry.macId;
	pCmd->max_lifetime = ENDIAN_SWAP16(maxlifetime);
	retval = wlexecuteCommand(netdev, HostCmd_CMD_SET_MAX_PACKET_LIFETIME);
	MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
	return retval;
}
int
...
int wlFwSetAPBss(struct net_device *netdev, wlfacilitate_e facility)
{
    ...
    WlLogPrint(MARVEL_DEBUG_ALL, __func__,"AP bss %s", (facility == WL_ENABLE) ? "enable": "disable");
...
int wlFwSetWmm(struct net_device *netdev,wlfacilitate_e facility)
{
    ...
    WLDBG_ENTER(DBG_LEVEL_0);
    WlLogPrint(MARVEL_DEBUG_ALL, __func__,"SetWmms %d",facility);
    MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);
...
/* guijinlin add 2012-06-27 start */
int wlFwSetIEsExt(struct net_device *netdev,UINT32 SupportedMCSMask)
{
    struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
    vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
    MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
    ...
    WlLogPrint(MARVEL_DEBUG_ALL, __func__,"set_mcs_mask %d\n",SupportedMCSMask);
    ...
    retval = 0;
    pCmd->IeListLen =0;
    /* guijinlin add 2012-06-28 start */
#if defined(MBSS)
    if(vmacSta_p->master == NULL)
    {
        ...
        {
            (NETDEV_PRIV_P(struct wlprivate, wlpptr->vdev[i]))->vmacSta_p->mcs_mask = SupportedMCSMask;
            i++;
        }
    }
    vmacSta_p->mcs_mask = SupportedMCSMask;
#endif
    /* guijinlin add 2012-06-28 end */
    ...
    if (*(mib->QoSOptImpl))
    {
        ...
        pCmd->IeListLen  += AddHT_IE_Ext(wlpptr->vmacSta_p,(IEEEtypes_HT_Element_t*)&(pCmd->IeList[retval]),SupportedMCSMask);
        retval = pCmd->IeListLen;   
...
{
    struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
    //WlLogPrint(MARVEL_DEBUG_ALL, __func__,"%s send cmd 0x%04x=%s", netdev->name,0x8000 | cmd,wlgetCmdString(0x8000 | cmd));
    ...
    if (wlChkAdapter(netdev)  && (!wlpptr->wlpd_p->inSendCmd))
    {
        ...
        {
            WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"timeout");
    ...
    wlpptr->wlpd_p->inSendCmd = FALSE;
    WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"no adapter plugged in");
    return FAIL;
}
...
static int wlwaitForComplete(struct net_device *netdev, u_int16_t cmdCode)
{
    ...
    if (currIteration == 0)
    {
        WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"%s: cmd 0x%04x=%s timed out\n",
...
int wlFwApplySettings(struct net_device *netdev)
{
    ...
    */
    ...
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting dev mac failed");
        retval = FAIL;
    }
#endif
    if (wlFwGetRegionCode(netdev))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"getting Region code failed");
        retval = FAIL;
    }
    if (wlFwSetAntenna(netdev, WL_ANTENNATYPE_RX))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting RX antenna failed");
        retval = FAIL;
    }
    if (wlFwSetAntenna(netdev, WL_ANTENNATYPE_TX))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting TX antenna failed");
        retval = FAIL;
    }
    if( wlFwSetHTStbc(netdev, *mib->mib_HtStbc))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting HT STBC failed");
        retval = FAIL;
    }
#if defined(SOC_W8764)
    if( wlFwSetBFType(netdev, *mib->mib_bftype))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting BF TYPEv");
        retval = FAIL;
    }
	if( wlFwMaxPacketLifetime(netdev, *mib->mib_MaxPacketLifetime))
	{
		WLDBG_EXIT_INFO(DBG_LEVEL_0, "setting Max Packet Lifetime");
		retval = FAIL;
	}
#endif
    if (wlFwSetRadio(netdev, WL_ENABLE, mib->StationConfig->mib_preAmble))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting auto preamble failed");
        retval = FAIL;
    }
    if (wlchannelSet(netdev, PhyDSSSTable->CurrChan, PhyDSSSTable->Chanflag, 1))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting channel failed");
        retval = FAIL;
    }
    if (wlFwSetRegionCode(netdev, *(mib->mib_regionCode)))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting region code failed");
        retval = FAIL;
    }
#ifdef MRVL_DFS
    if(DecideDFSOperation(netdev, bChannelChanged, currDFSMode, newDFSMode , mib))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"DFS Operation failed");
        retval = FAIL;
    }
#endif //MRVL_DFS
    if (wlFwGetPwrCalTable(netdev))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"getting cal power table failed failed");
        retval = FAIL;
    }
    if( wlFwSetMaxTxPwr(netdev))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting Max Tx Power failed");
        retval = FAIL;
    }
    if (wlFwSetTxPower(netdev, HostCmd_ACT_GEN_SET_LIST, 0))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting tx power failed");
        retval = FAIL;
    }
#if defined(SOC_W8366)||defined(SOC_W8764)
    if (wlFwSetAntenna(netdev, WL_ANTENNATYPE_TX2))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting TX antenna2 failed");
        retval = FAIL;
    }
#endif
    if( wlFwSetCDD(netdev, *mib->mib_CDD))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting CDD failed");
        retval = FAIL;
    }
    if (wlFwSetIEs(netdev))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting IEs failed");
        retval = FAIL;
    }
    if (wlFwSetAPBcastSSID(netdev, *(mib->mib_broadcastssid)))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting hidden ssid failed");
        retval = FAIL;
    }
    if( wlFwSetWmm(netdev,*(mib->QoSOptImpl)))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting qos option failed");
        retval = FAIL;
    }
    if( wlFwSetApMode(netdev,*(mib->mib_ApMode)))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting AP MODE failed");
        retval = FAIL;
    }
    if( wlFwHTGI(netdev,*mib_guardInterval_p))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting HT GI failed");
        retval = FAIL;
    }
    if( wlFwSetAdaptMode(netdev))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting Adpat mode failed");
        retval = FAIL;
    }
    if( wlFwSetCSAdaptMode(netdev))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting CS Adpat mode failed");
        retval = FAIL;
    }
#ifdef RXPATHOPT
    if( wlFwSetRxPathOpt(netdev, *(mib->mib_RxPathOpt)))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting RxPathOpt failed");
        retval = FAIL;
    }
#endif  
    ...
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting DwdsStaMode mode failed");
        retval = FAIL;
    }
#endif  
    ...
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting Fw Flush timer failed");
        retval = FAIL;
    }
    if(*(mib->QoSOptImpl))
    {
    ...
    if (wlFwSetApBeacon(netdev))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting AP beacon failed");
        retval = FAIL;
    }
#endif
    /* NOTE: Sequence of hostcmd:
    ...
    */
    ...
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"set Optimization level failed");
        retval = FAIL;
    }
    /* NOTE: Sequence of hostcmd:
    wlFwSetEdcaParam
    ...
    */
    ...
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting tx rate failed");
        retval = FAIL;
    }
    if(wlFwSetRTSThreshold(netdev, *(mib->mib_RtsThresh)))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting rts threshold failed");
        retval = FAIL;
    }
#ifdef SINGLE_DEV_INTERFACE
    if (wlFwSetAPBss(netdev, WL_ENABLE))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"enabling AP bss failed");
        retval = FAIL;
    }
#endif
    {
        extern int set_sta_aging_time(vmacApInfo_t *vmacSta_p,int minutes);
        set_sta_aging_time(vmacSta_p,*(mib->mib_agingtime));
    }
    {
    ...
    if( wlFwSetSpectrumMgmt( netdev, mib_SpectrumMagament_p->spectrumManagement ))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"enabling spectrum management failed");
        retval = FAIL;
    }
        ...
        (PhyDSSSTable->Chanflag.FreqBand == FREQ_BAND_5GHZ ))
    {
        ...
        {
            WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting power constraint info element failed");
            retval = FAIL;
        }
    }
        ...
        mib_SpectrumMagament_p->multiDomainCapability )
    {
        if( wlFwSetCountryCode( netdev, mib_SpectrumMagament_p->countryCode ) )
        {
            WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting country code info element failed");
            retval = FAIL;
        }
    }
    else 
    ...
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"set N protection failed");
        retval = FAIL;
    }
#ifdef AMPDU_SUPPORT
    if(*(mib->mib_rifsQNum)!=0)
        ...
        {
            WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"set RIFS failed");
            retval = FAIL;
        }
#endif
#ifdef CLIENTONLY   
...
int wlFwMultiBssApplySettings(struct net_device *netdev)
{
    ...
    if (wlFwSetIEs(netdev))
    {
        WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"setting IEs failed");
        retval = FAIL;
    }
    if (wlFwSetAPBcastSSID(netdev, *(mib->mib_broadcastssid)))
    {
        WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"setting hidden ssid failed");
        retval = FAIL;
    }
    if (wlFwSetApBeacon(netdev))
    {
        WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"setting AP beacon failed");
        retval = FAIL;
    }
    if (wlFwSetAPBss(netdev, WL_ENABLE))
    {
        WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"enabling AP bss failed");
        retval = FAIL;
    }
#ifdef IEEE80211_DH
    if( wlFwSetSpectrumMgmt( netdev, mib_SpectrumMagament_p->spectrumManagement ))
    {
        WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"enabling spectrum management failed");
        retval = FAIL;
    }
    /* power constraint ino IE should appear in beacon only on 5GHz */
    if(( mib_SpectrumMagament_p->spectrumManagement ) &&
        ...
        ||(*(mib->mib_ApMode) == AP_MODE_5GHZ_N_ONLY)))
    {
        if(wlFwSetPowerConstraint( netdev, mib_SpectrumMagament_p->powerConstraint ))
        {
            WLDBG_WARNING(DBG_LEVEL_0, "setting power constraint info element failed");
            retval = FAIL;
        }
    }
    ...
    if((*(mib->pMib_11nAggrMode) & WL_MODE_AMSDU_TX_MASK) ||AnyDevAmsduEnabled(wlpptr->master))
    {
        ...
        {
            WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"setting Fw Flush timer failed");
            retval = FAIL;
        }
    }
    else
    ...
    if( combIE == NULL )
    {
        WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"No memory left for WPS IE failed");
        retval = FAIL;
    }
    else
    {
        ...
        if( vmacSta_p->thisbeaconIE.Len != 0 )
        {
            ...
            {
                WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"Setting Beacon WSC IE failed");
                retval = FAIL;
            }
        }
        if( vmacSta_p->thisprobeRespIE.Len != 0 )
        {
            ...
            {
                WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"Setting Probe Response WSC IE failed");
                retval = FAIL;
            }
        }
        kfree(combIE);
    ...
    if(*(mib->mib_rifsQNum)!=0)
        if (wlFwSetRifs(netdev, *(mib->mib_rifsQNum)))
        {
            WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"set RIFS failed");
            retval = FAIL;
        }
#endif
        wlFwSetNewStn(netdev,vmacSta_p->macStaAddr, 0, 0, 0, NULL,0,0);  //add new station
...
int wlFwApplyClientSettings(struct net_device *netdev)
{
    ...
    {
        WlLogPrint(MARVEL_DEBUG_ERROR, __func__,"enabling Sta mode failed");
        retval = FAIL;
    }
...
static int wlFwSetAdaptMode(struct net_device *netdev)
{
    ...
    WLDBG_ENTER(DBG_LEVEL_0);
    WlLogPrint(MARVEL_DEBUG_ALL, __func__,"SetAdaptMode mode %d",*(mib->mib_RateAdaptMode));
    MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);
...
static int wlFwSetCSAdaptMode(struct net_device *netdev)
{
    ...
    WLDBG_ENTER(DBG_LEVEL_0);
    WlLogPrint(MARVEL_DEBUG_ALL, __func__,"SetCSAdaptMode mode %d",*(mib->mib_CSMode));
...
static int wlFwSetNProt(struct net_device *netdev, UINT32 mode)   
{
    ...
    WlLogPrint(MARVEL_DEBUG_ALL, __func__,"N prot mode %d", mode);
...
static int wlFwSetOptimizationLevel(struct net_device *netdev, UINT8 mode)    
{
    ...
    WlLogPrint(MARVEL_DEBUG_ALL, __func__,"Optimization %d", mode);
...
{   
    ...
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting wep keyto sta fail\n");
        retval = FAIL;
    }
    if(wlFwSetWpaTkipMode(netdev,staaddr))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting tkip sta fail\n");
        retval = FAIL;
    }
    if(wlFwSetWpaAesMode(netdev,staaddr))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting aes fail\n");
        retval = FAIL;
    }   
    if (wlFwSetMixedWpaWpa2Mode(netdev,staaddr))
    {
        WlLogPrint(MARVEL_DEBUG_WARNING, __func__,"setting mixed mode sta fail\n");
        retval = FAIL;
    }
    return retval;
}
...
int wlFwSetHTStbc( struct net_device *netdev, UINT32 mode)
{
    ...
    WlLogPrint(MARVEL_DEBUG_ALL, __func__,"SetHTStbc mode %d",mode);
#ifdef MFG_SUPPORT
    if (wlpptr->mfgEnable)
    {       
        return SUCCESS;
...
int wlFwSetRxPathOpt(struct net_device *netdev, UINT32 rxPathOpt)
{
    ...
    WlLogPrint(MARVEL_DEBUG_ALL, __func__,"Set RXPATHOPT to %d", rxPathOpt );
...
int wlFwSetDwdsStaMode(struct net_device *netdev, UINT32 enable)
{
    ...
    WlLogPrint(MARVEL_DEBUG_ALL, __func__,"Set DwdsStaMode to %d", enable );
...
int wlFwSetFwFlushTimer(struct net_device *netdev, UINT32 usecs)
{
    ...
    WlLogPrint(MARVEL_DEBUG_ALL, __func__,"Set FwFlushTimer to %d usecs", usecs );
...
#endif
    WlLogPrint(MARVEL_DEBUG_ALL, __func__,"SetCDD mode %d",cdd_mode);
    MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);
...
int wlFwSetBFType( struct net_device *netdev, UINT32 mode)
{
    ...
    WlLogPrint(MARVEL_DEBUG_ALL, __func__,"SetBFType mode %d",mode);
#ifdef MFG_SUPPORT
    if (wlpptr->mfgEnable)
    {       
        return SUCCESS;
