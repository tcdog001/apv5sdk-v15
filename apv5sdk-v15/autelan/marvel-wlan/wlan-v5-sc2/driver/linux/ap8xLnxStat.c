	...
	{
		{
			...
			p += sprintf(p, "\n-------------------------------\n" );
			p += wlFwGetHwStats(netdev, p, NULL);
		}
	}
	len = (p - page) - off;
	if (len < 0)
...
int ap8x_stat_proc_register(struct net_device *dev,int index)
{
...
#define WL_CASE(x) \
    ...
    if(wlpptr->vmacSta_p->VMacEntry.phyHwMacIndx == 1)
    {
        index += MAX_VMAC_MIB_INSTANCE;
    }
    ...
    memset(&devstat[devstat_index],0,sizeof(struct dev_stat));
...
int ap8x_stat_proc_unregister(struct net_device *dev,int index)
{
    int i;
    struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
	...
	//for (i = 0; i < WL_MAXIMUM_INSTANCES ; i++)
	{
        ...
        {
            printk("not find devstat,unregister failed!\n");
		}
	}
	return 0;
}
