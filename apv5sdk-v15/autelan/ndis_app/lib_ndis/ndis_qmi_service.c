...
static int ndis_get_qmi_status(int ndis_fd);
...
int ndis_qmi_connect(int ndis_fd, char *apn, char *username, char *passwd, int auth)
{
	...
	p_ndis_info->ndis_ifr.ifr_data = (void*)&ndis_cmd;
	p_ndis_info->current_status = LC_CONNECTING;
...
//	usleep(300);
	...
	p_ndis_info->current_status = LC_GETTING_IP;		
	return 0;
}
/********************************************************************
 *
...
{
	ndis_info_t* p_ndis_info = (ndis_info_t*)ndis_fd;
	int   conn_err_number = ndis_qmi_connect(ndis_fd, apn, username, passwd, auth);
...
//	int i = 10;
	while(i--)
	{
		...
		usleep(200);
		connect_status =ndis_get_qmi_status(ndis_fd);
		cprintf("%s:..connect_status=%d\n",__func__,connect_status);
		if(connect_status==LC_DISCONNECTED)
		{
			usleep(300);
			connect_status =ndis_get_qmi_status(ndis_fd);
			break;
...
//	int   conn_err_number = 0;
	...
	p_ndis_info->current_status = l_ndis_status.connection_status;
	return p_ndis_info->current_status;
}
/********************************************************************
 *
 *         Name:  ndis_get_status
...
//	int   conn_err_number = 0;
	...
	if((l_ndis_status.connection_status == LC_CONNECTING) || (l_ndis_status.connection_status == LC_GETTING_IP))
	{
		pipinfo->i32status = NDIS_CONNECTING;
	}else
	if((l_ndis_status.connection_status == LC_IP_GETTED) && (l_ndis_status.connection_status < LC_DISCONNECTING))
	{
...
//	int i = 0;
	...
	p_ndis_info->current_status = LC_DISCONNECTED; ////may not currect if not call ndis_get_status
