...
const static char *IFCONFIG = "/sbin/ifconfig";
const static char *NETMASK = "netmask";
const static char *ROUTEADD_NET = "/sbin/route add -net";
const static char *ROUTEADD_HOST = "/sbin/route add -host";
const static char *KILLALL = "/usr/bin/killall";
const static char *UDHCPC = "/sbin/udhcpc";
const static char *NTPCLIENT = "/usr/sbin/ntpclient";
const static char *HOSTAPD = "/usr/sbin/hostapd";
const static char *IWCONFIG = "/usr/sbin/iwconfig";
const static char *IWPRIV = "/usr/sbin/iwpriv";
const static char *SYSLOGD = "/sbin/syslogd";
const static char *WLANCONFIG = "/usr/sbin/wlanconfig";
...
static int conf_netwrok_set_ip( char *str_ifname, char *str_ipaddr)
{
...
static int conf_netwrok_set_netmask( char *str_ifname, char *str_netmask)
{
	MXML_NODE* pst_node = NULL;
	int res = IFERR_NO_ERROR;
	const char *XPATH = "/config/network/lan";
...
static int ap_network_set_ip( char *str_ifname, char *str_ipaddr)
{
	char str_tmp_cmd[MAX_CMD_LEN];
	memset( str_tmp_cmd, 0, MAX_CMD_LEN );
	sprintf( str_tmp_cmd, "%s %s %s", IFCONFIG, str_ifname, str_ipaddr );
	return (0==system(str_tmp_cmd)) ? IFERR_NO_ERROR: IFERR_COM_EXEC_FAILED;
}
static int ap_network_set_netmask( char *str_ifname, char *str_netmask)
{
	char str_tmp_cmd[MAX_CMD_LEN];
	memset( str_tmp_cmd, 0, MAX_CMD_LEN );
	sprintf( str_tmp_cmd, "%s %s %s", IFCONFIG, str_ifname,str_netmask );
	return (0==system(str_tmp_cmd)) ? IFERR_NO_ERROR: IFERR_COM_EXEC_FAILED;
}

//int network_set_ip( char *str_ifname, char *str_ipaddr, char *str_netmask)
int network_set_ip( char *str_ifname, char *str_ipaddr)
{
	int fd_mutex;
	int res = IFERR_NO_ERROR;
	...
	if( (res = ap_network_set_ip( str_ifname, str_ipaddr)) != IFERR_NO_ERROR )
	{
	...
	res = conf_netwrok_set_ip( str_ifname, str_ipaddr );
...
int network_set_netmask( char *str_ifname, char *str_netmask)
{
	int fd_mutex;
	int res = IFERR_NO_ERROR;
	...
	if( (res = ap_network_set_netmask( str_ifname, str_netmask)) != IFERR_NO_ERROR )
	{
	    flock( fd_mutex, LOCK_UN );
	    close(fd_mutex);
	    return res;
	...
	res = conf_netwrok_set_netmask( str_ifname, str_netmask );
...
static int conf_wireless_addif_vlan( char *str_ifname, char *str_vlanname)
{
    ...
    if(strcmp(str_vlanname,"default")==0)
	{
...
int wireless_get_bssid( unsigned int un_vap_num,char *str_bssid, unsigned int un_size)
{
	...
	sprintf(str_tmp_cmd,"%s %s |grep -o -i -e '[0-9a-fA-F]\\{1,2\\}:[0-9a-fA-F]\\{1,2\\}:[0-9a-fA-F]\\{1,2\\}:[0-9a-fA-F]\\{1,2\\}:[0-9a-fA-F]\\{1,2\\}:[0-9a-fA-F]\\{1,2\\}'",IWCONFIG,str_vap_if);
	stream = popen( str_tmp_cmd, "r" );
...
static int ap_wireless_config_mode( unsigned int un_vap_num, char *str_mode )
{
	...
	memset( str_tmp_cmd, 0, MAX_CMD_LEN );
	sprintf(str_tmp_cmd,"%s %s channel 0", IWCONFIG,str_vap_if);
	system(str_tmp_cmd);
	memset( str_tmp_cmd, 0, MAX_CMD_LEN );
	sprintf( str_tmp_cmd, "%s %s %s %s", IWPRIV, str_vap_if, "mode", str_mode );
    system(str_tmp_cmd);
	memset( str_tmp_cmd, 0, MAX_CMD_LEN );
	sprintf(str_tmp_cmd,"%s %s channel %s", IWCONFIG,str_vap_if,tmp);
	return system(str_tmp_cmd);		
}
...
static int ap_wireless_create_vap( char *str_wifi, char *str_ifname )
{
	...
	system(str_tmp_cmd);
	memset( str_tmp_cmd, 0, MAX_CMD_LEN );
	sprintf(str_tmp_cmd,"/usr/sbin/brctl addif default %s",str_ifname);
	return system(str_tmp_cmd);
}
...
static int ap_wireless_addif_vlan(  char *str_ifname,char *str_vlanname )
{
	char str_tmp_cmd[MAX_CMD_LEN];
	memset( str_tmp_cmd, 0, MAX_CMD_LEN );
	sprintf( str_tmp_cmd, "%s %s %s", "/usr/sbin/brctl addif", str_ifname, str_vlanname );
	return system(str_tmp_cmd);
}
...
static int ap_vlan_create( char *str_vlanname)
{
	char str_tmp_cmd[MAX_CMD_LEN];
	memset( str_tmp_cmd, 0, MAX_CMD_LEN );
	sprintf( str_tmp_cmd, "%s %s", "/usr/sbin/brctl addbr", str_vlanname);
	return system(str_tmp_cmd);
}
...
static int ap_vlan_destroy( char *str_vlanname)
{
	char str_tmp_cmd[MAX_CMD_LEN];
	memset( str_tmp_cmd, 0, MAX_CMD_LEN );
	sprintf( str_tmp_cmd, "%s %s", "/usr/sbin/brctl delbr", str_vlanname);
	return system(str_tmp_cmd);
}
...
static int ap_vlan_addport( char *str_vlanname,char *str_portname)
{
	...
	sprintf( str_tmp_cmd, "%s %s", "/usr/sbin/brctl addbr", str_vlanname);
	system(str_tmp_cmd);
	sprintf( str_tmp_cmd, "%s %s %s", "/sbin/vconfig add",str_portname, strpbrk(str_vlanname,"12345678"));
	system(str_tmp_cmd);
	
	sprintf( str_tmp_cmd, "%s %s%s%s %s", IFCONFIG, str_portname,".",strpbrk(str_vlanname,"12345678"),"up");
	system(str_tmp_cmd);

  sprintf( str_tmp_cmd, "%s %s %s%s%s", "/usr/sbin/brctl addif", str_vlanname,str_portname,".",strpbrk(str_vlanname,"12345678"));
	return system(str_tmp_cmd);
}
...
static int ap_vlan_delport( char *str_vlanname,char *str_portname)
{
	...
	sprintf( str_tmp_cmd, "%s %s %s%s%s", "/usr/sbin/brctl delif", str_vlanname,str_portname,".",strpbrk(str_vlanname,"12345678"));
	system(str_tmp_cmd);

  sprintf( str_tmp_cmd, "%s %s%s%s", "/sbin/vconfig rem",str_portname,"." ,strpbrk(str_vlanname,"12345678"));
	system(str_tmp_cmd);
...
int ap_system_show(void)
{
	...
	sprintf( str_tmp_cmd, "%s", "/usr/sbin/showsysinfo");
	return system(str_tmp_cmd);
}
...
static int ap_wireless_config_vlanvid( unsigned int un_vap_num,char *br_name)
{
	...
	get_data_by_path( CONF_FILE, str_tmp_path, "if",
                             str_vap_if, MAX_STR_LEN );
	memset( str_tmp_cmd, 0, MAX_CMD_LEN );
	sprintf( str_tmp_cmd, "%s %s %s", "/usr/sbin/brctl addif", br_name, str_vap_if);
	return system(str_tmp_cmd);
}
...
static int ap_stp_config_enable( char *str_ifname,char * str_enable)
{
	...
	{
	    sprintf( str_tmp_cmd, "%s %s %s", "/usr/sbin/brctl stp", str_ifname, "on");
	}
	else
	{
        sprintf( str_tmp_cmd, "%s %s %s", "/usr/sbin/brctl stp", str_ifname, "off");
	}
	return system(str_tmp_cmd);
}
...
static int ap_stp_config_bridgeprio( char *str_ifname,char *str_bridgeprio)
{
	char str_tmp_cmd[MAX_CMD_LEN];
	memset( str_tmp_cmd, 0, MAX_CMD_LEN );
	sprintf( str_tmp_cmd, "%s %s %s", "/usr/sbin/brctl setbridgeprio", str_ifname, str_bridgeprio);
	return system(str_tmp_cmd);
}
...
static int ap_stp_config_setfd( char *str_ifname,char *str_setfd)
{
	char str_tmp_cmd[MAX_CMD_LEN];
	memset( str_tmp_cmd, 0, MAX_CMD_LEN );
	sprintf( str_tmp_cmd, "%s %s %s", "/usr/sbin/brctl setfd", str_ifname, str_setfd);
	return system(str_tmp_cmd);
}
...
static int ap_stp_config_hello( char *str_ifname,char *str_hello)
{
	char str_tmp_cmd[MAX_CMD_LEN];
	memset( str_tmp_cmd, 0, MAX_CMD_LEN );
	sprintf( str_tmp_cmd, "%s %s %s", "/usr/sbin/brctl sethello", str_ifname, str_hello);
	return system(str_tmp_cmd);
}
...
static int ap_stp_config_maxage( char *str_ifname,char *str_maxage)
{
	char str_tmp_cmd[MAX_CMD_LEN];
	memset( str_tmp_cmd, 0, MAX_CMD_LEN );
	sprintf( str_tmp_cmd, "%s %s %s", "/usr/sbin/brctl setmaxage", str_ifname, str_maxage);
	return system(str_tmp_cmd);
}
...
//get if_name and ip_addr by vid, if there is not any vlan , we use lan default
    ...
    if( RADIUS == flag_sec_mode )
    {
        ...
        res += system( str_tmp_cmd );
        memset( str_tmp_data1, 0, MAX_STR_LEN );
        ret = get_data_by_path( CONF_FILE, str_tmp_path, "asharekey",
                                str_tmp_data1, MAX_STR_LEN );
        if( ret != IFERR_NO_ERROR )
        {
            return IFERR_ARG_WRONG;
        ...
        res += system( str_tmp_cmd );
        memset( str_tmp_data1, 0, MAX_STR_LEN );
        ret = get_data_by_path( CONF_FILE, str_tmp_path, "radiusip1",
                                str_tmp_data1, MAX_STR_LEN );
        ...
        memset( str_tmp_data1, 0, MAX_STR_LEN );
        ret = get_data_by_path( CONF_FILE, str_tmp_path, "authport1",
                                str_tmp_data1, MAX_STR_LEN );
        if( IFERR_NO_ERROR == ret )
        {
        ...
        memset( str_tmp_data1, 0, MAX_STR_LEN );
        ret = get_data_by_path( CONF_FILE, str_tmp_path, "sharekey1",
                                str_tmp_data1, MAX_STR_LEN );
        if( IFERR_NO_ERROR == ret )
        {
        ...
        memset( str_tmp_data1, 0, MAX_STR_LEN );
        ret = get_data_by_path( CONF_FILE, str_tmp_path, "radiusaip1",
                                str_tmp_data1, MAX_STR_LEN );
        if( IFERR_NO_ERROR == ret )
        {
        ...
        memset( str_tmp_data1, 0, MAX_STR_LEN );
        ret = get_data_by_path( CONF_FILE, str_tmp_path, "accepport1",
                                str_tmp_data1, MAX_STR_LEN );
        if( IFERR_NO_ERROR == ret )
        {
        ...
        memset( str_tmp_data1, 0, MAX_STR_LEN );
        ret = get_data_by_path( CONF_FILE, str_tmp_path, "asharekey1",
                                str_tmp_data1, MAX_STR_LEN );
        if( IFERR_NO_ERROR == ret )
        {
...
int wireless_sec_set_authmode( unsigned int un_vap_num, char *str_authmode)
{
     int ret = XERR_NO_ERROR;
     char str_tmp_path[MAX_PATH_LEN];
     sprintf( str_tmp_path, "/config/network/vap%d", un_vap_num );
     ret=set_data_by_path( CONF_FILE, str_tmp_path, "authmode", str_authmode ); 
     return (XERR_NO_ERROR==ret) ? IFERR_NO_ERROR : IFERR_FILE_WRITE;
}

int wireless_sec_get_authmode( unsigned int un_vap_num, char *str_authmode, unsigned int un_size )
{
    char str_tmp_path[MAX_PATH_LEN];
    sprintf( str_tmp_path, "/config/network/vap%d", un_vap_num );
    return get_data_by_path( CONF_FILE, str_tmp_path, "authmode",
                             str_authmode , un_size );
}

int wireless_sec_set_wepkeytype( unsigned int un_vap_num, char *str_key_type)
{
     int ret = XERR_NO_ERROR;
     char str_tmp_path[MAX_PATH_LEN];
     sprintf( str_tmp_path, "/config/network/vap%d", un_vap_num );
     ret=set_data_by_path( CONF_FILE, str_tmp_path, "wepkeytype", str_key_type ); 
     return (XERR_NO_ERROR==ret) ? IFERR_NO_ERROR : IFERR_FILE_WRITE;
}

int wireless_sec_get_wepkeytype( unsigned int un_vap_num, char *str_key_type, unsigned int un_size )
{
    char str_tmp_path[MAX_PATH_LEN];
    sprintf( str_tmp_path, "/config/network/vap%d", un_vap_num );
    return get_data_by_path( CONF_FILE, str_tmp_path, "wepkeytype", str_key_type , un_size );
}
                              ...
                              unsigned int un_size )
{
   ...
   ret = get_data_by_path( CONF_FILE, str_tmp_path, "authmode",
    ...
    {
        retu= get_data_by_path( CONF_FILE, str_tmp_path, "encryption",
                                 str_encrypt , un_size );
    ...
    if( 0 == strcmp(str_cur_mode, "3") )
    {
     ...
     retu=get_data_by_path( CONF_FILE, str_tmp_path, "wepbit",
                                 str_encrypt , un_size );
		return retu;
}
...
int wireless_sec_set_wepkey_level( unsigned int un_vap_num,  char *str_key_level)
{
    char str_tmp_path[MAX_PATH_LEN];
    sprintf( str_tmp_path, "/config/network/vap%d", un_vap_num );
    
    return set_data_by_path( CONF_FILE, str_tmp_path, "wepbit",str_key_level);
...
static int ap_wireless_sec_set_wpaencrypt( unsigned int un_vap_num,  char *str_wpaencrypt )
{
    char str_old_type[MAX_STR_LEN];
    char str_tmp_path[MAX_PATH_LEN];
    int ret = XERR_NO_ERROR;
...
static int conf_wireless_sec_set_wpaencrypt( unsigned int un_vap_num, 
                                         char *str_wpakey )
{
    char str_tmp_path[MAX_PATH_LEN];
    int ret = XERR_NO_ERROR;
    ...
    ret = set_data_by_path( CONF_FILE, str_tmp_path, "encryption", str_wpakey);
    
    return (XERR_NO_ERROR==ret) ? IFERR_NO_ERROR : IFERR_FILE_WRITE;    
}


int wireless_sec_set_wpaencrypt( unsigned int un_vap_num, char *str_wpaencrypt, int flag )
{ 
    int fd_mutex;
	int res = IFERR_NO_ERROR;
	char str_curwpaencrypt[70]={0};
	if(wireless_sec_get_wpaencrypt( un_vap_num,str_curwpaencrypt,70) == IFERR_NO_ERROR)
	{
	    if(strcmp(str_curwpaencrypt,str_wpaencrypt) == 0)
	    {
		  return IFERR_ARG_INVALID;
	    }
    res = conf_wireless_sec_set_wpaencrypt( un_vap_num, str_wpaencrypt );
    ...
    if( flag )
    {
        ...
        res = ap_wireless_sec_set_wpaencrypt(un_vap_num, str_wpaencrypt );
        flock( fd_mutex, LOCK_UN );
        close( fd_mutex );
	    }
    }
                             ...
                             unsigned int un_size )
{
    ...
    int res = IFERR_NO_ERROR;
    sprintf( str_tmp_path, "/config/network/vap%d", un_vap_num );
    res=get_data_by_path( CONF_FILE, str_tmp_path, "passphrase",
                             str_wpakey, un_size );
    return res;
}
...
static int conf_wireless_radius_set_authip( char *str_flag, char *str_ipaddr )
{
    ...
    ret = set_data_by_path( CONF_FILE, str_tmp_path, 
                            strcmp(str_flag, "1")?"radiusip1":"radiusip", 
                            str_ipaddr ); //modify by sys0508
                                            ...
                                            unsigned int un_size )
{
    ...
    ret = get_data_by_path( CONF_FILE, str_tmp_path, 
                            strcmp(str_flag, "1")?"radiusip1":"radiusip", 
                            str_ipaddr, un_size ); //modify by sys0508
...
static int conf_wireless_radius_set_authport( char *str_flag, char *str_port )
{
    ...
    ret = set_data_by_path( CONF_FILE, str_tmp_path, 
                            strcmp(str_flag, "1")?"authport1":"authport", 
                            str_port ); //modify by sys0508
                                              ...
                                              unsigned int un_size )
{
    ...
    ret = get_data_by_path( CONF_FILE, str_tmp_path, 
                            strcmp(str_flag, "1")?"authport1":"authport", 
                            str_port, un_size ); //modify by sys0508
...
static int conf_wireless_radius_set_acctip( char *str_flag, char *str_ipaddr )
{
    ...
    ret = set_data_by_path( CONF_FILE, str_tmp_path, 
                            strcmp(str_flag, "1")?"radiusaip1":"radiusaip", 
                            str_ipaddr ); //modify by sys0508
...
static int conf_wireless_radius_set_acctport( char *str_flag, char *str_port )
{
    ...
    ret = set_data_by_path( CONF_FILE, str_tmp_path, 
                            strcmp(str_flag, "1")?"accepport1":"acceptport", 
                            str_port );       //modify by sys0508
...
static int conf_wireless_radius_get_acctport( char *str_flag, char *str_port,
                                            unsigned int un_size )
{
    char str_tmp_path[MAX_PATH_LEN];
    int ret = XERR_NO_ERROR;
    ...
    ret = get_data_by_path( CONF_FILE, str_tmp_path, 
                            strcmp(str_flag, "1")?"acceptport1":"acceptport", 
                            str_port,un_size ); //modify by sys0508
...
int wireless_radius_get_acctport( char *str_flag, char *str_port,
                                unsigned int un_size )
{ 
    return conf_wireless_radius_get_acctport( str_flag, str_port, un_size );
}
...
static int conf_wireless_radius_set_sharekey( char *str_flag, char *str_key )
{
    ...
    ret = set_data_by_path( CONF_FILE, str_tmp_path, 
                            strcmp(str_flag, "1")?"sharekey1":"sharekey", 
                            str_key ); //modify by sys0508
                                              ...
                                              unsigned int un_size )
{
    ...
    ret = get_data_by_path( CONF_FILE, str_tmp_path, 
                            strcmp(str_flag, "1")?"sharekey1":"sharekey",
                            str_key, un_size );   //modify by sys0508
...
//add by sys0508  start
static int conf_wireless_radius_set_asharekey( char *str_flag, char *str_key )
{
    char str_tmp_path[MAX_PATH_LEN];
    int ret = XERR_NO_ERROR;
    ...
    ret = set_data_by_path( CONF_FILE, str_tmp_path, 
                            strcmp(str_flag, "1")?"asharekey1":"asharekey", 
                            str_key );
...
int wireless_radius_set_asharekey( char *str_flag, char *str_key )
{ 
    char str_curkey[70]={0};
	if(wireless_radius_get_asharekey( str_flag,str_curkey, 70) == IFERR_NO_ERROR)
	{
	    if(strcmp(str_curkey,str_key) == 0)
	    {
			return IFERR_ARG_INVALID;   
	    }
        return conf_wireless_radius_set_asharekey( str_flag, str_key );
	}
	return IFERR_FILE_READ;
}


static int conf_wireless_radius_get_asharekey( char *str_flag, char *str_key,
                                              unsigned int un_size )
{
    char str_tmp_path[MAX_PATH_LEN];
    int ret = XERR_NO_ERROR;
...
int wireless_radius_get_asharekey( char *str_flag, char *str_key, 
                                  unsigned int un_size )
{ 
    return conf_wireless_radius_get_asharekey( str_flag, str_key, un_size );
}
