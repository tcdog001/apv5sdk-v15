...
*
* filename: httpda.c
* description:  implementation for gSOAP HTTP Digest Authentication plugin.Supports both Basic and Digest authentication.
* 
*
* 
************************************************************************************/
...
{ 
  ...
  if (key && !strcmp(key, "Authorization") && data->nonce != NULL )
  {
...
static int http_da_prepareinit(struct soap *soap)
{
  ...
  else
  {
    ...
    if (soap->userid && soap->passwd && data->nonce != NULL)
    {
      md5_handler(soap, &data->context, MD5_INIT, NULL, 0);
