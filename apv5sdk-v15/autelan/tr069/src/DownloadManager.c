...
* All rights reserved.
*
* filename: downloadmanager.c   
* description:  implementation for downloadmanager application 
*  -use curl library for downloading file
*  -answers to cwmp process
* 
************************************************************************************/
...
/***                                                                     ***/
/*** ex. of execution:                                                   ***/
/*** ./downloadmanager --FileType:'1 Firmware Upgrade Image' --URL:'http://host:port'  --Username: --Password: --FileSize:1000 --TargetFileName: --DelaySeconds:1 --CWMPAddress:localhost:8093                            ***/
/***************************************************************************/ 
...
int nErrorCode=0;  //update by wangjr 091125
...
int write_to_file(void *buffer, size_t size, size_t nmemb, void *stream)
{
                            ...
                            const char *_proxyuserpwd)
{
  ...
  dprintf(LOG_LEVEL1, "download_funct\n");
  dprintf(LOG_LEVEL3, "_url: %s\n", _url);
  dprintf(LOG_LEVEL3, "_targetfilename: %s\n", _targetfilename);
  ...
  if(curl)
  {
    ...
    dprintf(LOG_LEVEL3, "szFileName: %s\n", szFileName);
    dprintf(LOG_LEVEL3, "CURLE_OK: %d\n", CURLE_OK);

    outfile = fopen(szFileName, "w");
    res = curl_easy_setopt(curl, CURLOPT_URL, _url);
    dprintf(LOG_LEVEL3, "after curl_easy_setopt(curl, CURLOPT_URL, res: %d\n", res);

    if ((strcmp(_user, "") != 0) && (strcmp(_passwd, "") != 0)) {
	  sprintf(szTemp, "%s:%s", _user, _passwd);
      res = curl_easy_setopt(curl, CURLOPT_USERPWD, szTemp);
      dprintf(LOG_LEVEL3, "after curl_easy_setopt(curl, CURLOPT_USERPWD, res: %d\n", res);
    }

    #if 0
    if (strcmp(_proxyhost, "") != 0) {
      res = curl_easy_setopt(curl, CURLOPT_PROXY, _proxyhost);
      dprintf(LOG_LEVEL3, "after curl_easy_setopt(curl, CURLOPT_PROXY, res: %d\n", res);

      res = curl_easy_setopt(curl, CURLOPT_PROXYPORT, _proxyport);
      dprintf(LOG_LEVEL3, "after curl_easy_setopt(curl, CURLOPT_PROXYPORT, res: %d\n", res);
      ...
      if ((strcmp(_proxyuserid, "") != 0) && (strcmp(_proxyuserpwd, "") != 0))
      {
        ...
        dprintf(LOG_LEVEL3, "after curl_easy_setopt(curl, CURLOPT_PROXYUSERPWD, res: %d\n", res);
      }
    }
	#endif
    ...
    dprintf(LOG_LEVEL3, "after curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, res: %d\n", res);

    /* Set a pointer to our struct to pass to the callback */
    res = curl_easy_setopt(curl, CURLOPT_WRITEDATA, outfile);
    dprintf(LOG_LEVEL3, "after curl_easy_setopt(curl, CURLOPT_WRITEDATA, res: %d\n", res);

    res = curl_easy_perform(curl);
    if(res == 1)  //add by wangjr 091130
    {
		nErrorCode = 9013;
    ...
    dprintf(LOG_LEVEL3, "after curl_easy_perform, res: %d\n", res);

    fclose(outfile);

    res = curl_easy_getinfo(curl, CURLINFO_SIZE_DOWNLOAD, &DownloadSize);
    dprintf(LOG_LEVEL3, "after curl_easy_getinfo(curl, CURLINFO_SIZE_DOWNLOAD, res: %d\n", res);
