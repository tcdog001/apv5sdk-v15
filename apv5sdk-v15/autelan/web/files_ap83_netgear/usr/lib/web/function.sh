...
writeflash()
{
   ...
   local xml_lan_proto=`config_getoption "$xml_config" proto`
   local xml_lan_ipaddr=`config_getoption "$xml_config" ipaddr`
   local xml_lan_netmask=`config_getoption "$xml_config" netmask`
   local xml_lan_nameserver1=`config_getoption "$xml_config" nameserver1`

   local xml_sroute=`config_read /config/network/sroute`
   local xml_sroute_route1=`config_getoption "$xml_sroute" route1`
   
   local xml_default_vlan=`config_read /config/network/default_vlan`
   local xml_vid=`config_getoption "$xml_default_vlan" vid`
   
   local magic="1464287568"
#pei add
   CONFIG_XML=/etc/defaults/config.xml
   local xml_default_config=`config_read /config/network/lan`
   local xml_lan_default_proto=`config_getoption "$xml_default_config" proto`
   local xml_lan_default_ipaddr=`config_getoption "$xml_default_config" ipaddr`
   local xml_lan_default_netmask=`config_getoption "$xml_default_config" netmask`
   local xml_lan_default_nameserver1=`config_getoption "$xml_default_config" nameserver1`

   local xml_default_sroute=`config_read /config/network/sroute`
   local xml_sroute_default_route1=`config_getoption "$xml_default_sroute" route1`
   
   local xml_default_default_vlan=`config_read /config/network/default_vlan`
   local xml_default_vid=`config_getoption "$xml_default_default_vlan" vid`

   CONFIG_XML=/etc/config/config.xml
   local flag_same=1
if [ "${xml_lan_proto}" != "${xml_lan_default_proto}" ]; then
	flag_same=0
fi
if [ "${xml_lan_ipaddr}" != "${xml_lan_default_ipaddr}" ]; then
	flag_same=0
fi
if [ "${xml_lan_netmask}" != "${xml_lan_default_netmask}" ]; then
	flag_same=0
fi
if [ "${xml_lan_nameserver1}" != "${xml_lan_default_nameserver1}" ]; then
	flag_same=0
fi
if [ "${xml_sroute_route1}" != "${xml_sroute_default_route1}" ]; then
	flag_same=0
fi
if [ "${xml_vid}" != "${xml_default_vid}" ]; then
	flag_same=0
fi
if [ "${flag_same}" -eq 1 ]; then
	echo "all IP,vlan setting are same."
	xml_lan_proto="static"
	xml_lan_ipaddr="0.0.0.0"
	xml_lan_netmask="0.0.0.0"
	xml_lan_nameserver1="0.0.0.0"
	xml_sroute_route1="0.0.0.0"
	xml_vid="0"
	magic="0"
fi
#pei add end   

   pro_ctl_util -M "$magic"  > /dev/null 2>&1
   pro_ctl_util -V "$xml_vid"  > /dev/null 2>&1
   if [ "$xml_lan_proto" == "static" ]; then
      pro_ctl_util -P "0"  > /dev/null 2>&1
      pro_ctl_util -I "$xml_lan_ipaddr"  > /dev/null 2>&1
      pro_ctl_util -S "$xml_lan_netmask"  > /dev/null 2>&1
      pro_ctl_util -G "$xml_sroute_route1"  > /dev/null 2>&1
      pro_ctl_util -D "$xml_lan_nameserver1"  > /dev/null 2>&1
   fi
   if [ "$xml_lan_proto" == "dhcp" ]; then    
      pro_ctl_util -P "1"  > /dev/null 2>&1
   fi
...
#
# ac_dns_setup(nameserver1, nameserver2)
#
ac_dns_setup()
{
    local nameserver1="$1"
   ...
   local xml_config=`config_read /config/network/lan`
   local xml_proto=`config_getoption "$xml_config" proto`
    if [ "$xml_proto" == "static" ]; then
       xml_config=`config_setoption "$xml_config" nameserver_config "static"`
       xml_config=`config_setoption "$xml_config" nameserver1 "$nameserver1"`
       xml_config=`config_setoption "$xml_config" nameserver2 "$nameserver2"`
       config_write /config/network/lan "$xml_config"
    fi
...
thin_vlan_setup()
{
   ...
   eval create="$FORM_vlan"
   eval vid="$FORM_vlanid"

   xml_config=`config_read /config/network/default_vlan`
        xml_config=`config_setoption "$xml_config" create "$create"`
        xml_config=`config_setoption "$xml_config" vid "$vid"`
        config_write /config/network/default_vlan "$xml_config"
...
advanceset_setup()
{
    ...
    local xml_wifi=`config_read /config/network/$xml_wlan_wifi`
	    ...
	    xml_wifi=`config_setoption "$xml_wifi" txpower "$txpower"`
