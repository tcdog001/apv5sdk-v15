...
function fetchObjectsByTagName(selTagName, parentLevel, forceFetch)
{
	parents='';
	...
	switch (DOMtype)
	{
		case "std":
		{
			BrowserObjects = eval(parents+'document.getElementsByTagName(selTagName)');
		}
		break;

		case "ie4":
		{
			BrowserObjects = eval(parents+'document.all.tags(selTagName)');
		}
		break;

		case "ns4":
		{
			BrowserObjects = eval(parents+'document.layers[selTagName]');
		}
		break;
	}
