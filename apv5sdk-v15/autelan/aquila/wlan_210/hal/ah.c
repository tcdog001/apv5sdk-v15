    ...
    struct hal_reg_parm *hal_conf_parm, HAL_STATUS *error)
{
    ...
    case AR9300_DEVID_EMU_PCIE: //Added by duanmingzhe fix can not probe PCIE
    ...
    case HAL_CAP_TXPOW:     /* global tx power limit  */
        ...
        case 4:         /* extra power for certain rates */
            *result = AH_PRIVATE(ah)->ah_extra_txpow;
            return HAL_OK;
		/*zhaoyang1 transplant from 717*/
        case 5:                                /*added by chenming to obtain maxtxpower-issue num PCAPVXN-78*/
            *result = AH_PRIVATE(ah)->ah_maxPower;
            return HAL_OK;
		/*zhaoyang1 transplant end*/
        }
        return HAL_ENOTSUPP;
                         ...
                         void *valBuff)
{
    ...
    */
    ...
    switch(p)
    {
        ...
        case HAL_PARAM_DEBUG_NL:
            {
                u_int32_t tmp = *(u_int32_t *)valBuff;
                if(tmp < 127)
                    ah->ah_noise_level = tmp;
                else
                    return -1;
            }
            break;
