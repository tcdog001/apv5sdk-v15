    ...
    u_int8_t * pPDADCValues, u_int16_t numXpdGains)
{
    ...
    u_int8_t   minPwrT4[AR9287_NUM_PD_GAINS] = {0}; //AUTELAN-Added:Modified by duanmingzhe for fixed bug from coverity check
    u_int8_t   maxPwrT4[AR9287_NUM_PD_GAINS] = {0}; //AUTELAN-Added:Modified by duanmingzhe for fixed bug from coverity check
    int16_t   vpdStep;
    int16_t   tmpVal;
...
ar9287EepromSetPowerCalTable(struct ath_hal *ah, ar9287_eeprom_t *pEepData, HAL_CHANNEL_INTERNAL *chan, int16_t *pTxPowerIndexOffset)
{
    ...
    u_int8_t  pdadcValues[AR9287_NUM_PDADC_VALUES] = {0}; //AUTELAN-Added:Modified by duanmingzhe for fixed bug from coverity check
    u_int16_t gainBoundaries[AR9287_PD_GAINS_IN_MASK] = {0}; //AUTELAN-Added:Modified by duanmingzhe for fixed bug from coverity check
