...
#if ATH_SUPPORT_AP_WDS_COMBO
		if (OS_MEMCMP(&(wh->i_addr1[0]), &(sc->sc_myaddr[0]), 3) && OS_MEMCMP(&(wh->i_addr1[4]), &(sc->sc_myaddr[4]), 2)) {
			if (((wh->i_addr1[0] & sc->sc_bssidmask[0]) == (sc->sc_myaddr[0] & sc->sc_bssidmask[0])) &&
			((wh->i_addr1[IEEE80211_ADDR_LEN - 1] & sc->sc_bssidmask[IEEE80211_ADDR_LEN - 1]) == 
			(sc->sc_myaddr[IEEE80211_ADDR_LEN - 1] & sc->sc_bssidmask[IEEE80211_ADDR_LEN - 1])))  {
#else
...
#if ATH_SUPPORT_AP_WDS_COMBO
		if (OS_MEMCMP(&(wh->i_addr1[0]), &(sc->sc_myaddr[0]), IEEE80211_ADDR_LEN - 1)) {
			if (((wh->i_addr1[0] & sc->sc_bssidmask[0]) == (sc->sc_myaddr[0] & sc->sc_bssidmask[0])) &&
			((wh->i_addr1[IEEE80211_ADDR_LEN - 1] & sc->sc_bssidmask[IEEE80211_ADDR_LEN - 1]) == 
			(sc->sc_myaddr[IEEE80211_ADDR_LEN - 1] & sc->sc_bssidmask[IEEE80211_ADDR_LEN - 1])))  {
#else
    ...
    if (index >= rxtid->baw_size) {
        ...
        while (index >= rxtid->baw_size) {
            ...
            if (rxbuf->rx_wbuf != NULL) {
				...
				/*autelan - hxh: add for icmp trace print */
				if(wbuf_to_indicate != NULL)
					ICMP_TRACE(wbuf_to_indicate, __func__, __LINE__);
				/*autelan - hxh: add for icmp trace print  -end*/
