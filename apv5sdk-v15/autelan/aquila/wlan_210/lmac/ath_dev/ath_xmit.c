...
extern u_int32_t dbg_print;
extern int (*kes_debug_print_handle)(const char *fmt, ...);
//extern int ICMP_SEND( struct ieee80211vap *vap );

int ICMP_TRACE(wbuf_t wbuf, char *func, int line)
{
                ...
                if( llc->llc_snap.ether_type == htons(ETHERTYPE_IP) )
                {
                    if(iphdr->protocol == 1)
                    {
						...
						if(icmptype != 0 && icmptype !=8)
							return 0;
                        //printk("%s:%d[time:%lu ms] send icmp:wbuf->len:%d, sequence=%d, icmptype = %d\n", func, line, ((jiffies / HZ) * 1000) + (jiffies % HZ) * (1000 / HZ), wbuf->len ,seq, icmptype);

                        if(kes_debug_print_handle)
            		   {
            		    	kes_debug_print_handle(KERN_EMERG "%s:%d[time:%lu ms] send icmp:wbuf->len:%d, sequence=%d, icmptype = %d\n", func, line, ((jiffies / HZ) * 1000) + (jiffies % HZ) * (1000 / HZ), wbuf->len ,seq, icmptype);
            		   }
                    
                        if(dbg_print & 0x02)
                        {
                            printk("%s:%d,wbuf is bellow 2:\n", func, line);
                            for (i = 0; i < pkt_len;  )
                            {
                                printk("%02x ", (u_int8_t)tmp[i]);
                                i++;
                                if(i %16 == 0)
                                {
                                    printk("\n");
...
EXPORT_SYMBOL(ICMP_TRACE);

void ICMP_TRACE_ETHER(wbuf_t wbuf, char *func, int line)
{
    struct icmphdr *icmpheader = NULL;
    struct iphdr * iphdr = NULL;
    u_int8_t icmptype = 0;
    ...
    if( ( dbg_print & 0x01 ) && NULL != wbuf)
    {
        if(wbuf->len > (sizeof(struct ethhdr) + sizeof(struct iphdr) + sizeof(struct icmphdr)))        //802.3 frame
        {
             ...
             iphdr = (struct iphdr *)((u_int8_t *)wbuf->data + sizeof(struct ethhdr));
             if(iphdr->protocol == 1)
             {
                 icmpheader = (struct icmphdr *)((u_int8_t *)iphdr + sizeof(struct iphdr)); 
                 icmptype = icmpheader->type;
                 seq = icmpheader->un.echo.sequence;
                 printk("%s:%d[time:%lu ms] send icmp from ethernet:wbuf->len:%d, sequence=%d, icmptype = %d\n", func, line, ((jiffies / HZ) * 1000) + (jiffies % HZ) * (1000 / HZ), wbuf->len, seq, icmptype);
                  if(kes_debug_print_handle)
                 {
                      kes_debug_print_handle(KERN_EMERG "%s:%d[time:%lu ms] send icmp from ethernet:wbuf->len:%d, sequence=%d, icmptype = %d\n", func, line, ((jiffies / HZ) * 1000) + (jiffies % HZ) * (1000 / HZ), wbuf->len, seq, icmptype);
                 }

                 if(dbg_print & 0x02)
                 {
                     printk("%s:%d,wbuf-len:%d, wbuf is bellow:\n",func, line, wbuf->len);
                     for (i = 0; i < wbuf->len; )
                     {
                         printk("%02x ", wbuf->data[i]);
                         i++;
...
EXPORT_SYMBOL(ICMP_TRACE_ETHER);

//extern u_int32_t aute_latency_wait;

int ICMP_PRASE(wbuf_t wbuf, struct ieee80211_node *ni,u_int32_t *tmp_len)
{
    struct icmphdr *icmpheader = NULL;
    struct iphdr * iphdr = NULL;
    struct llc *llc = NULL;
                ...
                if( llc->llc_snap.ether_type == htons(ETHERTYPE_IP) )
                {
                    ...
                    {   if((loop_num%10000) == 8)
                        {   
                            if(iphdr->daddr == 0)
                            {
                                printk("Fatal error iphdr=0x%x,ni=0x%x\n",iphdr,ni);
                                ni->aute_latency_wait = 0;
                            }
                            else
                            {
                                if(dbg_print & 0x08)
                                {
                                    printk("learn the ip address ni->saddr:0x%x,ni->daddr:0x%x, ethtype=%u\n",ni->saddr,ni->daddr,llc->llc_snap.ether_type);
                                    if(dbg_print & 0x10)
                                    {
                                        printk("%s:%d,wbuf-len:%d, wbuf is bellow:\n",__func__, __LINE__, wbuf->len);
                                        for (i = 0; i < wbuf->len; )
                                        {
                                             printk("%02x ", wbuf->data[i]);
                                             i++;
...
#define    N(a)    (sizeof(a)/sizeof(a[0]))
    ...
    if (!ATH_TXQ_SETUP(sc, qnum)) {
        ...
        //zhaoenjuan print axq_totalqueued
         printk("%s :the axq_totalqueued is zero(%d)",__func__,txq->axq_totalqueued);
        if(kes_debug_print_handle)
        {
...
#define DESC2PA(_sc, _va)	\
    ...
    if (sc->sc_enhanceddmasupport) {
		...
		if(dbg_print & 0x01)
		{
			struct ath_buf * tmp_bf = bf;
			TAILQ_FOREACH(tmp_bf, head, bf_list) 
			//while(tmp_bf != NULL)
			{
				if(tmp_bf->bf_isicmp)
        			ICMP_TRACE(tmp_bf->bf_mpdu, (char *)__func__, __LINE__);
				//tmp_bf = tmp_bf->bf_next;
			}
		}
		#endif
		ICMP_TRACE(bf->bf_mpdu, (char *)__func__, __LINE__);
        ath_hal_puttxbuf(ah, txq->axq_qnum, bf->bf_daddr);
        ...
        sc->sc_stats.ast_tx_bytes +=wbuf_get_pktlen(bf->bf_mpdu);//added by zhaoenjuan for wifi  tx statistic 2013-10-24
       //printk("%s: sc->sc_stats.ast_tx_bytes <wbuf_get_pktlen(bf->bf_mpdu)> =%lu\n",__func__,sc->sc_stats.ast_tx_bytes);(ÎÞ¼ÆÊý)
...
__ath_tx_prepare(struct ath_softc *sc, wbuf_t wbuf, ieee80211_tx_control_t *txctl)
{
    ...
    struct ieee80211vap *vap = NULL;    //Start:Added by zhanghu for nrates and ntries
	...
	/*zhaoyang1 transplant end*/
    vap = ni->ni_vap;    //Start:Added by zhanghu for nrates and ntries
    ...
    /*zhaoyang1 transplant end*/
            ...
            if (
                ...
                if (likely(!txctl->use_minrate)) {
                    ...
                    if(TATAL_N_TIME(vap->iv_nrates)){
                        rcs[0].tries = GET_N_TRIES(vap->iv_ntries, 0);
                        rcs[1].tries = 0;
                        rcs[2].tries = 0;
                        rcs[3].tries = 0;
                    }
...
/*Autelan-Added:duanmingzhe for patch tx-stuck patchs from case 01146407*/			
        ...
        /*Start:Added by zhanghu for nrates and ntries*/
        if(TATAL_N_TIME(vap->iv_nrates) && txctl->isdata ){
            rcs[0].tries = GET_N_TRIES(vap->iv_ntries, 0);
            rcs[1].tries = 0;
            rcs[2].tries = 0;
            rcs[3].tries = 0;
...
#define FOR_BIT(parm)    (sizeof(parm)*8)
        ...
        if((ic->ic_rate_mask & ((u_int64_t)1 << (FOR_BIT(ic->ic_rate_mask) - 1))) && txctl->min_rate == 0){
            ...
            if(max == i){
                printk("%s:%u--can't find the vaild rate\n", __func__, __LINE__);
            }
            if(i < max && \
    ...
    /*yanggs transplant for cts*/
	/*lyb add start*/
	if (noise_sched&&((wh->i_fc[0]&IEEE80211_FC0_TYPE_MASK) == IEEE80211_FC0_TYPE_CTL)&&((wh->i_fc[0] & IEEE80211_FC0_SUBTYPE_MASK)== IEEE80211_FC0_SUBTYPE_CTS)) 
	{
		*(u_int16_t *)wh->i_dur = cpu_to_le16(cts_duration);
		//printk("i_dur:%d\n",*(u_int16_t *)wh->i_dur);
   	}
...
ath_tx_start_dma(wbuf_t wbuf, sg_t *sg, u_int32_t n_sg, void *arg)
{
    ...
    if (firstbf) {
...
#endif
        {
          ...
          sc->sc_stats.ast_txq_noaggr_packets[txq->axq_qnum]++;//added by zhaoej for stuck
    ...
    if(sc->sc_enhanceddmasupport)
    {
        ...
        if(bf==NULL)
        {
            if (txq->axq_headindex != txq->axq_tailindex)
                printk("dump_txq_desc: ERR head %d tail %d\n",
...
ath_tx_tasklet(ath_dev_t dev)
{
	...
	//AUTELAN:added for beacon stuck check
