...
struct txqinfo qcu_info_wifi0[10];
struct txqinfo qcu_info_wifi1[10];
EXPORT_SYMBOL(qcu_info_wifi0);
EXPORT_SYMBOL(qcu_info_wifi1);
...
ath_tx_edma_tasklet(ath_dev_t dev)
{
    ...
    for (;;) {
		...
		if(txqprint)
		{
			if(!printdevid && !strcmp(sc->sc_osdev->netdev->name,"wifi0"))
			{
			    int i;
				...
				for(i = 0;i < HAL_NUM_TX_QUEUES;i++)
				{
					...
					qcu_info_wifi0[i].txq_headindex = tmp_txq->axq_headindex;
					qcu_info_wifi0[i].txq_tailindex = tmp_txq->axq_tailindex;
					qcu_info_wifi0[i].txq_depth = tmp_txq->axq_depth;
					qcu_info_wifi0[i].txq_minfree = tmp_txq->axq_minfree;
					qcu_info_wifi0[i].txq_num_buf_used = tmp_txq->axq_num_buf_used;
					qcu_info_wifi0[i].txq_aggr_depth = tmp_txq->axq_aggr_depth;
					qcu_info_wifi0[i].txq_totalqueued = tmp_txq->axq_totalqueued;
					qcu_info_wifi0[i].txq_intrcnt = tmp_txq->axq_intrcnt;
					ATH_TXQ_UNLOCK(tmp_txq);
				}
				txqprint = 0;
			}

			if(printdevid && !strcmp(sc->sc_osdev->netdev->name,"wifi1"))
			{
			    int i;
			    struct ath_txq *tmp_txq;
				for(i = 0;i < HAL_NUM_TX_QUEUES;i++)
				{
					tmp_txq = &sc->sc_txq[i];				
					ATH_TXQ_LOCK(tmp_txq);
					qcu_info_wifi1[i].txq_headindex = tmp_txq->axq_headindex;
					qcu_info_wifi1[i].txq_tailindex = tmp_txq->axq_tailindex;
					qcu_info_wifi1[i].txq_depth = tmp_txq->axq_depth;
					qcu_info_wifi1[i].txq_minfree = tmp_txq->axq_minfree;
					qcu_info_wifi1[i].txq_num_buf_used = tmp_txq->axq_num_buf_used;
					qcu_info_wifi1[i].txq_aggr_depth = tmp_txq->axq_aggr_depth;
					qcu_info_wifi1[i].txq_totalqueued = tmp_txq->axq_totalqueued;
					qcu_info_wifi1[i].txq_intrcnt = tmp_txq->axq_intrcnt;
					ATH_TXQ_UNLOCK(tmp_txq);
				}
				txqprint = 0;
			}
...
{
    int error = 0;
	int8_t  i = 0;  //Added by duanmingzhe for print txq info
	...
	for(i = 0; i < 10; i++)	
	{	
		qcu_info_wifi0[i].txq_headindex = 0;
		qcu_info_wifi0[i].txq_tailindex = 0;
		qcu_info_wifi0[i].txq_depth = 0;
		qcu_info_wifi0[i].txq_minfree = 0;
		qcu_info_wifi0[i].txq_num_buf_used = 0;
		qcu_info_wifi0[i].txq_aggr_depth = 0;
		qcu_info_wifi0[i].txq_totalqueued = 0;
		qcu_info_wifi0[i].txq_intrcnt = 0;

		qcu_info_wifi1[i].txq_headindex = 0;
		qcu_info_wifi1[i].txq_tailindex = 0;
		qcu_info_wifi1[i].txq_depth = 0;
		qcu_info_wifi1[i].txq_minfree = 0;
		qcu_info_wifi1[i].txq_num_buf_used = 0;
		qcu_info_wifi1[i].txq_aggr_depth = 0;
		qcu_info_wifi1[i].txq_totalqueued = 0;
		qcu_info_wifi1[i].txq_intrcnt = 0;
	}
	/*End:Added by duanmingzhe for print txq info*/
    return error;
}
