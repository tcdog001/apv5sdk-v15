...
extern int ICMP_TRACE(struct ieee80211_frame * wh, char *func, int line);
        ...
        ath_bufhead *bf_head, ieee80211_tx_control_t *txctl)
{
    ...
    __11nstatsn(sc, tx_bytes,wbuf_get_pktlen(bf->bf_mpdu));//added by zhaoenjuan for wifi  tx statistic 2013-10-24
	/*zhaoyang modify for add CL101673.fix.patch for dusi*/
...
/*zhaoyang modify end*/
    if (!TAILQ_EMPTY(&tid->buf_q) || tid->paused || txq->axq_depth) {
        ...
        /*autelan - hxh: add for icmp trace print*/
		if(ICMP_TRACE(bf->bf_mpdu ,__func__, __LINE__))
			bf->bf_isicmp = 1;
    ...
    struct aggr_rifs_param *param, int mimoburst, int *prev_frames)
{
    ...
    do {
		...
		/*autelan - hxh: add for icmp trace print */
		if(bf->bf_mpdu != NULL)
			ICMP_TRACE(bf->bf_mpdu, __func__, __LINE__);
...
__11nstats(sc, aute_tx_schedule);//AUTELAN:zhaoenjuan added for tx_schedule statistic
...
#endif
    {
...
#endif
        {
			...
			if (!tid->cleanup_inprogress && !isnodegone &&
                ...
                {
                    struct ath_vap      *av = an->an_avp;
                    struct ieee80211vap *vap = av->av_if_data;
                    if(TATAL_N_TIME(vap->iv_nrates)){
                        sw_retry_limit = TATAL_N_COUNT(vap->iv_ntries);
                    }
                }
...
ath_complete_drained_tid_buf(struct ath_softc *sc, ath_bufhead *bf_q)
{
    ...
    for (;;) {
		...
		if (bf->bf_isbar)
		{
			...
			printk("%s complete bar!!!!! ",__func__);
			printk(" [%02x:%02x:%02x:%02x:%02x:%02x]:an =%p bf = %p\n",ni->ni_macaddr[0],ni->ni_macaddr[1],ni->ni_macaddr[2],ni->ni_macaddr[3],ni->ni_macaddr[4],ni->ni_macaddr[5],an,bf);
