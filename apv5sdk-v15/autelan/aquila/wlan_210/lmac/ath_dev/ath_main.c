...
#endif
/*AUTELAN-zhaoenjuan added for interface up & down*/
#define IS_UP(_dev) \
...
u_int32_t aute_air_flag = 0;
EXPORT_SYMBOL(aute_air_flag);
...
u_int32_t drv_rx_buffer_dump(struct ath_softc *sc)	
{
	...
	printk("RX HPQ depth is %d fifo size is %d rx_hp_cnt is %d\n",rxedma->rxfifodepth,rxedma->rxfifohwsize,rx_hp_cnt);
	rxedma = &sc->sc_rxedma[1];
	printk("RX HPQ depth is %d fifo size is %d rx_lp_cnt is %d\n",rxedma->rxfifodepth,rxedma->rxfifohwsize,rx_lp_cnt);
	printk("###############################################\n");
...
void aute_kernel_restart(u_int32_t error_code)
{
#if defined(PC018) || defined(APM82181) 
    {
        ...
        OS_DELAY(1000000);
        kes_debug_print_flag_handle("E");//AUTELAN-zhaoenjuan added for AP restart flag 2014-3-3
        del_timer(&dog_timer);
...
u_int32_t aute_air_busy_threshold = 99;
EXPORT_SYMBOL(aute_air_busy_threshold);

u_int32_t aute_air_send_num = 0;
EXPORT_SYMBOL(aute_air_send_num);

u_int32_t air_latency_monitor(struct ath_softc *sc)
{
    static u_int32_t loop_num;
    static u_int32_t con_flag;
    struct ieee80211vap *vap = NULL;
    struct ieee80211com *ic =NULL;
    ic = (struct ieee80211com *)(sc->sc_ieee);
    loop_num++;
    if((aute_latency_idle_enable == 1)/*&&(ni->aute_latency_wait == 1)*/)
    {
        TAILQ_FOREACH(vap, &ic->ic_vaps, iv_next)
        { 
            //printk("--air_latency_monitor-- %d,send the packet\n", __LINE__);
            if(NULL != sc->sc_ieee_ops->ICMP_TEST_SEND){
                sc->sc_ieee_ops->ICMP_TEST_SEND(vap);
            }
        }
        
        if((loop_num%60) == 0) //one minute
        {
            if((aute_air_bad_num + aute_air_drop_num) > aute_air_bad_threshhod)
            {
                printk("Air latency detect error, down&up the interface...bad number=%d,drop=%d,send num=%d\n",aute_air_bad_num,aute_air_drop_num,aute_air_send_num); 
                if(kes_debug_print_handle)
                {
                    kes_debug_print_handle(KERN_EMERG "Air latency detect error, down&up the interface...bad number=%d,drop=%d,send num=%d\n",aute_air_bad_num,aute_air_drop_num,aute_air_send_num);
                }
                aute_air_bad_num = 0;
                aute_air_drop_num = 0;
                if(con_flag == 1)
                    aute_air_flag = 1; /*continue air latency detect, need to reboot*/
                else
                    con_flag = 1;

                return AUTE_DRV_AIR_LATENCY_ERR;	
            }
            else
            {
                if(dbg_print & 0x08)
                {
                    printk("Air latency: one minute clear count\n");
 				    if(kes_debug_print_handle)
                    {
                        kes_debug_print_handle(KERN_EMERG "Air latency: one minute clear count\n");
                    }
				}
                aute_air_flag = 0;
                con_flag = 0;
...
u_int32_t drv_air_latency_dump(struct ath_softc *sc)	
{	
    int tidno;
    struct ath_atx_tid *tid;
    int count=0;
   ...
   /*AUTELAN-Begin:zhaoenjuan added for down &up interface*/ 
    TAILQ_FOREACH(vap, &ic->ic_vaps, iv_next)
    {  
        struct net_device *dev = ((osif_dev *)vap->iv_ifp)->netdev;
        printk("Down the interface...%s\n",dev->name);
        if(IS_UP(dev))
        {
            vap->iv_osif_stop(dev);
        }
        vap->iv_kick_all(dev, 1);
        //aute_latency_wait = 0;
        printk("Up the interface....\n");
    }
   /*AUTELAN-End:zhaoenjuan added for down &up interface*/

    if(aute_air_flag == 1)
    {
        printk("Air latency detect continues error, rebooting...\n");  
        return AUTE_MONITOR_REBOOT_NOW;
    }
    else
        return AUTE_MONITOR_NO_REBOOT;
...
OS_TIMER_FUNC(autelan_drv_monitor)
{
    ...
    do{
		 ...
		 else if((loop_num % 5)  == 0)//per 5 sec check
    	 {
    ...
    else
    {
       ...
       do{
            ...
            error_code = air_latency_monitor(sc);//per sec
               if(error_code > 0)
                  goto handle;
...
handle:
    ...
    switch (error_code)
    {
        ...
        case AUTE_DRV_AIR_LATENCY_ERR:
            reboot_flag = drv_air_latency_dump(sc);
            break;
        default:
...
static int
ath_vap_stopping_lmac(ath_dev_t dev, int if_id)
{
...
static int
ath_vap_down_lmac(ath_dev_t dev, int if_id, u_int flags)
{
    struct ath_softc *sc = ATH_DEV_TO_SC(dev);
    struct ath_hal *ah = sc->sc_ah;
    struct ath_vap *avp;
...
static int
ath_suspend_lmac(ath_dev_t dev)
{
    struct ath_softc *sc = ATH_DEV_TO_SC(dev);
    struct ath_hal   *ah = sc->sc_ah;
        ...
        if (status & HAL_INT_FATAL) {
            ...
            if (((AH_TRUE == sc->sc_hang_check) && ath_hw_hang_check(sc)) ||
                ...
                if((AH_TRUE == sc->sc_hang_check) && ath_hw_hang_check(sc))
                {
                   if(kes_debug_print_handle)
                   	{
                   	   kes_debug_print_handle(KERN_EMERG "autelan-1-ath_bstuck_tasklet: stuck beacon by hang_check--sc_hang_check=%d\n", sc->sc_hang_check);
                   	}
                }
...
#endif
{
    ...
    struct ath_vap *avp;
    int rx_mitigation;
    u_int32_t error_code;  //AUTELAN:zhaoenjuan modified,interface id not create,restart the ap 
...
#else
   /*AUTELAN-Begin:zhaoenjuan modified,interface id not create,restart the ap */ 
    if (if_id >= ATH_BCBUF ) {
   ...
   /*AUTELAN-ENd:zhaoenjuan modified,interface id not create,restart the ap*/ 
#endif    
