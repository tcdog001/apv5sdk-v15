...
*/
u_int32_t aute_rx_clear=0;
u_int32_t aute_rx_frame=0;
u_int32_t aute_tx_frame=0;
...
ath_beacon_tasklet(struct ath_softc *sc, int *needmark)
{
    ...
    if (ath_hal_numtxpending(ah, sc->sc_bhalq) != 0)
    {
        ...
        aute_rx_clear = rx_clear;
        aute_rx_frame = rx_frame;
        aute_tx_frame = tx_frame;
        sc->sc_bmisscount++;
#ifdef ATH_SUPPORT_DFS         
	...
	if((sc->sc_3stream_sigflag1) && (sc->sc_3stream_sigflag2)) {
        ...
        if (sc->sc_bmisscount < (BSTUCK_THRESH_PERVAP * sc->sc_nvaps)) {
	     ...
	     {
	         kes_debug_print_handle(KERN_EMERG "autelan-2-0-%s: rx_clear=%d, rx_frame=%d, tx_frame=%d, sc_noise=%d, sc_bmisscount=%d\n", __func__, rx_clear, rx_frame, tx_frame, sc->sc_noise, sc->sc_bmisscount);
	     }
		   ...
		   if(kes_debug_print_handle)
		   {
		       kes_debug_print_handle(KERN_EMERG "autelan-2-1-%s: sc_noise=%d, sc_bmisscount=%d\n", __func__, sc->sc_noise, sc->sc_bmisscount);
		   }
            }
		   ...
		   if(kes_debug_print_handle)
		   {
		       kes_debug_print_handle(KERN_EMERG "autelan-2-2-%s: sc_noise=%d, sc_bmisscount=%d\n", __func__, sc->sc_noise, sc->sc_bmisscount);
		   }
		   ...
		   if(kes_debug_print_handle)
		   {
		       kes_debug_print_handle(KERN_EMERG "autelan-2-3-%s: missed %u consecutive beacons\n", __func__, sc->sc_bmisscount);
		   }
                ...
                if(kes_debug_print_handle)
		   {
		       kes_debug_print_handle(KERN_EMERG "autelan-2-4-%s: missed %u consecutive beacons\n", __func__, sc->sc_bmisscount);
		   }
            ...
            if (sc->sc_noreset) {
                ...
                if (show_cycles && rx_clear >= 99) {
                    ...
                    if (rx_frame >= 99) {
						...
						if(kes_debug_print_handle)
						{
							kes_debug_print_handle(KERN_EMERG "Busy environment detected\n");
						}
                    } else {
						...
						if(kes_debug_print_handle)
						{
							kes_debug_print_handle(KERN_EMERG "Interference detected\n");
						}
                    }

                    printk("Warning: Set BB hang by software, rx_clear=%d, rx_frame=%d, tx_frame=%d\n",
                           rx_clear, rx_frame, tx_frame);
					
					if(kes_debug_print_handle)
					{
						kes_debug_print_handle(KERN_EMERG "Warning: Set BB hang by software, rx_clear=%d, rx_frame=%d, tx_frame=%d\n",
                           rx_clear, rx_frame, tx_frame);
					}
    ...
    if (bfaddr != 0)
    {
        ...
        sc->sc_stats.ast_be_xmit_bytes += wbuf_get_pktlen(bf->bf_mpdu);//added by zhaoenjuan for wifi  tx statistic 2013-10-24
#ifdef ATH_ADDITIONAL_STATS
