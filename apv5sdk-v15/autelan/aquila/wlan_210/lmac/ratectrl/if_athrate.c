...
ath_rate_getmaxphyrate(struct ath_softc *sc, struct ath_node *an,uint32_t shortgi)
{
	...
	if(an->an_avp->av_opmode == HAL_M_IBSS){
		/*zhaoyang1 transplant from 717*/
		user_rate = ath_rate_getIBSSmaxphyrate(pRateTable,sc->sc_tx_numchains,sc->sc_curmode);/*modified by chenming for rate display*/
		/*zhaoyang1 transplant end*/
	}
	/* otherwise */
	else{
...
if(sc->sc_curmode >= WIRELESS_MODE_11NA_HT20 && sc->sc_curmode <= WIRELESS_MODE_11NG_HT40MINUS)
{
    u_int32_t hal_txchainmask,hal_rxchainmask;
    u_int8_t total_tx_numchains ,total_rx_numchains;
    (void) ath_hal_gettxchainmask(sc->sc_ah, &hal_txchainmask);
    (void) ath_hal_getrxchainmask(sc->sc_ah, &hal_rxchainmask);
    total_tx_numchains = sc->sc_mask2chains[hal_txchainmask];
    total_rx_numchains = sc->sc_mask2chains[hal_rxchainmask];
    return user_rate*(sc->sc_tx_numchains + sc->sc_rx_numchains)/(total_tx_numchains + total_rx_numchains);
}
else 
    return user_rate;
                      ...
                      )
{
    ...
    if((TATAL_N_TIME(vap->iv_nrates))){
        numRates = TATAL_N_TIME(vap->iv_nrates);        
    }
    ...
    if((TATAL_N_TIME(vap->iv_nrates))){
        ...
        ASSERT(numRates <= 4);
        for(; i < numRates; i++){
            series[i].tries = GET_N_TRIES(vap->iv_ntries, i);
        }
        for(; i < 4; i++){
            series[i].tries = 0;
        }
