...
/*AUTELAN-Added-Begin:duanmingzhe for get queue info*/
extern void ath_dump_txquedepth(struct ath_softc *sc);
/*AUTELAN-Added-End:duanmingzhe for get queue info*/
extern void autelan_node_table_info(char * caller,struct ieee80211com *ic,struct ath_softc_net80211 *scn);  //AUTELAN-Added:duanmingzhe for get queue info
...
/*<Begin : caizhibang modify for WBS-4 */
 const unsigned short mcs_rate[32][2][2]= { 650,720,1350,1500,
                                                           1300,1440,2700,3000,
                                                            1950,2170,4050,4500,
                                                            2600,2890,5400,6000,
                                                            3900,4330,8100,9000,
                                                            5200,5780,10800,12000,
                                                            5850,6500,12150,13500,
                                                            6500,7220,13500,15000,
                                                            1300,1440,2700,3000,
                                                            2600,2890,5400,6000,
                                                            3900,4330,8100,9000,
                                                            5200,5780,10800,12000,
                                                            7800,8670,16200,18000,
                                                            10400,11560,21600,24000,
                                                            11700,13000,24300,27000,
                                                            13000,14440,27000,30000,
                                                            1950,2170,4050,4500,
                                                            3900,4330,8100,9000,
                                                            5850,6500,12150,13500,
                                                            7800,8670,16200,18000,
                                                            11700,13000,24300,27000,
                                                            15600,17330,32400,36000,
                                                            17550,19500,36450,40500,
                                                            19500,21670,40500,45000,
                                                            2600,2880,5400,6000,
                                                            5200,5760,10800,12000,
                                                            7800,8680,16200,18000,
                                                            10400,11560,21600,24000,
                                                            15600,17320,32400,36000,
                                                            20800,23120,43200,48000,
                                                            23400,26000,48600,54000,
                                                            26000,28880,54000,60000
                                                      };
/*End : caizhibang modify for WBS-4> */

struct ieee80211_autelan_addr autelan_addr[8] = {0};
    ...
    struct iw_point *data, char *extra)
{
    ...
    struct rssi_stats        *is_rssi_stats = req->vap_rssi_stats; //add by zhaoej  for guangdong mobile
    ...
    memcpy(is_rssi_stats, vap->iv_stats.is_rssi_stats, RSSI_RANGE_NUM * sizeof(struct rssi_stats));//add by zhaoej  for guangdong mobile
   ...
   if(se_htcap)
    {
      ...
      /*Begin: modified by caizhibang, for WBS-4 */
      if((*(se_htcap) & 0x02) == 0x02)
      /*End: modified by caizhibang, for WBS-4 */
      {
           cwn_ht2040 = 1;
      }
      else
      ...
      if((cwn_ht2040 == 1) &&(((*se_htcap) & 0x40) == 0x40))
      {
         shortgi = 1;
      }
      else if((cwn_ht2040 == 1) &&(((*se_htcap) & 0x40) == 0x0))
      {
          shortgi = 0;
      }

      if((cwn_ht2040 == 0) &&(((*se_htcap) & 0x20) == 0x20))
      {
          shortgi = 1;
      }
      else if((cwn_ht2040 == 0) &&(((*se_htcap) & 0x20) == 0))
      {
           shortgi = 0;
      }

       
      if((*(se_htcap+6) & 0xff) == 0xff)
      {
          space_stream = 32;
      }
      else if((*(se_htcap+5) & 0xff) == 0xff)
      {
            space_stream = 24;    
      }
      else if((*(se_htcap+4) & 0xff) == 0xff)
      {
            space_stream = 16;
      }
      else
     ...
     for(i =0; i < space_stream; i++)
     {
              iwe.u.bitrate.value = mcs_rate[i][cwn_ht2040][shortgi] * 10000;
              current_val = iwe_stream_add_value(CURRENT_EV,
                current_val, end_buf, &iwe,
                IW_EV_PARAM_LEN);
     }
...
static int
ieee80211_ioctl_autelan_arp_filter(struct net_device *dev, struct iwreq *iwr)
{
	wlan_if_t vap = NETDEV_TO_VAP(dev);
	struct ieee80211com   *ic = vap->iv_ic;
	struct ieee80211_autelan_arp_filter ik;
	...
	switch (ik.type) { 
        case IEEE80211_PARAM_ARP_FILTER_SET_SWITCH :
            if(1 == ik.arg1) {
                vap->iv_arp_filter_switch = 1;
            } else if(0 == ik.arg1) {
                vap->iv_arp_filter_switch = 0;
            } else {
                return -EINVAL;
            }
            break;
        case IEEE80211_PARAM_ARP_FILTER_GET_SWITCH :
            flag = vap->iv_arp_filter_switch;
            copy_to_user(iwr->u.data.pointer, &(flag), sizeof(flag));
            break;
        case IEEE80211_PARAM_ARP_FILTER_ADD_MAC :
            flag = ieee80211_arpfilter_addmac(vap, ik.macaddr);
            copy_to_user(iwr->u.data.pointer, &(flag), sizeof(flag));
            break;
        case IEEE80211_PARAM_ARP_FILTER_DEL_MAC :
            flag = ieee80211_arpfilter_delmac(vap, ik.macaddr);
            copy_to_user(iwr->u.data.pointer, &(flag), sizeof(flag));
            break;
        case IEEE80211_PARAM_ARP_FILTER_LIST_MAC :
            ieee80211_arpfilter_getlist(vap, iwr->u.data.pointer, 
                                        iwr->u.data.length, &num_mac);
            /*return the number by iwr->u.data.length */
            iwr->u.data.length = num_mac;
            break;
	}
...
#undef IBSSDFS_RECOVER_MASK              
    ...
    /*Start:Added by zhanghu for nrates and ntries*/
    case IEEE80211_PARAM_N_RATES:
        vap->iv_nrates = (value & 0x01010101);
        break;
    case IEEE80211_PARAM_N_TRIES:
        if(TATAL_N_COUNT(vap->iv_ntries) > 10){
            printk("tries must be less than 10.\n");
        }else{
            vap->iv_ntries = value;
        }
        break;
    /*End:Added by zhanghu for nrates and ntries*/
    ...
    void *w, char *extra)
{
	...
	case IEEE80211_PARAM_QUEUE_INFO:
	{
		...
		if(sc != NULL)
		{
			...
			printk("RX HPQ depth is %d fifo size is %d \n",rxedma->rxfifodepth,rxedma->rxfifohwsize);
	...
	/*AUTELAN-Added-Begin:duanmingzhe for get node table*/
	case IEEE80211_PARAM_NODE_TABLE:
	{
		struct ath_softc_net80211 *scn = ATH_SOFTC_NET80211(ic);
		autelan_node_table_info(__func__,ic,scn);
		break;
	}
	/*AUTELAN-Added-End:duanmingzhe for get node table*/
    case IEEE80211_PARAM_PUREN:
...
/*ended by chenming*/
    ...
    */
    ...
    /*Start:Added by zhanghu for nrates and ntries*/
    case IEEE80211_PARAM_N_RATES:
        param[0] = vap->iv_nrates;
        break;
    case IEEE80211_PARAM_N_TRIES:
        param[0] = vap->iv_ntries;
        break;
    /*End:Added by zhanghu for nrates and ntries*/
 /* add by zhaohan@autelan.com Control the sending freq of deauth frames, 2013-12-12*/    
    case IEEE80211_PARAM_DEAUTH_SEND_INTERVAL:
...
#define AGING_TIMEOUT_SEC 60
    ...
    for(i = 0;i < nr;i++)
    {
        sr->isr_rates[i] = ((*(rates +2 + i))&0x7f)*100/2;
    }
   // memcpy(sr->isr_rates, rates+2, nr);
    ...
    if (rates) {
        ...
        for(i=0;i<nxr;i++)
        {
            sr->isr_rates[nr+i] = ((*(rates + 2 + i))&0x7f)*100/2;
        }
   ...
   do{
           ...
           if(se_htcap)
           {
               ...
               for(i =0; i < space_stream; i++)
               {
                    sr->isr_rates[nr+nxr+i] = mcs_rate[i][cwn_ht2040][shortgi];
               }
               sr->isr_nrates  += space_stream;
           }          
    }while(0);
    /*End: caizhibang modify for WBS-4> */
    cp = (u_int8_t *)(sr+1);
...
get_sta_info(void *arg, wlan_node_t node)
{
    ...
    /*Begin: added by zhanghu for the bug of  PCAPVXNN-107 2013-03-19*/
    wlan_dev_t ic = wlan_vap_get_devhandle(vap);
    struct ath_softc *sc = (ATH_SOFTC_NET80211(ic))->sc_dev;
    /*End: added by zhanghu for the bug of  PCAPVXNN-107 2013-03-19*/
    ...
    {
        int32_t ret = 0;
        ret = wlan_get_bss_dot11Rate(vap, si->isi_rates, IEEE80211_RATE_MAXSIZE);

        if(-EINVAL == ret){
            printk("error wlan_get_bss_dot11Rate\n");
        }
    }
    /*End: added by zhanghu for the bug of  PCAPVXNN-107 2013-03-19*/
    ...
    si->isi_htcap = wlan_node_get_htcap(node);
     /*<AUTELAN-Added-Begin:caizhibang add for data & mgmt frame count */
     si->isi_refcnt = ieee80211_node_refcnt(node);
      /*AUTELAN-Added-End:caizhibang add for data & mgmt frame count> */
...
/*ended by chenming*/
    ...
    /*Start:Added by zhanghu for nrates and ntries*/
    { IEEE80211_PARAM_N_RATES,
        IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "set_nrates" },
    { IEEE80211_PARAM_N_RATES,
        0,IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_nrates" },
        
    { IEEE80211_PARAM_N_TRIES,
        IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "set_ntries" },
    { IEEE80211_PARAM_N_TRIES,
        0,IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_ntries" },
    /*End:Added by zhanghu for nrates and ntries*/
	...
	/*AUTELAN-Added-End:duanmingzhe for get queue info*/
	/*AUTELAN-Added-Begin:duanmingzhe for get node table*/
    { IEEE80211_PARAM_NODE_TABLE,
        0,IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_nodetable" },
	/*AUTELAN-Added-End:duanmingzhe for get node table*/
	/* add by zhaohan@autelan.com. Control the sending freq of deauth frames, 2013-12-12 */
...
ieee80211_ioctl_autelan_niflag(struct net_device *dev, struct iwreq *iwr)
{
	...
	switch (ik.type) {
		...
		case SET_SAFE1:
		    for(i=0;i<8;i++)
		    {
		        if(autelan_addr[i].is_use == 1)
		        {
    		        if(strncmp(autelan_addr[i].if_name,iwr->ifr_name,strlen(iwr->ifr_name))==0)
    		            break;
		        }else if(autelan_addr[i].is_use == 0)
		        {
		            break;
		        }
		    }
		    if(i<8)
		    {
		        if(SET_CPE==ik.type)
		        {
    		        memset(autelan_addr[i].if_name,0x00,16);
    		        memset(&(autelan_addr[i].cpe_addr),0x00,4);
    		        memcpy(autelan_addr[i].if_name,iwr->ifr_name,strlen(iwr->ifr_name));
    		        memcpy(&(autelan_addr[i].cpe_addr),ik.ni_portal_ipaddr,4);
    		        autelan_addr[i].is_use = 1;
    		    }else if(SET_PORTAL==ik.type)
    		    {
    		        memset(autelan_addr[i].if_name,0x00,16);
    		        memset(&(autelan_addr[i].portal_addr),0x00,4);
    		        memcpy(autelan_addr[i].if_name,iwr->ifr_name,strlen(iwr->ifr_name));
    		        memcpy(&(autelan_addr[i].portal_addr),ik.ni_portal_ipaddr,4);
    		        autelan_addr[i].is_use = 1;
    		    }else if(SET_SAFE1==ik.type)
    		    {
    		        memset(autelan_addr[i].if_name,0x00,16);
    		        memset(&(autelan_addr[i].safe1_addr),0x00,4);
    		        memcpy(autelan_addr[i].if_name,iwr->ifr_name,strlen(iwr->ifr_name));
    		        memcpy(&(autelan_addr[i].safe1_addr),ik.ni_portal_ipaddr,4);
    		        autelan_addr[i].is_use = 1;
				}else
    		    {
    		        memset(autelan_addr[i].if_name,0x00,16);
    		        memset(&(autelan_addr[i].safe_addr),0x00,4);
    		        memcpy(autelan_addr[i].if_name,iwr->ifr_name,strlen(iwr->ifr_name));
    		        memcpy(&(autelan_addr[i].safe_addr),ik.ni_portal_ipaddr,4);
    		        autelan_addr[i].is_use = 1;
    		    }
		    }else
		    {
		        printk("Max num of ath is 8,no space for more\n");
		        return -EFAULT;
		    }
		    break;
		case GET_ADDR:
		    ...
		    {
		        ik.ni_localflags = 1;		    
    			ik.ni_portal_ipaddr[0] = ((uint8_t*)(&(autelan_addr[i].portal_addr)))[0];
    			ik.ni_portal_ipaddr[1] = ((uint8_t*)(&(autelan_addr[i].portal_addr)))[1];
    			ik.ni_portal_ipaddr[2] = ((uint8_t*)(&(autelan_addr[i].portal_addr)))[2];
    			ik.ni_portal_ipaddr[3] = ((uint8_t*)(&(autelan_addr[i].portal_addr)))[3];
    			ik.ni_pce_ipaddr[0] = ((uint8_t*)(&(autelan_addr[i].cpe_addr)))[0];
    			ik.ni_pce_ipaddr[1] = ((uint8_t*)(&(autelan_addr[i].cpe_addr)))[1];
    			ik.ni_pce_ipaddr[2] = ((uint8_t*)(&(autelan_addr[i].cpe_addr)))[2];
    			ik.ni_pce_ipaddr[3] = ((uint8_t*)(&(autelan_addr[i].cpe_addr)))[3];
    			ik.ni_safe_ipaddr[0] = ((uint8_t*)(&(autelan_addr[i].safe_addr)))[0];
    			ik.ni_safe_ipaddr[1] = ((uint8_t*)(&(autelan_addr[i].safe_addr)))[1];
    			ik.ni_safe_ipaddr[2] = ((uint8_t*)(&(autelan_addr[i].safe_addr)))[2];
    			ik.ni_safe_ipaddr[3] = ((uint8_t*)(&(autelan_addr[i].safe_addr)))[3];
    			ik.ni_safe1_ipaddr[0] = ((uint8_t*)(&(autelan_addr[i].safe1_addr)))[0];
    			ik.ni_safe1_ipaddr[1] = ((uint8_t*)(&(autelan_addr[i].safe1_addr)))[1];
    			ik.ni_safe1_ipaddr[2] = ((uint8_t*)(&(autelan_addr[i].safe1_addr)))[2];
    			ik.ni_safe1_ipaddr[3] = ((uint8_t*)(&(autelan_addr[i].safe1_addr)))[3];
...
/* temporary to be removed later, until _getstastats() is available */
    ...
    /*Begin:Added by WangJia for arp filter 2013-04-11*/
    case IEEE80211_IOCTL_ARP_FILTER:
        return ieee80211_ioctl_autelan_arp_filter(dev, (struct iwreq *)ifr);
    /*End:Added by WangJia for arp filter 2013-04-11*/
	/*zhaoyang add for data forward base users*/
	case IEEE80211_IOCTL_SET_NIFLAG:
	case IEEE80211_IOCTL_SET_CPE:
	case IEEE80211_IOCTL_SET_PORTAL:
	case IEEE80211_IOCTL_GET_ADDR:
...
static int
ATH_SYSCTL_DECL(ieee80211_sysctl_set_channelchangecount, ctl, write, filp, buffer,
	lenp, ppos)
{
	struct ieee80211vap *vap = ctl->extra1;
	u_int val;
	...
	if (write) {
		ret = ATH_SYSCTL_PROC_DOINTVEC(ctl, write, filp, buffer,
			lenp, ppos);
		if (ret == 0)
			vap->vap_channelchangecount= val;
	} else {
		val = vap->vap_channelchangecount;
		ret = ATH_SYSCTL_PROC_DOINTVEC(ctl, write, filp, buffer,
			lenp, ppos);
	}
	return ret;
...
static const ctl_table ieee80211_sysctl_template[] = {
	...
	/*End:Added by pengdecai for set maxstaion number 2012-03-06*/
	{ .ctl_name = CTL_AUTO,
		.procname = "channelchangecount",
		.mode	  = 0644,
		.proc_handler = ieee80211_sysctl_set_channelchangecount
	  },
