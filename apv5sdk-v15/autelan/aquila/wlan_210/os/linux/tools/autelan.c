...
#define IEEE80211_IOCTL_ARP_FILTER      (SIOCDEVPRIVATE+22) /*Added by wangjia for arp filter*/
...
#define IEEE80211_IOCTL_SET_NIFLAG      (SIOCDEVPRIVATE+25)
/*zhaoyang add end*/
...
#define IEEE80211_IOCTL_SET_CPE         (SIOCDEVPRIVATE+26)
#define IEEE80211_IOCTL_SET_PORTAL         (SIOCDEVPRIVATE+27)
#define IEEE80211_IOCTL_GET_ADDR        (SIOCDEVPRIVATE+28)
/*zhaoyang1 add end*/
...
#define SET_SAFE1           7
	...
	u_int8_t        ni_pce_ipaddr[4];
	u_int8_t        ni_safe_ipaddr[4];
	u_int8_t        ni_safe1_ipaddr[4];
};
/*zhaoyang modify end*/
...
void autelan_tunnel_ctl_help()
{
	...
	printf("\t[interface]\t\tsetcpe     [IP](Format 1 1 1 1,not 1.1.1.1)\n");
	printf("\t[interface]\t\tsetportal  [IP](Format 1 1 1 1,not 1.1.1.1)\n");
	printf("\t[interface]\t\tsetsafe    [IP](Format 1 1 1 1,not 1.1.1.1)\n");
	printf("\t[interface]\t\tsetsafe1   [IP](Format 1 1 1 1,not 1.1.1.1)\n");
	printf("\t[interface]\t\tgetaddr        (The cpe or portal address)\n");
	/*End: zhaoyang1 add for getting infomation of niflag 2013-03-23*/
	printf("\n");
}
...
static void setaddr(int argc,char **argv,int io_type)
{
    if(argc != 8)
    {
        autelan_tunnel_ctl_help();
	...
	}else
	{
		...
		{
		    parm.type = SET_SAFE;
		    io_type = IEEE80211_IOCTL_SET_PORTAL;
        }
		else if(2==io_type)
		{
			parm.type = SET_SAFE1;
		    ...
		    parm.type = (IEEE80211_IOCTL_SET_CPE==io_type)?SET_CPE:SET_PORTAL;
		}
		parm.ni_portal_ipaddr[0] = atoi(argv[4]);
		parm.ni_portal_ipaddr[1] = atoi(argv[5]);
		parm.ni_portal_ipaddr[2] = atoi(argv[6]);
		parm.ni_portal_ipaddr[3] = atoi(argv[7]);
		...
		iwr.u.data.pointer = (void *) buf;
		iwr.u.data.length = sizeof(buf);
		if(autelan_ioctl(&iwr, io_type) !=0)
		{
			printf("autelan.c:set cpe or portal addr error\n");
            return;
        }
        memset(&parm, 0, sizeof(struct ieee80211_autelan_tunnel_ctl));
		memcpy(&parm, buf, sizeof(struct ieee80211_autelan_tunnel_ctl));
...
static void getaddr(int argc,char **argv)
{
    if(argc != 4)
    {
        autelan_tunnel_ctl_help();
	...
	{	
		...
		iwr.u.data.pointer = (void *) buf;
		iwr.u.data.length = sizeof(buf);
		if(autelan_ioctl(&iwr, IEEE80211_IOCTL_GET_ADDR) !=0)
		{
			printf("autelan.c:get addr error.\n");
            return;
        }
		memset(&parm, 0, sizeof(struct ieee80211_autelan_tunnel_ctl));
		memcpy(&parm, buf, sizeof(struct ieee80211_autelan_tunnel_ctl));

		if(parm.ni_localflags == 1)
		{
    		printf(" PORTAL IP: %d.%d.%d.%d\n"
    		       " CPE    IP: %d.%d.%d.%d\n" 
				   " SAFE	 IP: %d.%d.%d.%d\n"  
				   " SAFE1	 IP: %d.%d.%d.%d\n",  
    				parm.ni_portal_ipaddr[0], parm.ni_portal_ipaddr[1], 
    				parm.ni_portal_ipaddr[2], parm.ni_portal_ipaddr[3],
    				parm.ni_pce_ipaddr[0], parm.ni_pce_ipaddr[1], 
    				parm.ni_pce_ipaddr[2], parm.ni_pce_ipaddr[3],
    				parm.ni_safe_ipaddr[0],parm.ni_safe_ipaddr[1],
    				parm.ni_safe_ipaddr[2],parm.ni_safe_ipaddr[3],
					parm.ni_safe1_ipaddr[0], parm.ni_safe1_ipaddr[1], 
					parm.ni_safe1_ipaddr[2], parm.ni_safe1_ipaddr[3]);
        }else if(parm.ni_localflags == 2)
        {
            printf("No IP address set.\n");
        }
...
int autelan_tunnel_ctl(int argc, char** argv)
{
	...
	/*End: zhaoyang1 add for getting infomation of niflag 2013-03-23*/
	else if(strncmp (argv[3], "setcpe", strlen("setcpe")) == 0)
	{
	    setaddr(argc,argv,IEEE80211_IOCTL_SET_CPE);
	}
	else if(strncmp (argv[3], "setportal", strlen("setportal")) == 0)
	{
	    setaddr(argc,argv,IEEE80211_IOCTL_SET_PORTAL);
	}	
	else if(strncmp (argv[3], "setsafe1", strlen("setsafe1")) == 0)
	{
		setaddr(argc,argv,2);
	}
	else if(strncmp (argv[3], "setsafe", strlen("setsafe")) == 0)
	{
	    setaddr(argc,argv,1);
	}
	else if(strncmp (argv[3], "getaddr", strlen("getaddr")) == 0)
	{
	    getaddr(argc,argv);
	}
	else {
...
static void autelan_help () 
{
	...
	printf("\tarp_filter\t... ...\n"); //Added by WangJia for arp filter 2013-04-11
