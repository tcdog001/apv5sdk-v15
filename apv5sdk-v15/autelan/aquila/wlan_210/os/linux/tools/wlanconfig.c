	...
	if (streq(cmd, "create")) {
		...
		if (argc > 3) {
			...
			if (streq(arg, "sta"))
				...
				list_wme(ifname);
            /*<Begin:caizhibang add list for sequence number of stations */
                   else if(streq(arg, "all"))
                        list_stations_all(ifname);
            /*End:caizhibang add list for sequence number of stations>*/
...
/*<AUTELAN-Added-Begin:caizhibang add for data & mgmt frame count */
static void
list_stations_all(const char *ifname)
{
...
list_scan(const char *ifname)
{
	...
	do {
		...
		printf("%-14.*s  %s  %3d  %3dM %2d:%-2d  %3d %-4.4s"
			...
			/*zhaoyang1 transplant end*/
                   /*<Begin : caizhibang modify for WBS-4 */
			, get_max_mcs_rate(sr->isr_rates, sr->isr_nrates)
			/*End: caizhibang modify for WBS-4> */
