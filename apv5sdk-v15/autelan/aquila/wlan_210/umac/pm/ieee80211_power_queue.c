...
ieee80211_node_saveq_age(struct ieee80211_node *ni)
{
	...
	struct ath_atx_tid *tid = NULL;
	struct ath_node *an = ((struct ath_node_net80211 *)(ni))->an_sta;
	struct ieee80211_frame_bar *bar = NULL;
	struct ieee80211com *ic = vap->iv_ic;
	osdev_t os_handle = ic->ic_osdev;
	struct ath_softc *sc = NULL;
	//AUTELAN-Added-End:duanmingzhe for tid stuck
	...
	if ((IEEE80211_NODE_SAVEQ_QLEN(dataq) != 0) ||
        ...
        while (IEEE80211_NODE_SAVEQ_POLL(mgtq, wbuf) != NULL &&
			...
			Before release the wbuf, we need to check the wbuf
			is bar frame?  
			If it is, call ATH_TX_RESUME_TID
			******************************************/
			wh = (struct ieee80211_frame *)wbuf_header(wbuf);
			subtype = wh->i_fc[0] & IEEE80211_FC0_SUBTYPE_MASK; 
			if(subtype == IEEE80211_FC0_SUBTYPE_BAR)
...
ieee80211_node_saveq_flush(struct ieee80211_node *ni)
{
	...
	if (IEEE80211_NODE_SAVEQ_QLEN(mgtq) != 0) {
		...
		for (;;) {
			...
			//AUTELAN-Added-Begin:duanmingzhe for tid stuck
			/******************************************
			If the wbuf send failed
			we check the wbuf is bar frame?  
			If it is, call ATH_TX_RESUME_TID
			******************************************/
			//ieee80211_send_mgmt(ni->ni_vap,ni,wbuf,true);
			if(ieee80211_send_mgmt(ni->ni_vap,ni,wbuf,true))
			{
				wh = (struct ieee80211_frame *)wbuf_header(wbuf);
				subtype = wh->i_fc[0] & IEEE80211_FC0_SUBTYPE_MASK; 	
				if(subtype == IEEE80211_FC0_SUBTYPE_BAR)
			...
			else
			{
				...
				if(subtype == IEEE80211_FC0_SUBTYPE_BAR)
				{
					bar = (struct ieee80211_frame_bar *) wbuf_header(wbuf);
				    tidno = (le16toh(bar->i_ctl) & IEEE80211_BAR_CTL_TID_M) >> IEEE80211_BAR_CTL_TID_S;
					//printk("%s node: %02x:%02x:%02x:%02x:%02x:%02x send_ok ",__func__,ni->ni_macaddr[0],ni->ni_macaddr[1],ni->ni_macaddr[2],ni->ni_macaddr[3],ni->ni_macaddr[4],ni->ni_macaddr[5]);
					//tid = &an->an_tx_tid[tidno];
					//printk("AUTELAN tid[%d] pause (pause = %d)!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n",tidno,tid->paused);
				}
			}
			//AUTELAN-Added-End:duanmingzhe for tid stuck
		}
...
ieee80211_node_saveq_send(struct ieee80211_node *ni, int frame_type)
{
    ...
    if (frame_type == IEEE80211_FC0_TYPE_MGT) {
		...
		//AUTELAN-Added-Begin:duanmingzhe for tid stuck
        if(ieee80211_send_mgmt(vap,ni,wbuf,true))
        {
			wh = (struct ieee80211_frame *)wbuf_header(wbuf);
			subtype = wh->i_fc[0] & IEEE80211_FC0_SUBTYPE_MASK; 	
			if(subtype == IEEE80211_FC0_SUBTYPE_BAR)
