...
extern HAL_BOOL	ar9300PrintKeyCache(struct ath_hal *ah); //AUTELAN-Added:duanmingzhe for get queue info
...
void autelan_node_table_info(char * caller,struct ieee80211com *ic,struct ath_softc_net80211 *scn)
{
	int i = 0;
	struct ieee80211_node *tmp_ni = NULL;
	struct ieee80211_node_table *nt = &ic->ic_sta;
	//struct ath_softc_net80211 *scn_t = ATH_SOFTC_NET80211(ic);
	//struct ath_softc *sc = scn_t->sc_dev;
	struct ath_softc *sc = ATH_DEV_TO_SC(scn->sc_dev);
	struct ath_hal *ah = sc->sc_ah;
		
	printk("%s scn->sc_keyixmap[keyix] BEGIN****************************\n",caller);
	for(i = 0; i < ATH_KEYMAX;i++)
	{
		tmp_ni = scn->sc_keyixmap[i];
		if(tmp_ni == NULL)
			continue;
		//if((ni_macaddr == NULL) || (mac_addr_cmp(tmp_ni->ni_macaddr,ni_macaddr) == 0))
		printk("%s scn->sc_keyixmap[%d] == ni %s ni->ni_ucastkey.wk_keyix = %d is_auth(%d)\n",caller,i,ether_sprintf(tmp_ni->ni_macaddr),tmp_ni->ni_ucastkey.wk_keyix,ieee80211_node_is_authorized(tmp_ni));
	}

	tmp_ni = NULL;
	
	printk("Iterate ni BEGIN****************************\n");
	TAILQ_FOREACH(tmp_ni, &nt->nt_node, ni_list)
	{
		if(tmp_ni == NULL)
			continue;
		//if((ni_macaddr == NULL) || (mac_addr_cmp(tmp_ni->ni_macaddr,ni_macaddr) == 0))
		printk("%s ni %s ni->ni_ucastkey.wk_keyix = %d is_auth(%d)\n",caller,ether_sprintf(tmp_ni->ni_macaddr),tmp_ni->ni_ucastkey.wk_keyix,ieee80211_node_is_authorized(tmp_ni));
	}
	
	tmp_ni = NULL;
	printk("hash ni BEGIN****************************\n");	
	
	for(i = 0; i < 32;i++)
	{
		LIST_FOREACH(tmp_ni, &nt->nt_hash[i], ni_hash)
		{
			
			if(tmp_ni == NULL)
				continue;
			//if((ni_macaddr == NULL) || (mac_addr_cmp(tmp_ni->ni_macaddr,ni_macaddr) == 0))
			printk("%s hash_index %d ni %s ni->ni_ucastkey.wk_keyix = %d is_auth(%d)\n",caller,i,ether_sprintf(tmp_ni->ni_macaddr),tmp_ni->ni_ucastkey.wk_keyix,ieee80211_node_is_authorized(tmp_ni));
		}
	}
	
	printk("HW key BEGIN****************************\n");	
	ar9300PrintKeyCache(ah);
	
	printk("\n\n");
}
EXPORT_SYMBOL(autelan_node_table_info);
/*---------------------
 * Support routines
 *---------------------
 */
               ...
               const u_int8_t      bssid[IEEE80211_ADDR_LEN])
{
	...
	/*ljy--modified end*/
	/*zhaoyang1 transplant end*/
    vap->aute_ping_test_ni_last = NULL;
    ...
    /*AUTELAN--zhaoenjuan added for down &up interface*/
    vap->iv_osif_stop = osif_vap_stop;     
    vap->iv_kick_all = osif_vap_init; 
    /*AUTELAN--zhaoenjuan added for down &up interface end*/
...
ath_tx_send(wbuf_t wbuf)
{
    ...
    if(wbuf != NULL)
    	ICMP_TRACE(wbuf, __func__, __LINE__);
...
ath_tx_mgt_send(struct ieee80211com *ic, wbuf_t wbuf)
{
    ...
    if (!error) {
        ...
        sc->sc_stats.ast_tx_mgmt_bytes += wbuf_get_pktlen(wbuf);//added by zhaoenjuan for wifi  tx statistic 2013-10-24
...
/*autelan - hxh: add for icmp trace print */
extern int ICMP_TRACE(wbuf_t wbuf, char *func, int line);
extern u_int32_t aute_latency_idle_enable;
extern int ICMP_PRASE(wbuf_t wbuf, struct ieee80211_node *ni,u_int32_t *tmp_len);
int
...
/*End : caizhibang modify for autelan 802.3 tunnel 2012-5-25 >*/    
    ...
    */
    ...
    if(aute_latency_idle_enable == 1)
    {
        //temp code for debug
        //ieee80211_dump_pkt(ic, wbuf_header(wbuf), wbuf_get_pktlen(wbuf) + IEEE80211_CRC_LEN,
          //                 rx_status->rateKbps, rx_status->rssi);

        tmp_seq = ICMP_PRASE(wbuf,ni,&tmp_len);

        if(tmp_len == ping_test_len)
        {
            int i,flag=0;	    	
            for (i = 0; i < 16; i++)
            {
                if(ping_trace_test[i].ni == ni)
                {
                    if((ping_trace_test[i].seq == tmp_seq) && (ping_trace_test[i].flag == 1))
                    {
                        u_int32_t timeout;
                        u_int32_t time_interval;
                        timeout =((jiffies / HZ) * 1000) + (jiffies % HZ) * (1000 / HZ);
                        if(timeout < ping_trace_test[i].timeout)
                        {
                            time_interval = 0;
                        }
                        else
                        {
                            time_interval = timeout - ping_trace_test[i].timeout;
                        }
                                           
                        if(dbg_print & 0x08)
                            printk("ping test find: time_interval %u ms,last_time= %u ms,current_time = %u ms,ni=0x%x,tmp_seq=%d\n",time_interval,ping_trace_test[i].timeout,timeout,ni,tmp_seq);

            			if(time_interval > ping_timeout_threshod)
            				aute_air_bad_num++;

                        ping_trace_test[i].seq = 0;
                        ping_trace_test[i].ni = NULL;
                        ping_trace_test[i].flag = 0;
            			flag =1;
            			break;
                    }
                }
            ...
            if(flag == 1)//find a ping test packet
    	    {
...
extern int PING_SEND( struct ieee80211vap *vap );
...
struct ieee80211_ops net80211_ops = {
	...
	ieee80211_dump_pkt_debug,               /*dump_pkt*/
	/*zhaoyang modify end*/
    PING_SEND,
};
