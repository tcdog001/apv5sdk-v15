...
#endif
	/*suzhaoyu add end*/
 /*liuhj add for tunnel local ctl by sta state for LTEFI start*/
 	u_int16_t ni_localflags;
	u_int8_t ni_portal_ipaddr[4];
 /*liuhj add for tunnel local ctl by sta state for LTEFI end*/
    u_int32_t saddr;
	u_int32_t daddr;
    u_int8_t ni_srcmac[6];
    u_int8_t aute_latency_wait;
...
/*Begin:Added by WangJia for Node List & ARP Filter 2013-03-29*/
#define NODE_LIST_HASHSIZE        32
#define NODE_LIST_DEFAULT_MAX_NUM 128

#define	NODE_LIST_HASH(addr)	\
    (((const u_int8_t *)(addr))[IEEE80211_ADDR_LEN - 1] % NODE_LIST_HASHSIZE)

#define NODE_LIST_FLAG_NONE       0x00000000
#define NODE_LIST_FLAG_ARP_FILTER 0x00000001

struct ieee80211_node_entry
{
    /** 
     * list element for linking on node list 
     **/
    TAILQ_ENTRY(ieee80211_node_entry)  ne_list; 

    /** 
     * list element for linking on node list 
     **/
    LIST_ENTRY(ieee80211_node_entry)   ne_hash; 

    /**
     * function switchs this ni has
     **/
    u_int32_t     ne_flags;
    
    /**
     * distinguish each other with MAC
     **/
    u_int8_t      ne_macaddr[IEEE80211_ADDR_LEN];
};

typedef struct ieee80211_node_entry* ieee80211_node_entry_t;

struct ieee80211_node_list
{
    osdev_t                               nl_osdev;
    spinlock_t                            nl_lock;

    TAILQ_HEAD(, ieee80211_node_entry)    nl_list;

    /*Max number of entries allowed in the node list */
    u_int32_t                             nl_max_num;
    /*Current number of entries allowed in the node list */
    u_int32_t                             nl_curr_num;
    
    /**
     * using a simple hash table to improve search efficiency
     **/
    ATH_LIST_HEAD(, ieee80211_node_entry) nl_hash[NODE_LIST_HASHSIZE];
};

typedef struct ieee80211_node_list* ieee80211_node_list_t;

int ieee80211_node_list_attach(wlan_if_t vap);
int ieee80211_node_list_detach(wlan_if_t vap);

int ieee80211_arpfilter_addmac(wlan_if_t vap, const u_int8_t mac[IEEE80211_ADDR_LEN]);
int ieee80211_arpfilter_delmac(wlan_if_t vap, const u_int8_t mac[IEEE80211_ADDR_LEN]);
int ieee80211_arpfilter_findmac(wlan_if_t vap, const u_int8_t mac[IEEE80211_ADDR_LEN]);
int ieee80211_arpfilter_getlist(wlan_if_t vap, u_int8_t *macList, int len, int *num_mac);

/*End:Added by WangJia for Node List & ARP Filter 2013-03-29*/
