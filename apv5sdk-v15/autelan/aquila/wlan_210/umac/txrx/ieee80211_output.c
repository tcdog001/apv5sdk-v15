...
/*Autelan-Begin:hxh added for ping delay check*/
#define ICMP_DATA_LEN 45

extern int ICMP_TRACE(wbuf_t wbuf, char *func, int line);
extern u_int32_t aute_air_bad_num;
extern u_int32_t aute_air_drop_num;

extern aute_ping_test_t  ping_trace_test[10];
extern u_int32_t aute_air_send_num;
extern u_int32_t aute_air_flag;

int PING_SEND( struct ieee80211vap *vap )
{
    wbuf_t wbuf = NULL;
    ...
    struct ieee80211_node *ni_iter = NULL;
    ...
    TAILQ_FOREACH_SAFE(ni_iter, &nt->nt_node, ni_list, ni_next) {
        ieee80211_ref_node(ni_iter);
        if (vap == ni_iter->ni_vap && ni_iter->ni_associd != 0 && ni_iter->aute_latency_wait == 1)  {
             ni = ni_iter;
             ieee80211_free_node(ni_iter);
             break;
         }
         ieee80211_free_node(ni_iter);
    } 
    ...
    if( ni->saddr == 0 )
    {
        //not learn the ip address ,can not send
        if(dbg_print & 0x08)
            printk("Wait to learn the IP address,ni=0x%x,ni->saddr=%d\n",ni,ni->saddr);
        ni->aute_latency_wait = 0;
        aute_air_flag = 0;
        aute_air_bad_num = 0;//clear the statistics
        aute_air_drop_num = 0;
        return -ENOMEM;
    }

    //user update and clear the statistics
    if(vap->aute_ping_test_ni_last != ni)
    {
        if(dbg_print & 0x08)
            printk("Switch to the new ni node,ni=0x%x,ni->saddr=%d,ni_last=0x%x\n",ni,ni->saddr,vap->aute_ping_test_ni_last);
    
        aute_air_flag = 0;
        aute_air_bad_num = 0;
        aute_air_drop_num = 0;
        vap->aute_ping_test_ni_last = ni;
    }
    ...
    *(u_int16_t *)wh->i_seq =
    ...
    i = seq % 16;
    if(ping_trace_test[i].flag == 1)
        aute_air_drop_num++;

    ping_trace_test[i].flag =1;
    ping_trace_test[i].seq = seq;
    ping_trace_test[i].ni = ni;
    ping_trace_test[i].timeout = ((jiffies / HZ) * 1000) + (jiffies % HZ) * (1000 / HZ);
    do_gettimeofday(&ping_trace_test[i].time_send);
    seq++;

    ieee80211_free_node(ni);

    aute_air_send_num++;
    return wlan_vap_send(vap, skb);

}
EXPORT_SYMBOL(PING_SEND);
/*Autelan-End:hxh added for ping delay check*/
    ...
    if (thinap) {
        if (wbuf->protocol == htons(0x0019))
        {
            ...
            else 
            {
                ...
                if(traffic_balance)
                {
                    /*Begin: Added by wangjia, 2013-01-21. */
                    u_int8_t  subtype = wh->i_fc[0] & IEEE80211_FC0_SUBTYPE_MASK;
                    if((2 == traffic_balance) &&
                       ((IEEE80211_FC0_SUBTYPE_AUTH == subtype) ||
                        (IEEE80211_FC0_SUBTYPE_ASSOC_RESP == subtype) ||
                        (IEEE80211_FC0_SUBTYPE_REASSOC_RESP == subtype))) {
                        u_int8_t  reassoc = 0;
                        u_int16_t reason  = 0;
                        ...
                        if(IEEE80211_FC0_SUBTYPE_AUTH == subtype) {
                            ...
                            header += sizeof(struct ieee80211_frame) + 2; // FC + Cap Info(2)
                            ...
                            if(IEEE80211_FC0_SUBTYPE_ASSOC_RESP == subtype) {
                                reassoc = 0;
                            } else if(IEEE80211_FC0_SUBTYPE_REASSOC_RESP == subtype) {
                                reassoc = 1;
                            }
                            reason = le16toh(*(u_int16_t *)header);
                            retval = ieee80211_send_assocresp(ni, reassoc, reason);
                               
                            goto bad;
                        }
                    }
                    /*End: Added by wangjia, 2013-01-21. */
...
/*End:Modify by zhaoyang1 for data forward base users 2012-11-24 */
		...
		else if(vap->vap_splitmac == 4)//lte mode 
		{
			;
		}
