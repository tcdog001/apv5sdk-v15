...
ieee80211_send_nulldata(struct ieee80211_node *ni, int pwr_save)
{
	...
	if(noise_sched)
	{
		...
		*(u_int16_t *)&cts->i_dur[0] = 0;
	    cts->i_fc[1] = 0;
	    cts->i_fc[0] = IEEE80211_FC0_TYPE_CTL | IEEE80211_FC0_SUBTYPE_CTS;
	    IEEE80211_ADDR_COPY(cts->i_ra, ni->ni_bssid);
	}else
	{
    /*yanggs transplant end*/
    ieee80211_send_setup(vap, ni, wh,
    ...
    )
{
	...
	IEEE80211_NOTE_MGMT_DEBUG(vap, ni, 
	  	" <FAIL> [Step 02 - SEND AUTH] %s: thinap = %d, traffic_balance = %d\n", 
	  	__func__, thinap, traffic_balance);
	return 0;	
    }
    ...
    *((u_int16_t *)frm) = htole16(status); frm += 2;
    ...
    wbuf_set_pktlen(wbuf, (frm - (u_int8_t *)wbuf_header(wbuf)));
    vap->iv_stats.is_tx_authresp ++;//add by zhaoenjuan for auth response 2013-03-12
    IEEE80211_NOTE_MGMT_DEBUG(vap, ni, 
...
ieee80211_recv_deauth(struct ieee80211_node *ni, wbuf_t wbuf, int subtype)
{
    ...
    frm = (u_int8_t *)&wh[1];
    efrm = wbuf_header(wbuf) + wbuf_get_pktlen(wbuf);
    /*Begin:add by zhaoenjuan for guangdong mobile statistic deauth 2013-03-12*/
     vap->iv_stats.is_rx_deauth ++;
    /*End:add by zhaoenjuan for guangdong mobile statistic deauth 2013-03-12*/
    IEEE80211_VERIFY_ADDR(ni);
	...
	/*zhaoyang1 transplant from 717*/
	/*pengruofeng--add for wids 2011-5-30*/ 
   if (vap->spoof_detect && ni != vap->iv_bss && vap->iv_opmode == (IEEE80211_M_HOSTAP|IEEE80211_M_BTAMP) )
   {
...
ieee80211_recv_disassoc(struct ieee80211_node *ni, wbuf_t wbuf, int subtype)
{
    ...
    frm = (u_int8_t *)&wh[1];
    efrm = wbuf_header(wbuf) + wbuf_get_pktlen(wbuf);
    /*Begin:add by zhaoenjuan for guangdong mobile statistic disassoc*/
    vap->iv_stats.is_rx_disassoc ++;
    /*End:add by zhaoenjuan for guangdong mobile statistic disassoc*/
	/*zhaoyang1 transplant from 717*/
                    ...
                    struct ieee80211_rx_status *rs)
{
    ...
    case IEEE80211_FC0_SUBTYPE_PROBE_REQ:
         vap->iv_stats.is_rx_probereq++;  /*Add by zhaoenjuan for proreq 2013-03-12*/
