...
/*zhaoyang1 transplant from 717*/
	...
	else 
    {
		...
		if (thinap && traffic_balance) { 
			IEEE80211_NOTE_MGMT_DEBUG(vap, ni, 
				" <INFO> [Step %s - RECV %s REQ] %s: thinap = %d, traffic_balance = %d\n", 
				reassoc ? "04" : "03", reassoc ? "REASSOC" : "ASSOC", __func__, thinap, traffic_balance);
			goto balance_code;
		}

     /*Begin:Add by zhaoenjuan for AssResp/proreq 2013-03-12*/
   if (reassoc){
		  vap->iv_stats.is_tx_reassocresp++;
		} else {
	      vap->iv_stats.is_tx_assocresp++;
        }
    /*End:Add  by zhaoenjuan for AssResp/proreq 2012-03-12*/
        IEEE80211_NOTE_MGMT_DEBUG(vap, ni, 
...
/*zhaoyang1 transplant end*/
        ...
        /*
         *  zhouke add for printk revc mgmt packet addr and ni mac; 
         */
	    IEEE80211_NOTE_MGMT_DEBUG(vap, ni, 
                       ...
                       u_int8_t *challenge, u_int8_t challenge_length, wbuf_t wbuf)
{
    ...
    do {
        ...
        if (ni != vap->iv_bss)  {
           ...
           */
		  IEEE80211_NOTE_MAC_MGMT_DEBUG(vap, wh->i_addr2, 
		  	" <INFO> [Step 02 - RECV AUTH] %s: station send auth in same vap(0x%02X%04X), report delete ni mac %s\n", 
		  	__func__, ni->ni_maintype, ni->ni_subtype,ether_sprintf(ni->ni_macaddr));
