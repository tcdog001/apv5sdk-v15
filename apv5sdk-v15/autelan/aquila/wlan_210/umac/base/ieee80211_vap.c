...
#define IEEE80211_C_OPMODE                                              \
    ...
    ieee80211_node_list_attach(vap);
...
/*lyb add start*/
static void send_null_data_timer_fn(unsigned long data)
{
	...
	{
		for(i=0;i<1;i++)
			ieee80211_send_nulldata(vap->iv_bss,true);
	}
	if(noise_sched)
		mod_timer(&(vap->send_null_data_timer), jiffies + HZ/noise_sched);
	else
		mod_timer(&(vap->send_null_data_timer), jiffies + HZ/100);
}
/*lyb add end*/
static int vap_setup_timer(struct ieee80211vap *vap)
{
	/*lyb add start*/
	init_timer(&(vap->send_null_data_timer));

	vap->send_null_data_timer.function = send_null_data_timer_fn;
	vap->send_null_data_timer.data = (unsigned long) vap;
	vap->send_null_data_timer.expires = jiffies + HZ/100;
	add_timer(&(vap->send_null_data_timer));
	/*lyb add end*/
...
ieee80211_vap_attach(struct ieee80211vap *vap)
{
	...
	/*zhaoyang1 transplant from 717*/
    /*<Begin:caizhibang modify for cts 2013-07-09 */
    /*yanggs transplant for cts*/
    if(noise_sched)
    {
         vap_setup_timer(vap);
    ...
    /*yanggs transplant end*/
    /*End:caizhibang modify for cts 2013-07-09> */
...
ieee80211_vap_detach(struct ieee80211vap *vap)
{
    ...
    if(noise_sched)
    {
	  del_timer(&(vap->send_null_data_timer));
    }
	/*lyb add end*/
    /*yanggs transplant end*/
    /*End:caizhibang modify for cts 2013-07-09> */
    ...
    /*Begin:Added by WangJia for arp filter 2013-04-11*/
    ieee80211_node_list_detach(vap);
    /*End:Added by WangJia for arp filter 2013-04-11*/
