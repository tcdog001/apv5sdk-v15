...
extern u_int32_t rate_threshold_debug;  //added by zhanghu for incresing rate
extern u_int8_t rate_index[3][IEEE80211_RATE_XRATE_SIZE + 1];   //added by zhanghu for incresing rate
...
static int
ieee80211_brs_rate_check(struct ieee80211_rateset *srs, struct ieee80211_rateset *nrs, struct ieee80211com *ic)
{
    /*add a paramter name ic by zhanghu for control rate*/
...
#define FOR_BIT(parm)    (sizeof(parm)*8)
    ...
    for (i = 0; i < srs->rs_nrates; i++) {
        if ( 1 == rate_threshold_debug || 
            ((ic->ic_rate_mask & ((u_int64_t)1 << (FOR_BIT(ic->ic_rate_mask) - 1))) && (ic_rate_mask & (1 << (i+max))) && (ic_rate_mask & (1 << i))) ||
            ( !(ic->ic_rate_mask & ((u_int64_t)1 << (FOR_BIT(ic->ic_rate_mask) - 1))) && (srs->rs_rates[i] & IEEE80211_RATE_BASIC))) {
                      ...
                      int flags)
{
    ...
    if (flags & IEEE80211_F_DOBRS)
        if (!ieee80211_brs_rate_check(irs, rs, ic))
            return 0;
...
ieee80211_check_node_rates(struct ieee80211_node *ni)
{
    ...
    if (!ieee80211_brs_rate_check(srs, &ni->ni_rates, vap->iv_ic))
        return 0;
...
/*
 * Query operational data rates
 * u_int32_t *rates was modified from u_int8_t *rates 
 * by chenming for rate display
 */
/*zhaoyang1 transplant end*/
int
wlan_get_bss_rates(wlan_if_t vaphandle,
                           u_int32_t *rates, u_int32_t len,
                           u_int32_t *nrates)
{
    struct ieee80211vap *vap = vaphandle;
    ...
    for (i = 0; i < len; i++) {
	/*zhaoyang1 transplant from 717*/
        rates[i] = IEEE80211_RV(rs->rs_rates[i])*1000/2;/*modified by chenming for rate display*/
	/*zhaoyang1 transplant end*/
    }
    *nrates = rs->rs_nrates;
    ieee80211_free_node(ni);
...
int32_t wlan_get_bss_dot11Rate(wlan_if_t vaphandle, u_int8_t *rates, u_int32_t len)
{
    struct ieee80211vap *vap = vaphandle;
    struct ieee80211_node *ni;
    struct ieee80211_rateset *rs;
