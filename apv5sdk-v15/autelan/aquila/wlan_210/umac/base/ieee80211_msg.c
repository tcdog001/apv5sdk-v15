...
/*lijiyong add begin for display the timestamp in debug mode*/
#include <linux/timer.h>
void ieee80211_note_print_timestamp(void)
{
    struct timeval current_time;
    do_gettimeofday(&current_time);
    printk("%ld.%06ld: ", (long) current_time.tv_sec, (long) current_time.tv_usec);
}
/*lijiyong add end*/
...
void ieee80211_note_mac(struct ieee80211vap *vap, u_int8_t *mac,const  char *fmt, ...)
{
     ...
     va_start(ap, fmt);
     /*lijiyong add begin for display the timestamp in debug mod*/
     ieee80211_note_print_timestamp();
     /*lijiyong add end*/
     vsnprintf (tmp,(OS_TEMP_BUF_SIZE - (tmp - tmp_buf)), fmt, ap);
     va_end(ap);
     printk("%s",tmp_buf);
