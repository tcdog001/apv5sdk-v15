...
//	u_int8_t ie_len = 0;
	...
	case WAPI_STA_AGING:
	case WAPI_UNICAST_REKEY:
	//zhaoyang modify WAPI_MUTIL_REKEY to WAPI_MULTI_REKEY
	case WAPI_MULTI_REKEY:
	/*zhaoyang modify for address redirection trap CHINAMOBILE-120*/
	case WAPI_ADDR_RD_TRAP:
	case WAPI_CHALLENGE_REPLAY_TRAP:
	/*zhaoyang modify end*/
	case WAPI_STA_STATS:
		break;
	case WAPI_WAI_REQUEST:
	{
		/*zhaoyang modify for replay trap CHINAMOBILE-119
        OS_MEMCPY(sta_msg.gsn, vap->iv_wapimsk_keys.wk_txiv, IEEE80211_WPI_SMS4_IVLEN);
		sta_msg.datalen += IEEE80211_WPI_SMS4_IVLEN;
		ie_len = ie[1] + 2;
		OS_MEMCPY(sta_msg.wie, ie, ie_len);
		sta_msg.datalen += ie_len;
		break;
...
EXPORT_SYMBOL(wapi_callback_wai);
/*zhaoyang modify end*/
#if 0
/*zhaoyang1 transplant end*/
