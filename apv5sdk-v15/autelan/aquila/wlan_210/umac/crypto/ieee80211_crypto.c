...
#define    IEEE80211_WEP_MINLEN                            \
	...
	if(k == NULL)
	{
		...
		//printk("%s:line %d,ni->ni_stats.ns_wpi_no_key_error=%d\n",__func__,__LINE__,ni->ni_stats.ns_wpi_no_key_error);
		return NULL;
	}
	/*zhaoyang modify end*/
	/*zhaoyang1 transplant end*/
	...
	if ((vap->weakiv_detect) && (vap->iv_opmode == IEEE80211_M_HOSTAP) && (cip->ic_cipher == IEEE80211_CIPHER_WEP))
	{
		u_int32_t wep_iv;
		wep_iv = 0;
		
		memcpy(&wep_iv, wbuf->data + hdrlen, IEEE80211_WEP_IVLEN);
		wep_iv = wep_iv >> 8;
		printk("%s: ni->ni_wepiv_last %x, wep_iv %x.\n", __func__, ni->ni_wepiv_last, wep_iv);

		if (ni->ni_wepiv_last != wep_iv)
		{
			ni->ni_wepiv_last = wep_iv;
			ni->ni_sameiv_count = 0;
		}
		else
		{
			ni->ni_sameiv_count++;
		...
		if (ni->ni_sameiv_count >= vap->iv_weakiv_threshold)
		{
