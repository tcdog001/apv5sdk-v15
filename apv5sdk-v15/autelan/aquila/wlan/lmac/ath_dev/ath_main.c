...
/*Begin:modified by zhaoej for queue stuck */
#define QSTUCK_TIMER_VAL 10000 /* 10 sec */

#define ATH_TXQ_STUCK_DEBUG_FLAG  0x01 //debug flag for print the debug info or not

#define ATH_BEACON_STUCK_CHECK_FLAG  0x01
#define ATH_DATA_STUCK_CHECK_FLAG  0x02
#define ATH_STATS_STUCK_CHECK_FLAG  0x04
#define ATH_MAC_STUCK_CHECK_FLAG  0x08
#define ATH_TXQ_STUCK_ACT_FLAG  0x10 //Reboot or not


u_int8_t wifi0_reboot_abnormity_queue=0;
u_int8_t wifi1_reboot_abnormity_queue=0;

u_int32_t wifi0_timer_times=0;
u_int32_t wifi1_timer_times=0;

u_int32_t wifi0_error_timer_times=0;
u_int32_t wifi1_error_timer_times=0;

u_int32_t wifi0_nobuffs_old = 0;
u_int32_t wifi1_nobuffs_old = 0;

u_int32_t wifi0_ast_tx_beacon_old = 0;
u_int32_t wifi1_ast_tx_beacon_old = 0;

u_int32_t wifi0_tx_beacon_same_times = 0;
u_int32_t wifi1_tx_beacon_same_times = 0;

u_int32_t wifi0_txq_packets_old = 0;
u_int32_t wifi1_txq_packets_old = 0;
u_int32_t reg_txq1_old;
u_int32_t reg1_same_times = 0;

u_int32_t wifi0_axq_totalqueued_times = 0;
u_int32_t wifi1_axq_totalqueued_times = 0;

/*modified by zhaoej for stuck*/
...
{
	//printk("coming into air_latency_monitor!\n");
    ...
    if((wifipos_ping_send_enable == 1))
    {
        ...
        { 
            if(NULL != sc->sc_ieee_ops->WIFIPOS_PING_SEND){
				//printk("WIFIPOS: send icmp packets to assoc sta now!\n");
...
OS_TIMER_FUNC(ath_txq_stuck_checker)
{
	...
	struct ath_stats *stats = NULL;
	u_int32_t reg_txq1;
	u_int32_t pkts_sum = 0;
	struct ieee80211vap *vap = NULL;
	struct ieee80211com *ic =NULL;
	int flag =0;
    /*Begin:add by zhaoej for AP restart due to axq_totalqueued is zero for a long time */
    struct ieee80211_node *ni= NULL;
    struct ieee80211_node_table *nt = NULL;
    int authorized_flag = 0;
    /*End:add by zhaoej for AP restart due to axq_totalqueued is zero for a long time */
	int reset_flag =0;

	OS_GET_TIMER_ARG(sc, struct ath_softc *);

	if(ath_txq_stuck_enable == 0)
		goto out;
	...
	{	
		...
		if(ath_txq_stuck_enable&ATH_STATS_STUCK_CHECK_FLAG)
		{
			TAILQ_FOREACH(vap, &ic->ic_vaps, iv_next)
			{   
				if(vap->iv_sta_assoc > 0)
           ...
           /*End:add by zhaoej for AP restart due to axq_totalqueued is zero for a long time */
			txq = &sc->sc_txq[HAL_TX_QUEUE_DATA];// modify from HAL_TX_QUEUE_CAB to HAL_TX_QUEUE_BEACON  to DATA
			// this code is for debug     beacon axq_totalqueued is always zero.
			if(ath_txq_stuck_debug&ATH_TXQ_STUCK_DEBUG_FLAG)
			{
				printk("\nwifi0:\n");
                printk("txq_packet[1]=%d,11n_tx_pkts=%d,beacon=%d\n",sc->sc_stats.ast_txq_packets[1],sc->sc_stats.ast_11n_stats.tx_pkts,sc->sc_stats.ast_be_xmit);
                printk("wifi0_nobuf[1]    = %d,pkt_sum[1]    = %d\n",sc->sc_stats.ast_txq_nobuf[1],(sc->sc_stats.ast_txq_noaggr_packets[1] + sc->sc_stats.ast_txq_packets[1]));
                printk("wifi0_nobuffs_old = %d,pkt_sum_old[1]= %d\n",wifi0_nobuffs_old,wifi0_txq_packets_old);
				printk("txq->axq_totalqueued=%d\n",txq->axq_totalqueued);
				{
					int i = 0;
					struct ath_txq *txq_tmp;
					for(i = 0; i < 10; i++)
					{
						txq_tmp = &sc->sc_txq[i];
						printk("depth = %u	txbuf_free = %u minfree = %u  num_buf_used = %u\n",txq_tmp->axq_depth,sc->sc_txbuf_free,txq_tmp->axq_minfree,txq_tmp->axq_num_buf_used);
					}
				}
			}
			...
			if(ath_txq_stuck_debug&ATH_TXQ_STUCK_DEBUG_FLAG)
			{
				...
				TAILQ_FOREACH(tmp_bf, &txq->axq_q, bf_list)
					count++;
				printk("TXQ[1] have %d pkts!\n",count);
			}

			/*in the workaround, sometimes everything is 0.
			axq_totalqueued should never to be 0 because that beacon always be sent*/
            if((txq->axq_totalqueued == 0)&&(flag == 1)&&(authorized_flag==1))//modified by zhaoej for interface not up but system restart & totalqueued is zero
			{
				wifi0_axq_totalqueued_times++;
				if(wifi0_axq_totalqueued_times > 6)
				{
					printk("autelan--wifi0: restarting system due to axq_totalqueued is zero for a long time\n");
					if(kes_debug_print_handle)
					{
						kes_debug_print_handle(KERN_EMERG "autelan--wifi0: restarting system due to axq_totalqueued is zero\n");  
					}

					/*wangyu add the stop_watchdog function for APV5*/
#if defined(PC018) || defined(APM82181)
                    if(ath_txq_stuck_enable&ATH_TXQ_STUCK_ACT_FLAG)
                    {
                        del_timer(&dog_timer);
					...
					printk("==============The system is rebooting and dump info=============\n");
					for (i=0;i<HAL_NUM_TX_QUEUES;i++)
					{
						txq = &sc->sc_txq[i];
						ATH_TXQ_LOCK(txq);
						printk("    QCU = %u\n",i);
						printk("headindex = %u    tailindex = %u\n",txq->axq_headindex,txq->axq_tailindex);
						printk("depth = %u	txbuf_free = %u minfree = %u  num_buf_used = %u\n",txq->axq_depth,sc->sc_txbuf_free,txq->axq_minfree,txq->axq_num_buf_used);
						printk("aggr_depth = %u axq_totalqueued = %u intrcnt = %u\n",txq->axq_aggr_depth,txq->axq_totalqueued,txq->axq_intrcnt);

						ATH_TXQ_UNLOCK(txq);
					}
					stats = (struct ath_stats *) &sc->sc_stats;
					for (i=0;i<10;++i) {
						if(sc->sc_stats.ast_txq_packets[i]+sc->sc_stats.ast_txq_noaggr_packets[i]){
							printk("TXQ[%d] tx_sum_pkts :%d ,no buffs %d\n",i,(sc->sc_stats.ast_txq_packets[i]+sc->sc_stats.ast_txq_noaggr_packets[i]), stats->ast_txq_nobuf[i]);
						}
					}
                    autelan_DumpRegs(sc->sc_ah);
                    //ar9300PrintReg(sc->sc_ah,print_all_reg);

					printk("========================Dump info end=========================\n");
					if(ath_txq_stuck_enable&ATH_TXQ_STUCK_ACT_FLAG)
					{
						OS_DELAY(1000000);
						//OS_SLEEP(1000);
						kernel_restart("wifi0: restarting system due to axq_totalqueued is zero for a long time\n");
					}
#endif
				}
			}
		...
		/*data queue stuck check*/
		if(ath_txq_stuck_enable&ATH_DATA_STUCK_CHECK_FLAG)
		{
			if(ath_txq_stuck_enable&ATH_MAC_STUCK_CHECK_FLAG)
			{
				/*Atheros MAC stuck check, only record the info, not reset the queue for now*/
				if (AH_TRUE == ath_hal_is_mac_hung(sc->sc_ah))
				{
					printk("autelan--wifi0: atheros mac check function [ath_hal_is_mac_hung] find hung!!!\n");
					if(kes_debug_print_handle)
						kes_debug_print_handle(KERN_EMERG "autelan--wifi0: atheros mac check function [ath_hal_is_mac_hung] find hung!!!\n");  
                    autelan_DumpRegs(sc->sc_ah);
                    //ar9300PrintReg(sc->sc_ah,print_all_reg);
					printk("======================MAC hang Dump reg end=======================\n\n");
				}
			}

		       /*data queue maybe abnormal*/    
		       txq = &sc->sc_txq[HAL_TX_QUEUE_DATA];
		       if(txq->axq_depth >= 1)
			{
				reg_txq1 = OS_REG_READ(sc->sc_ah, AR_Q1_TXDP);
				if(ath_txq_stuck_debug&ATH_TXQ_STUCK_DEBUG_FLAG)
				{
					printk("wifi0:reg_txq1=0x%x, reg_txq1_old = 0x%x,reg1_same_times=%d\n",reg_txq1,reg_txq1_old,reg1_same_times);		
					printk("depth = %u	txbuf_free = %u minfree = %u  num_buf_used = %u\n",txq->axq_depth,sc->sc_txbuf_free,txq->axq_minfree,txq->axq_num_buf_used);
				}
				
				if(reg_txq1_old == reg_txq1)
				{
					reg1_same_times++;
					if(reg1_same_times > 3)
					{
						/* hardware q stuck detected !!!! reset hardware */
						printk("autelan--wifi0 resetting hardware for data Q stuck due to qdeath=%d but the descriptor is not changed\n",txq->axq_depth);
						if(kes_debug_print_handle)
						{
							kes_debug_print_handle(KERN_EMERG "autelan--wifi0 resetting hardware for data Q stuck due to qdeath=%d but the descriptor is not changed\n",txq->axq_depth);  
						}
						autelan_DumpRegs(sc->sc_ah);
				...
				{
					reg1_same_times = 0;
					reg_txq1_old = reg_txq1;
				}
			}
			else
		       {
				/*soft try to put data packet to the hardware delay handle queue already*/
				if(pkts_sum == wifi0_txq_packets_old)
				{
					if(wifi0_nobuffs_old != sc->sc_stats.ast_txq_nobuf[1])
					{
						/* hardware q stuck detected !!!! reset hardware */
						printk("autelan--wifi0 resetting hardware for data Q stuck due to txq_packet no change but nobuff number increase.\n");
						printk("cur_pkts_sum=%d,old_pkts_sum=%d,nobuf=%d,old_nobuf=%d\n",pkts_sum,wifi0_txq_packets_old,sc->sc_stats.ast_txq_nobuf[1],wifi0_nobuffs_old);
						printk("depth = %u	txbuf_free = %u minfree = %u  num_buf_used = %u\n",txq->axq_depth,sc->sc_txbuf_free,txq->axq_minfree,txq->axq_num_buf_used);
						if(kes_debug_print_handle)
						{
							kes_debug_print_handle(KERN_EMERG "autelan--wifi0 resetting hardware for data Q stuck due to txq_packet no change but nobuff increase number.\n");  
							kes_debug_print_handle(KERN_EMERG "cur_pkts_sum=%d,old_pkts_sum=%d,nobuf=%d,old_nobuf=%d\n",pkts_sum,wifi0_txq_packets_old,sc->sc_stats.ast_txq_nobuf[1],wifi0_nobuffs_old);
							kes_debug_print_handle(KERN_EMERG "depth = %u	txbuf_free = %u minfree = %u  num_buf_used = %u\n",txq->axq_depth,sc->sc_txbuf_free,txq->axq_minfree,txq->axq_num_buf_used);
						}
						autelan_DumpRegs(sc->sc_ah);
						sc->sc_reset_type = ATH_RESET_NOLOSS;
						ath_internal_reset(sc);
						...
						sc->sc_stats.ast_resetOnError++;
						reset_flag = 1;
						wifi0_nobuffs_old = sc->sc_stats.ast_txq_nobuf[1];
					}
				}
	    		else
	    		{
				wifi0_txq_packets_old = pkts_sum;
				wifi0_nobuffs_old = sc->sc_stats.ast_txq_nobuf[1];
	    		}
		      }
		}

		/*beacon queue maybe abnormal*/
        if(wifi0_ast_tx_beacon_old == sc->sc_stats.ast_be_xmit)/*soft try to send beacon already*/
        {
           wifi0_tx_beacon_same_times ++;
           if(wifi0_tx_beacon_same_times > 3)
            {
               /* hardware q stuck detected !!!! reset hardware */
               DPRINTF(sc, ATH_DEBUG_ANY, "\n autelan-- %s wifi0, resetting hardware for Q stuck ",__func__);            
               if(ath_txq_stuck_debug&ATH_TXQ_STUCK_DEBUG_FLAG)
               printk("wifi0:beacon not sent by soft sc->sc_stats.ast_be_xmit=%d, wifi0_ast_tx_beacon_old = %d\n",sc->sc_stats.ast_be_xmit,wifi0_ast_tx_beacon_old);
               printk("Autelan--wifi0 resetting hardware for beacon stuck\n");
               if(kes_debug_print_handle)
                {
                  kes_debug_print_handle(KERN_EMERG "Autelan--wifi0 resetting hardware for beacon stuck\n");  
                }
			   autelan_DumpRegs(sc->sc_ah);
               sc->sc_reset_type = ATH_RESET_NOLOSS;
               ath_internal_reset(sc);
               ...
               wifi0_tx_beacon_same_times = 0;   
          }
       }
       else 
       {
         wifi0_ast_tx_beacon_old = sc->sc_stats.ast_be_xmit;
       }
   
       if(ath_txq_stuck_enable&ATH_BEACON_STUCK_CHECK_FLAG)
        {
    	...
    	if(wifi0_timer_times > 18)
    	{
    		if(wifi0_timer_times < wifi0_error_timer_times)
    			wifi0_error_timer_times = 0;
    
    		if(reset_flag == 1)
    		{
    			if((wifi0_timer_times - wifi0_error_timer_times) <= 18)
    				wifi0_reboot_abnormity_queue=1;
    			else 
    				wifi0_error_timer_times = wifi0_timer_times;
    		}
    	}
       
       	if(wifi0_reboot_abnormity_queue == 1)
    	{
    
    		printk("autelan--wifi0: restarting system due to stuck 2 times in three minutes\n");
    		if(kes_debug_print_handle)
    		{
    			kes_debug_print_handle(KERN_EMERG "autelan--wifi0: restarting system due to stuck 2 times in three minutes\n");  
    		}
			...
			if(ath_txq_stuck_enable&ATH_TXQ_STUCK_ACT_FLAG)
			{
				OS_DELAY(1000000);
				//OS_SLEEP(1000);
				printk("restating system delay 1 seconds\n");
				kernel_restart("autelan--wifi0: restarting system due to stuck\n");
			}
#endif
		}
	}
...
static
OS_TIMER_FUNC(ath_txq_stuck_checker_wifi1)
{
	struct ath_softc *sc;
	struct ath_txq *txq;
	u_int16_t i;
...
#ifdef ATH_SUPPORT_DFS
	if (sc->sc_dfs && sc->sc_dfs->sc_dfswait) {
		OS_SET_TIMER(&sc->sc_qstuck_wifi1, 60000 /* ms */);
		return;
	}
#endif
	...
	if(memcmp(sc->sc_osdev->netdev->name,"wifi1",5) == 0)
	{	
       	wifi1_timer_times++;
		...
		if(ath_txq_stuck_enable&ATH_STATS_STUCK_CHECK_FLAG)
		{
           ...
           /*End:add by zhaoej for AP restart due to axq_totalqueued is zero for a long time */
			...
			if(ath_txq_stuck_debug&ATH_TXQ_STUCK_DEBUG_FLAG)
			{
                printk("\nwifi1:\n");
                printk("txq_packet[1]=%d,11n_tx_pkts=%d,beacon=%d\n",sc->sc_stats.ast_txq_packets[1],sc->sc_stats.ast_11n_stats.tx_pkts,sc->sc_stats.ast_be_xmit);
                printk("wifi1_nobuf[1]    = %d,pkt_sum[1]    = %d\n",sc->sc_stats.ast_txq_nobuf[1],(sc->sc_stats.ast_txq_noaggr_packets[1] + sc->sc_stats.ast_txq_packets[1]));
                printk("wifi1_nobuffs_old = %d,pkt_sum_old[1]= %d\n",wifi1_nobuffs_old,wifi1_txq_packets_old);
				printk("txq->axq_totalqueued=%d\n",txq->axq_totalqueued);
				{
					int i = 0;
					struct ath_txq *txq_tmp;
			...
			/*in the workaround, sometimes everything is 0.
			axq_totalqueued should never to be 0 because of the beacon*/
			if((txq->axq_totalqueued == 0)&&(flag == 1)&&(authorized_flag==1))//modified by zhaoej for interface not up but system restart & totalqueued is zero
			{
				wifi1_axq_totalqueued_times++;
				if(wifi1_axq_totalqueued_times > 6)
				{
					printk("autelan--wifi1: restarting system due to axq_totalqueued is zero for a long time\n");
					if(kes_debug_print_handle)
					{
						kes_debug_print_handle(KERN_EMERG "autelan--wifi1: restarting system due to axq_totalqueued is zero\n");  
					}
					...
					if(ath_txq_stuck_enable&ATH_TXQ_STUCK_ACT_FLAG)
					{
						OS_DELAY(1000000);
						//OS_SLEEP(1000);
						kernel_restart("wifi1: restarting system due to axq_totalqueued is zero for a long time\n");
					}
#endif
				}
			}
		...
		if(ath_txq_stuck_enable&ATH_DATA_STUCK_CHECK_FLAG)
		{
			...
			if(ath_txq_stuck_enable&ATH_MAC_STUCK_CHECK_FLAG)
			{
				...
				if (AH_TRUE == ath_hal_is_mac_hung(sc->sc_ah))
				{
					printk("autelan--wifi1: atheros mac check function [ath_hal_is_mac_hung] find hung!!!\n");
					if(kes_debug_print_handle)
						kes_debug_print_handle(KERN_EMERG "autelan--wifi1: atheros mac check function [ath_hal_is_mac_hung] find hung!!!\n");  
                    autelan_DumpRegs(sc->sc_ah);
                    //ar9300PrintReg(sc->sc_ah,print_all_reg);
					printk("======================MAC hang Dump reg end=======================\n\n");
				}
		       ...
		       if(txq->axq_depth >= 1)
			{
				...
				if(ath_txq_stuck_debug&ATH_TXQ_STUCK_DEBUG_FLAG)
				{
					printk("wifi1:reg_txq1=0x%x, reg_txq1_old = 0x%x,reg1_same_times=%d\n",reg_txq1,reg_txq1_old,reg1_same_times);
					printk("depth = %u	txbuf_free = %u minfree = %u  num_buf_used = %u\n",txq->axq_depth,sc->sc_txbuf_free,txq->axq_minfree,txq->axq_num_buf_used);
				}
				...
				if(reg_txq1_old == reg_txq1)
				{
					...
					{
						/* hardware q stuck detected !!!! reset hardware */
						DPRINTF(sc, ATH_DEBUG_ANY, "\n autelan-- %s wifi1, resetting hardware for Q stuck ",__func__);            
						printk("autelan--wifi1 resetting hardware for Q stuck due to qdeath=%d but the descriptor is not changed\n",txq->axq_depth);
						if(kes_debug_print_handle)
						{
							kes_debug_print_handle(KERN_EMERG "autelan--wifi1 resetting hardware for Q stuck due to qdeath=%d but the descriptor is not changed\n",txq->axq_depth);  
							kes_debug_print_handle(KERN_EMERG "depth = %u	txbuf_free = %u minfree = %u  num_buf_used = %u\n",txq->axq_depth,sc->sc_txbuf_free,txq->axq_minfree,txq->axq_num_buf_used);
						}
						sc->sc_reset_type = ATH_RESET_NOLOSS;
						ath_internal_reset(sc);
		       ...
		       {
				/*soft try to put data packet to the hardware delay handle queue already*/
				if(pkts_sum == wifi1_txq_packets_old)
				{
					if(wifi1_nobuffs_old != sc->sc_stats.ast_txq_nobuf[1])
					{
						/* hardware q stuck detected !!!! reset hardware */
						printk("autelan--wifi1 resetting hardware for Q stuck due to txq_packet no change but nobuff number increase.\n");
						printk("cur_pkts_sum=%d,old_pkts_sum=%d,nobuf=%d,old_nobuf=%d\n",pkts_sum,wifi1_txq_packets_old,sc->sc_stats.ast_txq_nobuf[1],wifi0_nobuffs_old);
						printk("depth = %u	txbuf_free = %u minfree = %u  num_buf_used = %u\n",txq->axq_depth,sc->sc_txbuf_free,txq->axq_minfree,txq->axq_num_buf_used);
						if(kes_debug_print_handle)
						{
							kes_debug_print_handle(KERN_EMERG "autelan--wifi1 resetting hardware for Q stuck due to txq_packet no change but nobuff increase number.\n");  
							kes_debug_print_handle(KERN_EMERG "cur_pkts_sum=%d,old_pkts_sum=%d,nobuf=%d,old_nobuf=%d\n",pkts_sum,wifi0_txq_packets_old,sc->sc_stats.ast_txq_nobuf[1],wifi0_nobuffs_old);
							kes_debug_print_handle(KERN_EMERG "depth = %u	txbuf_free = %u minfree = %u  num_buf_used = %u\n",txq->axq_depth,sc->sc_txbuf_free,txq->axq_minfree,txq->axq_num_buf_used);
						}
						autelan_DumpRegs(sc->sc_ah);
						...
						sc->sc_stats.ast_resetOnError++;
						reset_flag = 1;
						wifi1_nobuffs_old = sc->sc_stats.ast_txq_nobuf[1];
					}
				}
		    		else
		    		{
					wifi1_txq_packets_old = pkts_sum;
					wifi1_nobuffs_old = sc->sc_stats.ast_txq_nobuf[1];
		    		}
		        }
		}
		...
		/*beacon queue maybe abnormal*/
        if(wifi1_ast_tx_beacon_old == sc->sc_stats.ast_be_xmit)/*soft try to send beacon already*/
        {
            wifi1_tx_beacon_same_times ++;
            if(wifi1_tx_beacon_same_times > 3)
            {
                        /* hardware q stuck detected !!!! reset hardware */
              DPRINTF(sc, ATH_DEBUG_ANY, "\n autelan-- %s wifi1, resetting hardware for Q stuck ",__func__);            
              if(ath_txq_stuck_debug&ATH_TXQ_STUCK_DEBUG_FLAG)
              printk("wifi1:beacon not sent by soft sc->sc_stats.ast_be_xmit=%d, wifi1_ast_tx_beacon_old = %d\n",sc->sc_stats.ast_be_xmit,wifi1_ast_tx_beacon_old);
              printk("Autelan--wifi1 resetting hardware for beacon stuck\n");
              if(kes_debug_print_handle)
               {
                 kes_debug_print_handle(KERN_EMERG "Autelan--wifi1 resetting hardware for beacon stuck\n");  
               }
		      autelan_DumpRegs(sc->sc_ah);
              sc->sc_reset_type = ATH_RESET_NOLOSS;
              ath_internal_reset(sc);
             ...
             reset_flag = 1;
              ...
              wifi1_tx_beacon_same_times = 0;
           }
        }
        else 
        {
           wifi1_ast_tx_beacon_old = sc->sc_stats.ast_be_xmit;
        }
		...
		if(wifi1_timer_times > 18)
		{
			if(wifi1_timer_times < wifi1_error_timer_times)
				wifi1_error_timer_times = 0;

			if(reset_flag == 1)
			{
				if((wifi1_timer_times - wifi1_error_timer_times) <= 18)
					wifi1_reboot_abnormity_queue=1;
				else 
					wifi1_error_timer_times = wifi1_timer_times;
			}
		}
       
       	if(wifi1_reboot_abnormity_queue == 1)
		{

			printk("autelan--wifi1: restarting system due to stuck 2 times in three minutes\n");
			if(kes_debug_print_handle)
			{
				kes_debug_print_handle(KERN_EMERG "autelan--wifi1: restarting system due to stuck 2 times in three minutes\n");  
			}
			...
			if(ath_txq_stuck_enable&ATH_TXQ_STUCK_ACT_FLAG)
			{
				...
				//OS_SLEEP(1000);
				printk("restating system delay 1 seconds\n");
				kernel_restart("autelan--wifi1: restarting system due to stuck\n");
			}
#endif
		}
	}
...
out:
    /*Begin: Yang Gongsheng modify for wifi1 check stuck 2013-05-09*/
    OS_SET_TIMER(&sc->sc_qstuck_wifi1, QSTUCK_TIMER_VAL);
    /*End: Yang Gongsheng modify for wifi1 check stuck 2013-05-09*/
    return;
	
}

/*End:modified by zhaoej for queue stuck*/
...
#define    N(a)    (sizeof(a)/sizeof(a[0]))
   ...
   if (error) goto bad2;
    ...
    if(sc->sc_enhanceddmasupport) {
        ...
        OS_SET_TIMER(&sc->sc_qstuck, QSTUCK_TIMER_VAL);
		/*zhaoyang modify for dusi check*/
		OS_INIT_TIMER(sc->sc_osdev, &sc->sc_qstuck_wifi1, ath_txq_stuck_checker_wifi1, sc);
        OS_SET_TIMER(&sc->sc_qstuck_wifi1, QSTUCK_TIMER_VAL);
		/*zhaoyang modify end*/
    }
#endif
...
ath_dev_free(ath_dev_t dev)
{
    ...
    if(sc->sc_enhanceddmasupport) {
		...
		/*zhaoyang modify for dusi check*/
		OS_CANCEL_TIMER(&sc->sc_qstuck_wifi1);
		/*zhaoyang modify end*/
    }
#endif
