...
u_int32_t ar_isr_txok_wifi0_count = 0;
u_int32_t ar_isr_txerr_wifi0_count = 0;
u_int32_t ar_isr_txeol_wifi0_count = 0;
u_int32_t ar_isr_txok_wifi1_count = 0;
u_int32_t ar_isr_txerr_wifi1_count = 0;
u_int32_t ar_isr_txeol_wifi1_count = 0;
EXPORT_SYMBOL(ar_isr_txok_wifi0_count);
EXPORT_SYMBOL(ar_isr_txerr_wifi0_count);
EXPORT_SYMBOL(ar_isr_txeol_wifi0_count);
EXPORT_SYMBOL(ar_isr_txok_wifi1_count);
EXPORT_SYMBOL(ar_isr_txerr_wifi1_count);
EXPORT_SYMBOL(ar_isr_txeol_wifi1_count);
        ...
        if (isr & (AR_ISR_TXOK | AR_ISR_TXERR | AR_ISR_TXEOL)) {
            *masked |= HAL_INT_TX;
			...
			if(ah->ah_devid == 0x31)
			{
      			if(isr & AR_ISR_TXOK)
      			{
      				ar_isr_txok_wifi0_count++;
      			}
      			if(isr & AR_ISR_TXERR)
      			{
      				ar_isr_txerr_wifi0_count++;
      			}
      			if(isr & AR_ISR_TXEOL)
      			{
      				ar_isr_txeol_wifi0_count++;
      			}
			}
			else if(ah->ah_devid == 0xabcd)
			{
      			if(isr & AR_ISR_TXOK)
      			{
      				ar_isr_txok_wifi1_count++;
      			}
      			if(isr & AR_ISR_TXERR)
      			{
      				ar_isr_txerr_wifi1_count++;
      			}
      			if(isr & AR_ISR_TXEOL)
      			{
      				ar_isr_txeol_wifi1_count++;
      			}
