...
/*pengruofeng add for frame stats 2011-5-9*/
    ...
    /*Begin:added by zhaoenjuan for management frame statistic 2013-03-13*/
    printf("receive probe_request  : %d\n",stats->is_rx_probereq);
    printf("transmit probe_response: %d\n",stats->is_tx_proberesp);
    printf("receive total_auth     : %d\n",stats->is_rx_auth_try);
    printf("receive success_auth   : %d\n",stats->is_rx_auth_ok);
    printf("transmit total_auth    : %d\n",stats->is_tx_authresp);
    printf("receive total_deauth   : %d\n",stats->is_rx_deauth);
    printf("transmit total_deauth  : %d\n",stats->is_deauth_state);
    
    printf("receive sucess_assoc   : %d\n",stats->is_assoc_success);
    printf("receive total_reassoc  : %d\n",stats->is_rx_reassoc);
    printf("receive sucess_reassoc : %d\n",stats->is_rx_reassoc_success);
    printf("transmit sucess_assoc  : %d\n",stats->is_tx_assocresp);
    printf("transmit sucess_reassoc: %d\n",stats->is_tx_reassocresp);
    printf("receive total_disassoc : %d\n",stats->is_rx_disassoc);
    printf("transmit total_disassoc: %d\n",stats->is_disassoc_state);
    
    /*End:added by zhaoenjuan for management frame statistic 2013-03-13*/
...
#define	STAT64(x,fmt) \
	...
	sprintf(mac_addr,"%02x:%02x:%02x:%02x:%02x:%02x",macaddr[0],
		macaddr[1],macaddr[2],macaddr[3],macaddr[4],macaddr[5]);

//	fprintf(fd, "%s:\n", ether_ntoa((const struct ether_addr*) macaddr));
	fprintf(fd, "%s:\n", mac_addr); //zengmin add for snmp 2013-08-15
