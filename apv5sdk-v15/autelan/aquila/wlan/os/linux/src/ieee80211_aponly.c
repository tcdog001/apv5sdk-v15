...
#endif      
			/*zhaoyang1 transplant from cmcc*/
			{
			 struct llc *llc; 
			 wbuf_t wbuf;
			 struct ieee80211_frame *wh;
        ...
        ieee80211_rx_status_t *rx_status, u_int8_t *chainreset)
{
    ...
    if (rx_status->ratecode & 0x80) 
    {
        ...
        ||sc->sc_curmode == WIRELESS_MODE_11NG_HT40MINUS) 
        {
            ...
            {
                /*Begin: Add by zhanghu for the bug that rx rate is more than the physical rate 2013-01-06 TESTBED-309*/
                if(rxs->rs_flags & HAL_RX_GI){
                /*End: Add by zhanghu for the bug that rx rate is more than the physical rate 2013-01-06 TESTBED-309*/
        ...
        {
            if (ic->ic_htcap & IEEE80211_HTCAP_C_SHORTGI20) 
            {
                /*Begin: Add by zhanghu for the bug that rx rate is more than the physical rate 2013-01-06 TESTBED-309*/
                if(rxs->rs_flags & HAL_RX_GI){
                /*End: Add by zhanghu for the bug that rx rate is more than the physical rate 2013-01-06 TESTBED-309*/
