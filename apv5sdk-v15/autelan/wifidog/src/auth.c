...
void
allowed_client(request *r)//add by liuhj
{
	t_authresponse	auth_response;
	char	*mac;
	char	p1[18] = {0}, mac_cli[18] = {0};
	s_config	*config = NULL;
	t_auth_serv	*auth_server = NULL;

	if(NULL == r->clientAddr)
		debug(LOG_DEBUG, "client ip is NULL");	

	debug(LOG_DEBUG, "client ip %s is", r->clientAddr);	
	
	if (!(mac = arp_get((char *)(r->clientAddr)))) {
		debug(LOG_DEBUG, "cannot get mac through ip %s", r->clientAddr);	
		free(mac);
		return;
	}
	debug(LOG_DEBUG, "client mac-0-%s", r->clientAddr, mac);	
    ...
    case AUTH_ALLOWED:
		/* Logged in successfully as a regular account */
		debug(LOG_INFO, "Got ALLOWED from central server authenticating from %s at %s - "
				"adding to firewall and redirecting them to portal", r->clientAddr, mac);
		fw_allow(r->clientAddr, mac, FW_MARK_KNOWN);
        served_this_session++;
	    break;

    default:
		debug(LOG_WARNING, "I don't know what the validation code %d means for from %s at %s - sending error message", auth_response.authcode, r->clientAddr, mac);
		send_http_page(r, "Internal Error", "We can not validate your request at this time");
	    break;
