...
#if 0
#include "auth.h"//add by liuhj
extern int NO_AuthServer;   //add by liuhj
#endif
...
thread_httpd(void *args)
{
	...
	if (httpdReadRequest(webserver, r) == 0) {
		...
		if (NO_AuthServer)
		{		
			debug(LOG_DEBUG, "allowed_client(r) start");
			allowed_client((request *)r);
			debug(LOG_DEBUG, "allowed_client(r) end");
		}
		/*end add by liuhj*/
#endif
