...
{
    t_client         *ptr;
//	debug(LOG_DEBUG, "client->ip %s, client->mac %s", ptr->ip, ptr->mac);
	debug(LOG_DEBUG, "client_list_find_by_ip %s", ip);
	if (NULL == ptr){
		debug(LOG_DEBUG, "ptr is NULL!!");
		return NULL;
	}
...
//	debug(LOG_DEBUG, "client->ip %s, client->mac %s", ptr->ip, ptr->mac);
	debug(LOG_DEBUG, "cannot find ptr->ip == ip");
