...
#else
	if (!is_online()) {
		debug(LOG_INFO, "---------------is_online not--------------");
		http_send_redirect_to_noauth(webserver->host, r, "Redirect to login page");
	}
	else if (!is_auth_online()) {
		debug(LOG_INFO, "---------------is_auth_online not--------------");
		http_send_redirect_to_noauth(webserver->host, r, "Redirect to login page");
	}
#endif
	else {
...
void http_send_redirect(request *r, char *url, char *text)
{
		...
		debug(LOG_INFO, "http_send_redirect url=[%s]", url); //add by liuhj
		debug(LOG_INFO, "http_send_redirect message=[%s]", message); //add by liuhj
		free(message);
}
