...
/*====================================================================*
 *
 *   signed manifest (void const * memory, ssize_t extent);
 *
...
/*====================================================================*
 *
 *   char * itoa (unsigned number, char buffer [], size_t length);
 *
...
static char * itoa (unsigned number, char buffer [], size_t length) 
...

{
	...
	while (length > 0) 
	{
		...
		* node = (struct TLVNode *)(offset);
		...
		{
			printf ("\t%s: %s\n", nvm_fields [type], data < (sizeof (nvm_chains) / sizeof (const char *))? nvm_chains [data]: itoa (data, string, sizeof (string)));
		}
