...

{
	...
	static char const * optv [] = 
	{
		...
		"b n\tbinary byte value is (n) [" OPTSTR (EFBU_BINARY) "]",
		"d x\treplace destination address with (x)",
		"e x\tethertype is (x) [" OPTSTR (EFBU_ETHERTYPE) "]",
		...
		"t n\ttransmit for (n) milliseconds [" OPTSTR (EFBU_TIMER) "]",
		"p n\tpause (n) milliseconds between frames [" OPTSTR (EFBU_PAUSE) "]",
		"v\tverbose messages",
		(char const *) (0)
	};
