...

{
	...
	{
		"aAbB:C:d:D:eEfFHi:IJ:K:l:mMn:N:op:P:qQrRst:TvW:w:x",
		"device [device] [...]",
		...
		"D x\tset Device Access Key (DAK) to (x) for option -E, -F and -J [" DAK1 "]",
		...
		"o\tread flash memory using VS_MODULE_OPERATION",
...
#include "../plc/plc.c"
	...
	while ((c = getoptv (argc, argv, optv)) != -1) 
	{
		...
		case 'F':
			...
			_clrbits (plc.flags, PLC_ERASE_DEVICE);
			_setbits (plc.flags, PLC_FLASH_DEVICE);
			break;
		case 'H':
