...

{
	...
	else if (_anyset (plc->flags, PLC_READ_MODULE)) 
	{
		if (AccessLevelPTS (plc) != -1) 
		{
	...
	else if (_anyset (plc->flags, PLC_WRITE_MODULE)) 
	{
...

{
	...
	static char const * optv [] = 
	{
		...
		"s n\tmodule sub-ID [" OPTSTR (PLC_SUBMODULEID) "]",
		"S n\tsession ID [" OPTSTR (PLCSESSION) "]",
		"t n\tmodule ID [" OPTSTR (PLC_MODULEID) "]",
		"q\tquiet mode",
		"v\tverbose mode",
		(char const *) (0)
	};
...
#include "../plc/plc.c"
	...
	while ((c = getoptv (argc, argv, optv)) != -1) 
	{
		...
		case 'm':
			...
			break;
		case 't':
			ModuleID = (uint16_t)(basespec (synonym (optarg, modules, SIZEOF (modules)), 16, sizeof (ModuleID)));
			break;
		case 's':
