...
#ifndef MAKEFILE
#include "../nda/EraseFlashSector.c"
#include "../nda/EraseFlashMemory2.c"
#include "../nda/ReadFlashMemory2.c"
#include "../nda/ModuleErase.c"
#include "../nda/AccessLevelPTS.c"
#endif
...

{
	while (count--) 
	{
		...
		if (_anyset (plc->flags, PLC_ERASE_SECTOR)) 
		{
			EraseFlashSector (plc);
		}
		if (_anyset (plc->flags, PLC_READ_PIB)) 
		{
			if (AccessLevelPTS (plc) != -1) 
			{
				ModuleRead (plc, &plc->pib, PLC_MOD_OP_READ_FLASH, PLC_MODULEID_PARAMETERS, 0);
			}
		}
		if (_anyset (plc->flags, PLC_READ_MAC)) 
		{
			if (AccessLevelPTS (plc) != -1) 
			{
				ModuleRead (plc, &plc->nvm, PLC_MOD_OP_READ_FLASH, PLC_MODULEID_FIRMWARE, 0);
			}
		}
		if (_anyset (plc->flags, PLC_READ_ALL)) 
		{
			if (AccessLevelPTS (plc) != -1) 
			{
				ReadFlashMemory2 (plc);
			}
		}
...

{
	...
	{
		"aAbB:d:D:eEfFi:HIJ:K:l:mMn:N:o:p:P:QqrRS:t:Tvw:xX:",
		"device [device] [...]",
		...
		"E\terase flash memory using VS_MODULE_OPERATION",
		...
		"n f\tread NVM from FLASH/SDRAM to file (f) using VS_MODULE_OPERATION",
		"N f\twrite firmware file (f) to flash memory using VS_MODULE_OPERATION",
		"o\tread all of flash memory using VS_MODULE_OPERATION",
		"p f\tread PIB from FLASH/SDRAM to file (f) using VS_MODULE_OPERATION",
...
#include "../plc/plc.c"
	...
	while ((c = getoptv (argc, argv, optv)) != -1) 
	{
		...
		case 'o':
			...
			_setbits (plc.flags, PLC_READ_ALL);
			break;
		case 'P':
			if (!checkfilename (optarg)) 
			{
		...
		case 'x':
			...
			break;
		case 'X':
			_setbits (plc.flags, PLC_ERASE_SECTOR);
			plc.sector = (unsigned)(basespec (optarg, 16, sizeof (plc.sector)));
			break;
		default:
			break;
		}
