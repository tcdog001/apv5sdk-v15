...

{
	...
	do 
	{
		...
		if (!module && !nvm_header.HEADERMINORVERSION) 
		{
			error ((plc->flags & PLC_BAILOUT), ENOTSUP, "Legacy SDRAM parameters detected");
		}
		if (!nvm_header.NEXTHEADER && !nvm_header.HEADERMINORVERSION) 
		{
			error ((plc->flags & PLC_BAILOUT), ENOTSUP, "Legacy firmware module detected");
		}
