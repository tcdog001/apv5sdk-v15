 ...
 *
 *   check memory resident PIB image; set found flag to indicate that
 *   a PIB has been found in the chain; return 0 for a good image and
 *   -1 for an bad image; 
 *
 *   the check performed here is not exhaustive but it is adequate;   
 *
...
/*====================================================================*
 *
 *   signed chkpib2 (char const * filename, flag_t flags);
 *
 *.  Qualcomm Atheros HomePlug AV Powerline Toolkit
 *:  Published 2009-2011 by Qualcomm Atheros. ALL RIGHTS RESERVED
 *;  For demonstration and evaluation only. Not for production use
...
static signed chkpib2 (char const * filename, flag_t flags) 

{
	...
	{
		return (pibchain2 (memory, extent, filename, flags));
	}
	else
	{
		return (pibimage1 (memory, extent, filename, flags));
	}
}
