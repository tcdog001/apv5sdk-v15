...
CWStateTransition CWWTPEnterConfigure() {
	...
	if(gEMenuConf.e_swtich == 1){
		/*	avoid a bug:
			when ap is quit, u delete a binded wlan from this wtp table.
		     	then ap connect to ac again,it won't recv the operate of delete wlan
		     	so,the mark of "create" of vapX will be existed for ever.
		*/

		/*prepare to recv new config of e-menu*/
		gEMenuConf.e_swtich = 0;

		CWClearEMenuConf();
		/*if(access("/jffs/config.xml_e_bak",R_OK) == 0){
			_system("cp /jffs/config.xml_e_bak /jffs/config.xml -rf");
		}*/
		
		_system("sys_ClearFat");
		CWLog("Connect to AC,e-menu is enable, switch to Thin.");
	}
	/* AC address */
	char str[128];
	...
	gRadioCount = radioCount;
	
	memset(&confArg,0,sizeof(confArg));
	strcpy(confArg.conf_arg,"radio_count");
	autelan_sprintf(confArg.conf_value,"%d",radioCount);
	CWSaveConfigInfo(CONF_WTPD,&confArg,CW_FALSE);

	memset(&confArg,0,sizeof(confArg));
	strcpy(confArg.conf_arg,"gNetworkPreferredFamily");
	autelan_sprintf(confArg.conf_value,"%d",gNetworkPreferredFamily);
	CWSaveConfigInfo(CONF_WTPD,&confArg,CW_FALSE);
	
	// send Configure Request
	seqNum = CWGetSeqNum();
	//add by yaoxiaochao: init wmm 10-24 
	int radio_count = CWGetRadioMaxCount();
	int radio_index = 0;
	for(radio_index = 0;radio_index < radio_count;radio_index++)
	{
		CWSaveWmmInlist(radio_index,0);
	}
...
CWBool CWSaveConfigureResponseMessage(CWProtocolConfigureResponseValues *configureResponse)
{
	...
	{CWWTPDebugLog("###AC IPv4 List NULL");}
	...
	if(configureResponse->echoRequestTimer>0)
	{
		...
		autelan_sprintf(confArg.conf_value,"%d",gEchoInterval);
		CWSaveConfigInfo(CONF_WTPD,&confArg,CW_TRUE);
	}
