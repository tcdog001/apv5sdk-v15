...
CWBool CWParseSettingsFile()
{
	...
	gSettingsFile = autelan_fopen (CW_SETTINGS_FILE, "rb");
	...
	while((line = (char*)CWGetCommand(gSettingsFile)) != NULL) 
	{
		...
		if (!strncmp(startTag+1, "IF_NAME", endTag-startTag-1))
		{
			...
			CW_CREATE_STRING_ERR(gInterfaceName, offset, {CW_FREE_OBJECT(line);return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY,NULL);});
