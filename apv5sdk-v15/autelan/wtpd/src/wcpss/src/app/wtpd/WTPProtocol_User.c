...
CWBool CWWTPGetBoardData(CWWTPVendorInfos *valPtr) {
	...
	valPtr->vendorInfosCount = 5; // we fill 3 information (just the required ones) 0 model number
//#if (defined(APXJ)||defined(XHAP93)||defined(XHPB44))
	if(gManuOption!=0)
		valPtr->vendorInfosCount = 6;
//#endif
	...
	(valPtr->vendorInfos)[0].vendorIdentifier = AUTELAN_VENDOR;
	(valPtr->vendorInfos)[0].type = CW_WTP_MODEL_NUMBER;
	(valPtr->vendorInfos)[0].length = strlen(WTPModelNum); /*pei 0926 for multi char *model */
	...
	*(((valPtr->vendorInfos)[0]).valuePtr) = 1000; // MODEL NUMBER  /*pei modified 0924*/   /* ap-code. pei 090519 */

	// my vendor identifier (IANA assigned "SMI Network Management Private Enterprise Code")
	(valPtr->vendorInfos)[1].vendorIdentifier = AUTELAN_VENDOR;
	(valPtr->vendorInfos)[1].type = CW_WTP_SERIAL_NUMBER;
	(valPtr->vendorInfos)[1].length = strlen(WTPBoardID);
	...
	*(((valPtr->vendorInfos)[1]).valuePtr) = 234567; // SERIAL NUMBER

	// WTP base MAC
	(valPtr->vendorInfos)[2].vendorIdentifier = AUTELAN_VENDOR;
	(valPtr->vendorInfos)[2].type = CW_BOARD_MAC_ADDRESS;
	(valPtr->vendorInfos)[2].length = 6; // mac length
	...
	*(((valPtr->vendorInfos)[2]).valuePtr) = 123456; // SERIAL NUMBER

	// WTP real model num
	(valPtr->vendorInfos)[3].vendorIdentifier = AUTELAN_VENDOR;
	(valPtr->vendorInfos)[3].type = CW_WTP_REAL_MODEL_NUMBER;    /* real-model. pei 090519 */
	(valPtr->vendorInfos)[3].length = strlen(WTPRealModelNum);
	CW_CREATE_OBJECT_SIZE_ERR(( ( (valPtr->vendorInfos)[3] ).valuePtr), (valPtr->vendorInfos)[3].length, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	*(((valPtr->vendorInfos)[3]).valuePtr) = 12345; // SERIAL NUMBER

	// WTP code version
	(valPtr->vendorInfos)[4].vendorIdentifier = AUTELAN_VENDOR;
	(valPtr->vendorInfos)[4].type = CW_WTP_CODE_VERSION;    /* real-model. pei 090519 */
	(valPtr->vendorInfos)[4].length = strlen(gWTPCodeVersion);
	CW_CREATE_OBJECT_SIZE_ERR(( ( (valPtr->vendorInfos)[4] ).valuePtr), (valPtr->vendorInfos)[4].length, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	*(((valPtr->vendorInfos)[4]).valuePtr) = 12345; // SERIAL NUMBER

//#if (defined(APXJ)||defined(XHAP93)||defined(XHPB44))
	if(gManuOption!=0){
		// WTP license num
		(valPtr->vendorInfos)[5].vendorIdentifier = AUTELAN_VENDOR;
		(valPtr->vendorInfos)[5].type = CW_WTP_MANUFACTURE_OPTION;    /* license num. pei 20100427 */
		(valPtr->vendorInfos)[5].length = sizeof(int);
		CW_CREATE_OBJECT_SIZE_ERR(( ( (valPtr->vendorInfos)[5] ).valuePtr), (valPtr->vendorInfos)[5].length, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
		*(((valPtr->vendorInfos)[5]).valuePtr) = 12345;
	}
//#endif
...
//	(valPtr->vendorInfos)[0].length = sizeof(long int); // just one int
	(valPtr->vendorInfos)[0].length = strlen(gWTPHardWareVersion); // just one int
	CW_CREATE_OBJECT_SIZE_ERR(( ( (valPtr->vendorInfos)[0] ).valuePtr), (valPtr->vendorInfos)[0].length, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	*(((valPtr->vendorInfos)[0]).valuePtr) = 123456; // HW version
	...
	((valPtr->vendorInfos)[1]).length = strlen(gWTPSoftWareVersion); // just one int
	CW_CREATE_OBJECT_SIZE_ERR(( ( (valPtr->vendorInfos)[1] ).valuePtr), (valPtr->vendorInfos)[0].length, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	*(((valPtr->vendorInfos)[1]).valuePtr) = 12347; // SW version
	//	strncpy(((valPtr->vendorInfos)[1]).valuePtr, gWTPSoftWareVersion, strlen(gWTPSoftWareVersion));
	// my vendor identifier (IANA assigned "SMI Network Management Private Enterprise Code")
	(valPtr->vendorInfos)[2].vendorIdentifier = 23456;
	(valPtr->vendorInfos)[2].type = CW_BOOT_VERSION;
	(valPtr->vendorInfos)[2].length = sizeof(int); // just one int
	CW_CREATE_OBJECT_SIZE_ERR(( ( (valPtr->vendorInfos)[2] ).valuePtr), (valPtr->vendorInfos)[2].length, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
...
__inline__ int CWWTPGetIPv4Address() {
	...
	if(0 != autelan_getsockname(gWTPSocket, (struct sockaddr*) &myAddr, &len))
		CWWTPDebugLog("getsockname error!");
	return autelan_ntohl(myAddr.sin_addr.s_addr); 	// TO-DO: this is garbage if we are an IPv6 client
}
...
int CWGetRadioMaxCount()
{
	...
	autelan_sprintf(temp_str, "grep \"<wifi_total>\" -C 1 /tmp/device_info.xml | awk -F '[><]' '/count/ {print $3}'");
	fp = autelan_popen(temp_str, "r");
	if(fp)
	{
		memset(str, 0, 20);
...
CWBool CWGetWTPRadiosAdminState(CWRadiosAdminInfo *valPtr) 
{
	...
	valPtr->radiosCount = CWGetRadioMaxCount();
	
	CW_CREATE_ARRAY_ERR(valPtr->radios, valPtr->radiosCount, CWRadioAdminInfoValues, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	
	for (i=0; i<valPtr->radiosCount; i++)
	{
		(valPtr->radios)[i].ID = i; // first radio
		autelan_sprintf(ifname, "wifi%d", (valPtr->radios)[i].ID);
		radioState = 1;
		CWGetInterfaceStatus(ifname, &radioState);
		if(radioState == 3)
		{
			(valPtr->radios)[i].state = DISABLED;
			(valPtr->radios)[i].cause = AD_RADIO_FAILURE;
		}
		else
		{
			(valPtr->radios)[i].state = ENABLED;
			(valPtr->radios)[i].cause = AD_NORMAL;
		}
		CWWTPDebugLog("radio %d state: %d, cause:%d", (valPtr->radios)[i].ID, (valPtr->radios)[i].state, (valPtr->radios)[i].cause);
	}
