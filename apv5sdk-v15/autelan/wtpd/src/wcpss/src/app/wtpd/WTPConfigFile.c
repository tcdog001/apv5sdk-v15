...
/*const char *CW_CONFIG_FILE = "/etc/wlan/config.wtp";*/
//const char *CW_CONFIG_FILE = "/jffs/config.wtp";
const char *CW_CONFIG_FILE = "/tmp/config.wtp";
...
CWBool CWConfigFileInitLib() {
	...
	gConfigValues[9].code = "</WTP_AC_DOMAIN_NAME> ";
	gConfigValues[9].value.str_value = NULL;

	gConfigValues[10].type = CW_STRING;
	gConfigValues[10].code = "</WTP_SOFTWARE_VERSION> ";
...
CWBool CWConfigFileDestroyLib() {
	...
	if(gConfigValues[9].value.str_value != NULL) {
		CW_CREATE_STRING_FROM_STRING_ERR(gWTPACDomainName, (gConfigValues[9].value.str_value), return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	}
	else
		CWWTPDebugLog("damain name get failed \n");
	
	if(gConfigValues[10].value.str_value != NULL) {
		CW_CREATE_STRING_FROM_STRING_ERR(gWTPSoftWareVersion, (gConfigValues[10].value.str_value), return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	}
	else
		CWWTPDebugLog("software version get failed \n");
