...
CWBool CWNetworkInitSocketClientConnect(CWSocket *sockPtr, CWNetworkLev4Address *addrPtr, int port)   /* pei test 1222 */
{
	...
	addr.sin6_port=autelan_htons(port);
	...
	if(addrPtr != NULL) {
        int res = autelan_connect((*sockPtr), ((struct sockaddr*)addrPtr), CWNetworkGetAddressSize(addrPtr));
		if(res < 0) {
...
//pei add for splictmac fast-forward 081115
	...
	CWWTPDebugLog("AC addr:%#x", autelan_inet_addr(str));

	memset(cmdbuf, 0, 100);
	autelan_sprintf(cmdbuf, "echo 32769 > /proc/sys/net/capwap/udp_port");
	_system(cmdbuf);
	autelan_sprintf(cmdbuf, "echo %#x > /proc/sys/net/capwap/ac_addr", autelan_inet_addr(str));
	_system(cmdbuf);
