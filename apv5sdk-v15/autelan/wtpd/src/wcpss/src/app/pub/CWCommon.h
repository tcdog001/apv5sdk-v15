...
#define		CW_PRINT_STRING_ARRAY(ar_name, ar_size)			{int i = 0; for(i = 0; i < (ar_size); i++) autelan_printf("[%d]: **%s**\n", i, ar_name[i]);}

// custom error
/*Begin:zengmin add for memset after malloc 2013-04-10*/
#define		CW_CREATE_OBJECT_ERR(obj_name, obj_type, on_err)	{obj_name = (obj_type*) (malloc(sizeof(obj_type))); if(obj_name == NULL) {on_err} else {memset(obj_name, 0, sizeof(obj_type));}}
#define		CW_CREATE_OBJECT_SIZE_ERR(obj_name, obj_size,on_err)	{obj_name = (malloc(obj_size)); if(obj_name == NULL) {on_err} else { memset(obj_name, 0, obj_size); }}
#define		CW_CREATE_ARRAY_ERR(ar_name, ar_size, ar_type, on_err)	{ar_name = (ar_type*) (malloc(sizeof(ar_type) * (ar_size))); if(ar_name == NULL) {on_err} else { memset(ar_name, 0, sizeof(ar_type) * (ar_size)); }}
#define		CW_CREATE_STRING_ERR(str_name, str_length, on_err)	{str_name = (char*) (malloc(sizeof(char) * ((str_length)+1) ) ); if(str_name == NULL) {on_err} else { memset(str_name, 0, sizeof(char) * ((str_length)+1)); }}
...
/*End:zengmin add for memset after malloc 2013-04-10*/
