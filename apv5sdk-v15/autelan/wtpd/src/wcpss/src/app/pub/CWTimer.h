...
__inline__ CWBool CWTimersInitLib() {
	autelan_sigemptyset(&mask);
    	autelan_sigaddset(&mask, SIGALRM);
	enable_interrupts();
	_clkini();
...
*/
	...
	if ( autelan_sigaction(TIMER_SIGNAL_TYPE, &siga, NULL) ) {
...
{ 
        ...
        if (-1 == autelan_setitimer(TIMER_INTERVAL_TYPE,&timer_value, (struct itimerval *)0)) { 
...
{ 
        struct timeval tp; 
        (void) autelan_gettimeofday(&tp,(struct timezone *)0); 
        /* ignore overflow */ 
...
void cancel_itimer() 
{ 
        if (-1 == autelan_setitimer(TIMER_INTERVAL_TYPE,&cancel_timer, 
                                       (struct itimerval *)0)) { 
