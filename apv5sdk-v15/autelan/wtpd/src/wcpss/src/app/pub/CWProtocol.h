...
#define 	CW_MSG_ELEMENT_WTP_STATIC_IP_CW_TYPE_V2			255

#define 	CW_MSG_ELEMENT_WTP_11N_MCS_CWMODE_GUARDINTERVAL			254 //11N parameter
#define 	CW_MSG_ELEMENT_ETH_SET_CW_TYPE			253
...
typedef struct {
	...
	enum {
		...
		CW_WTP_MANUFACTURE_OPTION = 7, /* value 0 manufacture is autelan value 1 is OEM, pei add for license num at 20100429 */
...
*  Author:sundl
*  Data: 2012/6/7
* Purpose:store ratrans data
*/
typedef struct _CWRtransmMsg{
	unsigned int seqnum;
...
typedef struct{
 ...
 CWBool CWAddRtransmMsgElementToList(CWRtransMsgList * list, char * msg,unsigned int msglen,unsigned int seqnum,unsigned int fragment);
//CWBool  CWDeleteRtransmMsgList(CWRtransMsgList * list,unsigned int seqnum);
