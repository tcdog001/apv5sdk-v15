...
char * CWGetCommand(FILE *configFile) {
	...
	if(buff != NULL && ret != NULL) {
		...
		CW_CREATE_STRING_ERR(command, len-1, {CW_FREE_OBJECT(buff);return NULL;}); //zengmin add free buff when malloc error by Coverity Resource leak  2013-06-08			
		memset(command,0, len-1);  //sdl fix a bug for pc-apv6
		strcpy(command, buff);
	}
...
//				CW_CREATE_ARRAY_ERR((gConfigValues[i].value.str_array_value), gConfigValues[i].count, char*, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	...
	gCWConfigFile = autelan_fopen (CW_CONFIG_FILE, "rb");
				...
				switch(gConfigValues[i].type) {
					...
					case CW_STRING:
						...
						if(!isCount) CW_CREATE_STRING_FROM_STRING_ERR(gConfigValues[i].value.str_value, 
										 myLine, 
										 {CW_FREE_OBJECT(line);return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL);});//zengmin add free line when malloc error by Coverity Resource leak  2013-06-08
						break;
					case CW_STRING_ARRAY:
						...
						while((line = CWGetCommand(gCWConfigFile)) != NULL && strcmp(line, gConfigValues[i].endCode)) {
							...
							if(isCount) gConfigValues[i].count++;
								...
								CW_CREATE_STRING_FROM_STRING_ERR((gConfigValues[i].value.str_array_value)[j], line, {CW_FREE_OBJECT(line);return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL);});////zengmin add free line when malloc error by Coverity Resource leak  2013-06-08
								j++;
							}
							CW_FREE_OBJECT(line);
						}
