...
static void timer_start(struct timeval *abs_to) {
	...
	autelan_gettimeofday(&abs_now, NULL);
	...
	rv = autelan_setitimer(TIMER_KIND, &relative, NULL);
	if (rv == -1) {
...
static void *cronometer(void *arg) {
	...
	autelan_sigaddset(&mask, SIG_TO_WAIT);
...
void timer_print() {
	...
	for(t = timerq.first, i = 0; t != NULL; t = t->next, i++) {
		...
		autelan_printf("Timer %d: id=<%d>, expire=<%u,%u>, in_use=<%d>, cancelled=<%d>\n",
...
//	printf("now time : %s [%u]\n",nowReadable, now);
	...
	autelan_sprintf(cmd,"echo %s [%lu] >/tmp/echo_time",nowReadable, now);
	autelan_system(cmd);
