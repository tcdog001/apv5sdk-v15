...
CWBool CWSecurityInitSessionClient(CWSocket sock, CWNetworkLev4Address *addrPtr, CWSafeList packetReceiveList, CWSecurityContext ctx, CWSecuritySession *sessionPtr, int *PMTUPtr) {
	...
	if (autelan_getsockname(sock, (struct sockaddr*)&peer, (void *)&peerlen) < 0) {
		SSL_free(*sessionPtr);
...
CWBool CWSecurityInitContext(CWSecurityContext *ctxPtr, const char *caList, const char *keyfile, const char *passw, CWBool isClient, int (*hackPtr)(void *)) {
	...
	if(caList != NULL) { // certificates
		...
		autelan_printf("OpenSSL PrivateSharedKey not ready\n");
		exit(0);
		/*
...
unsigned int CWSecurityPSKClientCB(SSL *ssl, const char *hint, char *identity,
    ...
    if(autelan_snprintf(identity, max_identity_len, "CLient_identity") < 0) return 0;
...
int psk_key2bn(const char *psk_key, unsigned char *psk, unsigned int max_psk_len) {
    ...
    if (!ret)
        {
        autelan_printf("Could not convert PSK key '%s' to BIGNUM\n", psk_key);
        if (bn)
            BN_free(bn);
        return 0;
        }
    if (BN_num_bytes(bn) > max_psk_len)
        {
        autelan_printf("psk buffer of callback is too small (%d) for key (%d)\n",
