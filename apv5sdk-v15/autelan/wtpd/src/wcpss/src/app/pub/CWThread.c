...
CWBool CWThreadCreateSem(CWThreadSem *semPtr, int value) {
	...
	#ifdef CW_USE_NAMED_SEMAPHORES
	{
		...
		autelan_snprintf(name, 32, "/CWSem-%d-%4.4d", getpid(), semCount++);
...
CWBool CWThreadCreateTimedSem(CWThreadTimedSem *semPtr, int value) {
	...
	if(	(autelan_bind((*semPtr)[0], (struct sockaddr*) &serverAddr, sizeof(serverAddr)) < 0) ||
		(autelan_bind((*semPtr)[1], (struct sockaddr*) &clientAddr, sizeof(clientAddr)) < 0) ||
		(autelan_connect((*semPtr)[1], (struct sockaddr*) &serverAddr, sizeof(serverAddr)) < 0) || // connect each socket to the other
		(autelan_connect((*semPtr)[0], (struct sockaddr*) &clientAddr, sizeof(clientAddr)) < 0)
	) {
		autelan_close((*semPtr)[0]);
		autelan_close((*semPtr)[1]);
		CWErrorRaiseSystemError(CW_ERROR_CREATING);
	}
...
CWBool CWThreadTimedSemIsZero(CWThreadTimedSem *semPtr) {
	...
	while((r=autelan_select(max((*semPtr)[1], (*semPtr)[0])+1, &fset, NULL, NULL, &timeout)) < 0) {
		if(errno == EINTR) {
			timeout.tv_sec = 0;
			timeout.tv_usec = 0;
			continue;
...
CWBool CWThreadTimedSemSetValue(CWThreadTimedSem *semPtr, int value) {
	...
	CW_REPEAT_FOREVER {
		...
		if(FD_ISSET((*semPtr)[0], &fset)) {
			while(autelan_read((*semPtr)[0], &dummy, 1) < 0) {
				if(errno == EINTR) continue;
				CWErrorRaiseSystemError(CW_ERROR_GENERAL);
			}
		}
		...
		if(FD_ISSET((*semPtr)[1], &fset)) {
			while(autelan_read((*semPtr)[1], &dummy, 1) < 0) {
				if(errno == EINTR) continue;
				CWErrorRaiseSystemError(CW_ERROR_GENERAL);
			}
		}
...
CWBool CWThreadTimedSemPost(CWThreadTimedSem *semPtr) {
	...
	while(autelan_send((*semPtr)[1], &dummy, 1, 0) < 0) {
		if(errno == EINTR) continue;
		CWErrorRaiseSystemError(CW_ERROR_SENDING);
	}
	...
	while((r = autelan_select(((*semPtr)[1])+1, &fset, NULL, NULL, &timeout)) <= 0) {
...
void CWExitThread() {
	//printf("\n*** Exit Thread ***\n");
	wid_syslog_notice("one wtp exit,thread autelan_close");
	pthread_exit((void *) 0);
}
...
void CWThreadSetSignals(int how, int num, ...) {
	...
	autelan_sigemptyset(&mask);
	
	va_start(args, num);
	
	for(; num > 0; num--) {
		autelan_sigaddset(&mask, va_arg(args, int));
	}
