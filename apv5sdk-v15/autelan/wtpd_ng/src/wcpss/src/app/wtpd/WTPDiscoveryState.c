...
#ifndef CW_NO_DTLS
	dpf("gDtlsSecurity:%d   gDtlsPolicy:%d\n", gDtlsSecurity, gDtlsPolicy);
#endif
...
//	dpf("gDtlsSecurity:%d   gDtlsPolicy:%d\n", gDtlsSecurity, gDtlsPolicy);
	...
	CWDebugLog("WTP Receives Discovery Response");
...
CWBool CWRenewACList()
{
	...
	for(i=0;i<gACInfoPtr->IPv4AddressesCount;i++)
	{
		...
		CW_CREATE_STRING_FROM_STRING_ERR(tmpACList[i].address, AcAddressStr, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	...
	{
		CW_CREATE_STRING_FROM_STRING_ERR(tmpACList[i].address, gCWACList[i-gACInfoPtr->IPv4AddressesCount].address, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
