...
{
	CWDebugLog("\n");
	CWDebugLog("#________ watchdog handle ________#");
	int retval;

 	retval = ioctl (leddev,WRCPU_WATCHDOG,NULL);/*down the blind*/
	if(retval !=0)
		CWDebugLog("watchdog write failed");

	if(!CWStartWatchDogTimer()) {
		CWDebugLog("CWStartWatchDogTimer failed\n");
		return;
	}
}
...
{
	printf("CWWTPStartWatchdog\n");
	if (!CWErr(CWStartWatchDogTimer())) 
	{
		CWDebugLog("CWStartWatchDogTimer error");
	}
	return CW_TRUE;
}
