	...
	for(i=0;i<qosCount;i++)
	{
		...
		for(ptr = wtp_wlan_list;ptr != NULL;ptr = ptr->next)
		{
			if(radioQosValues[i].radioID == ptr->radio_id)
			{
				...
				CWWTPDebugLog("configure update msg QosEnable:%d", gQosEnable);
				if(gQosEnable == 1 )
				{
...
CWBool CWBindingSetDirectSequenceControlValues(DirectSequeueControlValues *directSequeueControlValues, CWProtocolResultCode *resultCode)
{
	...
	*resultCode = CW_PROTOCOL_SUCCESS;
	...
	if(radio_found == CW_FALSE){
		CWDebugLog("The WTP dosen't have radio %d",directSequeueControlValues->radio_id);
	...
	CWWTPDebugLog("set radio %d channel %d", directSequeueControlValues->radio_id, channel);
	...
	if((gRunRetry == 0)||(channel !=0))
	{
		...
		set_channel_cmd(rid, wlanId, channel);
	...
	if(channel == 0)
	{
		if (gRunRetry == 0)
		{
            ...
            CWSetWifiCommit(directSequeueControlValues->radio_id);
#endif
			...
			set_channel_cmd(rid,wlanId,rechannel);
			for(ptr = wtp_wlan_list ; ptr != NULL ; ptr = ptr->next){
				if(ptr->radio_id == directSequeueControlValues->radio_id)
				{
					ptr->wlan_updown_time++;  //pei add 0225
...
#ifdef MARVELL_PLATFORM
            set_channel_cmd(rid, wlanId, gRadioInfoValue[directSequeueControlValues->radio_id].channel);
#endif
	...
	CWDebugLog("%s channel set OK the channel now is :%d", wlanname, channel);
	
	ConfArg confArg;
	memset(&confArg,0,sizeof(ConfArg));
	confArg.radioID = directSequeueControlValues->radio_id;
	strcpy(confArg.conf_arg,"channel");
	sprintf(confArg.conf_value,"%d",gRadioInfoValue[directSequeueControlValues->radio_id].channel);
...
CWBool CWBindingSetTxPower(TxPowerValues *txPowerValues, CWProtocolResultCode *resultCode)
{
	...
	CWBool radio_found = CW_FALSE;
	CWWTPWlan *ptr = NULL;
	int rid = txPowerValues->radio_id;
	int wid = 0;
	int return_type;
	...
	*resultCode = CW_PROTOCOL_SUCCESS;
	...
	if(radio_found == CW_FALSE){
		CWDebugLog("The WTP dosen't have radio %d",txPowerValues->radio_id);
...
//	printf("-------current_power:%d\n", txPowerValues->current_power);
	...
	sprintf(confArg.conf_value,"%d",gRadioInfoValue[txPowerValues->radio_id].txtype);
	CWSaveConfigInfo(CONF_RADIO,confArg,CW_TRUE);
	
	strcpy(confArg.conf_arg,"txpower");
	sprintf(confArg.conf_value,"%d",gRadioInfoValue[txPowerValues->radio_id].txpower);
	CWSaveConfigInfo(CONF_RADIO,confArg,CW_FALSE);

	CWSetTxpower(rid,wid,txtype, txpower);
	if(txtype==1)
		return_type=3;
	else
		return_type=2;
	...
	CWDebugLog("%s txpower set OK the power now is :%d dB",wlanname,txpower);
...
CWBool CWBindingSetRate(BindingRate *rateValues, CWProtocolResultCode *resultCode)
{
	...
	*resultCode = CW_PROTOCOL_SUCCESS;
	...
	if(radio_found == CW_FALSE){
		CWDebugLog("The WTP dosen't have radio %d",rateValues->radioID);
	...
	for(ptr = wtp_wlan_list ; ptr != NULL ; ptr = ptr->next){
		...
		{
			CWWTPGetWlanName(wlanname,ptr->radio_id,ptr->wlan_id);
			CWSetRate(ptr->radio_id,ptr->wlan_id,gRadioInfoValue[rateValues->radioID].radiotype,gRadioInfoValue[rateValues->radioID].ratemask);
	...
	CWDebugLog("%s rate set OK the rate now is :%d M", wlanname, rate/10);
...
CWBool CWBindingSetRadioType(BindingRadioType *radioTypeValues, CWProtocolResultCode *resultCode)
{
	...
	*resultCode = CW_PROTOCOL_SUCCESS;
	...
	if(radio_found == CW_FALSE){
		CWDebugLog("The WTP dosen't have radio %d",radioTypeValues->radioID);
		*resultCode = CW_PROTOCOL_FAILURE;
		return CW_TRUE;
	}
	...
	for(ptr = wtp_wlan_list ; ptr != NULL ; ptr = ptr->next){
		if(ptr->radio_id == radioTypeValues->radioID)
		{
			...
			CWWTPGetWlanName(ifname,ptr->radio_id,ptr->wlan_id);
			CWSetInterfaceUp(ifname,0);
			CWSetRadioType(ptr->radio_id, ptr->wlan_id, radiotype);
		}
	}

	if(gApScanningEnable == 1)
	{
		CWSetRadioType(radioTypeValues->radioID, 0, radiotype);
	}
	...
	sprintf(confArg.conf_value,"%d",radioTypeValues->radiotype);
	CWSaveConfigInfo(CONF_RADIO,confArg,CW_TRUE);
	...
	*resultCode=*resultCode|channel;
	...
	CWSetChannel(radio_id,wlan_id,channel);
	...
	CWDebugLog("%s radio type set OK the radio type now is :%d",wlanname,radiotype);
...
CWBool CWBindingSetMacOperation(MacOperationValues *macOperationValues, CWProtocolResultCode *resultCode)
{
	...
	*resultCode = CW_PROTOCOL_SUCCESS;
	...
	if(radio_found == CW_FALSE){
		CWDebugLog("The WTP dosen't have radio %d", macOperationValues->radio_id);
		*resultCode = CW_PROTOCOL_FAILURE;
		return CW_TRUE;
	}
	...
	sprintf(confArg.conf_value,"%hd",gRadioInfoValue[macOperationValues->radio_id].fragThreshold);
	CWSaveConfigInfo(CONF_RADIO,confArg,CW_TRUE);
	
	memset(&confArg,0,sizeof(confArg));
	confArg.radioID = macOperationValues->radio_id;
	strcpy(confArg.conf_arg,"rtsThreshold");
	sprintf(confArg.conf_value,"%hd",gRadioInfoValue[macOperationValues->radio_id].rtsThreshold);
	CWSaveConfigInfo(CONF_RADIO,confArg,CW_FALSE);
	
	memset(&confArg,0,sizeof(confArg));
	confArg.radioID = macOperationValues->radio_id;
	strcpy(confArg.conf_arg,"shortRetry");
	sprintf(confArg.conf_value,"%d",gRadioInfoValue[macOperationValues->radio_id].shortRetry);
	CWSaveConfigInfo(CONF_RADIO,confArg,CW_FALSE);
	
	memset(&confArg,0,sizeof(confArg));
	confArg.radioID = macOperationValues->radio_id;
	strcpy(confArg.conf_arg,"longRetry");
	sprintf(confArg.conf_value,"%d",gRadioInfoValue[macOperationValues->radio_id].longRetry);
	CWSaveConfigInfo(CONF_RADIO,confArg,CW_FALSE);
	for(ptr = wtp_wlan_list ; ptr != NULL ; ptr = ptr->next){
		if(ptr->radio_id == macOperationValues->radio_id){
			CWWTPGetWlanName(wlanname,ptr->radio_id,ptr->wlan_id);
			...
			{
				if(debug_print)
					printf("Current mode fragmention must be off, value: 2346.\n");
				set_fragthreshold_cmd(wlanname,2346); 
			}
			else
				CWSetFragmentThreshold(wlanname, gRadioInfoValue[macOperationValues->radio_id].fragThreshold);
		}
	}

	/* set frag and rts for wifi interface */
	CWWTPGetWifiName(wifiname, macOperationValues->radio_id);
	if(gRadioInfoValue[macOperationValues->radio_id].radiotype&IEEE80211_PARAM_MODE_11n)      //11n
	{
		if(debug_print)
			printf("Current mode fragmention must be off, value: 2346.\n");
		set_fragthreshold_cmd(wifiname,2346); 
	}
	else
		CWSetFragmentThreshold(wifiname, gRadioInfoValue[macOperationValues->radio_id].fragThreshold);
	set_rtsthreshold_cmd(wifiname, gRadioInfoValue[macOperationValues->radio_id].rtsThreshold);
...
CWBool CWBindingSetRadioConfig(WTPRadioConfigurationValues *radioConfigurationValues, CWProtocolResultCode *resultCode)
{
	...
	*resultCode = CW_PROTOCOL_SUCCESS;
	...
	sprintf(cmdbuf, "/usr/sbin/set_countrycode %d %s >/dev/null 2>&1",radioConfigurationValues->radio_id ,CountryCode);
	system(cmdbuf);
	
	memset(cmdbuf,0,128);
	sprintf(cmdbuf, "iwpriv wifi%d setCountry %s >/dev/null",radioConfigurationValues->radio_id,CountryCode);
	if(debug_print)
		printf("set wifi%d CountryCode:[%s]\n",radioConfigurationValues->radio_id,CountryCode);
	system(cmdbuf);
	
	if(radio_found == CW_FALSE){
		CWDebugLog("The WTP dosen't have radio %d", radioConfigurationValues->radio_id);
		*resultCode = CW_PROTOCOL_FAILURE;
		return CW_TRUE;
	}
	...
	for(ptr = wtp_wlan_list; ptr != NULL; ptr = ptr->next){
	    if(ptr->radio_id == radioConfigurationValues->radio_id){
		set_radioconfig_cmd(ptr->radio_id, ptr->wlan_id, gRadioInfoValue[radioConfigurationValues->radio_id].preamble, 
	...
	sprintf(confArg.conf_value,"%d",gRadioInfoValue[radioConfigurationValues->radio_id].preamble);
	CWSaveConfigInfo(CONF_RADIO,confArg,CW_TRUE);
	
	memset(&confArg,0,sizeof(confArg));
	confArg.radioID = radioConfigurationValues->radio_id;
	strcpy(confArg.conf_arg,"dtim");
	sprintf(confArg.conf_value,"%d",gRadioInfoValue[radioConfigurationValues->radio_id].dtim);
	CWSaveConfigInfo(CONF_RADIO,confArg,CW_FALSE);
	
	memset(&confArg,0,sizeof(confArg));
	confArg.radioID = radioConfigurationValues->radio_id;
	strcpy(confArg.conf_arg,"beaconInterval");
	sprintf(confArg.conf_value,"%hd",gRadioInfoValue[radioConfigurationValues->radio_id].beaconInterval);
	CWSaveConfigInfo(CONF_RADIO,confArg,CW_FALSE);
...
{
	if(gRunRetry == 1){
		CWWTPDebugLog("%s,gRunRetry=1,Direct Return.",__func__);
		return CW_TRUE;
	}
	CWBool radio_found = CW_FALSE;
	int sock;
//	int i;
	...
	CWDebugLog("IN CWBindingAddWlan");
	//dpf("IN CWBindingAddWlan\n");
	...
	if(radio_found == CW_FALSE){
		CWDebugLog("The WTP dosen't have radio %d",addWlanValues->radio_id);
	...
	CWDebugLog("mac mode %d, tunnel mode %d\n",wlancreating->wlan_mac_mode,wlancreating->wlan_tunnel_mode);
	...
	*resultCode = CW_PROTOCOL_SUCCESS;
	...
	if(wlancreateOK == 1 && receiveframethread == 0){
		...
		if(!CWErr(CWCreateThread(&thread_receiveFrame_dot_11, CWWTPReceive802_11Frame, wlancreating,0))) {//(void*)&(wlancreating->wlan_id)
			CWDebugLog("Error starting Thread that receive 802.11 binding frame");
		}
	...
	if(wlancreateOK == 1){
		...
		//diaowq add for CMCC_TEST 802.3 tunnel @20110916
	  	 if((sockd3 = socket(PF_PACKET,SOCK_RAW,htons(ETH_P_ALL))) < 0){
			CWDebugLog("socket sockd3 init error");
		 }
		 strncpy(ifr.ifr_name,wlanname, sizeof(ifr.ifr_name));
		 if (ioctl(sockd3, SIOCGIFINDEX, &ifr) == -1){//bind to a interface
			CWDebugLog("sockd3 SIOCGIFINDEX2  failed");
		 }
		 memset(&toaddr3,'\0',sizeof(toaddr3));
		 toaddr3.sll_family = AF_PACKET;
		 toaddr3.sll_protocol = htons(0x0030);
		 toaddr3.sll_ifindex = ifr.ifr_ifindex;
		 if(bind(sockd3,(struct sockaddr *)&toaddr3,sizeof(toaddr3)) < 0){
			CWDebugLog(" sockd3 bind error");
		 }
		...
		if(debug_print){
			printf("wlancreating wlan_sendsock %d\n",wlancreating->wlan_sendsock);
			printf("wlancreating wlan_dot3sock %d\n",wlancreating->wlan_dot3sock);

		}
		CWDebugLog("wlancreating wlan_eapsock %d\n",wlancreating->wlan_eapsock);
	...
	for(i=0;i<deletewlancount;i++){
		...
		for(ptr=wtp_wlan_list;ptr != NULL;oldptr = ptr,ptr = ptr->next)
		{
			if((ptr->wlan_id == deleteWlanValues->wlan_id)&&(ptr->radio_id == deleteWlanValues->radio_id))
			{
				...
				char cmdbuf[128];
				memset(cmdbuf,0,128);
				sprintf(cmdbuf,"rm -rf /tmp/conf_wtpd/radio%d/wlaninfo/wlan%d",deleteWlanValues->radio_id,deleteWlanValues->wlan_id);
				_system(cmdbuf);
			//	gWlanIDMap[deleteWlanValues->wlan_id] = -1;//delete by wangxl @20121115
				CWWTPDebugLog("delete wlan:ath.%d-%d\n",deleteWlanValues->radio_id,deleteWlanValues->wlan_id);
				break;
			}
		}
		...
		sprintf(confArg.conf_value,"%d",wlan_count);
		CWSaveConfigInfo(CONF_RADIO,confArg,CW_TRUE);
/*		if(WLanTableDelete(&deleteWlanValues[i]) == CW_TRUE)
			delete_wlan(&deleteWlanValues[i]);
		else
			CWDebugLog("can't find the delete wlan");*/
	}
...
CWBool GetRadioId(unsigned char *mac_addr, unsigned char *radio_id)
{
	*radio_id = 0;
	...
	CWDebugLog("can't found the rid from mac_addr\n");
	if(debug_print)
		printf("%s,rid:%d\n",__func__,*radio_id);
	return CW_FALSE;
}
...
CWBool GetWlanId(DeleteSTAValues *deleteSTAValues, unsigned char *wlan_id)
{
	...
	CWDebugLog("can't found the sta\n");
	CWThreadMutexUnlock(&sta_table_mutex);
	return CW_FALSE;
}
CWBool CWBindingDeleteSta(DeleteSTAValues *deleteSTAValues,CWProtocolResultCode *resultCode)
...
#endif
	CWWTPDebugLog("%s,get wlan_id:%d", __func__,wlan_id);
	STATableDelete(deleteSTAValues);
	del_sta(sock, deleteSTAValues, &wlan_id);
	*resultCode = CW_PROTOCOL_SUCCESS;
	close(sock);
	return CW_TRUE;
}
...
*/
	...
	ret = set80211priv(sock,ifname,IEEE80211_IOCTL_SETKEY, (void *)&wk, sizeof(wk));
	return ret;
}
...
/*	int res = 0;  */
	...
	dpf("**********into set session key*********\n");
	for(i=0;i<6;i++){
		dpf("%02x ",broadcastmac[i]);
	}
	dpf("CWBindingSetSTASessionKey\n");
...
//			res = wapid_ioctl(wlanname, P80211_IOCTL_SETWAPI_INFO, buf, buf_len); 
			...
			wapi_set_key(sock,"WPI-SMS4",staSessionKey->mac,wlanname,key_idx,(u8 *)buf,32/*key_len*/,1);
			sleep(3);
			if(debug_print)
			{
				printf("!!!!!!1!!!!!!!!mcast-key:%s\n", staSessionKey->key);
				printf("buf_len:%d\nwlanname:%s\n", buf_len, wlanname);
			}
			...
			CW_FREE_OBJECT(staSessionKey->mac);
   			staSessionKey->mac = NULL;
			madwifi_set_key(sock,staSessionKey,ieee80211_InfoEleValues->wtp_ie->pairwise_cipher, ieee80211_InfoEleValues->radio_id,ieee80211_InfoEleValues->wlan_id);
	}
#if 0
		if(isWPAfirstKey){
			CWDebugLog("First WPA key so up the interface");
	...
	else
	{
		if(ieee80211_InfoEleValues->wtp_ie->pairwise_cipher == WTP_WAPI_CIPHER_SMS4)
		{
			...
			char *tmp;
			CW_CREATE_STRING_ERR(tmp, MAC_ADDR_LEN, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
		    ...
		    key_idx = (unsigned int)((staSessionKey->flags>>12)&~(0xfffC));
			wapi_set_key(sock,"WPI-SMS4",staSessionKey->mac,wlanname,key_idx,(u8 *)buf,32/*key_len*/,0);
...
CWBool acw_binding_set_ampdu(ampdu_op_type*set_ampdu_values, CWProtocolResultCode *resultCode)
{
	...
	*resultCode = CW_PROTOCOL_SUCCESS;
	...
	if(radio_found == CW_FALSE){
		CWDebugLog("The WTP dosen't have radio %d",set_ampdu_values->radio_id);
		*resultCode = CW_PROTOCOL_FAILURE;
		return CW_TRUE;
	}
	...
	if(set_ampdu_values->op_type==RADIO)
	{
		...
		CWWTPGetWifiName(wlanname,set_ampdu_values->radio_id);
		CWSetWifiAmpdu(wlanname,set_ampdu_values->state, set_ampdu_values->ampdulimit, set_ampdu_values->ampduframes);
		...
		CWSetInterfaceUp(wlanname,0);
		CWSetAmpdu(wlanname,set_ampdu_values->state, set_ampdu_values->ampdulimit, set_ampdu_values->ampduframes);
		CWWTPDebugLog("wlanname=%s set_ampdu_values->state=%d set_ampdu_values->ampdulimit=%d ampduframes=%d!",wlanname,set_ampdu_values->state,set_ampdu_values->ampdulimit,set_ampdu_values->ampduframes);
		 CWSetInterfaceUp(wlanname,1);		
	}
       
	CWDebugLog("%s ampdu set OK the ampdu_limit now is :%d dB,the ampdu_frames is :%d",wlanname,set_ampdu_values->ampdulimit,set_ampdu_values->ampduframes);
...
CWBool acw_binding_set_amsdu(amsdu_op_type*set_amsdu_values, CWProtocolResultCode *resultCode)
{
	...
	*resultCode = CW_PROTOCOL_SUCCESS;
	...
	if(radio_found == CW_FALSE){
		CWDebugLog("The WTP dosen't have radio %d",set_amsdu_values->radio_id);
		*resultCode = CW_PROTOCOL_FAILURE;
		return CW_TRUE;
	}
       ...
       if(set_amsdu_values->op_type==RADIO){
		...
		for(ptr = wtp_wlan_list ; ptr != NULL ; ptr = ptr->next){
			if(ptr->radio_id == set_amsdu_values->radio_id)
			{
				ptr->wlan_updown_time++;  //pei add 0225
				CWWTPGetWlanName(wlanname,ptr->radio_id,ptr->wlan_id);
				CWSetAmsdu(wlanname,set_amsdu_values->state, set_amsdu_values->amsdulimit);
	...
	CWDebugLog("%s amsdu set OK the amsdu_limit now is :%d dB,the amsdu_frames is :%d",wlanname,set_amsdu_values->amsdulimit,set_amsdu_values->amsduframes);
...
CWBool acw_binding_set_puren_mixed(puren_mixed_op_type*set_puren_mixed_values, CWProtocolResultCode *resultCode)
{
	...
	*resultCode = CW_PROTOCOL_SUCCESS;
	...
	if(radio_found == CW_FALSE){
		CWDebugLog("The WTP dosen't have radio %d",set_puren_mixed_values->radio_id);
		*resultCode = CW_PROTOCOL_FAILURE;
		return CW_TRUE;
	}
	...
	CWSetPuren(wlanname, set_puren_mixed_values->state);
	...
	CWDebugLog("%s puren set OK the puren  now is :%d ",wlanname,set_puren_mixed_values->state);
...
CWBool acw_binding_set_dhcp_snooping(dhcp_snooping_type*set_dhcp_snooping_values, CWProtocolResultCode *resultCode)
{
	...
	*resultCode = CW_PROTOCOL_SUCCESS;
	...
	if(radio_found == CW_FALSE){
		CWDebugLog("The WTP dosen't have radio %d",set_dhcp_snooping_values->radio_id);
		*resultCode = CW_PROTOCOL_FAILURE;
		return CW_TRUE;
	}

	CWSet_dhcp_snooping(set_dhcp_snooping_values->state);

	CWDebugLog("%s dhcpsnooping set OK the state  now is :%d ",set_dhcp_snooping_values->state);
	return CW_TRUE;
...
CWBool acw_binding_set_sta_info_report(sta_info_repport_type*set_sta_info_report_values, CWProtocolResultCode *resultCode)
{
	...
	*resultCode = CW_PROTOCOL_SUCCESS;
	...
	if(radio_found == CW_FALSE){
		CWDebugLog("The WTP dosen't have radio %d",set_sta_info_report_values->radio_id);
		*resultCode = CW_PROTOCOL_FAILURE;
		return CW_TRUE;
	}
	...
	CWDebugLog(" sta_report_by_dhcpsnooping set OK the state  now is :%d ",set_sta_info_report_values->state);
...
CWBool acw_binding_set_channel_extoffset(channel_extoffset_op_type *set_channel_extoffset_values, CWProtocolResultCode *resultCode)
{
	...
	*resultCode = CW_PROTOCOL_SUCCESS;
	...
	if(radio_found == CW_FALSE){
		CWDebugLog("The WTP dosen't have radio %d",set_channel_extoffset_values->radio_id);
		*resultCode = CW_PROTOCOL_FAILURE;
		return CW_TRUE;
	}
	...
	memset(cmdbuf, 0, 128);
	CWSetChannel(set_channel_extoffset_values->radio_id,set_channel_extoffset_values->wlan_id,channel);
	...
	CWDebugLog("%s extoffset set OK the extoffset  now is :%d ",wlanname,set_channel_extoffset_values->state);

	ConfArg confArg;
	memset(&confArg,0,sizeof(ConfArg));
	confArg.radioID = set_channel_extoffset_values->radio_id;
	strcpy(confArg.conf_arg,"extoffset");
	sprintf(confArg.conf_value,"%d",set_channel_extoffset_values->state);
	CWSaveConfigInfo(CONF_RADIO,confArg,CW_TRUE);
...
CWBool acw_binding_set_tx_chainmask(tx_chainmask_op_type *set_tx_chainmask_value, CWProtocolResultCode *resultCode)
{
	...
	*resultCode = CW_PROTOCOL_SUCCESS;
	...
	if(radio_found == CW_FALSE){
		CWDebugLog("The WTP dosen't have radio %d",set_tx_chainmask_value->radio_id);
		*resultCode = CW_PROTOCOL_FAILURE;
		return CW_TRUE;
	}
	...
	if(set_tx_chainmask_value->op_type==RADIO){
		...
		CWWTPGetWifiName(wlanname,set_tx_chainmask_value->radio_id);	
		CWSetTxChainMask(wlanname, set_tx_chainmask_value->state);
...
//		_system(cmdbuf);
		CWSetTx_ChainMask(wlanname, set_tx_chainmask_value->state);
...
//		_system(cmdbuf);
	...
	CWDebugLog("%s tx_chainmask set OK the tx_chainmask  now is :%d  ",wlanname,set_tx_chainmask_value->state);
...
CWBool acw_binding_set_rx_chainmask(rx_chainmask_op_type *set_rx_chainmask_value, CWProtocolResultCode *resultCode)
{
	...
	*resultCode = CW_PROTOCOL_SUCCESS;
	...
	if(radio_found == CW_FALSE){
		CWDebugLog("The WTP dosen't have radio %d",set_rx_chainmask_value->radio_id);
		*resultCode = CW_PROTOCOL_FAILURE;
		return CW_TRUE;
	}
	...
	if(set_rx_chainmask_value->op_type==RADIO){
		...
		CWWTPGetWifiName(wlanname,set_rx_chainmask_value->radio_id);	
...
//		sprintf(cmdbuf,"/sbin/ifconfig %s down > /dev/null", wlanname); 
//		_system(cmdbuf);
		CWSetRx_ChainMask(wlanname, set_rx_chainmask_value->state);
...
//		_system(cmdbuf);
	...
	CWDebugLog("%s rx_chainmask set OK the rx_chainmask  now is :%d  ",wlanname,set_rx_chainmask_value->state);
...
CWBool acw_binding_set_ack_timeout(ack_timeout_op_type *set_ack_timeout_value, CWProtocolResultCode *resultCode)
{
	...
	sprintf(cmdbuf,"acktimeout wifi%d set %d", set_ack_timeout_value->radio_id, set_ack_timeout_value->distance); 
	system(cmdbuf);
	if(debug_print)
		printf("cmd : %s\n",cmdbuf);
	...
	if(debug_print)
		printf("wifiname=wifi%d set_ack_timeout_value->state=%d set_ack_timeout_value->distance=%d !\n",set_ack_timeout_value->radio_id,set_ack_timeout_value->state,set_ack_timeout_value->distance);

	CWDebugLog("wifi%d ack_timeout set OK the ack_timeout distance now is :%d  ",set_ack_timeout_value->radio_id,set_ack_timeout_value->distance);
...
{
	CWDebugLog("Parsing DireceSequeueControl element");
	CWParseMessageElementStart();
...
{
	CWDebugLog("Parsing ADD wlan message element");
	CWWTPDebugLog("%s:",__func__);
	CWCaptrue(len,(unsigned char*)msgPtr->msg);
	CWParseMessageElementStart();
	valPtr->radio_id = CWProtocolRetrieve8(msgPtr);
	...
	dpf("type:%d key_length:%d\n", valPtr->key_status, valPtr->key_length);
	int i;
	for(i = 0 ; i < valPtr->key_length; i++){
		dpf("%02x ",valPtr->key[i]);
	}
	dpf("\nkey end\n");
	...
	CWWTPDebugLog("%s,macmode %d ,tunnel mode %d",__func__,valPtr->mac_mode,valPtr->tunnel_mode);
	valPtr->suppress_ssid = CWProtocolRetrieve8(msgPtr);
	valPtr->ssid = CWProtocolRetrieveStr(msgPtr,len+4-msgPtr->offset);

	valPtr->wlan_id = CWCreateLocalWlanID(valPtr->radio_id, valPtr->wlan_id);
...
{
	CWDebugLog("Parsing Information Element message element");
...
{
	int vlanTag;
	CWDebugLog("Parsing wlan vlan message element");
	CWParseMessageElementStart();
...
//	dpf("radioId:%d wlanId:%d vlanTag:%x 8021qTypeId:%x userPriority:%d canonicalFormat:%d vlanId:%d\n",
...
{
	CWDebugLog("Parsing wapi message element");
	CWParseMessageElementStart();
...
//	dpf("cerAsType:%d cerAsLen:%d cerAsValue:%s cerAeType:%d cerAeLen:%d cerAeValue:%s asType:%d asLen:%d asValue:%s\n",
...
{
	CWDebugLog("Parsing delete wlan message element");
	CWParseMessageElementStart();
	valPtr->radio_id = CWProtocolRetrieve8(msgPtr);
	valPtr->wlan_id = CWProtocolRetrieve8(msgPtr);
...
{
	CWDebugLog("Parsing update wlan message element");
	CWParseMessageElementStart();
	valPtr->radio_id = CWProtocolRetrieve8(msgPtr);
	valPtr->wlan_id = CWProtocolRetrieve8(msgPtr);
	valPtr->capabilities = CWProtocolRetrieve16(msgPtr);
...
{
	CWDebugLog("Parse add sta message values");
	CWParseMessageElementStart();
	valPtr->radio_id = CWProtocolRetrieve8(msgPtr);
...
{
	CWDebugLog("Parse delete sta message values");
	CWParseMessageElementStart();
	valPtr->radio_id = CWProtocolRetrieve8(msgPtr);
	valPtr->mac_length = CWProtocolRetrieve8(msgPtr);
	valPtr->mac_addr =  (unsigned char *)CWProtocolRetrieveStr(msgPtr,(int)valPtr->mac_length);
...
//	CWCaptrue(len,msgPtr->msg);
	CWDebugLog("Parsing sta session key element");
	CWParseMessageElementStart();
...
CWBool CWParseRateSet2(CWProtocolMessage *msgPtr, int len ,BindingRate *valPtr)	
{
	...
	int count;
	CWDebugLog("Parse rate set message values");
	CWParseMessageElementStart();
	valPtr->radioID = CWProtocolRetrieve8(msgPtr);
...
CWBool CWGetRateMask(unsigned short *rate, int count,int radioid)
{
	...
	unsigned int basic_rate_mask_all = 0;
	...
	{
		m = 0;
		if (debug_print) printf("%d,",rate[i]);
		while(m<ratenum)
		{
			rateoffset = 0;
			if(rate[i] == ptr[m]) //found
			{
	...
	basic_rate_mask_all = ratemask & basic_rate_mask_all;  
	if(basic_rate_mask_all != 0)
	{ //basic rateset mask
		if(radiotype&IEEE80211_PARAM_MODE_11g)
		{
			if((basic_rate_mask_all & 0xF) && (basic_rate_mask_all & 0x150))					 //{ 24 , *,   12£¬*£¬6£¬11£¬5.5£¬2£¬1} ,only select b rate for basic rate
				ratemask = ratemask | ((basic_rate_mask_all & 0xF)  << basic_rate_offset_skip);
			else
				ratemask = ratemask | (basic_rate_mask_all << basic_rate_offset_skip );
		}
		else
				ratemask = ratemask | (basic_rate_mask_all << basic_rate_offset_skip );
	}
...
CWBool CWParseRateSet(CWProtocolMessage *msgPtr, int len ,BindingRate *valPtr)				//pei add 0714
{
	...
	count = length/2;
	valPtr->rate = CWProtocolRetrieve16(msgPtr);
	CW_CREATE_ARRAY_ERR(rate, count*sizeof(unsigned short), unsigned short int, CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
...
{
	CWDebugLog("Parse radio type set message values");
	CWParseMessageElementStart();
	valPtr->radioID = CWProtocolRetrieve8(msgPtr);
...
{
	CWDebugLog("Parse mac operation message values");
	CWParseMessageElementStart();
	valPtr->radio_id = CWProtocolRetrieve8(msgPtr);
	valPtr->reserved = CWProtocolRetrieve8(msgPtr);
...
{
	CWDebugLog("Parse WTP radio configuration message values");
	CWParseMessageElementStart();
	valPtr->radio_id = CWProtocolRetrieve8(msgPtr);
	...
	{
		int i=0;
		printf("***wifi%d country_code:",valPtr->radio_id);
		for(i=0;i<4;i++)
		{
			printf("%c", valPtr->country_code[i]);
		}
		printf("-\n");
	}
	CWParseMessageElementEnd();
...
{
	CWDebugLog("Parse neighbor dead interval set message values");
	CWParseMessageElementStart();
...
{
	CWDebugLog("Parse radio ampdu configuration message values");
	CWParseMessageElementStart();
...
{
	CWDebugLog("Parse radio amsdu configuration message values");
	CWParseMessageElementStart();
	valPtr->op_type= CWProtocolRetrieve8(msgPtr);
	(msgPtr->offset) += 1; //jump wtp_radio_set_type 
	valPtr->radio_id = CWProtocolRetrieve8(msgPtr);
...
{
	CWDebugLog("Parse dhcp snooping configuration message values");
	CWParseMessageElementStart();
	valPtr->op_type= CWProtocolRetrieve8(msgPtr);
	(msgPtr->offset) += 1; //jump wtp_radio_set_type 
	valPtr->radio_id = CWProtocolRetrieve8(msgPtr);
...
{
	CWDebugLog("Parse sta info report configuration message values");
	CWParseMessageElementStart();
	valPtr->op_type= CWProtocolRetrieve8(msgPtr);
	(msgPtr->offset) += 1; //jump wtp_radio_set_type 
	valPtr->radio_id = CWProtocolRetrieve8(msgPtr);
...
{
	CWDebugLog("Parse channel_extoffset configuration message values");
	CWParseMessageElementStart();
	valPtr->op_type= CWProtocolRetrieve8(msgPtr);
	(msgPtr->offset) += 1; //jump wtp_radio_set_type 
	valPtr->radio_id = CWProtocolRetrieve8(msgPtr);
...
{
	CWDebugLog("Parse tx_chainmask configuration message values");
	CWParseMessageElementStart();
	valPtr->op_type= CWProtocolRetrieve8(msgPtr);
	(msgPtr->offset) += 1; //jump wtp_radio_set_type 
	valPtr->radio_id = CWProtocolRetrieve8(msgPtr);
...
{
	CWDebugLog("Parse rx_chainmask configuration message values");
	CWParseMessageElementStart();
	valPtr->op_type= CWProtocolRetrieve8(msgPtr);
	(msgPtr->offset) += 1; //jump wtp_radio_set_type 
	valPtr->radio_id = CWProtocolRetrieve8(msgPtr);
...
{
	CWDebugLog("Parse ack_timeout configuration message values");
	CWParseMessageElementStart();
	valPtr->op_type = CWProtocolRetrieve8(msgPtr);
	(msgPtr->offset) += 1; //jump wtp_radio_set_type 
	valPtr->radio_id = CWProtocolRetrieve8(msgPtr);
...
CWBool CWBindingSaveConfigurationUpdateRequest(void* bindingValuesPtr, CWProtocolResultCode* resultCode)
{
	...
	*resultCode = CW_PROTOCOL_SUCCESS;
	...
	if(bindingPtr->neighborDeadInterval !=0)
	{
		...
		memset(cmdbuf, 0, 128);
		CWSetNeighborDeadInterval(thinapCheckThreshold);
		
		ConfArg confArg;
		memset(&confArg,0,sizeof(confArg));
		strcpy(confArg.conf_arg,"gCWNeighborDeadInterval");
		sprintf(confArg.conf_value,"%d",gCWNeighborDeadInterval);
	...
	{
		CWDebugLog("config the Radio Type!");
	...
	{
		CWDebugLog("config the Mac Operation!");
	...
	{
		CWDebugLog("config the WTP Radio Configuration!");
	...
	if(radioID != 10)
	{
		...
		for(ptr = wtp_wlan_list ; ptr != NULL ; ptr = ptr->next)
		{
			if(ptr->radio_id == radioID)
			{
				...
				sprintf(confArg.conf_value,"%d",wlan_updown_time);
...
CWBool CWBindingParseConfigurationUpdateRequest (char *msg, int len, void **valuesPtr) 
{
	...
	*valuesPtr = (void *)auxBindingPtr;
	...
	while(completeMsg.offset < len) {
		...
		CWDebugLog("message element type %d",elemType);
		switch(elemType) {
	...
	while(completeMsg.offset < len) {
		...
		switch(type) {
			...
			case BINDING_MSG_ELEMENT_TYPE_DIRECT_SEQUEUE_CONTROL:{
				...
				j++;
				CWDebugLog("BINDING_MSG_ELEMENT_TYPE_DIRECT_SEQUEUE_CONTROL Parsed");
				break;
			}
			case BINDING_MSG_ELEMENT_TYPE_TX_POWER:{
				...
				k++;
				CWDebugLog("BINDING_MSG_ELEMENT_TYPE_TX_POWER Parsed");
				break;
			}
			case BINDING_MSG_ELEMENT_TYPE_IEEE80211_RATE_SET:{	     		 //pei add 0715
				 CWDebugLog("get the rate set message");
				...
				l++;
				CWDebugLog("BINDING_MSG_ELEMENT_TYPE_IEEE80211_RATE_SET Parsed");
				break;
			}
			case BINDING_MSG_ELEMENT_TYPE_IEEE80211_RADIO_TYPE_SET:{	//pei add 0715
				 CWDebugLog("get the radio type set message");
				...
				m++;
				CWDebugLog("BINDING_MSG_ELEMENT_TYPE_IEEE80211_RADIO_TYPE_SET Parsed");
				break;
			}
			case BINDING_MSG_ELEMENT_TYPE_IEEE80211_MAC_OPERATE:{	     //pei add 0722
				 CWDebugLog("get the mac operation message");
				...
				n++;
				CWDebugLog("BINDING_MSG_ELEMENT_TYPE_IEEE80211_MAC_OPERATE Parsed");
				break;
			}
			case BINDING_MSG_ELEMENT_TYPE_IEEE80211_WTP_RADIO_CONFIGURATON:{	//pei add 0722
				 CWDebugLog("get the WTP radio configuration message");
				...
				t++;
				CWDebugLog("BINDING_MSG_ELEMENT_TYPE_IEEE80211_WTP_RADIO_CONFIGURATON Parsed");
				break;
			}
			case BINDING_MSG_ELEMENT_TYPE_WTP_RADIO_SET:{
				completeMsg.offset+=1;
				...
				switch(value_in_case_type){
					case DHCP_SNOOPING:
						CWDebugLog("get the DHCP_SNOOPING configuration message");
						(completeMsg.offset)-=2;
					...
					break;
					case STA_INFO_REPORT:
						CWDebugLog("get the STA_INFO_REPORT  message");
						(completeMsg.offset)-=2;
					...
					break;
					case AMPDU_OP:
						CWDebugLog("get the AMPDU_OP configuration message");
						(completeMsg.offset)-=2;
					...
					/*yuan add ...*/
					case AMSDU_OP:
						CWDebugLog("get the AMSDU_OP configuration message");
						(completeMsg.offset)-=2;
					...
					break;
					case PUREN_MIXED_OP:
						CWDebugLog("get the PUREN_MIXED_OP   message");
						(completeMsg.offset)-=2;
					...
					break;
					case CHANNEL_EXTOFFSET:
						CWDebugLog("get the CHANNEL_EXTOFFSET configuration message");
						(completeMsg.offset)-=2;
					...
					break;
					case TX_CHAINMASK:
						CWDebugLog("get the TX_CHAINMASK configuration message");
						(completeMsg.offset)-=2;
					...
					break;
					case RX_CHAINMASK:
						CWDebugLog("get the RX_CHAINMASK configuration message");
						(completeMsg.offset)-=2;
					...
					break;
					case ACK_TIMEOUT:
						CWDebugLog("get the ACK_TIMEOUT configuration message");
						(completeMsg.offset)-=2;
...
{	
	...
	*resultCode = CW_PROTOCOL_SUCCESS;
	...
	if (bindingPtr->qosCount>0) 
	{
		//yuanding add start
#ifdef MARVELL_PLATFORM
		memcpy(&gRadioQosValues, bindingPtr->radioQosValues, sizeof(Radio_QosValues));
#else
	...
	for(i=0;i<bindingPtr->directSequeueControlCount;i++)
	{
		...
		strcpy(confArg.conf_arg,"channel");
		sprintf(confArg.conf_value,"%d",gRadioInfoValue[radioId].channel);
		CWSaveConfigInfo(CONF_RADIO,confArg,CW_TRUE);
	}
	...
	for(i=0;i<bindingPtr->txPowerCount;i++)
	{
		...
		confArg.radioID = radioId;
		strcpy(confArg.conf_arg,"txtype");
		sprintf(confArg.conf_value,"%d",gRadioInfoValue[radioId].txtype);
		CWSaveConfigInfo(CONF_RADIO,confArg,CW_FALSE);
		
		memset(&confArg,0,sizeof(confArg));
		confArg.radioID = radioId;
		strcpy(confArg.conf_arg,"txpower");
		sprintf(confArg.conf_value,"%d",gRadioInfoValue[radioId].txpower);
		CWSaveConfigInfo(CONF_RADIO,confArg,CW_FALSE);
	}
	...
	for(i=0;i<bindingPtr->rateCount;i++)
	{
		...
		sprintf(confArg.conf_value,"%hd",gRadioInfoValue[radioId].rate);
		CWSaveConfigInfo(CONF_RADIO,confArg,CW_FALSE);
	}
	CW_FREE_OBJECT(bindingPtr->rateSetValues);
	for(i=0;i<bindingPtr->radioTypeCount;i++)
	{
		...
		confArg.radioID = radioId;
		strcpy(confArg.conf_arg,"radiotype");
		sprintf(confArg.conf_value,"%d",gRadioInfoValue[radioId].radiotype);
		CWSaveConfigInfo(CONF_RADIO,confArg,CW_FALSE);
	}
	CW_FREE_OBJECT(bindingPtr->radioTypeSetValues);
	...
	for(i=0;i<bindingPtr->macOperationCount;i++)
	{
		...
		confArg.radioID = radioId;
		strcpy(confArg.conf_arg,"fragThreshold");
		sprintf(confArg.conf_value,"%hd",gRadioInfoValue[radioId].fragThreshold);
		CWSaveConfigInfo(CONF_RADIO,confArg,CW_FALSE);

		memset(&confArg,0,sizeof(confArg));
		confArg.radioID = radioId;
		strcpy(confArg.conf_arg,"rtsThreshold");
		sprintf(confArg.conf_value,"%hd",gRadioInfoValue[radioId].rtsThreshold);
		CWSaveConfigInfo(CONF_RADIO,confArg,CW_FALSE);

		memset(&confArg,0,sizeof(confArg));
		confArg.radioID = radioId;
		strcpy(confArg.conf_arg,"shortRetry");
		sprintf(confArg.conf_value,"%d",gRadioInfoValue[radioId].shortRetry);
		CWSaveConfigInfo(CONF_RADIO,confArg,CW_FALSE);

		memset(&confArg,0,sizeof(confArg));
		confArg.radioID = radioId;		
		strcpy(confArg.conf_arg,"longRetry");
		sprintf(confArg.conf_value,"%d",gRadioInfoValue[radioId].longRetry);
		CWSaveConfigInfo(CONF_RADIO,confArg,CW_FALSE);
	}
	CW_FREE_OBJECT(bindingPtr->macOperation);
	...
	for(i=0;i<bindingPtr->radioConfigCount;i++)
	{
		...
		sprintf(cmdbuf, "/usr/sbin/set_countrycode %d %s >/dev/null 2>&1",radioId,CountryCode);
		_system(cmdbuf);
		
		memset(cmdbuf,0,128);
		sprintf(cmdbuf, "iwpriv wifi%d setCountry %s >/dev/null",radioId,CountryCode);
		_system(cmdbuf);
		...
		confArg.radioID = radioId;
		strcpy(confArg.conf_arg,"preamble");
		sprintf(confArg.conf_value,"%d",gRadioInfoValue[radioId].preamble);
		CWSaveConfigInfo(CONF_RADIO,confArg,CW_FALSE);
		
		memset(&confArg,0,sizeof(confArg));
		confArg.radioID = radioId;
		strcpy(confArg.conf_arg,"dtim");
		sprintf(confArg.conf_value,"%d",gRadioInfoValue[radioId].dtim);
		CWSaveConfigInfo(CONF_RADIO,confArg,CW_FALSE);
		
		memset(&confArg,0,sizeof(confArg));
		confArg.radioID = radioId;
		strcpy(confArg.conf_arg,"beaconInterval");
		sprintf(confArg.conf_value,"%hd",gRadioInfoValue[radioId].beaconInterval);
		CWSaveConfigInfo(CONF_RADIO,confArg,CW_FALSE);
	}
...
CWBool CWBindingParseConfigureResponse (char *msg, int len, void **valuesPtr){
	...
	CWDebugLog("Parsing Binding Configuration Response...");
...
CWBool CWBindingParseIEEE80211WlanConfigurationRequest(char *msg, int len, void **valuesPtr)
{
	...
	if(msg == NULL || valuesPtr == NULL) return CWErrorRaise(CW_ERROR_WRONG_ARG,NULL);
	CWDebugLog("Parse Binding IEEE80211configuration request");
	completeMsg.msg = msg;
	completeMsg.offset = 0;
	...
	*valuesPtr = (void *)auxBindingPtr;
	...
	while(completeMsg.offset < len) {
		...
		switch(elemType) {
			...
			case BINDING_MSG_ELEMENT_TYPE_IEEE80211_INFO_ELEMENT:              //pei add 0605
				...
				completeMsg.offset += elemLen;
				CWDebugLog("parse Information Element finished");}
				break;
			case BINDING_MSG_ELEMENT_TYPE_IEEE80211_WLAN_VLAN_INFO:         //pei add 0305
				CWDebugLog("get the wlan vlan message");
				if(!(CWParseWlanVlan(&completeMsg, elemLen,auxBindingPtr->wlanVlanValues))){
				completeMsg.offset += elemLen;
				CWDebugLog("parse wlan vlan finished");}
				break;
			case BINDING_MSG_ELEMENT_TYPE_WAPI_CER_INFO_ELEMENT:
				CWDebugLog("get the wapi message");
				if(!(CWParseWapi(&completeMsg, elemLen,auxBindingPtr->wapiValues))){
				completeMsg.offset += elemLen;
				CWDebugLog("parse wapi finished");}
				break;
			default:
				completeMsg.offset += elemLen;
				break;
	...
	CWDebugLog("Binding WLAN configuration Request Parsed");
	...
	*resultCode = CW_PROTOCOL_SUCCESS;
	...
	if(bindingPtr->addwlancount > 0&&bindingPtr->infoelecount > 0&&bindingPtr->wlanVlanCount > 0){
		...
		*created_wlan_id = bindingPtr->addWlanValues->wlan_id;
		...
		CWDebugLog("receive wlan id %d \n",bindingPtr->addWlanValues->wlan_id);
...
//		if(!CWBindingAddWlan(bindingPtr->addWlanValues, bindingPtr->ieee80211_InfoEleValues->wtp_ie, resultCode)){
	...
	if(bindingPtr->deleteWlanCout > 0){
		...
		*created_wlan_id = bindingPtr->deleteWlanValues->wlan_id;
		...
		CWDebugLog("receive wlan id %d \n",bindingPtr->deleteWlanValues->wlan_id);
	...
	if(bindingPtr->updateWlanCount > 0){
		...
		*created_wlan_id = bindingPtr->updateWlanValues->wlan_id;
		CWDebugLog("receive wlan id %d \n",bindingPtr->updateWlanValues->wlan_id);
    ...
    if(marvell_flag)
    {
        CWSetWifiCommit(*created_radio_id);
    }
#endif
...
CWBool CWBindingParseSTAConfigurationRequest(char *msg, int len, void **valuesPtr)
{
	...
	if(msg == NULL || valuesPtr == NULL) return CWErrorRaise(CW_ERROR_WRONG_ARG,NULL);
	CWDebugLog("CWBindingParseSTAConfigurationRequest");
	completeMsg.msg = msg;
	completeMsg.offset = 0;
	completeMsg.msgLen = len;
	...
	*valuesPtr = (void *)auxBindingPtr;
	...
	while(completeMsg.offset < len) {
		...
		switch(elemType) {
			case CW_MSG_ELEMENT_ADD_STATION_CW_TYPE:
				 CWDebugLog("get the add sta message");
				 if(!(CWParseAddSta(&completeMsg, elemLen,auxBindingPtr->addSTAValues))){
				 completeMsg.offset += elemLen;
				 CWDebugLog("parse add sta finished");}
				 break;
			case CW_MSG_ELEMENT_DELETE_STATION_CW_TYPE:
				 CWDebugLog("get the add sta message");
				 if(!(CWParseDeleteSta(&completeMsg, elemLen,auxBindingPtr->deleteSTAValues))){
				 completeMsg.offset += elemLen;
				 CWDebugLog("parse delete sta finished");}
				 break;
			case BINDING_MSG_ELEMENT_TYPE_STATION_SESSION_KEY:				
				 CWDebugLog("get sta session key message");
				 if(!(CWParseSTASessionKey(&completeMsg, elemLen,auxBindingPtr->staSessionKeyValues))){
				 completeMsg.offset += elemLen;
				 CWDebugLog("parse sta session key finished");}
				 break;
			case BINDING_MSG_ELEMENT_TYPE_IEEE80211_INFO_ELEMENT:              //pei add 0606
				 CWDebugLog("get ieee80211 info element message");
				 if(!(CWParseInformationElement(&completeMsg, elemLen,auxBindingPtr->ieee80211_InfoEleValues))){
				 completeMsg.offset += elemLen;
				 CWDebugLog("parse ieee80211 info element finished");}
				 break;
			default:
				completeMsg.offset += elemLen;
				break;
	...
	CWDebugLog("Binding STA configuration Request Parsed");
...
CWBool CWSaveAddwlanConfigInfo(CWWTPWlan *wlancreating)
{
	...
	sprintf(confArg.conf_value,"%d",wlancreating->radio_id);
	CWSaveConfigInfo(CONF_WLAN,confArg,CW_TRUE);
	...
	sprintf(confArg.conf_value,"%d",CWGetRemoteWlanID(wlancreating->radio_id,wlancreating->wlan_id));
	...
	sprintf(confArg.conf_value,"%hd",wlancreating->vlan_id);
	CWSaveConfigInfo(CONF_WLAN,confArg,CW_FALSE);
	...
	confArg.wlanID	= wlancreating->wlan_id;
	strcpy(confArg.conf_arg,"wlan_updown_time");
	sprintf(confArg.conf_value,"%d",wlancreating->wlan_updown_time);
	CWSaveConfigInfo(CONF_WLAN,confArg,CW_FALSE);
	...
	sprintf(confArg.conf_value,"%d",wlancreating->wlan_mac_mode);
	CWSaveConfigInfo(CONF_WLAN,confArg,CW_FALSE);
	...
	sprintf(confArg.conf_value,"%d",wlancreating->wlan_tunnel_mode);
	CWSaveConfigInfo(CONF_WLAN,confArg,CW_FALSE);
	return CW_TRUE;
}
CWBool CWSaveConfigInfo(Conf_Type conf_type,ConfArg confArg,CWBool chkdir)
{
	...
	if(CONF_WTPD == conf_type)
	{
		...
		sprintf(filepath,"%s/%s",CONF_WTPD_PATH,confArg.conf_arg);
...
{	
	...
	{	
		sprintf(cmdline,"echo 2 > /proc/sys/net/ath.%d-%d/vap_splitmac",changeTunnelMode->radio_id,changeTunnelMode->wlan_id);
	}
	
	if(changeTunnelMode->tunnel_mode==CW_802_DOT_11_TUNNEL)
	{
		sprintf(cmdline,"echo 1 > /proc/sys/net/ath.%d-%d/vap_splitmac",changeTunnelMode->radio_id,changeTunnelMode->wlan_id);
	}
	_system(cmdline);
	CWWTPDebugLog(cmdline);
	dpf(cmdline);
	*resultCode = CW_PROTOCOL_SUCCESS;
	return CW_TRUE;
}
