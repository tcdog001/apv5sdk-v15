 ...
 * 
 */
 #define  PWLANXSCALE   //hh2007.1.31
 int InterfaceScanInit=0;
 #ifdef PWLANXSCALE
 u_char         *
...
//printf("var_ipAddrEntry1\n");
    ...
    u_char         *cp;
    int             lowinterface = 0;
extern struct ifnet *ifnetaddr_list;
#ifndef freebsd2
    short           interface; //
#endif
#ifdef hpux11
  ...
  // displaylist();
    ...
    if (header_simple_table(vp,name,length,exact,var_len,write_method, 7)) //listnumber))
        return NULL;
    ...
    switch (vp->magic) {
    case IPADADDR:
        long_return =0; // pp->if_addr;
        return (u_char *)  & long_return;
    case IPADIFINDEX:
        long_return = i;
        return (u_char *) & long_return;
    case IPADNETMASK:
        long_return =0; //pp->ia_subnetmask;
        return (u_char *) & long_return;
    case IPADBCASTADDR:
        long_return =1; 
        return (u_char *) & long_return;
...
Address_Scan_Next(short *Index, struct ifnet *Retifnet)
{
    ...
    //        *Index = netsnmp_access_interface_index_find(ifr->ifr_name); //cz modified
	}
...
#define IP_ADDRINDEX_OFF	10
	...
	*var_len = sizeof(uint32_t);
    ...
    case IPADIFINDEX:
        ...
        long_return =
 //cz modified           netsnmp_access_interface_index_find(Lowentry.
