...
#include "../autelan-mib/autelan-sourceFile/wapi/mii_app.h"
#include "../autelan-mib/autelan-sourceFile/wapi/wapicgifunc.h"
#include "../autelan-mib/autelan-sourceFile/wapi/wapiPwlanAp.h"

unsigned long GetWirelessSpeed(char *strPrefix)
{
    int iMod;
    ...
    if(strcmp(strPrefix,"awlan")==0){
	    ...
	    iMod=atoi(ap_parameter);
    }
    else if(strcmp(strPrefix,"ath")==0){ // by sunyushuang0320
	    memset(ap_parameter,0,256);
            prop_count=load_prop(SEP_EQUAL,WIRELESS_CONF,properties);
...
unsigned long GetEthSpeed(char *strPrefix)
{
  	FILE *fp;
  	char buffer[256];
	char string[256];
	...
	memset (string, 0, 256);
	printf("strPrefix:%s\n",strPrefix);
	sprintf( buffer, "%s  %s  %s", "/usr/sbin/ethtool",strPrefix,"|awk '/Speed:/{print $2}'");
	printf("buffer:%s\n",buffer);
	fp=popen(buffer,"r");
	  if(fp)
		{
	  ...
	  printf("string:%s\n",string);
    if(strcmp(string,"1000Mb/s")==0)
    	{
    	return 1000000000;
    	}
    if(strcmp(string,"100Mb/s")==0)
    	{
    	return 100000000;
    	}
      if(strcmp(string,"10Mb/s")==0)
    	{
    	return 10000000;
    	}
}
...
//*/
//#ifdef linux //cz modified
#ifdef linux  //sys modified
 ...
 */
#if !defined (WIN32) && !defined (cygwin)
static int header_ifEntry(struct variable *vp,
               oid * name,
               size_t * length,
...
static int InterfaceScanInitFlag=0;
...
//printf("start var_ifEntry\n");
    ...
    char           *cp;
    int i;
    int listnumber= Interface_Scan_Get_Count();
 	static struct ifnet *pp;
  	static  char strmac[256];
  	static  char strifname[256];
    
 
	if(InterfaceScanInitFlag==0){
    	Interface_Scan_Init();
    	InterfaceScanInitFlag++;
	}
    ...
    if (header_simple_table(vp,name,length,exact,var_len,write_method, listnumber))
        return NULL;
    for (i = 1;( pp != NULL && i < name[*length - 1] ); pp=pp->if_next , i++)
    {
      // if( strcmp(pp->if_name,"eth")==0 ) { //2007.12.19
      //     if ( pp->if_unit==0)
	   //	       pp=pp->if_next;
   		//if(i==listnumber)
		//InterfaceScanInitFlag=0;
		//printf("InterfaceScanInitFlag=%d\n",InterfaceScanInitFlag);
		
     	//printf("if_name=%s  if_unit=%s  if_mtu=%d   if_flags=%d  if_metric=%d  if_hwaddr=%d  if_type=%d  if_speed=%d \n",
    	//        pp->if_name,pp->if_unit,pp->if_mtu,pp->if_flags,pp->if_metric,pp->if_hwaddr,pp->if_type,pp->if_speed);
       //	}
        DEBUGMSGTL(("wapi/wapias", "wapi -- %lu != %lu\n",i, name[*length - 1]));
    }
    if (pp == NULL)
    {
        DEBUGMSGTL(("wapi/wapias", "wapi -- no match: %lu\n",i));
        return NULL;
    }
    DEBUGMSGTL(("wapi/wapias", "wapi -- match: %lu\n", i));
	
    //lhhtemp=Interface_Scan_By_Index(interface, Name, &ifnet, &in_ifaddr);
   ...
   // while (if_ptr && strcmp(Name, if_ptr->name))
    ...
    case IFDESCR:
	 bzero(strifname, 50);
        strcpy(strifname,pp->if_name);
	  printf("if_name=%s\n", pp->if_name);
	 strcat(strifname,pp->if_unit);
	 printf("if_unit=%s\n", pp->if_unit);
	 *var_len=strlen(strifname);
        return (u_char *) strifname;
    case IFTYPE: 
            long_return = pp->if_type;
			 printf("if_type=%d\n", pp->if_type);
        return (u_char *) & long_return;
    case IFMTU:
            long_return = pp->if_mtu;
            return (u_char *) & long_return;        
    case IFSPEED:       

/*	    if(strcmp(pp->if_name,"eth")==0){    //zpz-2008.6.11
		long_return = g_ulMiiSpeed[get_dev_link("eth0")];
	    }
*/

	    if(strcmp(pp->if_name,"eth")==0){    //sys modify
	      printf("pp->if_name:%s\n",pp->if_name);
		bzero(strifname, 50);
        	 strcpy(strifname,pp->if_name);
		 strcat(strifname,pp->if_unit);
		long_return = GetEthSpeed(strifname); 
	    }
	    else if(strcmp(pp->if_name,"ath")==0 || strcmp(pp->if_name,"awlan")==0 ){
		long_return = GetWirelessSpeed(pp->if_name);  // by sunyushuang0320
	    }
	    else{
		long_return = (u_long)pp->if_speed;
	    }
        return (u_char *) & long_return;
    ...
    case IFPHYSADDRESS:
     bzero(strmac, 20);
     memcpy(strmac,pp->if_hwaddr,20);
     *var_len=6;
       if ((strmac[0] == 0) && (strmac[1] == 0) &&
            (strmac[2] == 0) && (strmac[3] == 0) &&
            (strmac[4] == 0) && (strmac[5] == 0))
            *var_len = 0;
	return (u_char *) strmac;	
      
    case IFADMINSTATUS:
        long_return = pp->if_flags & IFF_UP ? 1 : 2;
        return (u_char *) & long_return;
    case IFOPERSTATUS:
        long_return = pp->if_flags & IFF_RUNNING ? 1 : 2;
        return (u_char *) & long_return;
    case IFLASTCHANGE:
        long_return=0;
        return (u_char *) & long_return;
    case IFINOCTETS:
       long_return = (u_long) pp->if_ibytes;  //hh2007.12.20
        return (u_char *) & long_return;
    case IFINUCASTPKTS:
        
            long_return = (u_long) pp->if_ipackets;    
        return (u_char *) & long_return;
    case IFINNUCASTPKTS:
        ...
        return (u_char *) & long_return;
    case IFINDISCARDS:
        long_return = (u_long) pp->if_iqdrops;
        return (u_char *) & long_return;
    case IFINERRORS:
        long_return = (u_long) pp->if_ierrors;
        return (u_char *) & long_return;
    case IFINUNKNOWNPROTOS:
    ...
    case IFOUTOCTETS:
        ...
        long_return = (u_long) pp->if_obytes;

        return (u_char *) & long_return;
    case IFOUTUCASTPKTS:
         long_return = (u_long) pp->if_opackets;;
        return (u_char *) & long_return;
    case IFOUTNUCASTPKTS:
        ...
        return (u_char *) & long_return;
    case IFOUTDISCARDS:
        long_return = pp->if_snd.ifq_drops;
        return (u_char *) & long_return;
    case IFOUTERRORS:
        long_return = pp->if_oerrors;
        return (u_char *) & long_return;
    case IFOUTQLEN:
        long_return = pp->if_snd.ifq_len;
        return (u_char *) & long_return;
    case IFSPECIFIC:
        *var_len = nullOidLen;
...
#   else
   return g_ulMiiSpeed[get_dev_link("eth0")]; //_mii_if_speed(fd, ifr);
#   endif /* HAVE_LINUX_ETHTOOLS_H */
#else /*!linux*/	
    unsigned int retspeed = 10000000;
    return retspeed;
...
//    if(strcmp(ifname_buf,"eth0")==0) continue ;		
//	if(strcmp(ifname_buf,"lo")==0) continue ;
//	if(strcmp(ifname_buf,"ath15")==0) continue ;
//	if(strcmp(ifname_buf,"aath215")==0) continue ;
//	if(strcmp(ifname_buf,"wifi0")==0) continue ;
//	if(strcmp(ifname_buf,"wifi1")==0) continue ;
//	if(strcmp(ifname_buf,"br0")==0) continue ;


//	if((strcmp(ifname_buf,"eth1")!=0)&&(strncmp(ifname_buf,"awlan",5)!=0))continue;
        ...
        *ifnetaddr_ptr = nnew;
        ...
        for (ptr = nnew->if_name; *ptr && (*ptr < '0' || *ptr > '9'); ptr++);
...
//	SaveLogger("defined:ARPHRD_LOOPBACK");
	    switch (ifrq.ifr_hwaddr.sa_family) {
            case ARPHRD_ETHER:
				//modify by piyp 2008-12-26    // by sunyushuang0320
				if(strstr(nnew->if_name,"ath")!=0){  
					nnew->if_type = 184;   //PropBroadbandWirelessAccesspt2multipt radio Interface
...
#endif
 	{   //memset(strMsg,0,256);
	    printf("name:%s,type: %d,unit:%s,iSpeed:%d\n", nnew->if_name,nnew->if_type,nnew->if_unit,nnew->if_speed);
	    //SaveLogger( strMsg );
	}
	    ...
	    if (!nnew->if_type){
            ...
            nnew->if_speed = nnew->if_type == 6 ? /*MySpeedgetIfSpeed(fd, ifrq)*/10000000 :
