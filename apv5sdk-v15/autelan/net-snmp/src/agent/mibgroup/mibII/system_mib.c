...
int GetHostName( char *strName )
{
    ...
    get_prop("HOST_NAME", strName, properties, prop_count);

    return 0;
}

	        
int SaveHostName( char *strName )
{
   save_global_conf( SEP_EQUAL,BASIC_CONF,"","HOST_NAME", strName );
    return 0;
}
void
...
#if (defined (WIN32) && defined (HAVE_WIN32_PLATFORM_SDK)) || defined (mingw32)
  {
    ...
    /* default sysObjectID */
	{
	  ...
	  for(i=0;i<strlen(oid_str);i++)
	    ...
	    while( NULL != p)
		  {
			  version_sysoid[count++] = (oid)atoi(p);
			  p = strtok(NULL,",");
		   }
		version_sysoid[count++] = 2;
		version_sysoid[count++] = 3;
		version_sysoid[count++] = 1;
	   }   
