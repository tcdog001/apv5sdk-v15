...
int displaylist()
{
	int i;
	 ...
	 p=ifnetaddr_list;
	 for(i=1;i<8;i++){
	printf("if_name=%s  if_unit=%s  if_mtu=%d   if_flags=%d  if_metric=%d  if_hwaddr=%d  if_type=%d  if_speed=%d  if_addr=%d \n",
		   p->if_name,p->if_unit,p->if_mtu,p->if_flags,p->if_metric,p->if_hwaddr,p->if_type,p->if_speed,p->if_addr);
	p=p->if_next;
	}
...
#if HAVE_LINUX_ETHTOOLS_H
static u_int _ethtools_if_speed(int fd, struct ifreq ifr)
{
...
//	if(strcmp(ifname_buf,"aath215")==0) continue ;
	if((strncmp(ifname_buf,"eth",3)!=0)&&(strncmp(ifname_buf,"awlan",5)!=0)continue;
