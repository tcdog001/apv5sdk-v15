...
#include "autelan-sourceFile/wapi/iwlib.h"
...
static double mypow(double m,double n)
{
 	int pow = 0;
	double flag = 0;
	int i = 0;
...
#define DOT11WEPDEFAULTKEYINDEX		30
{DOT11WEPDEFAULTKEYINDEX,  ASN_INTEGER,  RONLY,   var_dot11WEPDefaultKeysTable, 3,  { 1,3,1,1 }},
          ...
          WriteMethod **write_method)
{
    ...
    if ((  exact && ( snmp_oid_compare ( rName, vp->namelen + 1, name, *length ) == 0 )) ||
      ...
      switch ( vp->magic ) {
        ...
        case DOT11MULTICASTTRANSMITTEDFRAMECOUNT:
          if ( co->haveMulticastTransmittedFrameCount       ) found = TRUE; break;
  ...
  *write_method = NULL;
  ...
  switch ( vp->magic ) {
    ...
    case DOT11MULTICASTTRANSMITTEDFRAMECOUNT: return ( UCHAR * ) &co->multicastTransmittedFrameCount;
            ...
            size_t   name_len)
{
  ...
  switch ( action ) {
    ...
    case ACTION:
		...
		memset(nSc.ifName,0,IFNAME_LEN + 1);
		nSc.ifIndex=name[name_len-1];
		if_indextoname(nSc.ifIndex,nSc.ifName);
		memset(string,0,SPRINT_MAX_LEN);
		memcpy(string,var_val,var_val_len);
		string[var_val_len]='\0';
		wireless_config_ssid(get_vnum_by_vname( nSc.ifName ) , string );
    case UNDO:
      break;
  ...
  switch ( action ) {
    ...
    case RESERVE1:
      ...
      if ( var_val_len > sizeof ( long_ret )){
       ...
       if ((long_ret < MIN_BEACON_INT)||(long_ret > MAX_BEACON_INT)) 
	   	{
            fprintf(stderr, "write to dot11BeaconPeriod:bad value\n");
    ...
    case ACTION:
		...
		wireless_config_bintval( get_vnum_by_vname(nSc.ifName), string);
            ...
            size_t   name_len)
{
  ...
  switch ( action ) {
    ...
    case RESERVE1:
	  ...
	  long_ret = *((long *) var_val);
       if ((long_ret < MIN_DTIM_INT)||(long_ret > MAX_DTIM_INT)) {
    ...
    case ACTION:
		...
		memset (string, 0, sizeof(string));
		sprintf(string,"%d",long_ret);
		wireless_config_dtim(get_vnum_by_vname(nSc.ifName), string);
            ...
            size_t   name_len)
{
  ...
  switch ( action ) {
    ...
    case RESERVE1:
      ...
      if ( var_val_len > sizeof ( long_ret )){
       ...
       if ((long_ret != 1)&&(long_ret != 2)) {
            fprintf(stderr, "write to dot11AuthenticationAlgorithmsEnable:bad value\n");
            return SNMP_ERR_WRONGVALUE;
        }
      break;
    ...
    case FREE:
		break;
    case ACTION:
		nAa.authenticationAlgorithmsEnable=long_ret;
		break;
    case UNDO:
      break;
...
{
  static unsigned char string[SPRINT_MAX_LEN];
  static unsigned char buffer[SPRINT_MAX_LEN];
  static unsigned char value[SPRINT_MAX_LEN];
  static unsigned char type [SPRINT_MAX_LEN];
  char *p;
  int i=0,j=0;
  int size;
            ...
            size_t   name_len)
{
  ...
  switch ( action ) {
    ...
    case RESERVE1:
	  ...
	  long_ret = *((long *) var_val);
	  if((long_ret < MIN_RTS_INT)||(long_ret > MAX_RTS_INT)) 
      {
            fprintf(stderr, "write to dot11RTSThreshold:bad value\n");
            return SNMP_ERR_WRONGVALUE;
        }
      break;
    ...
    case FREE:
    case ACTION:
		memset(nOp.ifName,0,IFNAME_LEN + 1);
		nOp.ifIndex=name[name_len-1];
               printf("nOp.ifIndex==%d\n",nOp.ifIndex);
		if( NULL==if_indextoname(nOp.ifIndex,nOp.ifName))
		{
     			 syslog ( LOG_ERR, "SNMP %s - %s %s\n", 
       		        "ieee802dot11.load80211Structs()", nOp.ifName, "has no ifName" );
			 break;
		}

        memset (string, 0, sizeof(string));
	    sprintf(string,"%d",long_ret);
		wireless_config_rts(get_vnum_by_vname(nOp.ifName), string);
      break;
    case UNDO:
    case COMMIT:
      break;
            ...
            size_t   name_len)
{
  ...
  switch ( action ) {
    ...
    case RESERVE1:
	  ...
	  long_ret = *((long *) var_val);
       if ((long_ret < MIN_FRAG_INT)||(long_ret > MAX_FRAG_INT)) {
            fprintf(stderr, "write to dot11FragmentationThreshold:bad value\n");
            return SNMP_ERR_WRONGVALUE;
        }
      break;
    ...
    case ACTION:
		...
		memset (string, 0, sizeof(string));
		sprintf(string,"%d",long_ret);
		wireless_config_frag(get_vnum_by_vname(nOp.ifName), string);
    case UNDO:
      break;
  ...
  switch ( action ) {
    ...
    case RESERVE1:
      ...
      if ( var_val_len > sizeof ( long_ret )){
	   ...
	   long_ret = *((long *) var_val);
       if ((long_ret < MIN_TXPOWER_INT)||(long_ret > MAX_TXPOWER_INT)) {
            fprintf(stderr, "write to dot11CurrentTxPowerLevel:bad value\n");
            return SNMP_ERR_WRONGVALUE;}
      break;
    ...
    case ACTION:
		...
		memset(nPt.ifName,0,IFNAME_LEN + 1);
		nPt.ifIndex=name[name_len-1];
               printf("nPt.ifIndex==%d\n",nPt.ifIndex);
		if( NULL==if_indextoname(nPt.ifIndex,nPt.ifName))
		{
     			 syslog ( LOG_ERR, "SNMP %s - %s %s\n", 
       		        "ieee802dot11.load80211Structs()", nPt.ifName, "has no ifName" );
			 break;
		}
		memset (string, 0, sizeof(string));
	    sprintf(string,"%d",long_ret);
		wireless_config_txpower(get_vnum_by_vname(nPt.ifName),string);
    case UNDO:
      break;
            ...
            size_t   name_len)
{
  ...
  switch ( action ) {
    ...
    case RESERVE1:
	  ...
	  long_ret = *((long *) var_val);
       if ((long_ret < MIN_CHAN_INT)||(long_ret > MAX_CHAN_INT)) {
            fprintf(stderr, "write to dot11CurrentChannel:bad value\n");
            return SNMP_ERR_WRONGVALUE;
        }
      break;
    ...
    case ACTION:
		memset (string,0,128);
		memset (value,0,128);
		sprintf(value,"%s  %s  %s","/sbin/ifconfig",nPd.ifName,"down");
		system(value);
		sprintf(string,"%s  %s   %s  %d","/usr/sbin/iwconfig",nPd.ifName,"channel",long_ret);
		printf("string:%s\n",string );
		system(string);
		memset(value,0,128);
		sprintf(value,"%s  %s  %s","/sbin/ifconfig",nPd.ifName,"up");
		system(value);
              // wireless_config_channel(get_vnum_by_vname(nPd.ifName),string);
  break;
    case UNDO:
      break;
...
nSc.ifIndex = ifIndex;
{
   ...
   strcpy(nSc.ifName,ifName);
   sprintf( str_tmp_cmd, "%s %s %s %s", "/usr/sbin/iwpriv",nSc.ifName, "get_bintval", "| awk -F \":\"  '{print $2}'");
   fp=popen(str_tmp_cmd,"r");
   memset (string, 0, SPRINT_MAX_LEN);
   fgets(string,sizeof(string),fp);
   nSc.beaconPeriod=atoi(string);
   pclose(fp);
 }

//nSc.beaconPeriod=65530;
  

nSc.haveDTIMPeriod = TRUE;
nSc.ifIndex = ifIndex;
{
   FILE *fp;
   char str_tmp_cmd[SPRINT_MAX_LEN];
   memset( str_tmp_cmd, 0, SPRINT_MAX_LEN );
   strcpy(nSc.ifName,ifName);
   sprintf( str_tmp_cmd, "%s %s %s %s", "/usr/sbin/iwpriv",nSc.ifName, "get_dtim_period", "| awk -F \":\"  '{print $2}'");
   fp=popen(str_tmp_cmd,"r");
   memset (string, 0, SPRINT_MAX_LEN);
   fgets(string,sizeof(string),fp);
   nSc.DTIMPeriod=atoi(string);
   pclose(fp);
 }
...
//3
// 3
nCo.haveFailedCount= TRUE; 
{FILE *fp;
   char str_tmp_cmd[SPRINT_MAX_LEN];
   memset( str_tmp_cmd, 0, SPRINT_MAX_LEN );
   sprintf( str_tmp_cmd, "%s", "/usr/sbin/athstats |awk '/failed due to too many retries/{print $1}'");
   fp=popen(str_tmp_cmd,"r");
   memset (string, 0, SPRINT_MAX_LEN);
   fgets(string,sizeof(string),fp);
   nCo.failedCount=atoi(string);
   pclose(fp);
}

nCo.haveRetryCount= TRUE; 
{FILE *fp;
   char str_tmp_cmd[SPRINT_MAX_LEN];
   memset( str_tmp_cmd, 0, SPRINT_MAX_LEN );
   sprintf( str_tmp_cmd, "%s", "/usr/sbin/athstats |awk '/long on-chip tx retries/{print $1}'");
   fp=popen(str_tmp_cmd,"r");
   memset (string, 0, SPRINT_MAX_LEN);
   fgets(string,sizeof(string),fp);
   nCo.retryCount=atoi(string);
   pclose(fp);
}
//7
nCo.haveRTSSuccessCount= TRUE;
{FILE *fp;
   char str_tmp_cmd[SPRINT_MAX_LEN];
   memset( str_tmp_cmd, 0, SPRINT_MAX_LEN );
   sprintf( str_tmp_cmd, "%s", "/usr/sbin/athstats |awk '/tx frames with rts enabled/{print $1}'");
   fp=popen(str_tmp_cmd,"r");
   memset (string, 0, SPRINT_MAX_LEN);
   fgets(string,sizeof(string),fp);
   nCo.RTSSuccessCount=atoi(string);
   pclose(fp);
}
//9
nCo.haveACKFailureCount = TRUE;
{FILE *fp;
   char str_tmp_cmd[SPRINT_MAX_LEN];
   memset( str_tmp_cmd, 0, SPRINT_MAX_LEN );
   sprintf( str_tmp_cmd, "%s", "/usr/sbin/athstats |awk '/tx frames with no ack marked/{print $1}'");
   fp=popen(str_tmp_cmd,"r");
   memset (string, 0, SPRINT_MAX_LEN);
   fgets(string,sizeof(string),fp);
   nCo.ACKFailureCount=atoi(string);
   pclose(fp);
}
//10

nCo.haveReceivedFragmentCount = TRUE;
{FILE *fp;
   char str_tmp_cmd[SPRINT_MAX_LEN];
   memset( str_tmp_cmd, 0, SPRINT_MAX_LEN );
   sprintf( str_tmp_cmd, "%s", "/usr/sbin/athstats |awk '/rx management frames/{print $1}'");
   fp=popen(str_tmp_cmd,"r");
   memset (string, 0, SPRINT_MAX_LEN);
   fgets(string,sizeof(string),fp);
   nCo.receivedFragmentCount=atoi(string);
   pclose(fp);
}
...
nCo.haveFCSErrorCount= TRUE;
{
   ...
   memset (string, 0, SPRINT_MAX_LEN);
   fgets(string,sizeof(string),fp);
   nCo.FCSErrorCount=atoi(string);
   pclose(fp);	

 }

//13
nCo.haveTransmittedFrameCount = TRUE;
{FILE *fp;
   char str_tmp_cmd[SPRINT_MAX_LEN];
   memset( str_tmp_cmd, 0, SPRINT_MAX_LEN );
   sprintf( str_tmp_cmd, "%s", "/usr/sbin/athstats|awk '/tx management frames/{print $1}'");
   fp=popen(str_tmp_cmd,"r");
   memset (string, 0, SPRINT_MAX_LEN);
   fgets(string,sizeof(string),fp);
   nCo.transmittedFrameCount=atoi(string);
   pclose(fp);
}

//14
nCo.haveWEPUndecryptableCount= TRUE;
{FILE *fp;
   char str_tmp_cmd[SPRINT_MAX_LEN];
   memset( str_tmp_cmd, 0, SPRINT_MAX_LEN );
   nCo.ifIndex = ifIndex;
   ...
   memset (string, 0, SPRINT_MAX_LEN);
   fgets(string,sizeof(string),fp);
   nCo.WEPUndecryptableCount=atoi(string);
   pclose(fp);
}
...
nRi.haveManufacturerName = TRUE;
{
   FILE *fp;
   char str_tmp_cmd[128];
   memset(str_tmp_cmd,0,128);
   nRi.ifIndex=ifIndex;
   sprintf(str_tmp_cmd,"%s","/usr/sbin/showsysinfo |awk -F \":\"  '/Company/{print  $2}' ");
   fp=popen(str_tmp_cmd,"r");
   if(fp)
   	{
          memset(string,0,128);
	   ...
	   strcpy(nRi.manufacturerName,string);
	   pclose(fp);
       }
...
nRi.haveManufacturerProductVersion = TRUE;
{FILE *fp;
   char str_tmp_cmd1[SPRINT_MAX_LEN];
   char str_tmp_cmd2[SPRINT_MAX_LEN];
   char ver_tmp1[SPRINT_MAX_LEN];
   char ver_tmp2[SPRINT_MAX_LEN];
   char string_tmp[SPRINT_MAX_LEN];
   memset( str_tmp_cmd1, 0, SPRINT_MAX_LEN );
   memset(ver_tmp1, 0, SPRINT_MAX_LEN);
   sprintf( str_tmp_cmd1, "%s", "/usr/sbin/showsysinfo |awk -F \":\" '/Software/{print $2}'");
   fp=popen(str_tmp_cmd1,"r");
   fgets(ver_tmp1,sizeof(ver_tmp1),fp);
   ver_tmp1[strlen(ver_tmp1)-1] = '\0';
   sprintf(string,"%s %s %s","Software:",ver_tmp1,"Hardware:");
   pclose(fp);
   memset(str_tmp_cmd2, 0, SPRINT_MAX_LEN );
   memset(ver_tmp2, 0, SPRINT_MAX_LEN);
   sprintf(str_tmp_cmd2, "%s", "/usr/sbin/showsysinfo |awk -F \":\" '/Hardware/{print $2}'");
   fp=popen(str_tmp_cmd2,"r");
   fgets(ver_tmp2,sizeof(ver_tmp2),fp);
   strncat(string,ver_tmp2,sizeof(ver_tmp2));
   string[strlen(string)-1] = '\0';
   pclose(fp);
   strcpy(nRi.manufacturerProductVersion,string);
}
...
//var_dot11PhyTxPowerTable 
 ...
 if ( wi->has_range ){
   nPt.haveNumberSupportedPowerLevels= TRUE;
   nPt.numberSupportedPowerLevels =8; }
 	

nPt.haveTxPowerLevel1= TRUE;
nPt.TxPowerLevel1 = dbm2mwatt(20);

nPt.haveTxPowerLevel2= TRUE;
nPt.TxPowerLevel2 = dbm2mwatt(19);

nPt.haveTxPowerLevel3= TRUE;
nPt.TxPowerLevel3 = dbm2mwatt(18);

nPt.haveTxPowerLevel4= TRUE;
nPt.TxPowerLevel4 = dbm2mwatt(17);

nPt.haveTxPowerLevel5= TRUE;
nPt.TxPowerLevel5 = dbm2mwatt(15);

nPt.haveTxPowerLevel6= TRUE;
nPt.TxPowerLevel6 = dbm2mwatt(10);

nPt.haveTxPowerLevel7= TRUE;
nPt.TxPowerLevel7 = dbm2mwatt(6);

nPt.haveTxPowerLevel8= TRUE;
nPt.TxPowerLevel8 = dbm2mwatt(0);

 if ( wi->has_range ) {
    nPt.haveCurrentTxPowerLevel= TRUE;
	nPt.currentTxPowerLevel= wi->txpower.value;
 }
...
static void loadWiExt ( int skfd, char *ifname, struct wireless_info *wi )
{
  ...
  if ( ioctl ( skfd, SIOCGIWFREQ, &wrq ) >= 0 ) {
    wi->has_freq = 1;
    wi->freq = (( double ) wrq.u.freq.m ) * mypow( 10, wrq.u.freq.e );
  }
...
/* Get power level */
  if ( ioctl ( skfd, SIOCGIWTXPOW, &wrq ) >= 0 ) {
    wi->has_txpower = 1;
    memcpy ( &wi->txpower, &wrq.u.txpower, sizeof ( iwparam ));
  }
...
//  printf ( "%s freq: %g\n", "SIOCGIWFREQ", info.freq / GIGA );
  ...
  if ( info.txpower.flags & IW_TXPOW_MWATT )
    ...
    printf ( "%s txpower3: %d mW \n", "SIOCGIWRANGE",dbm2mwatt( info.txpower.value));
...
dbm2mwatt(int	in)
{
 ...
 return((int) (floor(mypow(10.0, (((double) in) / 10.0)))));
#endif	/* WE_NOLIBM */
}
