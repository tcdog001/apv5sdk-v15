...
#define  JFFS_SYSREBOOTFLAG      "/jffs/sysrebootflag"
#if HAVE_IO_H
#include <io.h>
#endif
#include <stdio.h>
...
extern void report_RF_trap() ;
extern int upgradSocketFD;
extern int cgiSocketFD;
 extern int read_upgrade_info();   
    extern void snmpd_process_for_driver();
	extern int sock_snmp_recv_cgi();
static int
receive(void)
{
    int             numfds;
...
#ifdef   UPGRADE_SELECT_FLAG		
       /*	¼ÓÈëÉý¼¶¼ì²â¾ä±ú*/
	 //printf("numfds=%d   upgradSocketFD=%d\n ", numfds,upgradSocketFD);
	 FD_SET(upgradSocketFD, &readfds);
        if (upgradSocketFD>=numfds)
    	     numfds=upgradSocketFD+1;
#endif    

#if 1		
	 FD_SET(cgiSocketFD, &readfds);
        if (cgiSocketFD>=numfds)
    	     numfds=cgiSocketFD+1;
#endif    
...
#ifdef   UPGRADE_SELECT_FLAG
            //printf("numfds=%d   upgradSocketFD=%d\n ", numfds,upgradSocketFD);
        if(FD_ISSET(upgradSocketFD, &readfds)) {
		    read_upgrade_info() ;	   
		    FD_CLR(upgradSocketFD, &readfds);		
            }  
#endif

#if 1
        if(FD_ISSET(cgiSocketFD, &readfds)) {
			sock_snmp_recv_cgi();
		    FD_CLR(cgiSocketFD, &readfds);		
        }  
#endif
