...
/* End :caizhibang modify for ZT tunnel 2012-2-24>*/				
   ...
   // u_int8_t tid;
    ...
    if (ni->ni_rssi < vap->iv_rssi_threshold)
    {
...
/*<Begin : caizhibang modify for ZT tunnel  2012-5-24*/
    if (thinap || ((vap->vap_splitmac == 2)&&(frametype != __constant_htons(ETHERTYPE_PAE)))) {
        if (vap->vap_splitmac) {
            /*zhaoyang modify for use tunnel or local base users*/
			if(vap->vap_splitmac == 2)//enable this function
			{
			      unsigned char addr1[ETHER_ADDR_LEN],addr2[ETHER_ADDR_LEN];
				unsigned short vlan_type=0;
				if(ni->ni_tunnel_flag == 1)//tunnel
				{
					/*
			        ...
			        */
					...
					vlan_type = eh->ether_type; 
                                IEEE80211_ADDR_COPY(addr1,eh->ether_dhost );
					IEEE80211_ADDR_COPY(addr2,eh->ether_shost);	
					
				      wbuf_pull(wbuf,sizeof(struct ether_header));
				      wbuf_push(wbuf,sizeof(struct vlan_ethhdr));
					  
    					veth=(struct vlan_ethhdr *)(wbuf_header(wbuf));
					
					//add vlan tag
					IEEE80211_ADDR_COPY(veth->h_dest, addr1);
					IEEE80211_ADDR_COPY(veth->h_source, addr2);

                                wbuf->sta_sid = ni->sta_sid;
		                   wbuf->protocol = htons(0x0030);
								
					veth->h_vlan_encapsulated_proto=htons(vlan_type);
					veth->h_vlan_proto = htons(ETH_P_8021Q);
					veth->h_vlan_TCI = htons(ni->vlan_id);
					sendtowtpd = 1;
					wbuf_capture = wbuf;
					wbuf = NULL;  
...
#if 0 //peiwh modified at 0628
				else if(ni->ni_tunnel_flag == 2)//local
				{
					goto localout;
				}
#endif
				else // bad ni_tunnel_flag
					goto bad;
					
			}
	/*zhaoyang modify end*/
/* End :caizhibang modify for ZT tunnel 2012-5-24>*/				
...
localout:
 /*zhaoyang modify end*/
 /* End :caizhibang modify for ZT tunnel 2012-5-24>*/				
        if (frametype == __constant_htons(ETHERTYPE_PAE) || frametype == __constant_htons(ETHERTYPE_PRE_PAE) || frametype == __constant_htons(ETHERTYPE_WAI)) 
...
#define HAS_SEQ(type, subtype)   (((type & 0x4) == 0) && ((type | subtype) != QOS_NULL))
   ...
   // struct llc *llc_type = NULL;
	...
	if((stop_transmit || ic->ic_stop_transmit) &&(vap->iv_opmode == IEEE80211_M_HOSTAP)&&(subtype!=IEEE80211_FC0_SUBTYPE_BEACON)){
...
ieee80211_iter_input_all(void *arg, struct ieee80211vap *vap, bool is_last_vap)
{
    ...
    /*Start: Add by zhanghu to statistics of every frame by the rate*/            
    if( IEEE80211_FC0_TYPE_DATA == params->type ){
        ieee80211_vap_statistics_rate_for_rx_frame(vap, params->rs);
    }
    /*End: Add by zhanghu to statistics of every frame by the rate*/  
