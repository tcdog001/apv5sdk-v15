    ...
    )
{
    ...
    *((u_int16_t *)frm) = htole16(status); frm += 2;
    ...
    /*Begin:added  by zhanghu to statistics the number that the ap send disassoc frame to the station being on line.  2012-11-01*/
    if (IEEE80211_STATUS_SUCCESS != status){
        vap->iv_stats.is_tx_auth_assoc_fail++;
    }
    /*End:added  by zhanghu to statistics the number that the ap send disassoc frame to the station being on line.  2012-11-01*/
