...
/*zhaoyang modify for dusi check*/
#define MAX_ABNORMITY_TIME_DELAY 3
#define MAX_ABNORMITY_MGMT_TIME 60
#define NOBUFFS_INCREASE_THRESHOLD 50
int wifi0_tx_abnormity;
int wifi1_tx_abnormity;
int wifi0_reboot_abnormity;
int wifi1_reboot_abnormity;
...
u_int32_t wifi0_ast_tx_packets_old = 0;
u_int32_t wifi1_ast_tx_packets_old = 0;
...
OS_TIMER_FUNC(ath_txq_stuck_checker)
{
 ...
 //   u_int16_t reset_required = 0;
	...
	//	printk("sc->sc_stats.ast_be_xmit=%d,wifi0_ast_tx_beacon_old=%d,wifi0_reboot_abnormity=%d\n",sc->sc_stats.ast_be_xmit,wifi0_ast_tx_beacon_old,wifi0_reboot_abnormity);
	//	printk("sc->sc_stats.ast_txq_packets[1]=%d,wifi0_txq_packets_old=%d,wifi0_tx_abnormity=%d\n",sc->sc_stats.ast_txq_packets[1],wifi0_txq_packets_old,wifi0_tx_abnormity);
	//	printk("sc->sc_stats.ast_txq_nobuf[1]=%d,wifi0_nobuffs_old=%d\n",sc->sc_stats.ast_txq_nobuf[1],wifi0_nobuffs_old);
	//	printk("sc->sc_stats.ast_11n_stats.tx_pkts=%d,wifi0_ast_tx_packets_old=%d\n",sc->sc_stats.ast_11n_stats.tx_pkts,wifi0_ast_tx_packets_old);
		
		if(sc->sc_stats.ast_txq_packets[1] != 0 && sc->sc_stats.ast_txq_packets[1] == wifi0_txq_packets_old)
		{
			wifi0_tx_abnormity ++;
			if(sc->sc_stats.ast_txq_nobuf[1]>=wifi0_nobuffs_old +NOBUFFS_INCREASE_THRESHOLD)
				wifi0_tx_abnormity +=2;
		}
		else
			wifi0_tx_abnormity = 0;

		if(wifi0_tx_abnormity >= max_abnormity_time)
		{		
			if(wifi0_nobuffs_old!= sc->sc_stats.ast_txq_nobuf[1])
			{
				 /* hardware q stuck detected !!!! reset hardware */
		        DPRINTF(sc, ATH_DEBUG_ANY, "\n %s wifi0, resetting hardware for Q stuck ",__func__);	        
				 printk("wifi0 big resetting hardware for Q stuck\n");
		       ...
		       // wifi0_tx_abnormity = 0;
				...
				if(wifi0_ast_tx_beacon_old == sc->sc_stats.ast_be_xmit)
				{
					wifi0_reboot_abnormity ++;
				}
				else
				{
					wifi0_reboot_abnormity =0;
				}
			}
			else
			{
				//nobuf no change
				if(sc->sc_stats.ast_11n_stats.tx_pkts != wifi0_ast_tx_packets_old)
					wifi0_tx_abnormity = 0;
				else// if(wifi0_tx_abnormity >= max_abnormity_time + MAX_ABNORMITY_TIME_DELAY)
				{
					 /* hardware q stuck detected !!!! reset hardware */
			        DPRINTF(sc, ATH_DEBUG_ANY, "\n %s wifi0, resetting hardware for Q stuck ",__func__);	        
					printk("wifi0 small resetting hardware for Q stuck\n");
			        sc->sc_reset_type = ATH_RESET_NOLOSS;
			        ath_internal_reset(sc);
					ath_radio_disable(sc);
					ath_radio_enable(sc);			
		...
		if(wifi0_reboot_abnormity >= MAX_ABNORMITY_MGMT_TIME)
		{
			printk("wifi0 Q stuck,now restarting system!\n");
			kernel_restart("wifi0 Q stuck,now restarting system!");
		}
		
		if(wifi0_tx_abnormity == 0)
		{
			wifi0_txq_packets_old = sc->sc_stats.ast_txq_packets[1];
			wifi0_nobuffs_old = sc->sc_stats.ast_txq_nobuf[1];
			wifi0_ast_tx_packets_old = sc->sc_stats.ast_11n_stats.tx_pkts;
		}

		if(wifi0_reboot_abnormity == 0)
			wifi0_ast_tx_beacon_old = sc->sc_stats.ast_be_xmit;
		
	}
	else if(memcmp(sc->sc_osdev->netdev->name,"wifi1",5) == 0)
	{
		if(sc->sc_stats.ast_txq_packets[1] != 0 && sc->sc_stats.ast_txq_packets[1] == wifi1_txq_packets_old)
		{
			wifi1_tx_abnormity ++;
			if(sc->sc_stats.ast_txq_nobuf[1]>=wifi1_nobuffs_old +NOBUFFS_INCREASE_THRESHOLD)
				wifi1_tx_abnormity +=2;
		}
		else
			wifi1_tx_abnormity = 0;

		if(wifi1_tx_abnormity >= max_abnormity_time)
		{		
			if(wifi1_nobuffs_old!= sc->sc_stats.ast_txq_nobuf[1])
			{
				 /* hardware q stuck detected !!!! reset hardware */
		        DPRINTF(sc, ATH_DEBUG_ANY, "\n %s wifi1, resetting hardware for Q stuck ",__func__);	      
				printk("wifi1 big resetting hardware for Q stuck\n");
		        sc->sc_reset_type = ATH_RESET_NOLOSS;
		        ath_internal_reset(sc);
				ath_radio_disable(sc);
				ath_radio_enable(sc);	       
		     ...
		     //   wifi1_tx_abnormity = 0;
		     	if(wifi1_ast_tx_beacon_old == sc->sc_stats.ast_be_xmit)
				{
					wifi1_reboot_abnormity ++;
				}
				else
				{
					wifi1_reboot_abnormity =0;
				}
			}
			else 
			{
				//nobuf no change
				if(sc->sc_stats.ast_11n_stats.tx_pkts != wifi1_ast_tx_packets_old)
					wifi1_tx_abnormity = 0;
				else// if(wifi1_tx_abnormity >= max_abnormity_time + MAX_ABNORMITY_TIME_DELAY)
				{
					 /* hardware q stuck detected !!!! reset hardware */
			        DPRINTF(sc, ATH_DEBUG_ANY, "\n %s wifi1, resetting hardware for Q stuck ",__func__);	        
					printk("wifi1 small resetting hardware for Q stuck\n");
			        sc->sc_reset_type = ATH_RESET_NOLOSS;
			        ath_internal_reset(sc);
					ath_radio_disable(sc);
					ath_radio_enable(sc);			
		...
		if(wifi1_reboot_abnormity >= MAX_ABNORMITY_MGMT_TIME)
		{
			printk("wifi1 Q stuck,now restarting system!\n");
			kernel_restart("wifi1 Q stuck,now restarting system!");
		}
		if(wifi1_tx_abnormity == 0)
		{
			wifi1_txq_packets_old = sc->sc_stats.ast_txq_packets[1];
			wifi1_nobuffs_old = sc->sc_stats.ast_txq_nobuf[1];
			wifi1_ast_tx_packets_old = sc->sc_stats.ast_11n_stats.tx_pkts;
		}

		if(wifi1_reboot_abnormity == 0)
