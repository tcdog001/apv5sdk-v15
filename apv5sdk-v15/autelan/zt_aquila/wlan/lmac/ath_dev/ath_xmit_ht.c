...
static int
ath_ni_statistics_rate_for_tx_frame(struct ath_buf *bf, ieee80211_tx_control_t *txctl)
{
...
#define MCS0    0x80
    ...
    if((rateCode & MCS0) && !txctl->use_minrate){
...
static int
ath_vap_statistics_rate_for_tx_frame(struct ath_buf *bf, ieee80211_tx_control_t *txctl)
{
    struct ath_node *an = (struct ath_node *) bf->bf_node;  
    struct ieee80211_node *ni = (struct ieee80211_node*)an->an_node;
...
/*zhaoyang modify end*/
    ...
    if(txctl->isdata && !txctl->ismcast){
        ath_ni_statistics_rate_for_tx_frame(bf, txctl);
    }
    if(txctl->isdata){
        ath_vap_statistics_rate_for_tx_frame(bf, txctl);
    }
    /*End: Add by zhanghu to statistics of every frame by the rate*/
...
#endif
						/*zhaoyang modify for add CL101673.fix.patch for dusi*/
						DPRINTF(sc, ATH_DEBUG_PWR_SAVE, "%s calling tx_sched_normal\n",
						__func__);
						/*zhaoyang modify end*/
                        ath_tx_sched_normal(sc, txq, tid);
                    }
