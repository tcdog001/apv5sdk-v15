...
#if 0
        // The following may have been true for Merlin, but unknown for Osprey.
        /* XXX */
        if (!AR_SREV_KITE(ah)) {
            /*
             * For WOW sleep, we reprogram the SerDes so that the PLL and
             * CHK REQ are both enabled. This uses more power but the Apple
             * team reported that otherwise, WOW sleep is unable and chip
             * may disappears.
             */
