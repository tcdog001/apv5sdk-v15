...
ieee80211_ioctl_tunnel_base_users(struct net_device *dev, struct iwreq *iwr)
{
	struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
	struct ieee80211com *ic = vap->iv_ic;
	struct ieee80211_node *ni = NULL;
	struct ieee80211_tunnel_base_users ik;
	
	memset(&ik, 0x00, sizeof(ik));

	if (copy_from_user(&ik, iwr->u.data.pointer, sizeof(ik))) {
		unsigned long  tmp_ret_val= copy_from_user(&ik, iwr->u.data.pointer, sizeof(ik));
		printf("=====copy_from_user,%lu====\n",tmp_ret_val);
		return -EFAULT;
	}

	ni = ieee80211_find_node(&ic->ic_sta,ik.ni_macaddr);
	if(ni == NULL)
	{
		printf("sta mac error.\n");
		return -EFAULT;
	}

	if(ik.ni_tunnel_flag == 1 || ik.ni_tunnel_flag == 2)
	{
		ni->ni_tunnel_flag = ik.ni_tunnel_flag;
		ni->vlan_id = ik.vlan_id;
/*<Begin : caizhibang modify for ZT tunnel  2012-5-24*/
                ni->sta_sid = ik.sta_sid;
/* End :caizhibang modify for ZT tunnel 2012-2-24>*/				
	}
	else 
	{
		ieee80211_free_node(ni); //Added by duanmingzhe for fixing memory leak
		printf("ni flag error\n");
		return -EFAULT;
	}
...
domlme(void *arg, wlan_node_t node)
{
    ...
    case IEEE80211_MLME_DEAUTH:
		...
		/*zhaoyang1 transplant from 717*/
		/*pengruofeng add start for management frame stats 2011-5-11*/
		node->ni_vap->iv_stats.is_deauth_ioctl++;
		/*pengruofeng add end 2011-5-11*/
		/*zhaoyang1 transplant end*/
        break;
    default:
...
ieee80211_ioctl_autelan_traffic_limit(struct net_device *dev, struct iwreq *iwr)
{
	...
	switch (ik.type) {
		...
		case GET_VAP_TRAFFIC_LIMIT_FLAG:
			copy_to_user(iwr->u.data.pointer, &(vap->vap_traffic_limit), sizeof(vap->vap_traffic_limit));
			break;
		...
		case GET_EVERY_NODE_TRAFFIC_LIMIT_FLAG:
			copy_to_user(iwr->u.data.pointer, &(vap->every_node_traffic_limit), sizeof(vap->every_node_traffic_limit));
			break;

		case SET_SPECIFIC_NODE_TRAFFIC_LIMIT_FLAG:
			/*ljy--modified begin to adjust policy of traffic limit*/
			//vap->specific_node_traffic_limit = ik.arg1;
			ni = ieee80211_find_node(&ic->ic_sta, ik.macaddr);
			if (ni == NULL)
				return -EINVAL;
			ni->ni_traffic_limit = ik.arg1;
			ieee80211_free_node(ni);
			/*ljy--modified end*/
			break;
		...
		case  GET_SPECIFIC_NODE_TRAFFIC_LIMIT_FLAG:
			/*ljy--modified begin to adjust policy of traffic limit*/
			//copy_to_user(iwr->u.data.pointer, &(vap->specific_node_traffic_limit), sizeof(vap->specific_node_traffic_limit));
			ni = ieee80211_find_node(&ic->ic_sta, ik.macaddr);
			if (ni == NULL)
				return -EINVAL;	
			copy_to_user(iwr->u.data.pointer, &(ni->ni_traffic_limit), sizeof(ni->ni_traffic_limit));
			ieee80211_free_node(ni);
			/*ljy--modified end*/
			break;
		...
		case GET_VAP_TRAFFIC_LIMIT_SEND:
			rate = (vap->vap_packet_max_number_send) * 8 / 1024;
			copy_to_user(iwr->u.data.pointer, &rate, sizeof(rate));
			break;
...
/* <Begin :caizhibang modify for ZT tunnel 2012-5-24*/				
	/*zhaoyang modify for use tunnel or local base users*/
	case IEEE80211_IOCTL_TUNNEL_BASE_USERS:
	{
//				printk("@@@@@@@@@@@@@==debug_print_ioctl==@@@@@@@@@@@\n");
		return ieee80211_ioctl_tunnel_base_users(dev, (struct iwreq *)ifr);
	}
	/*zhaoyang modify end*/
