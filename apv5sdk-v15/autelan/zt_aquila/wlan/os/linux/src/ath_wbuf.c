...
#define ALLOC_HEAD_LENGTH 144 /*modified  by zhanghu to kill the bug of the system handled up 2012-10-30 AZT-31*/
...
ath_rxbuf_alloc(struct ath_softc *sc, u_int32_t len)
{
    ...
    if (sc->sc_opmode == HAL_M_MONITOR) {
        ...
        /*Begin:Added by duanmingzhe for thinap add space for the prism header*/
        if (thinap) {
            nbf = adf_nbuf_alloc(NULL, len+ sizeof(wlan_ng_prism2_header),sizeof(wlan_ng_prism2_header), sc->sc_cachelsz);
            if (nbf == NULL) {
                DPRINTF(sc, ATH_DEBUG_ANY,
	...
	/*End:Added by duanmingzhe for thinap add space for the prism header*/
	/*zhaoyang1 transplant end*/
	{
	    /*Begin:modified  by zhanghu to kill the bug of the system handled up 2012-10-30 AZT-31*/
        nbf = adf_nbuf_alloc(NULL, len + ALLOC_HEAD_LENGTH, ALLOC_HEAD_LENGTH, sc->sc_cachelsz);
        /*End:modified  by zhanghu to kill the bug of the system handled up 2012-10-30 AZT-31*/
