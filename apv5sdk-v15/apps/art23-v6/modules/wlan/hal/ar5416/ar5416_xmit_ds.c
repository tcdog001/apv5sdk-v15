...
{
        struct ar5416_desc *ads = AR5416DESC(ds);
        struct ar5416_desc nads;

        HALASSERT((segLen[0] &~ AR_BufLen) == 0);

        OS_MEMCPY(&nads, ads, offsetof(struct ar5416_desc, u.tx.tx_status));
		OS_MEMZERO(&(nads.u.tx.tx_status), sizeof(nads.u.tx.tx_status));
        /* Set the buffer addresses */
        nads.ds_data = bufAddr[0];
        ...
        if (firstSeg) {
                 ...
                 */
                nads.ds_ctl1 |= segLen[0] | (lastSeg ? 0 : AR_TxMore);
                 ...
                 */
                nads.ds_ctl0 = 0;
                nads.ds_ctl1 = segLen[0];
#ifdef AH_NEED_DESC_SWAP
                nads.ds_ctl2 = __bswap32(AR5416DESC_CONST(ds0)->ds_ctl2);
                nads.ds_ctl3 = __bswap32(AR5416DESC_CONST(ds0)->ds_ctl3);
#else
                nads.ds_ctl2 = AR5416DESC_CONST(ds0)->ds_ctl2;
                nads.ds_ctl3 = AR5416DESC_CONST(ds0)->ds_ctl3;
#endif
                 ...
                 */
                nads.ds_ctl0 = 0;
                nads.ds_ctl1 = segLen[0] | AR_TxMore;
                nads.ds_ctl2 = 0;
                nads.ds_ctl3 = 0;
        }
        OS_MEMCPY(ads, &nads, sizeof(struct ar5416_desc));
        return AH_TRUE;
}
...
ar5416ClearTxDescStatus(struct ath_hal *ah, void *ds)
{
	...
	struct ar5416_desc nads;
	OS_MEMZERO(&(nads.u.tx.tx_status), sizeof(nads.u.tx.tx_status));
	ads->u.tx.tx_status = nads.u.tx.tx_status;
}
...
ar5416ProcTxDesc(struct ath_hal *ah, void *desc)
{
		...
		nads.u.tx.tx_status = ads->u.tx.tx_status;
		nads.ds_ctl3 = ads->ds_ctl3;
        /* Update software copies of the HW status */
        ntxstat.ts_seqnum = MS(nads.ds_txstatus9, AR_SeqNum);
        ntxstat.ts_tstamp = nads.AR_SendTimestamp;
        ntxstat.ts_status = 0;
        ntxstat.ts_flags  = 0;

        if (nads.ds_txstatus1 & AR_ExcessiveRetries)
            ntxstat.ts_status |= HAL_TXERR_XRETRY;
        if (nads.ds_txstatus1 & AR_Filtered)
            ntxstat.ts_status |= HAL_TXERR_FILT;
        if (nads.ds_txstatus1 & AR_FIFOUnderrun) {
            ntxstat.ts_status |= HAL_TXERR_FIFO;
	        ar5416UpdateTxTrigLevel(ah, AH_TRUE);
        }
        if (nads.ds_txstatus9 & AR_TxOpExceeded)
            ntxstat.ts_status |= HAL_TXERR_XTXOP;
        if (nads.ds_txstatus1 & AR_TxTimerExpired)
            ntxstat.ts_status |= HAL_TXERR_TIMER_EXPIRED;

        if (nads.ds_txstatus1 & AR_DescCfgErr)
            ntxstat.ts_flags |= HAL_TX_DESC_CFG_ERR;
        if (nads.ds_txstatus1 & AR_TxDataUnderrun) {
            ntxstat.ts_flags |= HAL_TX_DATA_UNDERRUN;
            ar5416UpdateTxTrigLevel(ah, AH_TRUE);
        }
        if (nads.ds_txstatus1 & AR_TxDelimUnderrun) {
            ntxstat.ts_flags |= HAL_TX_DELIM_UNDERRUN;
            ar5416UpdateTxTrigLevel(ah, AH_TRUE);
        }
        if (nads.ds_txstatus0 & AR_TxBaStatus) {
            ntxstat.ts_flags |= HAL_TX_BA;
            ntxstat.ba_low = nads.AR_BaBitmapLow;
            ntxstat.ba_high = nads.AR_BaBitmapHigh;
        }

        ntxstat.tid = (nads.ds_txstatus9 & 0xf0000000) >>28;
        /*
         * Extract the transmit rate.
         */
        ntxstat.ts_rateindex = MS(nads.ds_txstatus9, AR_FinalTxIdx);
        switch (ntxstat.ts_rateindex) {
        case 0:
            ntxstat.ts_ratecode = MS(nads.ds_ctl3, AR_XmitRate0);
            break;
        case 1:
            ntxstat.ts_ratecode = MS(nads.ds_ctl3, AR_XmitRate1);
            break;
        case 2:
            ntxstat.ts_ratecode = MS(nads.ds_ctl3, AR_XmitRate2);
            break;
        case 3:
            ntxstat.ts_ratecode = MS(nads.ds_ctl3, AR_XmitRate3);
            break;
        }

        ntxstat.ts_rssi =      MS(nads.ds_txstatus5, AR_TxRSSICombined);
        ntxstat.ts_rssi_ctl0 = MS(nads.ds_txstatus0, AR_TxRSSIAnt00);
        ntxstat.ts_rssi_ctl1 = MS(nads.ds_txstatus0, AR_TxRSSIAnt01);
        ntxstat.ts_rssi_ctl2 = MS(nads.ds_txstatus0, AR_TxRSSIAnt02);
        ntxstat.ts_rssi_ext0 = MS(nads.ds_txstatus5, AR_TxRSSIAnt10);
        ntxstat.ts_rssi_ext1 = MS(nads.ds_txstatus5, AR_TxRSSIAnt11);
        ntxstat.ts_rssi_ext2 = MS(nads.ds_txstatus5, AR_TxRSSIAnt12);
        ntxstat.evm0 = nads.AR_TxEVM0;
        ntxstat.evm1 = nads.AR_TxEVM1;
        ntxstat.evm2 = nads.AR_TxEVM2;
        ntxstat.ts_shortretry = MS(nads.ds_txstatus1, AR_RTSFailCnt);
        ntxstat.ts_longretry = MS(nads.ds_txstatus1, AR_DataFailCnt);
        ntxstat.ts_virtcol = MS(nads.ds_txstatus1, AR_VirtRetryCnt);
        ntxstat.ts_antenna = 0;

		ds->ds_txstat = ntxstat;
        return HAL_OK;
}
...
ar5416CalcTxAirtime(struct ath_hal *ah, void *ds, struct ath_tx_status *ts)
{
	 ...
	 * improve the d-cache efficiency.
	 */
	nads.u.tx.tx_ctl = ads->u.tx.tx_ctl;
     ...
     */
	if (status9 == 0) {
        return  MS(nads.ds_ctl4, AR_PacketDur0) * finalindex_tries;
	} else if (status9 == 1) {
        return (MS(nads.ds_ctl4, AR_PacketDur1) * finalindex_tries) +
               (MS(nads.ds_ctl2, AR_XmitDataTries0) * MS(nads.ds_ctl4, AR_PacketDur0));
	} else if (status9 == 2) {
        return (MS(nads.ds_ctl5, AR_PacketDur2) * finalindex_tries) +
               (MS(nads.ds_ctl2, AR_XmitDataTries1) * MS(nads.ds_ctl4, AR_PacketDur1)) +
               (MS(nads.ds_ctl2, AR_XmitDataTries0) * MS(nads.ds_ctl4, AR_PacketDur0));
	} else if (status9 == 3) {
        return (MS(nads.ds_ctl5, AR_PacketDur3) * finalindex_tries) +
               (MS(nads.ds_ctl2, AR_XmitDataTries2) * MS(nads.ds_ctl5, AR_PacketDur2)) +
               (MS(nads.ds_ctl2, AR_XmitDataTries1) * MS(nads.ds_ctl4, AR_PacketDur1)) +
               (MS(nads.ds_ctl2, AR_XmitDataTries0) * MS(nads.ds_ctl4, AR_PacketDur0));
	} else {
        HALASSERT(0);
