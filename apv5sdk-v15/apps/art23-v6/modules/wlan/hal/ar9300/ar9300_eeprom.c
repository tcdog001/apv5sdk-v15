...
ar9300OtpWrite(struct ath_hal *ah, u_int off, u_int32_t data)
{
	...
	if (AR_SREV_OSPREY(ah) || AR_SREV_POSEIDON(ah) || AR_SREV_HORNET(ah)) 
	{
		...
		value=1;
		for(it=0; it<100; it++){
		    value=(OS_REG_READ(ah,OTP_STATUS0_OTP_SM_BUSY))&0x3;
			if(value){ 
...
#define SUB_NUM_CTL_MODES_AT_2G_40 3    /* excluding HT40, EXT-OFDM, EXT-CCK */
    ...
    /* XXX: Antenna gain is only 1 chain on Osprey?
     *
...
#define INCREASE_MAXPOW_BY_THREE_CHAIN   10 /* 10*log10(3)*2 */
    ...
    HDPRINTF(ah, HAL_DBG_CALIBRATE, "%s[%d] +++chan %d,cfgctl 0x%04x  twiceAntennaReduction 0x%04x, twiceMaxRegulatoryPower 0x%04x powerLimit 0x%04x\n", __func__, __LINE__, 
                        chan->channel, cfgCtl, twiceAntennaReduction, twiceMaxRegulatoryPower, powerLimit);
		...
		if (!chan->paprdTableWriteDone) {
            for (i = 0;  i < 24; i++)
            {
                ...
                if (tmpPapdRateMask & (1 << i)) {
                    ...
                    HDPRINTF(ah, HAL_DBG_CALIBRATE, "%s[%d]: Chan %d Scale down targetPowerValT2[%d] = 0x%04x\n", __func__, __LINE__, chan->channel, i, targetPowerValT2[i]);
                }
    ...
    {
        HDPRINTF(ah, HAL_DBG_CALIBRATE, "%s: Chan %d Before tmpPapdRateMask = 0x%08x\n", __func__, chan->channel, tmpPapdRateMask);
        HDPRINTF(ah, HAL_DBG_EEPROM, "%s: APPLIED REGULATORY LIMITS\n", __func__);
        i = 0;
        while( i < ar9300RateSize)
        {
            HDPRINTF(ah, HAL_DBG_EEPROM, "%s: TPC[%02d] 0x%08x ",
