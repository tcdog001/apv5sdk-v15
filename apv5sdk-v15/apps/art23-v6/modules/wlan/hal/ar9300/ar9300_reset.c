...
#ifdef ART_BUILD
void init_OTP_HORNET(struct ath_hal *ah);
#endif
#endif
...
#define FAIL(_code)     do { ecode = _code; goto bad; } while (0)
	...
	*status = HAL_OK;
    ...
    ap->ah_bbhang_phyrestart_dis_state = 0;
     ...
     */
    if (((MS((AH_PRIVATE(ah)->ah_bbPanicLastStatus),
                AR_PHY_BB_WD_RX_OFDM_SM)) == 0xb) ||
            AH_PRIVATE(ah)->ah_bbpanic_rx_ofdm)
    {
        ar9300DisablePhyRestart(ah, 1);
        AH_PRIVATE(ah)->ah_bbpanic_rx_ofdm = 1;
    }
				 ...
				 int32_t iqc_coeff[])
{
        ...
        (i2_p_q2_a1_d1 <= iq_corr_a1_d1))  
    {
         ...
         //printf("Parameters's range checking failed!\n");
	     return AH_FALSE;
    }
...
void get_pll_info(unsigned int *cpu_freq, unsigned int *ddr_freq, unsigned int *ahb_freq)
{
    ...
    unsigned int    regBoot_Strap;
    unsigned int    regCpu_Pll_Config;
    unsigned int    regClock_Control;
    unsigned int    regPll_Dither_Frac;
    unsigned int    regPll_Dither;  
...
void init_OTP_HORNET(struct ath_hal *ah)
{
	...
	get_pll_info(&cpu_freq, &ddr_freq, &ahb_freq);
...
//	printf("reg %x = %x\n", OTP_PG_STROBE_PW_REG_V, temp_reg);
	OS_REG_WRITE(ah, OTP_PG_STROBE_PW_REG_V, (unsigned int)temp_reg);	//it need 5000ns for TSMC55nm
...
//	printf("reg %x = %x\n", OTP_RD_STROBE_PW_REG_V, temp_reg);
	OS_REG_WRITE(ah, OTP_RD_STROBE_PW_REG_V, (unsigned int)temp_reg - 1);	//it need 35ns for TSMC55nm
...
//	printf("reg %x = %x\n", OTP_VDDQ_HOLD_TIME_DELAY, temp_reg);
	OS_REG_WRITE(ah, OTP_VDDQ_HOLD_TIME_DELAY, (unsigned int)temp_reg - 1);	//it need 15ns for TSMC55nm
...
//	printf("reg %x = %x\n", OTP_PGENB_SETUP_HOLD_TIME_DELAY, temp_reg);
	OS_REG_WRITE(ah, OTP_PGENB_SETUP_HOLD_TIME_DELAY, (unsigned int)temp_reg - 1);	//it need 21.2ns for TSMC55nm
...
//	printf("reg %x = %x\n", OTP_CSB_ADDR_LOAD_SETUP_HOLD_DELAY, temp_reg);
	OS_REG_WRITE(ah, OTP_CSB_ADDR_LOAD_SETUP_HOLD_DELAY, (unsigned int)temp_reg - 1);	//it need 6.8ns for TSMC55nm
