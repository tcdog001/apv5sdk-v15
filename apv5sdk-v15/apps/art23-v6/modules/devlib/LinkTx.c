...
//#include "..\fusion-osprey\common\lmac\hal\ar9300\ar9300papd.h"
//#include "ar9300papd.h"

extern void ar9300SwizzlePaprdEntries(struct ath_hal*, unsigned int);
...
//            UserPrint(" c[%d]=%d",LinkTxQueuedLast,nd);
		...
		// copy it back to the shared memory
		//
		MyMemoryWrite(dptr,dr,TxDescriptorSize());
...
//    UserPrint("batch queue\n");
    ...
    if(nd>0)
    {
        ...
        //
        MyMemoryRead(LinkTxLoopDescriptor[last], descriptor, TxDescriptorSize());
//        UserPrint("link %d -> %d\n",last,first);
        //
        // check if it is done
        //
        ...
        else
        {
            ...
            MyMemoryWrite(LinkTxLoopDescriptor[last], descriptor, TxDescriptorSize());
...
//
int LinkTxComplete(int timeout, int (*ison)(), int (*done)(), int chipTemperature)
{
	int it;
...
//            UserPrint("(%d %d %d)",it,cindex,batch);
            ...
            {
                MyMemoryRead(LinkTxLoopDescriptor[cindex%LinkTxDescriptorMany], cdescriptor, TxDescriptorSize());	
                cd=cdescriptor;
            }
            else
            {
            ...
            if(LinkTxSplitDescriptor)
            {
		        ...
		        MyMemoryWrite(LinkTxLoopDescriptorStatus[it%LinkTxDescriptorMany],descriptor,TxDescriptorStatusSize());
            }
            //
...
static int LinkTxStatusLoopCreateSplit(int numDesc)
{
	...
	for(it=0; it<numDesc; it++)
	{
		...
		MyMemoryWrite(LinkTxLoopDescriptorStatus[it],dr,TxDescriptorStatusSize());
	}
...
int LinkTxSetup(int *rate, int nrate, int ir,
	...
	unsigned char *pattern, int npattern)
{
	...
	//
	if (AH)
		ar9300SwizzlePaprdEntries(AH,txchain);
	else
		printf("Yak! AH is NULL!\n");
    //
...
void LinkTxOptions(int controlfifo, int splitdescriptor, int aggstatus, int enablefirst)
{
    LinkTxFifo=controlfifo;
    LinkTxSplitDescriptor=splitdescriptor;
    LinkTxAggregateStatus=aggstatus;
    LinkTxEnableFirst=enablefirst;
}

void LinkTxForPAPD(int ChainNum)
{
