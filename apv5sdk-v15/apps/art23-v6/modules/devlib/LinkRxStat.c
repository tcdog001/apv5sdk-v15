...
struct rxStats *LinkRxStatTotalFetch()
{
		return &_LinkRxStatTotal;
}


struct rxStats *LinkRxStatFetch(int rate)
{
...
//    int isDuplicate;
    ...
    if(RxDescriptorFrameRxOk(descriptor)) //pStatsInfo->status2 & DESC_FRM_RCV_OK)
    {
		...
		//
        HistogramInsert(_LinkRxStatTotal.rssi,MRSSI,rssi);
        HistogramInsert(_LinkRxStat[rate].rssi,MRSSI,rssi);
		for(it=0; it<MCHAIN; it++)
		{
            HistogramInsert(_LinkRxStatTotal.rssic[it],MRSSI,rssic[it]);
            HistogramInsert(_LinkRxStat[rate].rssic[it],MRSSI,rssic[it]);
            HistogramInsert(_LinkRxStatTotal.rssie[it],MRSSI,rssie[it]);
            HistogramInsert(_LinkRxStat[rate].rssie[it],MRSSI,rssie[it]);
		}
		//
		...
		{
            HistogramInsert(_LinkRxStatTotal.evm[it],MEVM,evm[it]);
            HistogramInsert(_LinkRxStat[rate].evm[it],MEVM,evm[it]);
		}
#ifdef UNUSED
		//
		// ppm
	...
	else 
	{
		...
		//
        HistogramInsert(_LinkRxStatTotal.badrssi,MRSSI,rssi);
        HistogramInsert(_LinkRxStat[rate].badrssi,MRSSI,rssi);
		for(it=0; it<MCHAIN; it++)
		{
            HistogramInsert(_LinkRxStatTotal.badrssic[it],MRSSI,rssic[it]);
            HistogramInsert(_LinkRxStat[rate].badrssic[it],MRSSI,rssic[it]);
            HistogramInsert(_LinkRxStatTotal.badrssie[it],MRSSI,rssie[it]);
            HistogramInsert(_LinkRxStat[rate].badrssie[it],MRSSI,rssie[it]);
		}
		//
		...
		for(it=0; it<MSTREAM; it++)
		{
            HistogramInsert(_LinkRxStatTotal.badevm[it],MEVM,evm[it]);
            HistogramInsert(_LinkRxStat[rate].badevm[it],MEVM,evm[it]);
		}
	}
