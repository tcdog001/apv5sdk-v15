...
{   
	...
	ar9300TransmitPowerRegWrite(AH, pPwrArray);
	return 0;
}
//set power registers all to same value
int Ar9300TransmitPowerSet(int mode, double txp)
{
...
//	
int Ar9300TransmitGainSet(int mode, int txgain)
{	
    return ForceSingleGainTableOsprey(mode, txgain);
}


int Ar9300TransmitGainRead(int entry, unsigned int *rvalue, int *value, int max)
{
	unsigned int dac_gain = 0;
	OSPREY_TXGAIN_TBL *pGainTbl = NULL;
	int offset=0;
...
int Ar9300TransmitGainWrite(int entry, int *value, int nvalue)
{
    char fieldName[50];
