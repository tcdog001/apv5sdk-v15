...
void FieldSelect(struct _Field *field, int nfield)
{
    int it;
...
//
int FieldFindByAddress(unsigned int address, int low, int high, char **registerName, char **fieldName)
{	
...
int FieldFind(char *name, unsigned int *address, int *low, int *high)
{
	int it;
	char *dot;
...
int FieldWrite(char *name, unsigned int value)
{
	int low, high;
	...
	{
		UserPrint("cant find field %s\n",name);
	    return -1;
	}
}


int FieldRead(char *name, unsigned int *value)
{
	int low, high;
	unsigned int address;
    ...
    if(FieldFind(name, &address, &low, &high))
	{
	    MyFieldRead(address,low,high,value);
		return 0;
	}
	else
	{
...
int FieldList(char *pattern, void (*print)(char *name, unsigned int address, int low, int high))
{
	char name[MBUFFER];
	int count;
	int it;
