...
int MyRegisterRead(unsigned int address, unsigned int *value)
{
	unsigned int *rptr;
    //
...
int MyRegisterWrite(unsigned int address, unsigned int value)
{
	unsigned int *rptr;
    //
	// check alignment, registers must be addressed as 0, 4, 8, ...
...
int MyMemoryRead(unsigned int address, unsigned int *buffer, int many)
{
    uintptr_t localAddress = (uintptr_t)address;
	unsigned char *rptr;
...
int MyMemoryWrite(unsigned int address, unsigned int *buffer, int many)
{
    uintptr_t localAddress = (uintptr_t)address;
	unsigned char *rptr;
...
int AnwiDriverAttach(int devid)
{
...
uintptr_t AnwiDriverRegisterMap()
{
	if(globDrvInfo.pDevInfoArray[devIndex]!=0)
	{
		if(globDrvInfo.pDevInfoArray[devIndex]->pdkInfo!=0)
