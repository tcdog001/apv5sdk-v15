...
void TargetPowerGetCommand(int client)
{
	...
	int iband;
	char *name;	
	char buffer[MBUFFER],value[MBUFFER];
	char *v;
	int address;
	int error;
    int status, parseStatus=0, nc=0, lc=0;
	int index;
    int code;
		...
		else
		{
			...
			for(it=0; it<nfrequency; it++)
			{
				for(ich=0; ich<3; ich++)
				{
					if((1<<ich)&chain)
					{
						nf=ar9300NoiseFloorGet(AH, frequency[it], ich);
						nfp=ar9300NoiseFloorPowerGet(AH, frequency[it], ich);
