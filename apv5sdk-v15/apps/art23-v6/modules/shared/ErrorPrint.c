...
void ErrorPrintFunction(void (*print)(char *buffer))
{
	_ErrorPrint=print;
}


void ErrorLogFunction(void (*log)(char *buffer))
{
	_ErrorLog=log;
}
...
int ErrorHashCreate(struct _Error *m, int many)
{
    struct _Error **fpnew;
    int it, ip;
    int key;
...
int ErrorTypeResponseGet(int type)
{
...
int ErrorTypeResponseSet(int type, int response)
{
    if(type<sizeof(_ErrorResponse)/sizeof(_ErrorResponse[0]))
    {
...
int ErrorResponseSet(int code, int response)
{
    struct _Error *mptr;
...
void ErrorTypeAbbreviation(int onoff)
{
	UseAbbreviation=onoff;
}


int ErrorTypeParse(char *text)
{
...
//
void ErrorPauseFunction(void (*pause)(void))
{
    _ErrorPause=pause;
}
...
//
void ErrorBellFunction(void (*bell)(void))
{
    _ErrorBell=bell;
}
...
void ErrorForward(int art, int code, char *ttype, char *message)
{
    struct _Error *mptr;
    int response;
	int type;
