...
void CommandParameterReplacement(char * (*f)(char *name, char *buffer, int max))
{
    _ValueReplacement=f;
}
...
int CommandParameterMany()
{
    return ArgMany;
}


char *CommandParameterName(int arg)
{
...
int CommandParameterValueMany(int arg)
{
    if(arg>=0 && arg<ArgMany)
    {
...
char *CommandParameterValue(int arg, int value)
{
	return CommandParameterValueInternal(arg,value,_ValueReplacement);
}

char *CommandParameterValueNoReplacement(int arg, int value)
{
	return CommandParameterValueInternal(arg,value,0);
}

void backwardSetCmdProcess(char *buffer)
{
	int iIndex=0;
	char *pt, *pt1;
...
int CommandParse(char *buffer)
{
	...
	_CommandInput=Sduplicate(buffer);
