...
static const struct iwcmd giwcmd[] = {
 {SIOCSIWCOMMIT , "commit" },
...
struct timeval t1;
#if 0
static int ioctl_helper(int ret, unsigned long int req)
{
	const struct iwcmd *cmd;
	struct timeval t2,diff;
	gettimeofday(&t2, NULL);
	timersub(&t2, &t1, &diff);
	for (cmd = giwcmd; cmd->text!=NULL; ++cmd) {
		if (cmd->cmd == req) {
			break;
		}
	}
...
#define MY_IOCTL(_fd, _req, args...) (gettimeofday(&t1, NULL), ioctl_helper(ioctl(_fd, _req,##args), _req))
#endif
#define MY_IOCTL(_fd, _req, args...) ioctl(_fd, _req,##args)

static int wpa_driver_wext_flush_pmkid(void *priv);
static int wpa_driver_wext_get_range(void *priv);
static void wpa_driver_atheros_sleep_again(void *eloop_ctx, void *timeout_ctx)
{
	int ret;
	...
	iwr.u.power.disabled = 0; /* atheros specific flag to enable REC_POWER */
	ret = MY_IOCTL(drv->ioctl_sock, SIOCSIWPOWER, &iwr);
}

static int wpa_driver_atheros_wake_for_dhcp(struct wpa_driver_wext_data *drv)
{   /* dirty method to disable power management in order to get dhcp easily */
	struct iwreq iwr;
	os_memset(&iwr, 0, sizeof(iwr));
	os_strncpy(iwr.ifr_name, drv->ifname, IFNAMSIZ);
	iwr.u.power.disabled = 1; /* atheros specific flag to enable MAX_PERF_POWER */
	if(MY_IOCTL(drv->ioctl_sock, SIOCSIWPOWER, &iwr)>=0) {
		eloop_register_timeout(15, 0, wpa_driver_atheros_sleep_again, drv, drv->ctx);
	}
	return 0;		    
}
					  ...
					  int idx, u32 value)
{
	...
	if (MY_IOCTL(drv->ioctl_sock, SIOCSIWAUTH, &iwr) < 0) {
...
int wpa_driver_wext_get_bssid(void *priv, u8 *bssid)
{
	...
	if (MY_IOCTL(drv->ioctl_sock, SIOCGIWAP, &iwr) < 0) {
...
int wpa_driver_wext_set_bssid(void *priv, const u8 *bssid)
{
	...
	if (MY_IOCTL(drv->ioctl_sock, SIOCSIWAP, &iwr) < 0) {
...
int wpa_driver_wext_get_ssid(void *priv, u8 *ssid)
{
	...
	if (MY_IOCTL(drv->ioctl_sock, SIOCGIWESSID, &iwr) < 0) {
...
int wpa_driver_wext_set_ssid(void *priv, const u8 *ssid, size_t ssid_len)
{
	...
	if (MY_IOCTL(drv->ioctl_sock, SIOCSIWESSID, &iwr) < 0) {
...
int wpa_driver_wext_set_freq(void *priv, int freq)
{
	...
	if (MY_IOCTL(drv->ioctl_sock, SIOCSIWFREQ, &iwr) < 0) {
...
/* Atheros +++ */
				wpa_driver_atheros_wake_for_dhcp(drv);
/* Aterhos +++ */
			}
			break;
					      ...
					      const char *ifname, int *flags)
{
	...
	if (MY_IOCTL(drv->ioctl_sock, SIOCGIFFLAGS, (caddr_t) &ifr) < 0) {
					      ...
					      const char *ifname, int flags)
{
	...
	if (MY_IOCTL(drv->ioctl_sock, SIOCSIFFLAGS, (caddr_t) &ifr) < 0) {
		perror("SIOCSIFFLAGS");
		return -1;
	}
	return 0;
...
void * wpa_driver_wext_init(void *ctx, const char *ifname)
{
    ...
    wpa_printf(MSG_INFO, "%s exit", __func__);
...
int wpa_driver_wext_scan(void *priv, const u8 *ssid, size_t ssid_len)
{
	...
	if (MY_IOCTL(drv->ioctl_sock, SIOCSIWSCAN, &iwr) < 0) {
				     ...
				     size_t max_size)
{
	...
	for (;;) {
		...
		if (MY_IOCTL(drv->ioctl_sock, SIOCGIWSCAN, &iwr) == 0)
			break;
	...
	while (pos + IW_EV_LCP_LEN <= end) {
		...
		custom = pos + IW_EV_POINT_LEN;
        wpa_printf(MSG_WARNING, "we_version %d\n", drv->we_version_compiled);
        if (iwe->cmd == IWEVGENIE) {
            wpa_hexdump(MSG_INFO, "IWEVGNIE1", (const unsigned char *)&iwe, (IW_EV_LCP_LEN + IW_EV_POINT_LEN + 4));
        }
		if (drv->we_version_compiled > 18 &&
			...
			os_memcpy(dpos, pos + IW_EV_LCP_LEN,
				  sizeof(struct iw_event) - dlen);
            wpa_printf(MSG_WARNING, "Road one");
		} else {
			os_memcpy(&iwe_buf, pos, sizeof(struct iw_event));
			custom += IW_EV_POINT_OFF;
            wpa_printf(MSG_DEBUG, "Road two");
		}
        if (iwe->cmd == IWEVGENIE) {
            wpa_hexdump(MSG_WARNING, "IWEVGNIE2", (const unsigned char *)&iwe, (IW_EV_LCP_LEN + IW_EV_POINT_LEN + 4));
        }
		...
		case IWEVQUAL:
			if (ap_num < max_size) {
				...
				results[ap_num].level = iwe->u.qual.level - 256 + 10;
				// Atheros Patches ---
			}
			break;
		case SIOCGIWENCODE:
...
static int wpa_driver_wext_get_range(void *priv)
{
    ...
    while (1) {
    	if (MY_IOCTL(drv->ioctl_sock, SIOCGIWRANGE, &iwr) >= 0) {
            break;
    	} else {
    	    sleep(1);
    ...
    if (iwr.u.data.length >= minlen &&
		...
		wpa_printf(MSG_ERROR, "SIOCGIWRANGE: WE(compiled)=%d "
    ...
    wpa_printf(MSG_INFO, "%s exit %d", __func__, range->we_version_compiled);
				       ...
				       const u8 *key, size_t key_len)
{
	...
	if (MY_IOCTL(drv->ioctl_sock, SIOCSIWENCODEEXT, &iwr) < 0) {
		ret = errno == EOPNOTSUPP ? -2 : -1;
		if (errno == ENODEV) {
			/*
			    ...
			    const u8 *key, size_t key_len)
{
	...
	if (MY_IOCTL(drv->ioctl_sock, SIOCSIWENCODE, &iwr) < 0) {
				...
				const u8 *addr, int cmd, int reason_code)
{
	...
	if (MY_IOCTL(drv->ioctl_sock, SIOCSIWMLME, &iwr) < 0) {
				      ...
				      size_t ie_len)
{
	...
	if (MY_IOCTL(drv->ioctl_sock, SIOCSIWGENIE, &iwr) < 0) {
			  ...
			  struct wpa_driver_associate_params *params)
{
	...
	if (wpa_driver_wext_set_mode(drv, params->mode) < 0)
         ...
         * available */
        if (wpa_driver_wext_set_gen_ie(drv, params->wpa_ie,
                                       params->wpa_ie_len) < 0)
        {
            ret = -1;
        }
...
int wpa_driver_wext_set_mode(void *priv, int mode)
{
	...
	if (MY_IOCTL(drv->ioctl_sock, SIOCSIWMODE, &iwr) < 0) {
				 ...
				 u32 cmd, const u8 *bssid, const u8 *pmkid)
{
	...
	if (MY_IOCTL(drv->ioctl_sock, SIOCSIWPMKSA, &iwr) < 0) {
		if (errno != EOPNOTSUPP)
			 ...
			 struct prism2_hostapd_param *param, int len)
{
	...
	if (MY_IOCTL(drv->ioctl_sock, PRISM2_IOCTL_HOSTAPD, &iwr) < 0) {
...
static int wpa_driver_wext_open_mlme(struct wpa_driver_wext_data *drv)
{
	...
	*i++ = 1;
	...
	if (MY_IOCTL(drv->ioctl_sock, PRISM2_IOCTL_PRISM2_PARAM, &iwr) < 0) {
...
{
	struct wpa_driver_wext_data *drv = priv;
	if (os_strcmp(cmd, "RSSI")==0) {
		int rssi;
		char linebuf[1024];
		FILE *f = fopen("/proc/net/wireless", "r");
		int ifnamelen = strlen(drv->ifname);
		if (f) {
			while(fgets(linebuf, sizeof(linebuf)-1, f)) {
				char *p = linebuf;
				while(*p && isspace(*p)) ++p;				
				if(os_strncmp(p, drv->ifname, ifnamelen)==0 && p[ifnamelen]==':') {
					p += ifnamelen + 1;					
					/* skip status */
					while(*p && isspace(*p)) ++p;
		...
		// reply comes back in the form "LinkSpeed XX" where XX is the number we're interested in.
		...
		if(MY_IOCTL(drv->ioctl_sock, SIOCGIFHWADDR, &ifr)==0) {
		...
		return os_snprintf(buf, buf_len, "Scan-Channels = %u\n", 11);
	} else if (os_strncmp(cmd, "SCAN-CHANNELS ", 14)==0) {
		int allowChannels;
		if (sscanf(cmd, "%*s %d", &allowChannels)==1) {
			return 0; /* ignore it */
		}
		
	} else if (os_strncmp(cmd, "BTCOEXMODE ", 11)==1) {
		int mode;
		if (sscanf(cmd, "%*s %d", &mode)==1) {
			return 0; /* ignore it */
		}				
