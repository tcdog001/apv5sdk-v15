			   ...
			   const u8 *key, size_t key_len)
{
	...
	if (alg != WPA_ALG_SMS4 && seq_len > sizeof(u_int64_t)) {
			     ...
			     struct wpa_driver_associate_params *params)
{
	...
	if (params->wpa_ie_len && params->wpa_ie[0]!= 0x44 &&
...
static int wpa_driver_atheros_set_wapi(void * priv, int enabled)
{

        struct wpa_driver_atheros_data *drv = priv;

        return set80211param(drv, IEEE80211_PARAM_SETWAPI, 15, 1); /* enabled it */
...
const struct wpa_driver_ops wpa_driver_atheros_ops = {
	...
	.set_wapi 		= wpa_driver_atheros_set_wapi,
#ifdef EAP_WPS
#ifndef USE_INTEL_SDK
