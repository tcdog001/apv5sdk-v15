...
#if 0
static char * wapi_supplicant_ie_txt(char *pos, char *end, const char *proto,
					const u8 *ie, size_t ie_len)
{
    struct wapi_ie_data data;
    int ret;
    //wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_SCAN_RESULTS, " found WAPI IE\n" );
    printf("found WAPI IE\n");
    ret = os_snprintf(pos, end - pos, "[%s-", proto);
    if (ret < 0 || ret >= end - pos)
        return pos;
    pos += ret;
    printf("found WAPI IE 1\n");
    ...
    printf("found WAPI IE 2\n");

    if(data.akmnumber == 1) {
        if (ntohl( *data.akmlist) == WAPI_AKM_PSK ) {
            ret = os_snprintf(pos, end - pos, " PSK");
            if (ret < 0 || ret >= end - pos)
                return pos;
            pos += ret;
        }
        printf("found WAPI IE 3\n");
        if (ntohl ( *data.akmlist ) == WAPI_AKM_CERT ) {
	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	*value++ = '\0';
	...
	if (wpa_config_set(ssid, name, value, 0) < 0) {
		wpa_printf(MSG_ERROR, "CTRL_IFACE: Failed to set network "
			   "variable '%s'", name);
		return -1;
	}
