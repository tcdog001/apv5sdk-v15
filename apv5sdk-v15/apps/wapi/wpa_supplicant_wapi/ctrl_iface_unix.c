					      ...
					      void *sock_ctx)
{
	...
	if (reply) {
	    ...
	    wpa_printf(MSG_INFO, "reply = %s", "FAIL\n");
		sendto(sock, "FAIL\n", 5, 0, (struct sockaddr *) &from,
		       fromlen);
	} else if (reply_len == 2) {
	    wpa_printf(MSG_INFO, "reply = %s", "OK\n");
...
wpa_supplicant_ctrl_iface_init(struct wpa_supplicant *wpa_s)
{
	...
	if (mkdir(dir, S_IRWXU | S_IRWXG) < 0) {
		if (errno == EEXIST) {
			wpa_printf(MSG_INFO, "Using existing control "
				   "interface directory.");
		} else {
	...
	if (gid_set && chown(dir, -1, gid) < 0) {
		wpa_printf(MSG_ERROR, "chown[ctrl_interface]");
		goto fail;
	}

	if (os_strlen(dir) + 1 + os_strlen(wpa_s->ifname) >=
	    sizeof(addr.sun_path)) {
	    wpa_printf(MSG_ERROR, "directory length is too long");
		goto fail;
    }

	priv->sock = socket(PF_UNIX, SOCK_DGRAM, 0);
	if (priv->sock < 0) {
		wpa_printf(MSG_ERROR, "socket(PF_UNIX)");
		goto fail;
	}
	...
	if (bind(priv->sock, (struct sockaddr *) &addr, sizeof(addr)) < 0) {
		wpa_printf(MSG_INFO, "ctrl_iface bind(PF_UNIX) failed: %s",
			   strerror(errno));
		if (connect(priv->sock, (struct sockaddr *) &addr,
			...
			if (bind(priv->sock, (struct sockaddr *) &addr,
				 sizeof(addr)) < 0) {
				wpa_printf(MSG_ERROR, "bind(PF_UNIX)");
				goto fail;
			}
	...
	if (gid_set && chown(fname, -1, gid) < 0) {
		wpa_printf(MSG_ERROR, "chown[ctrl_interface/ifname]");
		goto fail;
	}

	if (chmod(fname, S_IRWXU | S_IRWXG) < 0) {
		wpa_printf(MSG_ERROR, "chmod[ctrl_interface/ifname--- ]");
		goto fail;
	}
	os_free(fname);
...
havesock:
#endif
    wpa_printf(MSG_INFO, "[%s] sock = %d", __func__, priv->sock);
