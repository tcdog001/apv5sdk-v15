...
/*====================================================================*
 *
 *   signed WritePIB (struct int6k *int6k);
 *   
 *   int6k.h
 *
 *   This plugin writes a file resident PIB image into device SDRAM
 *   using as many VS_WR_MOD messages as needed to complete transfer;  
 *   runtime firmware must be running for this to work; the PIB file
...
#pragma pack (pop)
	...
	uint32_t extent = lseek (int6k->PIB.file, 0, SEEK_END);
	uint32_t offset = lseek (int6k->PIB.file, 0, SEEK_SET);
	Request (int6k, "Write Parameters to Device");
	while (extent) 
	{
		...
		if (read (int6k->PIB.file, request->MBUFFER, length) < (signed)(length)) 
		{
	...
	Confirm (int6k, "Wrote %s", filepart (int6k->PIB.name));
	return (0);
}
