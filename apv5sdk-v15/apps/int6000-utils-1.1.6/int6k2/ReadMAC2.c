...
/*====================================================================*
 *   
 *   signed ReadMAC2 (struct int6k *int6k);
 *   
 *   int6k2.h
 *   
 *   Read MAC Firmware image from a device using ihpapi_UpdateDevice
 *   api function call, and as many ihpapi_TxFrame and ihpapi_RxFrame
 *   as needed. Note: The image read is from SDRAM, which may no contain
 *   the MAC Firmware.
 *   
...
#ifndef READMAC2_SOURCE
#define READMAC2_SOURCE
...
{
	Request (int6k, "Reading MAC from Device");
	if (ihpapi_UpdateDevice (int6k->channel->host, int6k->channel->peer, IHPAPI_HAR_UPLOAD_FW) == -1) 
	{
	...
	do 
	{
		...
		if (write (int6k->NVM.file, int6k->result->data.updateDev.buffer, (unsigned)(int6k->result->data.updateDev.bufferLen)) != int6k->result->data.updateDev.bufferLen) 
		{
