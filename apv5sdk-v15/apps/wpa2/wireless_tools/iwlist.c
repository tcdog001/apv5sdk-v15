	      ...
	      if((errno == E2BIG) && (range.we_version_compiled > 16))
		{
		  ...
		  if(wrq.u.data.length > buflen)
		    buflen = wrq.u.data.length;
		  	...
		  	{
		  		/*zhaoyang modify for buffer realloc fail*/
		  		if(buflen >= 134217728)
...
static int print_vap_rate_frame_info(int skfd,char *ifname,char *args[],int count)
{
    struct iwreq    wrq;
    ...
    /*building the iw_point struct for getting the data from the kernel */
    ...
    if(iw_get_ext(skfd, ifname, SIOCGIWVAPRATEFRAME, &wrq) < 0){
        fprintf(stderr, "Fail for getting statistics of every frame by the rate\n");
        return -1;
    }else{
        struct mac_rate_frame_info *rx_data = &(reqopt.vap_rx_data);
        struct mac_rate_frame_info *tx_data = &(reqopt.vap_tx_data);
        
        
        /* Display the interface rate info for every frame */        
        printf("----------%s---tx---------\n", ifname);
        printf("  %4dM/s %4dM/s %4sM/s %4dM/s  %4dM/s %4dM/s %4dM/s %4dM/s %4dM/s %4dM/s %4dM/s %4dM/s\n"
		, 1
		, 2
		, "5.5"
		, 11
        ...
        printf("%7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu\n\n"
		, (tx_data->rate)[0].count
        , (tx_data->rate)[1].count
        , (tx_data->rate)[2].count
        , (tx_data->rate)[3].count
        , (tx_data->rate)[4].count
        , (tx_data->rate)[5].count
        , (tx_data->rate)[6].count
        , (tx_data->rate)[7].count
        , (tx_data->rate)[8].count
        , (tx_data->rate)[9].count
        , (tx_data->rate)[10].count
        , (tx_data->rate)[11].count
	);
        printf("  MCS0~MCS7 : %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu\n"
        , tx_data->mcs[0]
        , tx_data->mcs[1]
        , tx_data->mcs[2]
        , tx_data->mcs[3]
        ...
        printf(" MCS8~MCS15 : %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu\n"
        , tx_data->mcs[8]
        , tx_data->mcs[9]
        , tx_data->mcs[10]
        , tx_data->mcs[11]
        ...
        printf("MCS16~MCS23 : %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu\n"
        , tx_data->mcs[16]
        , tx_data->mcs[17]
        , tx_data->mcs[18]
        , tx_data->mcs[19]
        ...
        printf("%7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu\n\n"
        , (rx_data->rate)[0].count
        , (rx_data->rate)[1].count
        , (rx_data->rate)[2].count
        , (rx_data->rate)[3].count
        , (rx_data->rate)[4].count
        , (rx_data->rate)[5].count
        , (rx_data->rate)[6].count
        , (rx_data->rate)[7].count
        , (rx_data->rate)[8].count
        , (rx_data->rate)[9].count
        , (rx_data->rate)[10].count
        , (rx_data->rate)[11].count
    );
        printf("  MCS0~MCS7 : %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu\n"
        , rx_data->mcs[0]
        , rx_data->mcs[1]
...
static int print_ni_rate_frame_info(int skfd,char *ifname,char *args[],int count)
{
    struct iwreq    wrq;
    struct iw_ni_rate_frame_req   reqopt;
    struct rssi_stats  *rssi_stats = NULL;
    ...
    /*get the data from the kernel*/
    if(iw_get_ext(skfd, ifname, SIOCGIWNIRATEFRAME, &wrq) < 0){
        fprintf(stderr, "Fail for getting statistics of every frame by the rate\n");
        return -1;
    }else{
        u_int32_t   i = 0;
        u_int32_t   ni_count = reqopt.ni_count;
        struct ni_info *ni_data = reqopt.ni_data;
        
        
        /* Display the interface rate info for every frame */ 
        if(ni_count > NI_COUNT){
            printf("too much ni node\n ");
            return -1;
        }        
        for(i=0; i < ni_count; i++){            
            rssi_stats = ni_data[i].ni_rssi_stats;
            printf("\n\n----MAC: %02X:%02X:%02X:%02X:%02X:%02X-----\n\n",
                ni_data[i].mac[0], ni_data[i].mac[1], ni_data[i].mac[2],
                ni_data[i].mac[3], ni_data[i].mac[4], ni_data[i].mac[5]);            
            printf("    ----------tx---------\n");
            printf("  %4dM/s %4dM/s %4sM/s %4dM/s  %4dM/s %4dM/s %4dM/s %4dM/s %4dM/s %4dM/s %4dM/s %4dM/s\n"
        		, 1
        		, 2
            ...
            printf("%7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu\n\n"
        		, ni_data[i].ni_tx_data.rate[0].count
        		, ni_data[i].ni_tx_data.rate[1].count
        		, ni_data[i].ni_tx_data.rate[2].count
        		, ni_data[i].ni_tx_data.rate[3].count
        		, ni_data[i].ni_tx_data.rate[4].count
        		, ni_data[i].ni_tx_data.rate[5].count
        		, ni_data[i].ni_tx_data.rate[6].count
        		, ni_data[i].ni_tx_data.rate[7].count
        		, ni_data[i].ni_tx_data.rate[8].count
        		, ni_data[i].ni_tx_data.rate[9].count
        		, ni_data[i].ni_tx_data.rate[10].count
        		, ni_data[i].ni_tx_data.rate[11].count
        	);

            printf("  MCS0~MCS7 : %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu\n"
                , ni_data[i].ni_tx_data.mcs[0]
                , ni_data[i].ni_tx_data.mcs[1]
                , ni_data[i].ni_tx_data.mcs[2]
                , ni_data[i].ni_tx_data.mcs[3]
                , ni_data[i].ni_tx_data.mcs[4]
                , ni_data[i].ni_tx_data.mcs[5]
                , ni_data[i].ni_tx_data.mcs[6]
                , ni_data[i].ni_tx_data.mcs[7]
            );
            printf(" MCS8~MCS15 : %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu\n"
                , ni_data[i].ni_tx_data.mcs[8]
                , ni_data[i].ni_tx_data.mcs[9]
                , ni_data[i].ni_tx_data.mcs[10]
                , ni_data[i].ni_tx_data.mcs[11]
                , ni_data[i].ni_tx_data.mcs[12]
                , ni_data[i].ni_tx_data.mcs[13]
                , ni_data[i].ni_tx_data.mcs[14]
                , ni_data[i].ni_tx_data.mcs[15]
            );
            printf("MCS16~MCS23 : %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu\n"
                , ni_data[i].ni_tx_data.mcs[16]
                , ni_data[i].ni_tx_data.mcs[17]
                , ni_data[i].ni_tx_data.mcs[18]
                , ni_data[i].ni_tx_data.mcs[19]
                , ni_data[i].ni_tx_data.mcs[20]
                , ni_data[i].ni_tx_data.mcs[21]
                , ni_data[i].ni_tx_data.mcs[22]
                , ni_data[i].ni_tx_data.mcs[23]
            );
            ...
            printf("%7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu\n\n"
        		, ni_data[i].ni_rx_data.rate[0].count
        		, ni_data[i].ni_rx_data.rate[1].count
        		, ni_data[i].ni_rx_data.rate[2].count
        		, ni_data[i].ni_rx_data.rate[3].count
        		, ni_data[i].ni_rx_data.rate[4].count
        		, ni_data[i].ni_rx_data.rate[5].count
        		, ni_data[i].ni_rx_data.rate[6].count
        		, ni_data[i].ni_rx_data.rate[7].count
        		, ni_data[i].ni_rx_data.rate[8].count
        		, ni_data[i].ni_rx_data.rate[9].count
        		, ni_data[i].ni_rx_data.rate[10].count
        		, ni_data[i].ni_rx_data.rate[11].count
        	);

            printf("  MCS0~MCS7 : %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu\n"
                , ni_data[i].ni_rx_data.mcs[0]
                , ni_data[i].ni_rx_data.mcs[1]
                , ni_data[i].ni_rx_data.mcs[2]
                , ni_data[i].ni_rx_data.mcs[3]
                , ni_data[i].ni_rx_data.mcs[4]
                , ni_data[i].ni_rx_data.mcs[5]
                , ni_data[i].ni_rx_data.mcs[6]
                , ni_data[i].ni_rx_data.mcs[7]
            );
            printf(" MCS8~MCS15 : %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu\n"
                , ni_data[i].ni_rx_data.mcs[8]
                , ni_data[i].ni_rx_data.mcs[9]
                , ni_data[i].ni_rx_data.mcs[10]
                , ni_data[i].ni_rx_data.mcs[11]
                , ni_data[i].ni_rx_data.mcs[12]
                , ni_data[i].ni_rx_data.mcs[13]
                , ni_data[i].ni_rx_data.mcs[14]
                , ni_data[i].ni_rx_data.mcs[15]
            );
            printf("MCS16~MCS23 : %7llu %7llu %7llu %7llu %7llu %7llu %7llu %7llu\n"
                , ni_data[i].ni_rx_data.mcs[16]
                , ni_data[i].ni_rx_data.mcs[17]
                , ni_data[i].ni_rx_data.mcs[18]
                , ni_data[i].ni_rx_data.mcs[19]
                , ni_data[i].ni_rx_data.mcs[20]
                , ni_data[i].ni_rx_data.mcs[21]
                , ni_data[i].ni_rx_data.mcs[22]
                , ni_data[i].ni_rx_data.mcs[23]
            );
            ...
            printf("%9llu, %9llu, %9llu, %9llu, %9llu, %9llu\n",
                rssi_stats[0].ns_rx_data,
                rssi_stats[1].ns_rx_data,
                rssi_stats[2].ns_rx_data,
                rssi_stats[3].ns_rx_data,
                rssi_stats[4].ns_rx_data,                
                rssi_stats[5].ns_rx_data
            );
            ...
            printf("%9llu, %9llu, %9llu, %9llu, %9llu, %9llu\n",
                rssi_stats[6].ns_rx_data,
                rssi_stats[7].ns_rx_data,
                rssi_stats[8].ns_rx_data,
                rssi_stats[9].ns_rx_data,
                rssi_stats[10].ns_rx_data,                
                rssi_stats[11].ns_rx_data
            );
            ...
            printf("%9llu, %9llu, %9llu, %9llu, %9llu\n",
                rssi_stats[12].ns_rx_data,
                rssi_stats[13].ns_rx_data,
                rssi_stats[14].ns_rx_data,
                rssi_stats[15].ns_rx_data,
                rssi_stats[16].ns_rx_data              
            );
            printf("\n");
        }
    }
...
static const struct iwlist_entry iwlist_cmds[] = {
  ...
  /*Start: Add by zhanghu to statistics of every frame by the rate*/
  { "vap_rate_frame", print_vap_rate_frame_info,  0, NULL },  
  { "ni_rate_frame", print_ni_rate_frame_info,  0, NULL },
  /*End: Add by zhanghu to statistics of every frame by the rate*/
