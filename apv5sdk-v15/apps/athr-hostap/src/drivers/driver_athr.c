...
static void driver_atheros_disconnect(struct driver_atheros_data *drv);
static int
...
static void driver_atheros_disconnect(struct driver_atheros_data *drv)
{
	...
	if (iwr.u.mode == IW_MODE_INFRA) {
		...
		if (driver_atheros_set_bssid(drv, null_bssid) < 0) { // ||
			//driver_atheros_set_ssid(drv, ssid, 32) < 0) {
				 ...
				 const char *bridge)
{
	...
	os_strncpy(ifr.ifr_name, "wifi0", IFNAMSIZ); //hard code radio???
