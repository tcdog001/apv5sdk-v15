			...
			const u8 *req_ies, size_t req_ies_len, int reassoc)
{
	...
	if (hapd->conf->wpa) {
		...
		if (sta->auth_alg == WLAN_AUTH_FT) {
			res = wpa_ft_validate_reassoc(sta->wpa_sm, req_ies,
						      req_ies_len);
...
skip_wpa_check:
	...
	len = p - buf;
	if (len)
		hostapd_set_assoc_resp_ie(hapd, buf, len);
	hostapd_sta_assoc(hapd, addr, reassoc, 0);
#endif /* CONFIG_IEEE80211R */
					  ...
					  const u8 *ies, size_t ies_len)
{
	...
	if (ies_len)
		hostapd_set_auth_ie(hapd, ies, ies_len);
	hostapd_sta_auth(hapd, dst, auth_transaction, status);
}
#endif /* CONFIG_IEEE80211R */
...
fail:
	if (resp_ies_len)
		hostapd_set_auth_ie(hapd, resp_ies, resp_ies_len);
