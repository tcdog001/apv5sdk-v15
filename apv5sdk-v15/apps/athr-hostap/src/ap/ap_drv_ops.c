...
int hostapd_set_assoc_resp_ie(struct hostapd_data *hapd, const u8 *ie,
			      size_t len)
{
	if (hapd->driver == NULL || hapd->driver->set_assoc_resp_ie == NULL)
		return 0;
	return hapd->driver->set_assoc_resp_ie(hapd->conf->iface,
					       hapd->drv_priv, ie, len);
}


int hostapd_set_auth_ie(struct hostapd_data *hapd, const u8 *ie, size_t len)
{
	if (hapd->driver == NULL || hapd->driver->set_auth_ie == NULL)
		return 0;
	return hapd->driver->set_auth_ie(hapd->conf->iface,
					 hapd->drv_priv, ie, len);
}
